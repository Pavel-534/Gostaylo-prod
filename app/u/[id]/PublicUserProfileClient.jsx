...
/** @typedef {{ ledgerBaseCurrency: string, exampleAmountThb: number, visitorCurrencyCode: string | null, formattedConverted: string | null, formattedThbBaseline: string | null }} LandingEarningPreview */

/**
 * @param {{ visitorCountry?: string, landingEarningPreview?: LandingEarningPreview | null }} props
 */
export default function PublicUserProfileClient({
  ...
  /** SSOT: активированные партнёры (`landing-meta.directPartnersInvited`), иначе регистрации по ссылке. */
  const teamProofCount = Number(
    landingMeta?.directPartnersInvited ?? landingMeta?.friendsInvited ?? 0,
  )
  ...
  const geoHintLine =
    ...
    landingEarningPreview.formattedThbBaseline
      ? t('stage76_landingEarningTeaser')
          ...
          .replace('{ledgerBase}', String(landingEarningPreview.ledgerBaseCurrency || 'THB'))
      : null
          ...
          <CardContent className="p-5 sm:p-7">
            <div className="flex flex-col sm:flex-row items-center gap-5 text-center sm:text-left">
              ...
              <div className="flex-1 space-y-3 min-w-0">
                ...
                {teamProofCount > 0 ? (
                  <div className="flex items-start gap-2 rounded-lg border border-border bg-muted/20 px-3 py-2.5 w-full">
                    <Users className="h-5 w-5 text-primary shrink-0 mt-0.5" aria-hidden />
                    <p className="text-sm text-foreground leading-snug text-left">
                      {t('stage74_4_teamSocialProof').replace(
                        '{count}',
                        teamProofCount.toLocaleString(countLocale),
                      )}
                    </p>
                  </div>
                ) : null}
                {geoHintLine ? (
                  <div className="rounded-lg border border-primary/25 bg-primary/5 px-3 py-2.5 w-full">
                    <p className="text-sm text-left font-medium text-foreground leading-snug">{geoHintLine}</p>
                    <p className="text-[11px] text-muted-foreground mt-1.5 leading-snug text-left">
                      {t('stage76_landingEarningDisclaimer').replace(
                        '{ledgerBase}',
                        String(landingEarningPreview.ledgerBaseCurrency || 'THB'),
                      )}
                    </p>
                  </div>
                ) : null}
                ...
                <div className="pt-1">
                  <Button asChild variant="outline" size="sm" className="border-border w-full sm:w-auto">
