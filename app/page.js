...
export default function FunnyRentHome() {
  ...
  return (
    <div className='min-h-screen bg-white'>
      {/* Header - Responsive with User Identity */}
          ...
          <div className='flex items-center justify-between h-14 sm:h-16'>
            ...
            <Link href='/' className='flex-shrink-0'>
              <div className='flex flex-col leading-none'>
                ...
                <span className='text-lg sm:text-xl font-black text-teal-600 tracking-tight ml-3 sm:ml-4 -mt-1'>Rent</span>
              </div>
            </Link>
            ...
            <div className='flex items-center gap-1.5 sm:gap-2'>
              ...
              >
                <Link href='/partner/listings/new'>
                  ...
                  {language === 'ru' ? 'Разместить объект' : 'List Property'}
                </Link>
              </Button>
              ...
              {currentUser ? (
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant='outline' size='sm' className='border-teal-200 hover:bg-teal-50 h-8 gap-1.5 px-2'>
                      <User className='h-4 w-4 text-teal-600' />
                      <span className='hidden sm:inline text-xs font-medium text-slate-700 max-w-[80px] truncate'>
                  ...
                  <DropdownMenuContent align='end' className='w-48'>
                    <div className='px-2 py-1.5 border-b'>
                      <p className='text-sm font-medium text-slate-900 truncate'>{currentUser.name || currentUser.email}</p>
                      <p className='text-xs text-slate-500'>{currentUser.role}</p>
                    </div>
                    <DropdownMenuItem onClick={() => router.push(
                      currentUser.role === 'ADMIN' ? '/admin/dashboard' :
                      currentUser.role === 'PARTNER' ? '/partner/dashboard' : '/renter/dashboard'
                    )}>
