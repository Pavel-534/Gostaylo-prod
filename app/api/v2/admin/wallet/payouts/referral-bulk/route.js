 ...
 * Stage 114.3 / 131.3 / 134 — массовый approve/reject очереди withdrawable_referral.
...
export async function POST(request) {
  ...
  if (action === 'reject') {
    const walletsBefore = await supabaseAdmin
      .from('user_wallets')
      .select(
        'user_id,referral_withdrawal_amount_thb,referral_withdrawal_metadata,withdrawable_balance_thb',
      )
      .in('user_id', userIds)
    ...
    for (const row of walletsBefore.data || []) {
      const fxMeta =
