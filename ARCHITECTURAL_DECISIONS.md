...
- **Public coordinates privacy (Stage 163.2 — implemented):** детерминированный fuzz 150–300 m on-read для verticals с **`mapLocationDisplayMode: privacy`**; exact coords для transport/yacht/tour; reveal после брони — SSOT **`lib/geo/listing-public-coordinates.js`**, viewer context **`lib/geo/public-coordinate-viewer-context.js`**, ADR **`docs/ADR/163-coordinate-privacy-ssot.md`**. PostGIS / **`listings.coordinates`** остаются истинными; obfuscation только в публичных read-path. Verify: **`npm run verify:coord-privacy`**.
...
- **RUB acquiring:** гость платит в RUB; учёт в THB. SSOT суммы для PSP — **`lib/services/payment-adapters/acquirer-charge-amount.js`**; флаги **`PAYMENT_ACQUIRER_RUB_ENABLED`**, **`PAYMENT_ACQUIRER_RUB_SHADOW`**. Курсовой **спред** — **`fx_markup_thb`** в snapshot при `payment_currency ≠ listing_base_currency` (в т.ч. **pay=THB × base≠THB**: surcharge в `total_guest_brutto`, Stage **200.88**; partner netto не режется).
...
- **Ambassador Program 3.0 (Accepted, ADR-131, Stage 131.0):** SSOT политики — **`docs/ADR/131-ambassador-3-0.md`**; SSOT параметров runtime — **`system_fintech_settings`** (`migrations/stage131_0_system_fintech_settings.sql`) + **`SystemConfigService`** (`lib/services/finance/system-config.service.js`). Owner waterfall: acquiring от **`guest_payment_thb`**, USN/НДС/bank reserve от **`owner_revenue`** (commission + FX markup владельцу, не в pool). Guest L2 (**45/12/43**) — флаг `ambassador_guest_l2_enabled` (P1); caps 500/50k; program cap **250k THB/мес** → `pending` + `metadata.cap_deferred`. Reinvestment launch **45%**. L3+ не monetized. Promo tank / welcome остаются в **`system_settings.general`**. *(Stage 128.x — только TanStack Query, ADR-128.)*
- **Ambassador Program 3.1 (Accepted, ADR-131A, 2026-08-19, владелец Pavel):** guest pool **L3** (42/10/5/43), program cap **1 000 000** THB/мес, staged L4/L5. Runtime **пока ADR-131** до PR Stage **131.A1**. L3 live ledger — только после `ambassador_guest_l3_enabled=true` + Legal track. Policy: **`docs/ADR/131A-ambassador-3-1-multi-level.md`**.
- **Team Analytics & Growth (Accepted, ADR-133, Stage 133.0):** SSOT — **`docs/ADR/133-team-analytics-growth.md`**. Breakdown L1/L2 «от амбассадора» — по **`referral_relations`** (direct `referee_id`), **не** по абсолютному **`ledger_depth`**. Агрегация KPI — Postgres RPC **`referral_team_analytics_for_referrer`** + **`lib/referral/build-referral-team-analytics.js`**; UI — tab «Команда» на **`/profile/referral`**, поле **`data.teamAnalytics`**. **`teamMembers[].earnedForReferrerThb`**. Retention v1: **`direct_partners_with_completed_host_booking`**. Legacy **`stats.monthlyL1EarnedThb`** / **`monthlyNetworkEarnedThb`** — та же ADR-133 семантика (RPC). Shadow L2 вне ledger — **`shadowL2Notice`**.
