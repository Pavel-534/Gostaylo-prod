...
- **Separate heuristics (sync when adding categories):** при **`categories.slug`** сначала расширяйте **`lib/config/category-behavior.js`** (`CATEGORY_SLUG_BEHAVIOR_OVERRIDES` / **`resolveCategoryBehavior`**); **`lib/listing-location-privacy.js`** и **`lib/partner-calendar-filters.js`** используют хелперы реестра (в т.ч. legacy **`categoryId`** → реестр). When **`categories`** gains a new slug, update these if the category needs distinct map/privacy or calendar grouping.
