...
function DeepLinkCard({ href, icon: Icon, title, description }) {
  return (
    <Link
      ...
      className={cn(
        MOBILE_FLAT_CARD_CLASS,
        'flex items-start gap-3 max-sm:border-b max-sm:border-slate-100 max-sm:py-3 sm:rounded-xl sm:border-slate-200 sm:bg-white sm:p-4 sm:shadow-sm sm:transition sm:hover:border-indigo-200 sm:hover:shadow-md',
      )}
    >
  ...
  return (
    <div className="min-h-screen bg-slate-50/90">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        {report ? (
          <>
            <Card className={cn(MOBILE_FLAT_CARD_CLASS, 'overflow-hidden sm:border-emerald-200/80 sm:bg-gradient-to-br sm:from-emerald-50 sm:via-white sm:to-white sm:shadow-lg')}>
            ...
            <Card className={cn(MOBILE_FLAT_CARD_CLASS, 'sm:border-slate-200/80 sm:shadow-md')}>
            ...
            {(report.referral?.rows || []).length > 0 ? (
              <Card className={cn(MOBILE_FLAT_CARD_CLASS, 'sm:border-slate-200/80 sm:shadow-sm')}>
                ...
                <CardContent className={MOBILE_FLAT_CARD_CONTENT_CLASS}>
                  <div className="sm:hidden space-y-0" data-testid="referral-mobile-cards">
                    {report.referral.rows.map((r) => (
                      <div key={r.id} className="border-b border-slate-100 py-3">
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <AdminTableAmount value={r.amountThb} showPlus={false} className="shrink-0 font-semibold" />
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            {report.ledger?.legs?.length > 0 ? (
              <Card className={cn(MOBILE_FLAT_CARD_CLASS, 'sm:border-slate-200/80 sm:shadow-sm')}>
                ...
                <CardContent className={MOBILE_FLAT_CARD_CONTENT_CLASS}>
                  <div className="sm:hidden space-y-0" data-testid="ledger-mobile-cards">
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            ...
                            <p className="mt-1 text-xs text-slate-500">RUB {leg.amountRub ?? '—'}</p>
                          </div>
                          <p className="shrink-0 font-semibold tabular-nums">{leg.amountThb}</p>
                        </div>
                      </div>
                    ))}
                  </div>
