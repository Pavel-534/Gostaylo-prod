...
async function ensureRubRate() {
  if (isSmokeExchangeRatesWriteBlocked()) {
    ...
    const thbPerRub = normalizeThbPerUnitRate('RUB', Number(data?.rate_to_thb))
    if (thbPerRub == null || thbPerRub <= 0) {
      throw new Error('exchange_rates RUB missing (e2e read-only on prod)')
    }
    return thbPerRubFromRubPerThb(1 / thbPerRub) ?? 2.8
  }
