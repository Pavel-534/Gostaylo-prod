...
/** Drop prior stage72 bookings on the reused listing (host_activation needs a clean slate). */
async function purgeStage72BookingsForListing(listingId) {
  ...
  const conversationIds = (convs || []).map((c) => c.id).filter(Boolean);
  ...
  await supabaseAdmin.from('invoices').delete().in('booking_id', bookingIds);
  await supabaseAdmin.from('bookings').delete().in('id', bookingIds);
}

/**
 * Stage 119.2 — COMPLETED (начисление) → диспут (clawback) → CANCELLED (SSOT transition).
 * @param {string} bookingId
 */
async function runReferralLifecycle1192Phase(bookingId) {
  const { onDisputeResolved } = await import('@/lib/services/marketing/referral-lifecycle-hook.js');
  const { transitionBookingStatus } = await import('@/lib/services/booking/booking-status.service.js');

  const { data: earnedRows, error: earnedErr } = await supabaseAdmin
    .from('referral_ledger')
    .select('id, status, amount_thb, referral_type')
    .eq('booking_id', bookingId)
    .eq('status', 'earned');
  if (earnedErr) throw new Error(`E2E_1192_LEDGER_READ: ${earnedErr.message}`);
  if (!earnedRows?.length) {
    throw new Error('E2E_1192_REQUIRES_EARNED_LEDGER: run distribute before lifecycle phase');
  }

  const genBefore = await PricingService.getGeneralPricingSettings();
  const promoPotBefore = Number(genBefore?.marketing_promo_pot ?? genBefore?.marketingPromoPot ?? 0);

  const walletSnap = async (userIds) => {
    const rows = [];
    for (const uid of userIds) {
      const { data: w } = await supabaseAdmin
        .from('user_wallets')
        .select('user_id,withdrawable_balance_thb,internal_credits_thb')
        .eq('user_id', uid)
        .maybeSingle();
      rows.push({
        userId: uid,
    ...
    ...new Set(
      (
        await supabaseAdmin
          ...
          .eq('booking_id', bookingId)
          .eq('status', 'earned')
      ).data?.map((r) => String(r.referrer_id || '')).filter(Boolean) || [],
    ),
  ];
  const walletsPre = await walletSnap(beneficiaryIds);

  const disputeRes = await onDisputeResolved({
    bookingId,
    disputeId: `e2e-dispute-${bookingId.slice(0, 8)}`,
    trigger: 'e2e_dispute_resolved',
    resolutionReason: 'E2E Stage 119.2 dispute clawback',
  });
  if (disputeRes.success === false) {
    throw new Error(`E2E_1192_DISPUTE_REVERT: ${disputeRes.error || 'failed'}`);
  }

  const cancelRes = await transitionBookingStatus(bookingId, 'CANCELLED', {
    scope: 'cancel',
    allowStaffCancelCompleted: true,
    actorContext: { trigger: 'e2e_dispute_cancel' },
  });
  if (!cancelRes.success) {
    throw new Error(`E2E_1192_CANCEL_TRANSITION: ${cancelRes.error || 'failed'}`);
  }

  const { data: openLedger } = await supabaseAdmin
    .from('referral_ledger')
    .select('id, status')
    .eq('booking_id', bookingId)
    .in('status', ['pending', 'earned']);
  if (openLedger?.length) {
    throw new Error(`E2E_1192_LEDGER_STILL_OPEN: ${openLedger.length} rows`);
  }

  const { data: revertedLedger } = await supabaseAdmin
    .from('referral_ledger')
    .select('id, status, metadata')
    .eq('booking_id', bookingId)
    .in('status', ['canceled', 'canceled_deficit']);
  if (!revertedLedger?.length) {
    throw new Error('E2E_1192_NO_REVERTED_LEDGER');
  }
  ...
  const walletsPost = await walletSnap(beneficiaryIds);

  const wdPre = walletsPre.reduce((s, r) => s + r.withdrawable, 0);
  const wdPost = walletsPost.reduce((s, r) => s + r.withdrawable, 0);
  const intPre = walletsPre.reduce((s, r) => s + r.internal, 0);
  const intPost = walletsPost.reduce((s, r) => s + r.internal, 0);

  if (wdPost > wdPre + 0.01 || intPost > intPre + 0.01) {
    throw new Error(
      `E2E_1192_WALLET_NOT_CLAWED: wd ${wdPre}→${wdPost} int ${intPre}→${intPost}`,
    );
  }
  ...
  const { data: bookingRow } = await supabaseAdmin
    ...
    .eq('id', bookingId)
    .maybeSingle();
  if (String(bookingRow?.status || '').toUpperCase() !== 'CANCELLED') {
    throw new Error(`E2E_1192_BOOKING_STATUS: ${bookingRow?.status}`);
  }

  return {
    success: true,
    earnedRowsBefore: earnedRows.length,
    revertedRows: revertedLedger.length,
    promoPotBefore,
    promoPotAfter,
    promoPotDelta: Math.round((promoPotAfter - promoPotBefore) * 100) / 100,
    walletClawback: { withdrawableBefore: wdPre, withdrawableAfter: wdPost, internalBefore: intPre, internalAfter: intPost },
    disputeRes: { skipped: disputeRes.skipped, revert: disputeRes.revert?.success },
    cancelRes: { referralLifecycle: cancelRes.referralLifecycle?.revert?.success },
    idempotentSecondDispute: await onDisputeResolved({ bookingId, trigger: 'e2e_dispute_idempotent' }),
  };
}
...
export async function runStage72ReferralCashflowFixture() {
  ...
  try {
    ...
    const patched = {
      ...
      partner_activation_bonus: 760,
      partnerActivationBonus: 760,
      partner_activation_bonus_thb: 760,
      mlm_level1_percent: 100,
      mlmLevel1Percent: 100,
    ...
    const listingId = process.env.E2E_STAGE72_LISTING_ID || STAGE72_REUSE_LISTING_ID;
    await purgeStage72BookingsForListing(listingId);
    const { error: listErr } = await supabaseAdmin.from('listings').upsert(
      {
        id: listingId,
        owner_id: idC,
        ...
        max_capacity: 1,
        metadata: { test_data_tag: E2E_TEST_DATA_TAG, e2e_fixture: 'stage72-cashflow' },
      },
      { onConflict: 'id' },
    );
    if (listErr) throw new Error(`listings upsert: ${listErr.message}`);
    ...
    const lifecycle1192 = await runReferralLifecycle1192Phase(bookingD);
