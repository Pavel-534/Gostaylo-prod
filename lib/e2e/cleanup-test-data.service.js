...
import { fetchNeverPaidCleanupBookingIds } from './never-paid-booking-cleanup.js'
import { purgeExpiredCalendarHoldBlocks } from '../calendar/purge-expired-hold-blocks.js'

const CHUNK = 150
/** Live money pipe — skip only if test-ledger purge RPC failed. */
...
async function fetchBookingIdsForListings(sb, listingIds, { allowMoneyStatuses = false } = {}) {
  const bookingIds = new Set()
  for (const part of chunks(listingIds)) {
    const { data, error } = await sb.from('bookings').select('id, status').in('listing_id', part)
    if (error) {
    ...
    for (const row of data || []) {
      ...
      if (!allowMoneyStatuses && LEDGER_PROTECTED_BOOKING_STATUSES.has(st)) {
        console.warn(
          `[cleanup-test-data] AUDIT_LEDGER_01: skip money booking ${row.id} status=${st}`,
        )
        continue
      }
      bookingIds.add(String(row.id))
    }
  }
  return [...bookingIds]
}
 ...
 * @param {{ dryRun?: boolean, protectListingIds?: string[], maxStorageDeletes?: number, maxProfiles?: number }} [opts]
 */
export async function runCleanupTestData(sb, opts = {}) {
  ...
  let ledgerPurgeOk = false
  let ledgerPurge = null
  if (!dryRun) {
    ledgerPurge = await purgeTestLedgerRows(sb, { scope: 'markers' })
    ledgerPurgeOk = ledgerPurge.success === true
  }
  const listingBookingIds = listingIds.length
    ? await fetchBookingIdsForListings(sb, listingIds, { allowMoneyStatuses: ledgerPurgeOk })
    : []
  const neverPaidIds = await fetchNeverPaidCleanupBookingIds(sb)
  const bookingIds = uniq([...listingBookingIds, ...neverPaidIds])
  ...
  const userPreview = await runAggressiveTestUserCleanup(sb, {
    dryRun: true,
  ...
  const userReport = await runAggressiveTestUserCleanup(sb, {
    dryRun: false,
    maxProfiles: opts.maxProfiles,
  })
  ...
  if (listingIds.length) {
    report.deleted.calendar_blocks += await safeDeleteIn(
      sb,
      'calendar_blocks',
      'listing_id',
      listingIds,
  ...
  const holdPurge = await purgeExpiredCalendarHoldBlocks(sb)
  report.deleted.calendar_blocks += holdPurge.deleted
