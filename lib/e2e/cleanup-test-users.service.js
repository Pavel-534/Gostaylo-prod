...
import { purgeTestLedgerRows } from './purge-test-ledger.js'
import { BOOKING_FINANCE_DASHBOARD_STATUSES } from '../booking/status-sets.js'

const CHUNK = 150
/** Fail-closed skip if purge_test_ledger_rows(markers) fails. After success, smoke money rows may go. */
const LEDGER_PROTECTED_BOOKING_STATUSES = new Set([
...
async function fetchBookingIdsForProfiles(sb, profileIds, listingIds) {
  ...
  for (const part of chunks(profileIds)) {
    const { data: br } = await sb.from('bookings').select('id, status').in('renter_id', part)
    for (const row of br || []) if (row?.id) bookingIds.add(String(row.id))
    const { data: bp } = await sb.from('bookings').select('id, status').in('partner_id', part)
    for (const row of bp || []) if (row?.id) bookingIds.add(String(row.id))
  }
  for (const part of chunks(listingIds)) {
    const { data: bl } = await sb.from('bookings').select('id, status').in('listing_id', part)
...
/**
 * Split booking ids into deletable vs ledger-protected (money statuses).
 * @returns {Promise<{ deletableIds: string[], protectedIds: string[] }>}
 */
async function partitionBookingsForLedgerSafeCleanup(sb, bookingIds) {
  const deletableIds = []
  const protectedIds = []
  for (const part of chunks(bookingIds)) {
    if (!part.length) continue
    const { data, error } = await sb.from('bookings').select('id, status').in('id', part)
    if (error) {
      console.warn('[cleanup-test-users] booking status partition:', error.message)
      // Fail closed: skip all deletes for this chunk
      protectedIds.push(...part)
      continue
    }
    const byId = new Map((data || []).map((r) => [String(r.id), String(r.status || '').toUpperCase()]))
    for (const id of part) {
      const st = byId.get(String(id)) || ''
      if (LEDGER_PROTECTED_BOOKING_STATUSES.has(st)) protectedIds.push(String(id))
      else deletableIds.push(String(id))
    }
  }
  if (protectedIds.length) {
    console.warn(
      `[cleanup-test-users] AUDIT_LEDGER_01: skipping ${protectedIds.length} money-status booking(s) (append-only ledger)`,
    )
  }
...
async function deletePartnerLedgerAccounts(sb, profileIds) {
  // Live append-only stays on. Orphan partner accounts are dropped by purge_test_ledger_rows.
  void sb
  void profileIds
  return { entriesDeleted: 0, accountsDeleted: 0 }
}

/**
 * @param {import('@supabase/supabase-js').SupabaseClient} sb
 * @param {{ dryRun?: boolean, protectedEmails?: string[], maxProfiles?: number }} [opts]
 */
export async function runAggressiveTestUserCleanup(sb, opts = {}) {
  const dryRun = opts.dryRun !== false
  const collected = await collectAggressiveTestProfileIds(sb, opts)
  const maxProfiles = Math.max(0, Math.floor(Number(opts.maxProfiles) || 0))
  const profileIds =
    maxProfiles > 0 ? collected.profileIds.slice(0, maxProfiles) : collected.profileIds
  const candidates =
    maxProfiles > 0 ? collected.candidates.slice(0, maxProfiles) : collected.candidates

  const report = {
    dryRun,
    profileCount: profileIds.length,
    profileCountTotal: collected.profileIds.length,
    candidates,
    listingCount: 0,
    bookingCount: 0,
    conversationCount: 0,
  ...
  const bookingIdsAll = await fetchBookingIdsForProfiles(sb, profileIds, listingIds)
  const ledgerPurge = dryRun ? { success: true } : await purgeTestLedgerRows(sb, { scope: 'markers' })
  const { deletableIds: bookingIds, protectedIds: protectedBookingIds } = ledgerPurge.success
    ? { deletableIds: bookingIdsAll, protectedIds: [] }
    : await partitionBookingsForLedgerSafeCleanup(sb, bookingIdsAll)
  ...
  report.bookingCount = bookingIdsAll.length
  report.conversationCount = conversationIds.length
  report.ledgerProtectedBookings = protectedBookingIds.length
  report.ledgerPurge = ledgerPurge
  ...
  if (ledgerPurge.success) {
    report.deleted.ledger_entries = Number(ledgerPurge.entries || 0)
    report.deleted.ledger_journals = Number(ledgerPurge.journals || 0)
    report.deleted.ledger_accounts = Number(ledgerPurge.partnerAccounts || 0)
  } else {
    if (journalIds.length) {
      console.warn(
        `[cleanup-test-users] AUDIT_LEDGER_01: purge failed; skipping ${journalIds.length} ledger journal id(s)`,
      )
    }
    report.deleted.ledger_entries = 0
    report.deleted.ledger_journals = 0
  ...
  for (const part of chunks(profileIds)) {
    await safeDeleteIn(sb, 'referral_attributions', 'referrer_id', part)
    await safeDeleteIn(sb, 'referral_attributions', 'converted_profile_id', part)
    await safeDeleteIn(sb, 'referrals', 'referrer_id', part)
    await safeDeleteIn(sb, 'referrals', 'referred_id', part)
    await safeDeleteIn(sb, 'activity_log', 'user_id', part)
    await safeDeleteIn(sb, 'telegram_link_codes', 'user_id', part)
    await safeDeleteIn(sb, 'messages', 'sender_id', part)
    await safeDeleteIn(sb, 'conversations', 'renter_id', part)
    await safeDeleteIn(sb, 'conversations', 'partner_id', part)
  }
