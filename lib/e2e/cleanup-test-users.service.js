...
import { BOOKING_FINANCE_DASHBOARD_STATUSES } from '@/lib/booking/status-sets.js'

const CHUNK = 150
/** AUDIT_LEDGER_01 C-L3 — never DELETE money-pipe bookings (journals are append-only). */
...
async function deletePartnerLedgerAccounts(sb, profileIds) {
  // AUDIT_LEDGER_01 C-L4: ledger_entries are append-only — do not DELETE.
  // Partner accounts with entries remain (RESTRICT); orphan SYSTEM accounts are never deleted here.
  void sb
  console.warn(
    `[cleanup-test-users] AUDIT_LEDGER_01: skipping ledger_accounts/entries delete for ${profileIds.length} profile(s)`,
  )
...
export async function runAggressiveTestUserCleanup(sb, opts = {}) {
  ...
  const { deletableIds: bookingIds, protectedIds: protectedBookingIds } =
  ...
  // AUDIT_LEDGER_01 C-L4: never DELETE journals/entries (append-only triggers)
  if (journalIds.length) {
    console.warn(
      `[cleanup-test-users] AUDIT_LEDGER_01: skipping delete of ${journalIds.length} ledger journal id(s)`,
    )
  }
