 ...
 */
export async function runStage120AttributionChainFixture() {
  ...
  const { data: catRow } = await supabaseAdmin.from('categories').select('id').limit(1).maybeSingle();
  if (!catRow?.id) throw new Error('E2E_120_NO_CATEGORY');
  ...
  if (dist.skipped && dist.reason !== 'ALREADY_EARNED') {
