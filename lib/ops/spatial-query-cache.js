...
/**
 * Stage 177.5.0 — polygon spatial cache key (fingerprint from discovery-geo-polygon).
 * @param {string} fingerprint
 * @param {string} [suffix]
 */
export function spatialPolygonCacheKey(fingerprint, suffix = '') {
  return `polygon:${String(fingerprint || '').slice(0, 32)}${suffix ? `:${suffix}` : ''}`
}
