...
import { runGatewayLedgerReconciliation } from '@/lib/services/finances/gateway-reconciliation.js'
import { detectSettleOrphans } from '@/lib/services/payout-batch/payout-batch-settle-two-phase.js'
...
/**
 * @returns {Promise<{ pendingFiscalCount: number, ledgerDriftPartners: Array<object>, alertsSent: string[], settleOrphans?: object, staleCron?: object }>}
 */
export async function runFinancialHealthScan() {
  ...
  const gatewayReconcile = await runGatewayLedgerReconciliation()
  if (gatewayReconcile?.alertSent) {
    alertsSent.push('GATEWAY_LEDGER_DRIFT')
  }

  const settleOrphans = await detectSettleOrphans({ limit: 100 })
  if (settleOrphans?.orphans?.length) {
    alertsSent.push('SETTLE_ORPHAN')
  }

  const staleCron = await runStaleCronMonitor()
  if (staleCron?.stale?.length) {
    alertsSent.push('STALE_CRON')
  }
  ...
  return {
    ...
    gatewayReconcile,
    settleOrphans: {
      scanned: settleOrphans.scanned || 0,
      orphanCount: settleOrphans.orphans?.length || 0,
      error: settleOrphans.error || null,
      sample: (settleOrphans.orphans || []).slice(0, 5),
    },
    staleCron: {
      checked: staleCron.checked || 0,
      staleCount: staleCron.stale?.length || 0,
      alertsSent: staleCron.alertsSent || [],
      sample: (staleCron.stale || []).slice(0, 5),
      error: staleCron.error || null,
    },
    alertsSent,
  }
}
