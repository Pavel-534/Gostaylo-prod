...
/** Пользовательский семантический поиск (эмбеддинг запроса) */
export const TASK_SEARCH_QUERY = 'search_query'
...
export async function getAiUsageDashboardData(period, opts = {}) {
  ...
  for (;;) {
    ...
    for (const r of chunk) {
      ...
      else if (tt === TASK_SEARCH_QUERY) searchQueryUsd += v
    }
  ...
  return {
    ...
    searchQueryUsd: Math.round(searchQueryUsd * 1_000_000) / 1_000_000,
    requestCount,
    recent: recentRows || [],
    error: aggError,
    period,
