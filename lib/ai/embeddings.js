...
import { isListingStatusEligibleForEmbedding } from '@/lib/ai/listing-embedding-policy'

export { LISTING_STATUSES_ELIGIBLE_FOR_EMBEDDING } from '@/lib/ai/listing-embedding-policy'
...
export async function updateListingEmbedding(listingId, semanticOverride = null) {
  ...
  const { data: row, error: fetchErr } = await supabaseAdmin
    .from('listings')
    .select(
      'id, owner_id, status, title, description, district, category_id, categories ( name, slug )',
    )
    .eq('id', id)
    .maybeSingle()
  ...
  if (!isListingStatusEligibleForEmbedding(row.status)) {
    return { ok: false, error: 'status_not_eligible_for_embedding', skipped: true }
  }
