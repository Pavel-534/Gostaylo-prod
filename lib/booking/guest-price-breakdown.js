...
export function buildGuestPriceBreakdownFromBooking(booking) {
  ...
  const taxAmountThb = n(fs.tax_amount_thb ?? taxSnap.amount_thb)
  const taxRatePercent = n(fs.tax_rate_percent ?? taxSnap.rate_percent)
 ...
 * @param {number} [p.taxAmountThb]
 * @param {number} [p.taxRatePercent]
...
export function buildGuestPriceBreakdownFromCheckoutTotals(p) {
  ...
  const taxAmountThb = n(p?.taxAmountThb)
  const taxRatePercent = n(p?.taxRatePercent)
