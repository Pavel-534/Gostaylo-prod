...
export const SYSTEM_BOOKING_STATUS_TRANSITIONS = Object.freeze({
  ...
  /** Check-in does not replace thaw — system cron may CHECKED_IN → THAWED when escrow_thaw_at due */
  CHECKED_IN: ['THAWED', 'COMPLETED', 'REFUNDED'],
  THAWED: ['READY_FOR_PAYOUT', 'COMPLETED', 'REFUNDED'],
  READY_FOR_PAYOUT: ['COMPLETED', 'REFUNDED'],
})
...
/**
 * Stage 125.3 — PAID_ESCROW may only be reached via EscrowService.moveToEscrow (RPC), not FSM UPDATE.
 * Callers pass `metadata.viaEscrowRpc: true` (set from escrow downstream only).
 */
export const DIRECT_PAID_ESCROW_TRANSITION_ERROR = 'direct_paid_escrow_transition_forbidden'

/**
 * @param {string} to
 * @param {{ viaEscrowRpc?: boolean }} [metadata]
 */
export function validatePaidEscrowFsmGuard(to, metadata = {}) {
  const toSt = String(to || '').toUpperCase()
  if (toSt !== 'PAID_ESCROW') return { ok: true }
  if (metadata.viaEscrowRpc === true) return { ok: true }
  return {
    ok: false,
    error: DIRECT_PAID_ESCROW_TRANSITION_ERROR,
    message:
      'Переход в PAID_ESCROW допускается только через EscrowService.moveToEscrow (RPC capture)',
  }
}
 ...
 * @param {string} from
 * @param {string} to
 * @param {{ checkedInAt?: string, updatedAt?: string, extraPatch?: object, viaEscrowRpc?: boolean }} [metadata]
 */
export function validateSystemBookingStatusTransition(from, to, metadata = {}) {
  const paidEscrowGuard = validatePaidEscrowFsmGuard(to, metadata)
  if (!paidEscrowGuard.ok) {
    return paidEscrowGuard
  }
