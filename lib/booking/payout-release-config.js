...
/**
 * Guest-facing checkout success body (escrow protection, no legacy check-out copy).
 * @param {EscrowThawBucket} bucket
 * @param {PayoutReleaseLang} lang
 * @returns {string}
 */
function buildGuestEscrowSuccessText(bucket, lang) {
  const brand = getSiteDisplayName()
  if (lang === 'en') {
    return `Payment successful. Your funds are securely protected by ${brand} escrow until your booking begins.`
  }
  if (lang === 'zh') {
    return `付款成功。您的资金由 ${brand} 托管保护，直至预订开始。`
  }
  if (lang === 'th') {
    return `ชำระเงินสำเร็จ เงินของคุณได้รับการคุ้มครองโดย escrow ของ ${brand} จนกว่าจะเริ่มการจอง`
  }
  return `Оплата успешна. Средства надёжно защищены эскроу-системой ${brand} до начала бронирования.`
}

/**
 * Guest-facing escrow timing on checkout success step 3 (category-aware SSOT).
 * @param {EscrowThawBucket} bucket
 * @param {PayoutReleaseLang} lang
 * @returns {string}
 */
function buildGuestEscrowStep3Text(bucket, lang) {
  const brand = getSiteDisplayName()
  if (bucket === 'housing') {
    if (lang === 'en') {
      return `Your payment stays in ${brand} escrow until check-in. Partner payout follows platform rules: release the day after check-in (6 PM), then a ${PAYOUT_HOLD_HOURS}-hour security hold.`
    }
    if (lang === 'zh') {
      return `付款由 ${brand} 托管保护至入住。合作伙伴收款按平台规则：入住次日 18:00 解冻，再等待 ${PAYOUT_HOLD_HOURS} 小时。`
    }
    if (lang === 'th') {
      return `เงินของคุณอยู่ใน escrow ของ ${brand} จนกว่าจะเช็คอิน การจ่ายพาร์ทเนอร์ตามกติกาแพลตฟอร์ม: ปลดวันถัดจากเช็คอิน (18:00) แล้วรอ ${PAYOUT_HOLD_HOURS} ชม.`
    }
    return `Ваши деньги защищены эскроу ${brand} до заселения. Выплата партнёру — на следующий день после заезда (18:00), затем ${PAYOUT_HOLD_HOURS} ч ожидания.`
  }
  if (bucket === 'transport') {
    if (lang === 'en') {
      return `Your payment stays in ${brand} escrow until the rental starts. Partner payout follows platform rules after service begins.`
    }
    if (lang === 'zh') {
      return `付款由 ${brand} 托管保护至租赁开始。合作伙伴收款按平台规则在服务开始后处理。`
    }
    if (lang === 'th') {
      return `เงินของคุณอยู่ใน escrow ของ ${brand} จนกว่าจะเริ่มเช่า การจ่ายพาร์ทเนอร์ตามกติกาแพลตฟอร์มหลังเริ่มบริการ`
    }
    return `Ваши деньги защищены эскроу ${brand} до начала аренды. Выплата партнёру — по правилам платформы после начала бронирования.`
  }
  if (lang === 'en') {
    return `Your payment stays in ${brand} escrow until the service begins. Partner payout follows platform rules (release 2h after start, then ${PAYOUT_HOLD_HOURS}h hold).`
  }
  if (lang === 'zh') {
    return `付款由 ${brand} 托管保护至服务开始。合作伙伴收款按平台规则（开始后 2 小时解冻，再等待 ${PAYOUT_HOLD_HOURS} 小时）。`
  }
  if (lang === 'th') {
    return `เงินของคุณอยู่ใน escrow ของ ${brand} จนกว่าจะเริ่มบริการ การจ่ายพาร์ทเนอร์ตามกติกา (ปลด 2 ชม. หลังเริ่ม แล้วรอ ${PAYOUT_HOLD_HOURS} ชม.)`
  }
  return `Ваши деньги защищены эскроу ${brand} до начала услуги. Выплата партнёру — через 2 ч после начала, затем ${PAYOUT_HOLD_HOURS} ч ожидания.`
}
...
function buildUiTextCatalog(bucket) {
  ...
  for (const lang of LANGS) {
    ...
    out[lang] = {
      ...
      guestEscrowSuccess: buildGuestEscrowSuccessText(bucket, lang),
      guestEscrowStep3: buildGuestEscrowStep3Text(bucket, lang),
    }
  }
  return out
}
...
export function getPayoutReleaseConfig(bookingOrContext) {
  ...
  if (thawAtRaw) {
    ...
    const iso = computeEscrowThawAt({
      ...
      listingMetadata:
        bookingOrContext?.listing?.metadata ??
        (bookingOrContext?.listingMetadata && typeof bookingOrContext.listingMetadata === 'object'
          ? bookingOrContext.listingMetadata
          : null),
    })
