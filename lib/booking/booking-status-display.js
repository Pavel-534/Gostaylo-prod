 ...
 * Переходы статусов — `lib/booking/status-transitions.js` (CHECKED_IN ≠ THAWED).
 */

import { BookingStatus } from '@/lib/services/escrow/constants.js'
import {
  ESCROW_OR_PAYOUT_PIPELINE_STATUSES,
  getBookingStatusOwnerHintRu,
...
/** @typedef {'PENDING'|'INQUIRY'|'CONFIRMED'|'AWAITING_PAYMENT'|'PAID'|'PAID_ESCROW'|'CHECKED_IN'|'THAWED'|'THAW_HOLD'|'DISPUTED'|'READY_FOR_PAYOUT'|'COMPLETED'|'CANCELLED'|'REFUNDED'} BookingUiStatus */
...
export const BOOKING_STATUS_DISPLAY = {
  ...
  INQUIRY: {
    labelKey: 'bookingStatus_inquiry',
    badgeClass: 'bg-violet-100 text-violet-800 border-violet-200',
    defaultRu: 'Запрос',
    defaultEn: 'Inquiry',
  },
  AWAITING_PAYMENT: {
    labelKey: 'bookingStatus_awaitingPayment',
    badgeClass: 'bg-amber-50 text-amber-900 border-amber-200',
    defaultRu: 'Ожидает оплаты',
    defaultEn: 'Awaiting payment',
  },
  PAID_ESCROW: {
  ...
  CHECKED_IN: {
    labelKey: 'bookingStatus_checkedIn',
    badgeClass: 'bg-sky-100 text-sky-900 border-sky-200',
    defaultRu: 'Гость заехал',
    defaultEn: 'Checked in',
  },
  THAWED: {
...
export function resolveBookingStatusBadge(booking, opts = {}) {
  ...
  const ownerHintRu =
    opts.ownerHint === false ? null : getBookingStatusOwnerHintRu(uiStatus)
  return {
    uiStatus,
...
export { getBookingStatusOwnerHintRu }
...
export { ESCROW_OR_PAYOUT_PIPELINE_STATUSES }
