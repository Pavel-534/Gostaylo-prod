...
import { STALE_UNPAID_STATUSES, isNeverPaidCleanupBooking } from '@/lib/e2e/never-paid-booking-cleanup.js'
...
/**
 * Stage 201.09 — cancel unpaid INQUIRY/PENDING/CONFIRMED/AWAITING_PAYMENT after check-out date.
 */
export async function processStaleUnpaidPastCheckout(options = {}) {
  const todayYmd = String(options.todayYmd || new Date().toISOString().slice(0, 10))
  const trigger = String(options.trigger || 'cron_stale_unpaid_past_checkout')
  const limit = Math.max(1, Math.min(500, Math.floor(Number(options.limit) || 200)))

  const { data: rows, error } = await supabaseAdmin
    .from('bookings')
    .select('id, status, check_out')
    .in('status', STALE_UNPAID_STATUSES)
    .limit(limit)

  if (error) {
    return { success: false, error: error.message || 'STALE_UNPAID_READ_FAILED', scanned: 0, cancelled: 0 }
  }
  ...
  for (const row of rows || []) {
    if (!isNeverPaidCleanupBooking(row, todayYmd)) {
      skipped += 1
      continue
    }
    ...
    const statusResult = await BookingService.updateStatus(row.id, 'CANCELLED', {
      reason: 'auto_expired_past_checkout_unpaid',
      referralTrigger: trigger,
      bookingStatusScope: 'cancel',
    })
    if (!statusResult?.success) {
