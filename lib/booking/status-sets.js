...
/**
 * AUDIT_MONEY_FLOW_04 P2 — cron thaw may leave CHECKED_IN (guest checked in; money still escrow).
 * CHECKED_IN ≠ THAWED: check-in is ops signal; thaw still required for payout path.
 */
export const ESCROW_THAW_SOURCE_STATUSES = Object.freeze([S.PAID_ESCROW, S.CHECKED_IN])

/** @param {string | null | undefined} status */
export function isEscrowThawSourceStatus(status) {
  return ESCROW_THAW_SOURCE_STATUSES.includes(String(status || '').toUpperCase())
}
...
/**
 * Partner finances + dashboard «Ожидает оплаты»: гость ещё не заплатил.
 * PENDING / CONFIRMED = заявка / подтверждение до checkout; AWAITING_PAYMENT = окно оплаты.
 * SSOT for `partner-finances-summary` and `GET /api/v2/partner/stats` (Stage 201.06).
 */
export const PARTNER_PENDING_PAYMENT_BOOKING_STATUSES = new Set([
  S.PENDING,
  S.CONFIRMED,
  S.AWAITING_PAYMENT,
])
...
/** Unified order card: гость может отменить (pre-pay + ledger-refund pipeline). Stage 202.16 */
