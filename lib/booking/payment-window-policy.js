...
const requireTranslations = createRequire(import.meta.url)
...
export function buildInvoicePaymentWindowSystemMessage(categorySlug, lang = 'ru') {
  // Lazy-load so pure hold/TTL helpers stay importable in unit tests without i18n graph.
  const { getUIText } = requireTranslations('../translations/index.js')
