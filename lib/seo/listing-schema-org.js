...
export function buildListingPrimaryJsonLdWithoutContext(listing, baseUrl, telephone) {
  ...
  if (shape === 'lodging') {
    ...
    const locality =
      district ||
      (typeof meta.city_label === 'string' && meta.city_label.trim()) ||
      (typeof meta.city === 'string' && meta.city.trim()) ||
      listing?.city_code ||
      listing?.region_code ||
      undefined
    const postalCode = district ? phuketPostalCodeForDistrict(district) : undefined
    const { mapLocationDisplayMode } = resolveCategoryBehavior(
      slug,
      listing?.categories?.wizard_profile ?? null,
    )
    const streetAddress = lodgingStreetAddressStub(listing, { mapLocationDisplayMode })
    ...
    const floor =
      Number.isFinite(areaN) && areaN > 0
        ...
        : undefined
    const addressRegion =
      (typeof meta.parent_location === 'string' && meta.parent_location.trim()) ||
      listing?.region_code ||
      undefined
    const addressCountry = listing?.country_code || meta.country_code || undefined
