...
/**
 * Stage 86.0: `@graph` = основная сущность + BreadcrumbList для ИИ/гео-иерархии.
 *
 * @param {object} listing
 * @param {string} baseUrl
 * @param {string|null} telephone
 */
export function buildListingStructuredDataPayload(listing, baseUrl, telephone) {
  const primary = buildListingPrimaryJsonLdWithoutContext(listing, baseUrl, telephone)
  const crumbs = buildListingBreadcrumbJsonLd(listing, baseUrl)
