...
export async function buildListingDetailOgMetadata({
  ...
  // Stage 200.71 — apex canonical (SSOT), trailing slash; OG url may still use request baseUrl
  const apex = getPublicSiteUrl().replace(/\/$/, '')
  const canonical = `${apex}/listings/${listingId}/`
  const ogUrl = `${String(baseUrl).replace(/\/$/, '')}/listings/${listingId}/`
