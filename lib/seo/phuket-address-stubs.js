...
/**
 * @param {{ address?: string | null, district?: string | null, metadata?: object }} listing
 * @param {{ mapLocationDisplayMode?: 'privacy' | 'exact' }} [options]
 * @returns {string} streetAddress для Schema.org PostalAddress
 */
export function lodgingStreetAddressStub(listing, options = {}) {
  const mode = String(options.mapLocationDisplayMode || 'privacy').toLowerCase()
  const privacy = mode !== 'exact'
