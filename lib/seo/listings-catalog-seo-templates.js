 ...
 * Перевод: `tr(key)` (= `t(lang)(key)` / `getUIText`); плейсхолдер `{brand}` в шаблонах подставляется **`getSiteDisplayName()`**; также `{categoryName}`, `{location}`, `{count}`.
 */

import { normalizeCategoryWizardProfileColumn } from '@/lib/config/category-wizard-profile-db'
import { resolveCatalogSeoPlaceContext } from '@/lib/seo/catalog-seo-geo'
 ...
 */
async function buildPlaceStrings(lang, whereRaw, tr) {
  const w =
  ...
  const lng = ['ru', 'en', 'zh', 'th'].includes(lang) ? lang : 'en'
  const ctx = await resolveCatalogSeoPlaceContext(w, lng)
  const whereDisp = ctx.whereDisplay || w

  let titleKey = 'catalogSeo_place_title_where_generic'
  let descKey = 'catalogSeo_place_desc_where_generic'
  if (ctx.market === 'ru') {
    titleKey = 'catalogSeo_place_title_where_ru'
    descKey = 'catalogSeo_place_desc_where_ru'
  } else if (ctx.market === 'th') {
    titleKey = 'catalogSeo_place_title_where_th'
    descKey = 'catalogSeo_place_desc_where_th'
  } else if (ctx.market === 'generic') {
    titleKey = 'catalogSeo_place_title_where_generic'
    descKey = 'catalogSeo_place_desc_where_generic'
  } else {
    titleKey = 'catalogSeo_place_title_where'
    descKey = 'catalogSeo_place_desc_where'
  }

  return {
    locationTitle: interpolateCatalogSeoTemplate(tr(titleKey), { where: whereDisp }),
    locationDesc: interpolateCatalogSeoTemplate(tr(descKey), { where: whereDisp }),
  }
}
 ...
 */
export async function buildCatalogSeoFromProfile(lang, where, categoryName, profileKey, totalCount, tr = t(lang)) {
  const name =
    (categoryName && String(categoryName).trim()) ||
    tr('catalogSeo_fallback_categoryName') ||
    'Listings'

  const { locationTitle, locationDesc } = await buildPlaceStrings(lang, where, tr)
