 ...
 * @param {{ skipHeavyBootstrap?: boolean }} [options] — Stage 201.104: skip catalog search (Search tab).
 */
export async function buildListingsCatalogMetadata(searchParams, bootstrap = null, options = {}) {
  let baseUrl
  try {
  ...
  const skipHeavy = options.skipHeavyBootstrap === true
  const boot =
    bootstrap ||
    (skipHeavy ? null : await getCatalogBootstrapFromSearchParams(searchParams))
  const catRows = boot?.categorySeoRows ?? (await fetchCategoriesSeoSnapshot())
  const listingCount = skipHeavy
    ? (boot?.listingCountForSeo ?? null)
    : (boot?.listingCountForSeo ?? (await fetchListingsCountForCatalogSeo(searchParams)))

  const { title, description } = await getListingsCatalogTitleAndDescriptionWithRows(
    lang,
    searchParams,
    catRows,
    listingCount,
