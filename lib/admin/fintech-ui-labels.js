...
/** Режим владельца — без англ. кодов в бейджах пулов. */
export const BATCH_STATUS_OWNER_RU = {
  DRAFT: 'Черновик пула',
  LOCKED: 'Готов к переводу в банк',
  EXPORTED: 'Файл для банка скачан',
  SETTLED: 'Выплата завершена',
  FAILED: 'Ошибка выплаты',
  CANCELLED: 'Отменён',
}

export function resolveBatchStatusLabel(status, ownerMode = false) {
  const key = String(status || '').toUpperCase()
  if (ownerMode) return BATCH_STATUS_OWNER_RU[key] || BATCH_STATUS_RU[key] || key
  return BATCH_STATUS_RU[key] || key
}
...
/** Режим владельца — без технических кодов рельсов. */
export const PAYOUT_RAIL_LABELS_OWNER = {
  TBANK_RU: 'Выплаты в рублях (Россия)',
  KG_CRYPTO: 'Международные (USDT)',
}

export function resolvePayoutRailLabel(rail, ownerMode = false) {
  const key = String(rail || '')
  if (ownerMode) return PAYOUT_RAIL_LABELS_OWNER[key] || PAYOUT_RAIL_LABELS[key] || key
  return PAYOUT_RAIL_LABELS[key] || key
}
