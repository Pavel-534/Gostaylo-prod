...
export function buildChatSafetySettingsPatch(body, prev = {}) {
  ...
  merged.searchRankPenaltyEnabled = merged.searchRankPenaltyEnabled !== false
  const prs = parseFloat(String(merged.searchRankPenaltyScore ?? ''))
  merged.searchRankPenaltyScore =
    Number.isFinite(prs) && prs > 0 ? prs : defaultChatSafety.searchRankPenaltyScore
