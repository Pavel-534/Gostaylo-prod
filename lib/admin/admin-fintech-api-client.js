  ...
  return dedupeClientRequest(
    ...
    async () => {
      ...
      return {
        ...
        yookassaOps: opsJson.success ? opsJson.data?.yookassaOps || null : null,
        ...
        preLiveReadiness: opsJson.success ? opsJson.data?.preLiveReadiness || null : null,
        liveMonitoring: opsJson.success ? opsJson.data?.liveMonitoring || null : null,
        errors: [],
      }
    },
...
export async function patchFintechPayoutBatch(id, action) {
  ...
  if (action === 'settled' || action === 'lock') {
    headers['Idempotency-Key'] =
      typeof crypto !== 'undefined' && crypto.randomUUID
        ? crypto.randomUUID()
        : `pb-${action}-${id}-${Date.now()}`
  }
...
export async function postFintechConversion(body) {
  const headers = { 'Content-Type': 'application/json' }
  const key = body?.idempotencyKey ? String(body.idempotencyKey).trim() : ''
...
/** Stage 131.0 — GET system_fintech_settings + waterfall preview. */
export async function fetchFintechSettings() {
  const res = await fetch('/api/admin/finances/fintech-settings', { cache: 'no-store', credentials: 'include' })
  const json = await readJson(res)
  return {
    ok: res.ok && json.success === true,
    data: json.data ?? null,
    error: json.error || json.message || (res.ok ? null : `HTTP ${res.status}`),
    message: json.message || null,
  }
}

/** Stage 131.1 — shadow L2 liability dashboard. */
export async function fetchShadowL2Liability() {
  const res = await fetch('/api/admin/finances/shadow-l2', { cache: 'no-store', credentials: 'include' })
  const json = await readJson(res)
  return {
    ok: res.ok && json.success === true,
    data: json.data ?? null,
...
/** Stage 131.0 — PUT system_fintech_settings (validated). */
export async function putFintechSettings(patch) {
  const res = await fetch('/api/admin/finances/fintech-settings', {
    method: 'PUT',
    credentials: 'include',
    headers: { 'Content-Type': 'application/json' },
