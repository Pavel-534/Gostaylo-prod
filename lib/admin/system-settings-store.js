 ...
 * Stage 202.42 — transient retry on reads only (Gateway Timeout / 502–504).
 */
...
export async function readSystemSettingsByKeys(keys) {
  ...
  const { data, error } = await withTransientSupabaseRead(
    () =>
      supabaseAdmin
        .from('system_settings')
        .select('id,key,value,updated_at')
        .in('key', unique),
    { attempts: 2, baseDelayMs: 250, label: 'system-settings-read' },
  )
