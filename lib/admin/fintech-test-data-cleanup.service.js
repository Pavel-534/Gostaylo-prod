...
import { pruneTreasuryOpsAlerts } from '@/lib/treasury/treasury-monitoring-alerts.js'
...
async function collectSmokeProfileIds(sb) {
  const ids = new Set()
  const { data: byId } = await sb.from('profiles').select('id, email, first_name, last_name, role').ilike('id', 'user-smoke-%')
  const { data: byEmail } = await sb
    .from('profiles')
    .select('id, email, first_name, last_name, role')
    .ilike('email', '%@smoke.invalid')
  const { data: byTestDomain } = await sb
    .from('profiles')
    .select('id, email, first_name, last_name, role')
    .ilike('email', '%@test.gostaylo.invalid')
  for (const row of [...(byId || []), ...(byEmail || []), ...(byTestDomain || [])]) {
    if (isFintechTestProfileRow(row)) ids.add(String(row.id))
  }
  try {
    const { collectAggressiveTestProfileIds } = await import('@/lib/e2e/cleanup-test-users.service.js')
    const { profileIds } = await collectAggressiveTestProfileIds(sb)
    for (const id of profileIds) ids.add(id)
  } catch {
    /* Next/webpack path only */
  }
  return [...ids]
}
 ...
 */
export async function collectTestBatchIds(sb, bookingIds) {
 ...
 */
export async function enrichTestBatchIds(sb, bookingIds, profileIds, batchIds) {
 ...
 * @param {string[]} profileIds
 */
export async function collectTestPayoutIds(sb, profileIds) {
  const ids = new Set()
 ...
 */
export async function collectAllTestLedgerJournalIds(sb, ctx) {
...
async function buildCleanupPlan(sb, opts = {}) {
  ...
  const { loadRecentTreasuryOpsAlerts } = await import('@/lib/treasury/treasury-monitoring-alerts.js')
  const recentTreasuryAlerts = await loadRecentTreasuryOpsAlerts(200)
  const treasuryAlertsWouldRemove = recentTreasuryAlerts.filter((a) =>
