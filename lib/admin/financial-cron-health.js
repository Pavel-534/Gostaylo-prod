...
export const FINANCIAL_CRON_HEALTH_JOBS = Object.freeze([
  ...
  },
  {
    ...
    label: 'Heal escrow (reconcile-confirmed-payments)',
    labelOwner: 'Дожим оплаты в эскроу',
    staleHours: 2,
  },
  {
    jobName: 'payout-batch-pools',
    label: 'Черновик пула выплат (пн/чт)',
    labelOwner: 'Формирование пула выплат партнёрам',
    staleHours: 96,
  },
  {
    jobName: 'ledger_shadow_reconcile',
    label: 'ADR-203 ledger shadow reconcile',
    labelOwner: 'Сверка status vs ledger (shadow)',
    staleHours: 26,
  },
  {
    jobName: 'financial-health-monitor',
    label: 'Мониторинг финансов (fiscal + ledger)',
    labelOwner: 'Проверка кассы и расхождений ledger',
    staleHours: 30,
  },
  {
    jobName: 'exchange-rates-refresh',
    label: 'Обновление курсов валют',
    labelOwner: 'Актуальные курсы для витрины и выплат',
    staleHours: 30,
  },
  {
    jobName: 'referral-reconciliation',
    label: 'Сверка реферального ledger',
    labelOwner: 'Проверка реферальных начислений',
    staleHours: 30,
  },
  {
    jobName: 'referral-unlock',
    label: 'Разблокировка реферальных бонусов (hold)',
    labelOwner: 'Перевод held-бонусов в доступные',
    staleHours: 30,
  },
])
...
export async function loadFinancialCronHealth(ownerLabels = false) {
  ...
  const lastAnyByJob = new Map()
  /** @type {Map<string, object>} */
  const lastSuccessByJob = new Map()
  for (const row of rows) {
    const key = row.job_name
    if (!lastAnyByJob.has(key)) lastAnyByJob.set(key, row)
    if (String(row.status || '') === 'success' && !lastSuccessByJob.has(key)) {
      lastSuccessByJob.set(key, row)
    }
  }

  const jobs = FINANCIAL_CRON_HEALTH_JOBS.map((def) => {
    const last = lastAnyByJob.get(def.jobName) || null
    const lastOk = lastSuccessByJob.get(def.jobName) || null
    // Freshness = last SUCCESS only (failed runs must not reset stale clock).
