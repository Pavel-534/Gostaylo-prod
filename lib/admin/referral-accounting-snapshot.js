...
import { resolveReferralSpendAlertFlags } from '@/lib/admin/referral-spend-alert-thresholds.js'
...
export async function loadReferralAccountingSnapshot(opts = {}) {
  ...
  const [monthEarnedRes, monthReservedRes, walletDebitsRes, heldRes] = await Promise.all([
    supabaseAdmin
  ...
  const monthlyReservedSpendThb = round2(
    (monthReservedRes?.data || []).reduce((acc, r) => acc + Number(r?.amount_thb || 0), 0),
  )
  ...
  const spendFlags = resolveReferralSpendAlertFlags(monthlyReservedSpendThb, alertPolicy)
  ...
  return {
    ...
    accounting: {
      ...
      totalWithdrawnThb,
      monthlyEarnedThb,
      monthlyReservedSpendThb,
      monthlyPaidActualThb: monthlyEarnedThb,
      monthlyWithdrawnThb,
      currentLiabilityThb,
      walletExposureThb,
      ...
      programCapThb: alertPolicy.programCapThb,
      programCapWarnPercent: alertPolicy.programCapWarnPercent,
      programCapWarnThb: alertPolicy.programCapWarnThb,
      monthlySpendPercent:
        alertPolicy.monthlySpendAlertThb > 0
          ? Math.min(100, Math.round((monthlyReservedSpendThb / alertPolicy.monthlySpendAlertThb) * 1000) / 10)
          : 0,
      monthlySpendRemainingThb: Math.max(0, round2(alertPolicy.monthlySpendAlertThb - monthlyReservedSpendThb)),
      monthlySpendAlertTriggered: spendFlags.earlyWarningTriggered,
      approachingCapTriggered: spendFlags.approachingCapTriggered,
      monthlySpendApproaching: spendFlags.approachingCapTriggered,
    },
  }
}
