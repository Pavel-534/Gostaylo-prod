 ...
 * Stage 201.57 — vehicle amenities from dictionary (not hardcoded parking/AC).
 */
import {
...
/** Особенности аренды транспорта (партнёрский редактор). */
export const VEHICLE_PARTNER_AMENITY_SLUGS = AMENITIES_DICTIONARY
  .filter((row) => row.categories.includes('vehicle'))
  .map((row) => row.slug)

/** Все slug'и, которые нормализуем при сохранении (жильё + туры + транспорт). */
export const AMENITY_SLUGS = [
  ...PROPERTY_PARTNER_AMENITY_SLUGS,
  ...TOUR_PARTNER_AMENITY_SLUGS,
