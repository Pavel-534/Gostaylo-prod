// Currency display utilities (symbols, list). FX numbers for UI come from
// GET /api/v2/exchange-rates → rateMap (THB per 1 unit); see TECHNICAL_MANIFESTO.md §1.

import { normalizeThbPerUnitRate } from '@/lib/finance/thb-per-unit-rate.js'

export const CURRENCIES = [
  { code: 'THB', symbol: '฿', name: 'Thai Baht', flag: '🇹🇭' },
  { code: 'USD', symbol: '$', name: 'US Dollar', flag: '🇺🇸' },
  { code: 'EUR', symbol: '€', name: 'Euro', flag: '🇪🇺' },
  { code: 'GBP', symbol: '£', name: 'British Pound', flag: '🇬🇧' },
  { code: 'RUB', symbol: '₽', name: 'Russian Ruble', flag: '🇷🇺' },
  { code: 'CNY', symbol: '¥', name: 'Chinese Yuan', flag: '🇨🇳' },
  { code: 'USDT', symbol: '₮', name: 'Tether', flag: '💎' },
  { code: 'JPY', symbol: '¥', name: 'Japanese Yen', flag: '🇯🇵' },
  { code: 'AUD', symbol: 'A$', name: 'Australian Dollar', flag: '🇦🇺' },
  { code: 'SGD', symbol: 'S$', name: 'Singapore Dollar', flag: '🇸🇬' },
  { code: 'KRW', symbol: '₩', name: 'Korean Won', flag: '🇰🇷' },
  { code: 'INR', symbol: '₹', name: 'Indian Rupee', flag: '🇮🇳' },
]

/** Header/footer currency switcher list (Stage 200.3 SSOT — no ad-hoc duplicates). */
export const UI_SWITCHER_CURRENCY_CODES = ['THB', 'USD', 'EUR', 'GBP', 'RUB', 'CNY', 'USDT']

export const UI_SWITCHER_CURRENCIES = UI_SWITCHER_CURRENCY_CODES.map(
  (code) => CURRENCIES.find((c) => c.code === code),
).filter(Boolean)

/** UI language code → BCP 47 for number grouping (thousands separators). */
export function languageToNumberLocale(language) {
  const l = String(language || 'en').toLowerCase().slice(0, 2)
  const map = { ru: 'ru-RU', en: 'en-US', zh: 'zh-CN', th: 'th-TH' }
  return map[l] || 'en-US'
}

/** ISO codes rendered as suffix (avoid ambiguous or broken glyphs — ฿→「B0」 on some Android). */
export const CURRENCY_CODE_SUFFIX_DISPLAY = new Set(['THB', 'USDT'])

/**
 * Format a native THB ledger amount as «1 234 THB» (no ฿ glyph).
 * @param {number} amountThb
 * @param {string} [language='en']
 */
export function formatThbAmountAsCode(amountThb, language = 'en') {
  if (amountThb == null || Number.isNaN(Number(amountThb))) {
    return '0 THB'
  }
  const rounded = Math.round(Number(amountThb))
  const formatted = rounded.toLocaleString(languageToNumberLocale(language), {
    minimumFractionDigits: 0,
    maximumFractionDigits: 0,
  })
  return `${formatted} THB`
}

/**
 * Format a native amount already in `currencyCode` (no FX conversion).
 * Stage 188.2 — ambassador hub isolation after rounded conversion.
 */
export function formatNativeAmountInCurrency(amount, currencyCode, language = 'en') {
  const code = String(currencyCode || 'THB').toUpperCase()
  const n = Number(amount)
  if (!Number.isFinite(n)) {
    if (code === 'THB') return '0 THB'
    const currencyInfo = CURRENCIES.find((c) => c.code === code)
    const symbol = currencyInfo?.symbol || code
    return `${symbol}0`
  }

  const maxFrac =
    code === 'JPY' || code === 'RUB' || code === 'USD' || code === 'USDT' ? 0 : 2
  const rounded = Math.round(n * 10 ** maxFrac) / 10 ** maxFrac
  const formatted = rounded.toLocaleString(languageToNumberLocale(language), {
    minimumFractionDigits: 0,
    maximumFractionDigits: maxFrac,
  })

  if (CURRENCY_CODE_SUFFIX_DISPLAY.has(code)) {
    return `${formatted} ${code}`
  }

  const currencyInfo = CURRENCIES.find((c) => c.code === code)
  const symbol = currencyInfo?.symbol || code
  return `${symbol}${formatted}`
}

/**
 * Format a THB amount for display in the selected currency.
 * @param {number} amountThb — value in THB (may be negative, e.g. discounts)
 * @param {string} [currency='THB']
 * @param {Record<string, number>} [exchangeRates] — THB per 1 unit (same as /api/v2/exchange-rates rateMap)
 * @param {string} [language='en'] — UI language for digit grouping (ru-RU spaces, en-US commas, etc.)
 */
export function formatPrice(amountThb, currency = 'THB', exchangeRates = {}, language = 'en') {
  const code = String(currency || 'THB').toUpperCase()

  if (amountThb == null || Number.isNaN(Number(amountThb))) {
    if (code === 'THB') return '0 THB'
    const currencyInfo = CURRENCIES.find((c) => c.code === code)
    const symbol = currencyInfo?.symbol || code
    return `${symbol}0`
  }

  let n = Number(amountThb)
  const rates = exchangeRates && typeof exchangeRates === 'object' ? exchangeRates : {}
  if (code !== 'THB' && rates[code] != null) {
    const rate = normalizeThbPerUnitRate(code, Number(rates[code]))
    if (rate != null && rate > 0) {
      n = n / rate
    }
  }

  const maxFrac = code === 'JPY' ? 0 : 2
  const rounded = Math.round(n * 10 ** maxFrac) / 10 ** maxFrac
  const formatted = rounded.toLocaleString(languageToNumberLocale(language), {
    minimumFractionDigits: 0,
    maximumFractionDigits: maxFrac,
  })

  if (CURRENCY_CODE_SUFFIX_DISPLAY.has(code)) {
    return `${formatted} ${code}`
  }

  const currencyInfo = CURRENCIES.find((c) => c.code === code)
  const symbol = currencyInfo?.symbol || code
  return `${symbol}${formatted}`
}

/**
 * «Голое» число для E2E (data-test-*): без символа валюты, согласовано с правилами отображения.
 * THB / RUB / EUR / GBP / CNY / USDT и др. (кроме USD и JPY) — целые единицы в выбранной валюте.
 * USD — ровно 2 знака после запятой (строка). JPY — целое.
 */
export function priceRawForTest(amountThb, currency = 'THB', exchangeRates = {}) {
  const code = currency || 'THB'
  if (amountThb == null || Number.isNaN(Number(amountThb))) {
    return code === 'USD' ? '0.00' : '0'
  }
  let n = Number(amountThb)
  const rates = exchangeRates && typeof exchangeRates === 'object' ? exchangeRates : {}
  if (code !== 'THB' && rates[code] != null) {
    const rate = normalizeThbPerUnitRate(code, Number(rates[code]))
    if (rate != null && rate > 0) {
      n = n / rate
    }
  }
  if (code === 'JPY') {
    return String(Math.round(n))
  }
  if (code === 'USD') {
    const rounded = Math.round(n * 100) / 100
    return rounded.toFixed(2)
  }
  return String(Math.round(n))
}

// Get currency info
export function getCurrencyInfo(code) {
  return CURRENCIES.find(c => c.code === code) || CURRENCIES[0]
}

/** ISO code → display symbol (SSOT). Unknown codes fall back to the code string. */
export function getCurrencySymbol(code) {
  const c = String(code || 'THB').toUpperCase()
  const info = CURRENCIES.find((x) => x.code === c)
  return info?.symbol || c
}

/** Метаданные для отображения (geo API и т.д.); неизвестный код — безопасный fallback. */
export function getCurrencyDisplayMeta(code) {
  const c = CURRENCIES.find((x) => x.code === code)
  if (c) return c
  return { code, symbol: code, name: code, flag: '🌐' }
}