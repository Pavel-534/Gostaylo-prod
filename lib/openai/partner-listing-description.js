...
function buildPartnerListingTranslateSystem() {
  const brand = getSiteDisplayName()
  return `You are a professional marketplace translator for ${brand}.
Return ONLY valid JSON (no markdown fences) with exactly these top-level keys: "ru", "en", "zh", "th".
Each locale object MUST have: "body", "seoTitle", "seoDescription".

Translate the SOURCE description faithfully into ru, en, zh (Simplified Chinese), and th.
- Preserve meaning, tone, and structure; do not invent amenities, prices, or distances.
- Keep the source-language "body" close to the provided source text (light polish only).
- seoTitle ≤58 chars (no brand suffix); seoDescription 140–158 chars, derived from the body.
Plain text in JSON strings; use \\n for newlines.`
}
 ...
 * @param {'generate'|'translate'} [input.mode]
 * @param {string} apiKey
...
export async function generatePartnerListingDescriptionQuad(input, apiKey) {
  ...
  const mode = input.mode === 'translate' ? 'translate' : 'generate'
  ...
  const userContent =
    mode === 'translate'
      ? `SOURCE description to translate:\n${String(input.existingDescription || '').trim()}\n\nListing facts (JSON):\n${JSON.stringify(facts, null, 2)}\n\nProduce faithful ru/en/zh/th translations as specified.`
  ...
  const res = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      model,
      temperature: mode === 'translate' ? 0.35 : 0.65,
      ...
      messages: [
        {
          ...
          content:
            mode === 'translate'
              ? buildPartnerListingTranslateSystem()
              : buildPartnerListingDescriptionSystem(),
        },
