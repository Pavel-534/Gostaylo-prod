/**
 * JWT для Supabase Realtime (`postgres_changes`): RLS в БД читает `request.jwt.claims`
 * через `public.current_profile_id()` (клейм **`profile_id`** / legacy userId), а не «сырой» `sub` как `profiles.id`.
 * После Stage 94.0 миграции 047 это же разрешение используется для core-таблиц; чат уже завязан на 022 + этот токен.
 * Подпись тем же секретом, что в Supabase Dashboard → Settings → API → JWT Secret (`SUPABASE_JWT_SECRET`).
 */
...
/**
 * @param {{ profileId: string, authUserId?: string | null, email?: string | null, role?: string | null }} params
 * @returns {string | null}
 */
export function createSupabaseRealtimeAccessToken(params) {
  ...
  const appRole = String(params?.role || '').trim().toUpperCase()
  ...
  return jwt.sign(
    {
      ...
      ...(appRole ? { user_metadata: { role: appRole } } : {}),
      ...(appRole ? { app_metadata: { role: appRole } } : {}),
      ...(email ? { email } : {}),
      iat: now,
      exp: now + 3600,
    },
