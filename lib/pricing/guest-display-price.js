...
/**
 * Единица цены для min/max фильтра и гистограммы поиска — совпадает с карточкой «за ночь».
 * При датах в поиске календарь уже в `listing.pricing` / `_pricing` (batch availability).
 *
 * @param {Record<string, unknown> | null | undefined} listing
 * @param {{ checkIn?: string, checkOut?: string } | null} [dates] — зарезервировано; даты учтены в `_pricing`
 * @returns {number}
 */
export function getGuestDisplayForSearchFilters(listing, dates) {
  void dates
