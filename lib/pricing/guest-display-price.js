/**
 * Stage 110.1 / 190.1 / 199 — SSOT гостевой цены на витрине (base + guest service fee %).
 * Каталог / поиск / SEO / избранное: `getGuestDisplayPerNight` (base + fee %).
 * Stay с batch-pricing: `getGuestDisplayForStay` → `guestPayableRoundedThb` (без повторного fee).
 * PDP stay hero + breakdown total: `getGuestPayableTotalThb` / `getPdpHeroGuestPriceThb`.
 * FX — retail `useFxRatesQuery({ retail: true })` на PDP и checkout (без mid-rate flicker).
...
import { resolveSearchStayTotalThb } from '@/lib/pricing/price-truth.js'
...
/**
 * Гостевая цена за выбранный период (Stage 199 — без double-fee).
 * Если batch `_pricing` уже guest-payable (`guestPayableRoundedThb` / `guestServiceFeeThb`) — берём как есть.
 * Иначе lodging subtotal + fee %.
 * @param {Record<string, unknown> | null | undefined} listing
 * @param {number} nights
 * @param {number} [guestServiceFeePercent]
 */
export function getGuestDisplayForStay(listing, nights, guestServiceFeePercent) {
  ...
  const pct = resolveGuestServiceFeePercent(listing, guestServiceFeePercent)
  const pr = listing.pricing ?? listing._pricing
  const fromBatch = resolveSearchStayTotalThb(pr, {
    nights: n,
    guestServiceFeePercent: pct,
    basePerNightThb: 0,
  })
