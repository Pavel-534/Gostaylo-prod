...
/**
 * Wizard live preview: L1 asset amount → THB canon → guest fee (storefront THB).
 * Stage 200.49 — `basePriceThb` form field is misnamed (ADR-181 asset amount in listingBaseCurrency).
 *
 * @param {number|string} assetAmount — partner L1 input (EUR/RUB/…), NOT necessarily THB
 ...
 * }} [ctx]
 */
export function computeWizardStorefrontPricePreview(assetAmount, pricingPolicy, ctx = {}) {
  const listingCurrency = String(ctx.listingBaseCurrency || 'THB').toUpperCase()
  // Prefer mid for L1→THB (ADR-181); fall back to retail map if mid not loaded yet
  const midRates =
    (ctx.midExchangeRates && typeof ctx.midExchangeRates === 'object'
      ? ctx.midExchangeRates
      : null) ||
    (ctx.exchangeRates && typeof ctx.exchangeRates === 'object' ? ctx.exchangeRates : null)

  const asset = Number(assetAmount) || 0
  let base = Math.round(asset)
  if (listingCurrency !== 'THB' && asset > 0) {
    const asThb = convertDisplayAmountToThb(asset, listingCurrency, midRates || { THB: 1 })
    // If FX missing, do not pretend asset units are THB (that produced ~₽4k for €1500)
    base = asThb > 0 ? asThb : 0
  }
  ...
  // Stage 200.86 — same listing currency: L1 × (1+fee), no retail round-trip
  let storefrontInListingCurrency = Math.round(asset * (1 + guestFeePercent / 100))
  if (listingCurrency === 'THB') {
    storefrontInListingCurrency = storefrontGuestDisplayThb
  }
  ...
  return {
    base,
    assetAmount: Math.round(asset * 100) / 100,
    guestFeePercent,
    guestFeeThb,
    storefrontGuestDisplayThb,
    /** Guest total in listing currency units (L1 × fee) — no retail FX */
    sitePriceSameCurrency: storefrontInListingCurrency,
    ...
    fxReady: listingCurrency === 'THB' || base > 0 || asset <= 0,
  }
}
