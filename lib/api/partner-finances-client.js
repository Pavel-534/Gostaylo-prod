...
export async function fetchPartnerBalanceBreakdown(
  { limit, offset, ledgerEntry } = {},
  fetchImpl = defaultFetch,
) {
  const qs = new URLSearchParams()
  if (limit != null) qs.set('limit', String(limit))
  if (offset != null) qs.set('offset', String(offset))
  if (ledgerEntry) qs.set('ledgerEntry', String(ledgerEntry))
  const suffix = qs.toString() ? `?${qs}` : ''
  const res = await fetchImpl(`/api/v2/partner/balance-breakdown${suffix}`)
...
/**
 * Period statement pack (Stage 211.2) — earned vs paid vs closing acts.
 * @param {{ from: string, to: string, axis?: 'created'|'checkout' }} params
 */
export async function fetchPartnerFinancesPeriodPack(
  { from, to, axis = 'created' },
  fetchImpl = defaultFetch,
) {
  const qs = new URLSearchParams({ from, to, axis })
  const res = await fetchImpl(`/api/v2/partner/finances-period?${qs}`)
  const json = await readJson(res)
  if (!res.ok) {
    throw new Error(json?.error || 'Failed to fetch period pack')
  }
  return json.data
}
...
export { fetchAuthMe } from '@/lib/api/auth-client'

/** Mid-market rate map (`retail=0`) for partner host display — prefer `usePartnerHostDisplayFx` in UI. */
export async function fetchExchangeRatesMid(fetchImpl = defaultFetch) {
...
export async function fetchPartnerSettlementDocuments(fetchImpl = defaultFetch) {
  const res = await fetchImpl('/api/v2/partner/settlement-documents')
  const json = await readJson(res)
  if (!res.ok || !json?.success) {
    throw new Error(json?.error || 'Failed to load settlement documents')
  }
  return json.data?.documents ?? []
}

export async function fetchPartnerSettlementDocumentDownloadUrl(
  { source, refId },
  fetchImpl = defaultFetch,
) {
  const q = new URLSearchParams({ source, refId })
  const res = await fetchImpl(`/api/v2/partner/settlement-documents/download?${q}`)
  const json = await readJson(res)
  if (!res.ok || !json?.success) {
    throw new Error(json?.error || 'download_failed')
  }
...
/**
 * Canonical partner statement download (CSV/PDF).
 * @param {{ from: string, to: string, format: 'csv'|'pdf', axis?: 'created'|'checkout' }} params
 */
export async function fetchPartnerFinancesExport(
  { from, to, format, axis = 'created' },
  fetchImpl = defaultFetch,
) {
  const qs = new URLSearchParams({ from, to, format, axis })
  const res = await fetchImpl(`/api/v2/partner/finances-export?${qs}`)
  if (!res.ok) {
    let detail = ''
    try {
    ...
    throw new Error(detail || 'Export request failed')
  }
  const blob = await res.blob()
  const fallback = `statement-${from}-${to}.${format === 'pdf' ? 'pdf' : 'csv'}`
  const filename = filenameFromContentDisposition(res.headers.get('Content-Disposition'), fallback)
  return { blob, filename }
}

/** @deprecated Prefer fetchPartnerFinancesExport({ format: 'pdf' }). */
export async function fetchFinancesStatementPdf({ from, to, axis = 'created' }, fetchImpl = defaultFetch) {
  const { blob } = await fetchPartnerFinancesExport({ from, to, format: 'pdf', axis }, fetchImpl)
  return blob
}
