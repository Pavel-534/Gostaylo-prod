 ...
 * Stage 202.44 — longer TTL when no date filter (browse pins); keep short when dates set.
...
export function mapPinsEdgeCacheControl({
  ...
  if (status === 200 && !viewerId) {
    ...
    return buildAnonymousPublicEdgeCacheControl({ sMaxAge: 45, staleWhileRevalidate: 90 })
  }
