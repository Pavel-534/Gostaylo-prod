/**
 * Stage 179.0a / 179.2a / 179.1 / 202.47 — conditional Vercel CDN Cache-Control
 * (anonymous-only public reads). ADR-163: map-pins / catalog search must not
 * edge-cache session-dependent coordinate reveal.
 */
 ...
 * Stage 202.44 — longer TTL when no date filter; 202.47 — bump browse CDN to cut Fluid CPU.
 * @param {{ viewerId?: string | null, status?: number, hasDateFilter?: boolean }} opts
 * @returns {string}
 */
export function mapPinsEdgeCacheControl({
  viewerId = null,
  status = 200,
  hasDateFilter = false,
} = {}) {
  if (status === 200 && !viewerId) {
    if (hasDateFilter) {
      return buildAnonymousPublicEdgeCacheControl({ sMaxAge: 10, staleWhileRevalidate: 30 })
    }
    return buildAnonymousPublicEdgeCacheControl({ sMaxAge: 90, staleWhileRevalidate: 180 })
  }
 ...
 * Stage 202.47 — longer TTL (catalog taxonomy is rarely mutated).
...
export function categoriesEdgeCacheControl({
  ...
  if (status === 200 && !isAdminRequest && !includeInactive) {
    return buildAnonymousPublicEdgeCacheControl({ sMaxAge: 600, staleWhileRevalidate: 1200 })
  }
  return PRIVATE_NO_STORE_CACHE_CONTROL
}
 ...
 * Stage 202.47 — modest TTL bump for simple anon browse.
