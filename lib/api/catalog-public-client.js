...
import {
  CACHE_KEY,
  TTL_LOCATION_SUGGEST_POPULAR_MS,
  TTL_LOCATION_SUGGEST_TYPED_MS,
  TTL_PUBLIC_STATS_MS,
  TTL_SEARCH_LOCATIONS_MS,
  TTL_SITE_FEATURES_MS,
} from '@/lib/api/client-fetch-policy'
import { suggestItemToWhereOption } from '@/lib/locations/suggest-item-to-option'
...
/**
 * Stage 158 — server-side location suggest (ranked by inventory).
 * @param {{ q?: string, lang?: string, limit?: number }} opts
 */
export async function fetchLocationSuggest(opts = {}) {
  ...
  const cacheKey = `${CACHE_KEY.locationSuggest}:${lang}:${q}:${limit}`

  return dedupeClientRequest(
    cacheKey,
    async () => {
      const res = await fetch(`/api/v2/search/locations/suggest?${params.toString()}`, {
        cache: 'default',
      })
      const data = await readJson(res)
      const items = (data?.data?.items ?? []).map(suggestItemToWhereOption)
      return {
        ok: res.ok && data.success,
        mode: data?.data?.mode ?? 'suggest',
        items,
        raw: data,
        status: res.status,
      }
    },
    { ttlMs: q ? TTL_LOCATION_SUGGEST_TYPED_MS : TTL_LOCATION_SUGGEST_POPULAR_MS },
  )
}
