/**
 * Stage 112.3 / 113.0 — SSOT partner listing fetch/patch (iCal sync manager).
 */
...
export async function fetchPartnerReputationHealth() {
  return dedupeClientRequest(
    'partner:reputation-health',
    async () => {
      const res = await fetch('/api/v2/partner/reputation-health', {
        credentials: 'include',
        cache: 'no-store',
      })
      const json = await readJson(res)
...
export async function postAirbnbImportPreview(body) {
