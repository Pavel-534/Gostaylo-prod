 ...
 * @param {{ hasDateFilter?: boolean }} [cacheOpts]
 */
function mapPinsJson(body, status, viewerId, cacheOpts = {}) {
  return NextResponse.json(body, {
    ...
    headers: edgeCacheResponseHeaders(
      mapPinsEdgeCacheControl({
        viewerId,
        status,
        hasDateFilter: cacheOpts.hasDateFilter === true,
      }),
    ),
  })
}
...
export async function runMapPinsGet(request) {
  ...
  try {
    ...
    // Stage 202.44 — overlap fee snapshot with spatial work (formulas unchanged; cached ~45s).
    const commissionPromise = getCommissionRate()
    ...
    if (serverClusters?.length) {
      ...
      return mapPinsJson(
        ...
        200,
        viewerId,
        { hasDateFilter: hasValidDateRange },
      )
    }
    ...
    // Stage 202.49 — decide metadata SELECT before PostgREST; client pin DTO never ships metadata.
    const jsMetadataFilters = metadataFiltersForJsPostFilter(
      filters.metadataFilters,
      discoveryPlan,
      unifiedPipeline,
    )
    const includeMetadata = Boolean(jsMetadataFilters)
    ...
    const commissionSnapshot = await commissionPromise
