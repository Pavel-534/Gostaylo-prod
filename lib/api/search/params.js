...
/** Service/nanny metadata URL facets (unified registry `service.*` SQL, Stage 177.4). */
export function serviceMetadataFiltersActive(metadataFilters) {
  if (!metadataFilters) return false
  return !!(
...
export function sqlMetadataFiltersActive(filters) {
  return Boolean(
    filters.instantBookingOnly ||
      ...
      serviceMetadataFiltersActive(filters.metadataFilters),
  )
}

export function getCacheKey(filters, catalogSort = 'recommended') {
  if (filters.checkIn || filters.checkOut || filters.lat != null || filters.lon != null) return null
  if (filters.mapBounds) return null
  if (filters.polygon) return null
  ...
  const sortKey = String(catalogSort || 'recommended').trim().toLowerCase() || 'recommended'
  return `${filters.category || 'all'}_${filters.limit}_${where}_${filters.q || ''}_${filters.minPrice || ''}_${filters.maxPrice || ''}_${sem}_${profile}_${sortKey}`
}

/**
 * Stage 179.1 — edge CDN eligibility (stricter than in-memory getCacheKey).
 * Allows category / where / price / sort / short q; excludes dates, geo, facets, guests>1, semantic.
 * @param {object} filters
 * @returns {boolean}
 */
export function isSimpleCatalogSearchForEdgeCache(filters) {
  if (!filters || typeof filters !== 'object') return false
  if (filters.semantic) return false
  const guests = Number(filters.guests)
  if (Number.isFinite(guests) && guests > 1) return false
  return getCacheKey(filters, 'recommended') != null
}
