import { getDistrictsForCity, resolveCanonicalCityLabelForGeo } from '@/lib/locations/city-district-map'
...
export async function buildSmartWhereOrClause(whereValue) {
  ...
  const canonicalCity = resolveCanonicalCityLabelForGeo(whereValue)
  const cityLabelForMeta = canonicalCity ?? whereValue
  const cityJson = JSON.stringify({ city: cityLabelForMeta })
  const orParts = []
  ...
  if (districts?.length) {
    ...
    // «Зонтик»: объявления с parent_location = Phuket / тем же каноном (район в district без слова Phuket в title)
    if (canonicalCity) {
      orParts.push(`metadata.cs.${JSON.stringify({ parent_location: canonicalCity })}`)
    }
