 ...
 * Stage 202.49 — omit `metadata` on browse path; include only when JS facet post-filter needs it.
...
/** Core columns for map markers (no fat JSONB). */
export const MAP_PINS_SELECT_CORE = `
  id,
  latitude,
  longitude,
  base_price_thb,
...
/** When JS metadata facet post-filter is active (vertical facets). */
export const MAP_PINS_SELECT_WITH_METADATA = `
  ${MAP_PINS_SELECT_CORE.trim()},
  metadata
`

/** @deprecated use resolveMapPinsSelect / MAP_PINS_SELECT_CORE — kept for older imports */
export const MAP_PINS_SELECT = MAP_PINS_SELECT_WITH_METADATA

/**
 * @param {{ includeMetadata?: boolean }} [opts]
 * @returns {string}
 */
export function resolveMapPinsSelect(opts = {}) {
  return opts.includeMetadata === true ? MAP_PINS_SELECT_WITH_METADATA : MAP_PINS_SELECT_CORE
}

/**
 * SQL-side E2E tag exclusion so lean selects (no metadata column) still hide test listings.
 * @param {import('@supabase/supabase-js').PostgrestFilterBuilder} q
 */
export function applyMapPinsE2eMetadataSqlGuard(q) {
  return q.not('metadata', 'cs', { test_data_tag: E2E_TEST_DATA_TAG })
}
...
export async function buildMapPinsQuery({
  ...
  discoveryPlan = null,
  /** Stage 202.49 — pull JSONB only when JS facet post-filter needs it */
  includeMetadata = false,
}) {
  const useUnifiedPlan = Boolean(discoveryPlan)
  let q = supabaseAdmin
    .from('listings')
    .select(resolveMapPinsSelect({ includeMetadata }))
    .eq('status', 'ACTIVE')
  ...
  q = applyMapPinsE2eMetadataSqlGuard(q)
...
export function mapPinRowToPayload(row, viewerContextOrOptions = null) {
  ...
  const currencyFields = catalogPublicCurrencyFields(row)
  return {
    id: String(row.id),
    lat: coords.latitude,
    ...
    basePriceAsset: currencyFields.basePriceAsset,
    guestServiceFeePercent: options.guestServiceFeePercent ?? null,
  }
}
