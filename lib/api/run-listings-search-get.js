...
import {
  ...
  catalogPublicCurrencyFields,
  catalogPublicGeoFields,
  pickLiteListingMetadata,
} from '@/lib/api/search/listing-search-payload';
...
import {
  metadataFiltersForJsPostFilter,
} from '@/lib/search/discovery-metadata-facet-page';
export { applySmartWhereFilter } from '@/lib/api/search/location-filter';

const cache = { data: null, timestamp: 0, TTL: 60 * 1000 };

/**
 * Stage 179.1 — catalog search JSON with conditional edge Cache-Control (ADR-163).
 * @param {unknown} body
 * @param {number} status
 * @param {{ viewerId?: string | null, isSimpleQuery?: boolean, extraHeaders?: Record<string, string> }} [opts]
 */
function listingsSearchJson(body, status, opts = {}) {
  const { viewerId = null, isSimpleQuery = false, extraHeaders = {} } = opts
  return NextResponse.json(body, {
    status,
    headers: {
      ...extraHeaders,
      ...edgeCacheResponseHeaders(
        listingsSearchEdgeCacheControl({ viewerId, status, isSimpleQuery }),
      ),
    },
  })
}
...
export async function runListingsSearchGet(request, options = {}) {
  ...
  if (!options.skipRateLimit) {
    ...
    if (rl) {
      return NextResponse.json(rl.body, {
        status: rl.status,
        headers: { ...rl.headers, 'Cache-Control': PRIVATE_NO_STORE_CACHE_CONTROL },
      });
    }
    if (sp0.get('semantic') === '1' && (sp0.get('q') || '').trim().length >= 2) {
      const srl = await rateLimitCheck(request, 'semantic_search');
      if (srl) {
        return NextResponse.json(srl.body, {
          status: srl.status,
          headers: { ...srl.headers, 'Cache-Control': PRIVATE_NO_STORE_CACHE_CONTROL },
        });
      }
    }
  }
  ...
  try {
    ...
    const filters = {
      ...
      polygon: Boolean(String(searchParams.get('polygon') || '').trim()),
      metadataFilters,
    ...
    const isSimpleQuery = isSimpleCatalogSearchForEdgeCache(filters);
    ...
    if (unifiedPipeline) {
      const parsed = await parseDiscoveryFiltersFromSearchParams(searchParams, {
        surface: 'catalog',
        isLite,
      });
      if (!parsed.ok) {
        return listingsSearchJson(
          {
            success: false,
          ...
          400,
          { viewerId, isSimpleQuery: false },
        );
      }
      discoveryPlan = await buildDiscoveryQueryPlan(parsed.value, { surface: 'catalog' });
    }

    const cacheKey = getCacheKey(filters, catalogSort);
    const now = Date.now();
    ...
    if (cacheKey && cache.data && cache.key === cacheKey && (now - cache.timestamp) < cache.TTL) {
      ...
      return listingsSearchJson(
        ...
        200,
        { viewerId, isSimpleQuery, extraHeaders: { 'X-Cache': 'HIT' } },
      );
    }
    ...
    let { data: rawListings, error } = { data: null, error: null };
    /** @type {{ mode: 'cursor', pageSize: number, next_cursor: string|null, hasMore: boolean, refillAttempts?: number } | null} */
    let discoveryPagination = null;
    ...
    if (unifiedPipeline && discoveryPlan) {
      const unifiedResult = await executeDiscoverySqlPlan(discoveryPlan, {
      ...
      rawListings = unifiedResult.data;
      error = unifiedResult.error;
      discoveryPlan = unifiedResult.plan;
      discoveryPagination = unifiedResult.pagination ?? null;
      discoveryAvailabilityStats = unifiedResult.availabilityStats ?? null;
    } else {
      const { query: legacyQuery } = await buildListingsQuery({
        supabaseAdmin,
        filters,
        fetchLimit,
        textOrClause,
      ...
      rawListings = legacyResult.data;
      error = legacyResult.error;
    }

    if (error) {
      console.error('[SEARCH API] Query error:', error);
      return listingsSearchJson({ success: false, error: error.message }, 500, {
        viewerId,
        isSimpleQuery: false,
      });
    }
    ...
    if ((rawListings?.length ?? 0) === 0 && qTrim.length >= 2 && textOrClause) {
      if (unifiedPipeline && discoveryPlan) {
        const resRelaxed = await executeDiscoverySqlPlan(discoveryPlan, {
          legacyFilters: filters,
          fetchLimit,
          textOrClause,
        ...
        if (!error) {
          ...
          discoveryPlan = resRelaxed.plan;
          discoveryPagination = resRelaxed.pagination ?? discoveryPagination;
          discoveryAvailabilityStats = resRelaxed.availabilityStats ?? discoveryAvailabilityStats;
        }
      } else {
        const { query: relaxedQuery } = await buildListingsQuery({
          supabaseAdmin,
          filters,
          fetchLimit,
          textOrClause,
        ...
        const resRelaxed = await relaxedQuery;
        error = resRelaxed.error;
        rawListings = resRelaxed.data;
      }
      if (error) {
    ...
    if (jsMetadataFilters) {
      listings = listings.filter((l) => listingMatchesMetadataFilters(l, jsMetadataFilters));
    }
    ...
    if (executorHandledAvailability) {
      ...
      filteredOutByAvailability = discoveryAvailabilityStats?.filteredOutByAvailability ?? 0;
      filteredOutByAvailabilityErrors =
        discoveryAvailabilityStats?.filteredOutByAvailabilityErrors ?? 0;
      filteredOutByCapacity = discoveryAvailabilityStats?.filteredOutByCapacity ?? 0;
    } else {
      ({
        availableListings,
    ...
    const usesDiscoveryCursorPagination = Boolean(discoveryPagination?.mode === 'cursor');

    const rankedListings = usesDiscoveryCursorPagination
      ? listingsForRanking
      : applyCatalogSort(listingsForRanking, catalogSort, {
          distanceKmById,
          trustByOwner,
          strikesByOwner,
          searchPenalty,
    ...
    const transformed = rankedListings.map((l) => {
      ...
      ...catalogPublicCurrencyFields(l),
      ...catalogPublicGeoFields(l),
      ...
      owner: l.owner,
      ownerVerified: String(l.owner?.verification_status || '').toUpperCase() === 'VERIFIED',
    ...
    const responseData = {
      ...
      meta: {
        ...
        spatialEngine: discoveryPlan?.spatial?.engine ?? (geoCenter ? (usedPostgisRadius ? 'postgis' : 'haversine') : null),
        registryFiltersApplied: discoveryRegistryFiltersMeta(discoveryPlan),
        ...(discoveryPagination ? { pagination: discoveryPagination } : {}),
      },
    };
    ...
    return listingsSearchJson(
      ...
      200,
      { viewerId, isSimpleQuery, extraHeaders: { 'X-Cache': 'MISS' } },
    );
  } catch (error) {
