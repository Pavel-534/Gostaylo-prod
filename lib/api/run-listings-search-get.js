...
/** Stage 54.0 — explicit projection (no `*`, no `embedding`); keep in sync with `transformed` map below. */
...
/**
 * Quote PG identifier value for PostgREST OR clause.
 */
...
/**
 * applySmartWhereFilter — глобальный поиск по иерархии country → region → city → district.
 *
 * Sprint 2026-02 Global DB:
 *   1) Если whereValue → разбираем через resolveWhereTarget (country/region/city/district)
 *   2) Schema-probe (geo-schema-probe.js): добавляем geo-предикаты ТОЛЬКО если соотв. колонка
 *      существует в БД. Это спасает от 500 SQLSTATE 42703 когда миграция ещё не применена.
 *   3) Строим PostgREST OR-clause:
 *      • Новые колонки: country_code / region_code / city_code (gated by schema probe)
 *      • Legacy: district.eq.{...} для всех known districts (Phuket fallback и custom)
 *      • metadata.cs (city in JSON) — для импортированных листингов
 *   4) Backward-compat: пока миграция не применена — работают только legacy district / metadata.
 *
 * @async — внутри await getGeoSchemaState()
 */
export async function applySmartWhereFilter(query, whereValue) {
  ...
  // Schema probe — какие новые колонки реально доступны
  ...
  if (target) {
    if (target.level === 'country') {
      ...
      if (schema.hasCityCode) target.cities?.forEach((c) => orParts.push(`city_code.eq.${pgOrValue(c)}`))
      // Districts фолбэк для не-мигрированных Phuket-листингов
  ...
  // Legacy slug→district fallback — оставляем рабочим как есть
  ...
  if (orParts.length === 0) {
    // Final ILIKE fallback (район без presets и без новых колонок)
    return query.or(`metadata.cs.${cityJson},district.ilike.%${whereValue}%`)
  }

  // Дедуп для аккуратной OR-строки
  const unique = Array.from(new Set(orParts))
  return query.or(unique.join(','))
}
...
function firstIntParam(sp, ...keys) {
  for (const k of keys) {
    const v = sp.get(k);
    if (v != null && v !== '') {
...
function parseBooleanSearchParam(sp, ...keys) {
  for (const k of keys) {
    const raw = sp.get(k);
...
function parseAmenitiesFromSearchParams(sp) {
...
function sqlMetadataFiltersActive(filters) {
  return Boolean(
...
export async function runListingsSearchGet(request, options = {}) {
  ...
  try {
    ...
    async function buildListingsQuery(includeTextSearchOr) {
      let q = supabaseAdmin
        .from('listings')
      ...
      if (filters.where && filters.where !== 'all') {
        q = await applySmartWhereFilter(q, filters.where);
      } else {
    ...
    if (!hasDateFilter) {
      ...
      /** IDs объявлений, которые были отклонены availability-RPC (не ошибки, а реально занятые) */
      ...
      /**
       * Soft-filter (Airbnb-стиль): если строгих результатов < 3,
       * добавляем недоступные объекты с флагом is_availability_mismatch=true,
       * чтобы страница не пустовала. Фронтенд отобразит их с пометкой "Уточняйте доступность".
       */
