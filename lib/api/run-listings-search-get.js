...
import {
  ...
  spatialRadiusActive,
  fetchListingIdsWithinRadius,
  finalizeRadiusFilteredListings,
...
import { sortListingsByReputationRanking } from '@/lib/api/search/ranking';
import {
...
export async function runListingsSearchGet(request, options = {}) {
  ...
  try {
    ...
    let spatialListingIds = null;
    let usedPostgisRadius = false;
    if (geoCenter) {
      spatialListingIds = await fetchListingIdsWithinRadius(filters.lat, filters.lon, filters.radiusKm);
      usedPostgisRadius = spatialListingIds !== null;
    }
    ...
    if (geoCenter && spatialRadius) {
      listings = finalizeRadiusFilteredListings(
        listings,
        spatialRadius,
        filters.limit,
        usedPostgisRadius,
    ...
    const rankedListings = sortListingsByReputationRanking(listingsForRanking, trustByOwner, {
      strikesByOwner,
      searchPenalty,
    }).slice(
      0,
