 ...
 *   2) Строим PostgREST OR-clause:
 *      • Новые колонки: country_code / region_code / city_code (если миграция применена)
 ...
 *   3) Backward-compat: если новые колонки IS NULL — старый district-фильтр всё равно работает.
 *
 * Это и есть "выдача всех объектов из дочерних городов и районов" при поиске по родителю.
 */
export function applySmartWhereFilter(query, whereValue) {
  ...
  if (target) {
    if (target.level === 'country') {
      orParts.push(`country_code.eq.${pgOrValue(target.countryCode)}`)
      // Все регионы / города этой страны — backward-compat для до-миграционных записей
  ...
  if (orParts.length === 0) {
    // Final ILIKE fallback (район без presets)
