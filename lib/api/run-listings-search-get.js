...
export async function runListingsSearchGet(request, options = {}) {
  ...
  try {
    ...
    const spatialSortUsed = parseSpatialSortUsed(searchParams, geoCenter);
    ...
    const rankedListings = (spatialSortUsed && distanceKmById.size > 0
      ? sortListingsByDistance(listingsForRanking, distanceKmById)
      : sortListingsByReputationRanking(listingsForRanking, trustByOwner, {
          strikesByOwner,
          searchPenalty,
        })
    ...
    const responseData = {
      ...
      meta: {
        ...
        stage: 'smart-v3',
        reputationRankingApplied: !(spatialSortUsed && distanceKmById.size > 0),
        payloadProfile: isLite ? 'lite' : 'full',
        priceHistogram: {
