...
import { computeCatalogGuestDisplayPriceThb } from '@/lib/pricing/catalog-guest-display-price.js';
...
export async function runListingsSearchGet(request, options = {}) {
  ...
  try {
    ...
    const catalogGuestFeePercent = commissionSnapshot?.guestServiceFeePercent;
    ...
    const rankedListings = sortListingsByReputationRanking(listingsForRanking, trustByOwner).slice(
      0,
    ...
    const transformed = rankedListings.map((l) => {
      ...
      guestDisplayPriceThb: computeCatalogGuestDisplayPriceThb(
        l.base_price_thb,
        catalogGuestFeePercent,
      ),
      commissionRate: (() => {
