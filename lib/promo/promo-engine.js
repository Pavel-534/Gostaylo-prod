...
export function promoIsActiveAt(promo, atMs = Date.now()) {
  ...
  if (promo.valid_from) {
    const startMs = new Date(promo.valid_from).getTime()
    if (!Number.isFinite(startMs) || startMs > atMs) {
 ...
 * Do not use `react` `cache()` here: keep this module usable from plain server paths (search API, pricing) and from any shared bundle without RSC-only assumptions.
 */
