...
export async function deliverProductFeedback(payload) {
  ...
  // Stage 202.0 — TELEGRAM_USER_FEEDBACK_TOPIC_ID when set (fallback: system-alerts).
  const tgHtml =
    ...
    `locale: <code>${escapeSystemAlertHtml(payload.language || 'ru')}</code>` +
    (currency ? ` · currency=<code>${escapeSystemAlertHtml(currency)}</code>` : '') +
    `\n` +
    (payload.userAgent
  ...
  void notifySystemAlert(tgHtml, { severity: 'INFO', userFeedbackTopic: true })
  ...
  if (supportTo) {
    ...
    const template = buildSimplePremiumEmailTemplate({
      ...
      paragraphs: [
        ...
        `Page: ${pageRef}`,
        `Locale: ${payload.language || 'ru'}${currency ? ` · Currency: ${currency}` : ''}`,
