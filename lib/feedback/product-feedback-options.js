...
/** UI switcher codes — reject arbitrary client strings in TG/email context. */
export const PRODUCT_FEEDBACK_CURRENCY_CODES = Object.freeze([
  'THB',
  'USD',
  'EUR',
  'GBP',
 ...
 * @returns {string | null}
 */
export function normalizeProductFeedbackCurrency(raw) {
  const c = String(raw || '')
    .trim()
    .toUpperCase()
  if (!c) return null
  return PRODUCT_FEEDBACK_CURRENCY_CODES.includes(c) ? c : null
}
...
export function validateProductFeedbackBody(body, user) {
  ...
  const currency = normalizeProductFeedbackCurrency(body?.currency)
