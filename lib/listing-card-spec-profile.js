 ...
 * @returns {string}
 */
export function formatListingFuelTypeLabel(metadata, language = 'en') {
  if (!metadata || typeof metadata !== 'object') return ''
  const raw = metadata.fuel_type ?? metadata.fuelType ?? ''
  const token = String(raw).toLowerCase().trim()
  if (!token || token === 'unset') return ''
  const labelKey = FUEL_TYPE_LABEL_KEYS[token]
  if (labelKey) {
 ...
 * @returns {number}
 */
export function getListingCardArea(listing) {
  const meta = listing.metadata && typeof listing.metadata === 'object' ? listing.metadata : {}
  const n = Number(meta.area ?? meta.squareMeters ?? meta.square_meters ?? 0)
  return Number.isFinite(n) && n > 0 ? n : 0
}
 ...
 * @returns {number|null}
 */
export function getListingCardVehicleYear(listing) {
  const meta = listing.metadata && typeof listing.metadata === 'object' ? listing.metadata : {}
  return normalizeVehicleModelYearForDisplay(meta.vehicle_year ?? meta.model_year ?? meta.year)
}
 ...
 * Stage 90.0 — плюс профиль владельца **`owner.is_verified` / `ownerVerified`** (подтверждение KYC админом → тот же знак в lite-каталоге).
...
export function listingQualifiesForTrustVerifiedMiniBadge(listing) {
  ...
  if (own && typeof own === 'object' && own.is_verified === true) return true
  const meta = listing.metadata && typeof listing.metadata === 'object' ? listing.metadata : {}
  if (
