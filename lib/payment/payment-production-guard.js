...
import { getExpectedUsdtForBooking } from '@/lib/booking-price-integrity.js'
 ...
 * Stage 130.4: optional MIR Controlled Live soft limit touch (TG only, non-blocking).
 *
 * @param {{ booking?: object | null, paymentMethod?: string | null }} [ctx]
 */
export async function assertGuestPaymentOperationsAllowed(ctx = {}) {
  ...
  const method = normalizeCheckoutPaymentMethod(
    ctx.paymentMethod || booking?.metadata?.paymentMethod || '',
  )
  if (booking && method === 'MIR') {
