...
export async function buildPublicLandingPayload(supabaseAdmin, userId) {
  ...
  const fintech = await SystemConfigService.getFintechConfig()
  ...
  const referralReinvestmentPercent = Math.min(
    95,
    Math.max(0, Number(fintech?.referralReinvestmentPercent ?? FINTECH_JS_DEFAULTS.referralReinvestmentPercent)),
  )
  const ambassadorGuestPoolL1Percent = Math.min(
    100,
    Math.max(0, Number(fintech?.ambassadorGuestPoolL1Percent ?? FINTECH_JS_DEFAULTS.ambassadorGuestPoolL1Percent)),
  )
  const referralSplitRatio = ambassadorGuestPoolL1Percent / 100
  ...
  for (const row of networkRows || []) {
    const depth = Number(row?.ledger_depth) || 1
    if (depth < 2) continue
    const iso = row?.earned_at || row?.updated_at
    if (!iso || yearMonthKeyInTimeZone(iso, statsTz) !== currentYm) continue
    monthlyNetworkEarnedThb += round2(row?.amount_thb)
  }
  monthlyNetworkEarnedThb = round2(monthlyNetworkEarnedThb)
  ...
  return {
    ...
    referralEstimator: {
      ...
      ambassadorGuestPoolL2Percent: Number(
        fintech?.ambassadorGuestPoolL2Percent ?? FINTECH_JS_DEFAULTS.ambassadorGuestPoolL2Percent,
      ),
      ambassadorGuestPoolRefereePercent: Number(
        fintech?.ambassadorGuestPoolRefereePercent ?? FINTECH_JS_DEFAULTS.ambassadorGuestPoolRefereePercent,
      ),
    },
    monthlyNetworkEarnedThb,
    publicGamification,
