...
export async function buildPublicLandingPayload(supabaseAdmin, userId) {
  ...
  const referralReinvestmentPercent = Math.min(
    95,
    Math.max(0, Number(fintech?.referralReinvestmentPercent ?? 45)),
  )
  const ambassadorGuestPoolL1Percent = Math.min(
    100,
    Math.max(0, Number(fintech?.ambassadorGuestPoolL1Percent ?? 45)),
  )
  ...
  return {
    ...
    referralEstimator: {
      ...
      ambassadorGuestPoolL2Percent: Number(fintech?.ambassadorGuestPoolL2Percent ?? 12),
      ambassadorGuestPoolRefereePercent: Number(fintech?.ambassadorGuestPoolRefereePercent ?? 43),
    },
    monthlyNetworkEarnedThb,
    publicGamification,
