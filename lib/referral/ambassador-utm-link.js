 ...
 *
 * Stage 200.10 — display/share use clean `/u` or `/go` URLs; UTM only on explicit channel copy.
 */
...
/**
 * Remove tracking query keys for professional message/share URLs.
 * Path identity (`/u/{id}`, `/go/{slug}`, `?ref=`) is preserved.
 *
 * @param {string} url
 * @returns {string}
 */
export function stripAmbassadorTrackingParams(url) {
...
/**
 * Absolute clean invite URL for pitches, QR, and native share.
 * @param {string} url
 * @returns {string}
 */
export function formatAmbassadorShareLink(url) {
  return stripAmbassadorTrackingParams(url)
}

/**
 * Compact caption for UI / Stories / under QR: `host/path` (no scheme, no UTM).
 * @param {string} url
 * @returns {string}
 */
export function formatAmbassadorLinkCaption(url) {
  const clean = stripAmbassadorTrackingParams(url)
  if (!clean) return ''
  try {
