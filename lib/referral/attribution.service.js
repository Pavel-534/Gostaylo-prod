...
export class ReferralAttributionService {
  ...
  static async assertTrackAllowed({ request, code, candidateUserId = null, fingerprint = null }) {
    ...
    return { allowed: true, referrerId, referralCode: resolved.data.code, referralCodeRow: resolved.data.referralCodeRow };
  }
  ...
  static async getAdminDashboard({
    ...
    const { data: convertedRows, error: convErr } = await applyBase(
      supabaseAdmin
        .from('referral_attributions')
        .select('id, converted_profile_id, booking_id, converted_at')
        .eq('status', 'converted')
    ...
    const { data: tableRows, error: tableErr } = await applyBase(
      supabaseAdmin
        .from('referral_attributions')
        .select(
          'id, click_id, referrer_id, referral_code, touch_type, landing_path, utm_source, utm_medium, utm_campaign, device_hash, status, converted_profile_id, booking_id, expires_at, converted_at, created_at',
        )
