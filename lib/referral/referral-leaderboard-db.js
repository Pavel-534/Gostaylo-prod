 ...
 * @returns {Promise<Array<{ referrerId: string, amountThb: number }>>}
 */
export async function aggregateReferralLeaderboardAlltimeFromDb(supabaseAdmin, limit = 10) {
  const lim = Math.min(100, Math.max(1, Math.floor(Number(limit) || 10)))
  ...
  return legacyAggregateReferralLeaderboardAlltime(supabaseAdmin, lim)
}
...
async function legacyAggregateReferralLeaderboardAlltime(supabaseAdmin, limit) {
  const sums = new Map()
  let offset = 0
