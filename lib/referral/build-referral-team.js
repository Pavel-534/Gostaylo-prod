...
export async function buildReferralTeamMembers(supabaseAdmin, referrerId, opts = {}) {
  ...
  const { data: earnedByRefereeRows } = await supabaseAdmin
    .from('referral_ledger')
    .select('referee_id, amount_thb')
    .eq('referrer_id', referrerId)
    .in('status', ['earned', 'earned_held'])
    .in('referee_id', refereeIdsOrdered);

  const earnedThbByReferee = {};
  for (const row of earnedByRefereeRows || []) {
    const rid = String(row.referee_id || '').trim();
    if (!rid) continue;
    earnedThbByReferee[rid] = Math.round(((earnedThbByReferee[rid] || 0) + (Number(row.amount_thb) || 0)) * 100) / 100;
  }
  ...
  const members = refereeIdsOrdered.map((rid) => {
    ...
    return {
      ...
      earnedForReferrerThb: earnedThbByReferee[rid] ?? 0,
    };
  });
