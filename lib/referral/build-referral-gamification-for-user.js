...
export async function buildReferralGamificationForUser(supabaseAdmin, profile, opts = {}) {
  ...
  let directPartnersInvited =
    opts.directPartnersInvited != null ? Number(opts.directPartnersInvited) : NaN
  if (!Number.isFinite(directPartnersInvited)) {
    directPartnersInvited = Number((await ReferralPnlService.countDirectPartnersInvited(profile.id)) || 0)
  }

  let friendsInvited = opts.friendsInvited != null ? Number(opts.friendsInvited) : NaN
  if (!Number.isFinite(friendsInvited)) {
    const { count } = await supabaseAdmin
      .from('referral_relations')
      .select('id', { head: true, count: 'exact' })
      .eq('referrer_id', profile.id)
    friendsInvited = Number(count || 0)
  }

  let totalLifetimeEarnedThb =
    opts.totalLifetimeEarnedThb != null ? Number(opts.totalLifetimeEarnedThb) : NaN
  if (!Number.isFinite(totalLifetimeEarnedThb)) {
    const { data: allEarned } = await supabaseAdmin
      .from('referral_ledger')
      .select('amount_thb')
      .eq('referrer_id', profile.id)
      .eq('status', 'earned')
    totalLifetimeEarnedThb = 0
    for (const row of allEarned || []) {
      totalLifetimeEarnedThb += Number(row?.amount_thb) || 0
    }
    totalLifetimeEarnedThb = Math.round(totalLifetimeEarnedThb * 100) / 100
  }
