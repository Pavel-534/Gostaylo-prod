/**
 * Генерация PDF-визитки амбассадора в браузере (минималистичный layout + марка бренда).
 ...
 *   pdfVariant?: 'standard' | 'elite',
 *   elitePartnerLine?: string,
 * }} opts
 *
 * Elite-оформление включается **только** при `pdfVariant === 'elite'` (SSOT; без скрытых флагов).
 */
export async function downloadAmbassadorCardPdf(opts) {
  ...
  const brandSubtitle = String(opts.brandSubtitle || '').trim() || brandName
  const elite = opts.pdfVariant === 'elite'
  const eliteLine = String(opts.elitePartnerLine || 'Elite Partner').trim() || 'Elite Partner'
  const ctaLine =
  ...
  if (elite) {
    ...
    doc.rect(3.2, 3.2, W - 6.4, H - 6.4, 'S')
    doc.setFillColor(252, 252, 253)
    doc.rect(4.2, 4.2, W - 8.4, H - 8.4, 'F')
  } else {
    doc.setFillColor(252, 252, 253)
    doc.rect(0, 0, W, H, 'F')
  ...
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(11)
  doc.text(brandName.toUpperCase(), textLeft, elite ? 15 : 13)

  doc.setFont('helvetica', 'normal')
  doc.setFontSize(6.2)
  doc.setTextColor(100, 116, 139)
  doc.text(brandSubtitle, textLeft, elite ? 19 : 17)

  doc.setFont('helvetica', 'bold')
  doc.setFontSize(6.5)
  doc.setTextColor(59, 130, 246)
  doc.text(officialStatusLine, textLeft, elite ? 22.5 : 20.5)

  if (elite) {
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(6)
    doc.setTextColor(180, 83, 9)
    doc.text(eliteLine, W - innerPad - 2, 11, { align: 'right' })
  }

  doc.setTextColor(15, 23, 42)
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(11)
  const nameLines = doc.splitTextToSize(displayName, elite ? 46 : 52)
  doc.text(nameLines, innerPad + 3, elite ? 31 : 29)

  doc.setFont('helvetica', 'normal')
  doc.setFontSize(8)
  doc.setTextColor(71, 85, 105)
  doc.text(ambassadorBadgeLabel, innerPad + 3, elite ? 31 + nameLines.length * 5 + 2 : 29 + nameLines.length * 5 + 2)

  doc.setFontSize(6.5)
  doc.setTextColor(100, 116, 139)
  const ctaLines = doc.splitTextToSize(ctaLine, elite ? 46 : 52)
  doc.text(ctaLines, innerPad + 3, H - 14)
  ...
  if (linkForQr) {
    ...
    const qrSize = 26
    doc.addImage(qrDataUrl, 'PNG', W - qrSize - innerPad - 3, elite ? 20 : 18, qrSize, qrSize)

    doc.setFontSize(5.5)
    doc.setTextColor(148, 163, 184)
    const linkLines = doc.splitTextToSize(linkForPrint, elite ? 46 : 52)
    doc.text(linkLines, innerPad + 3, H - 6)
  }
