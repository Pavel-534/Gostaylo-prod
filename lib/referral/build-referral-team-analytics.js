...
function normalizeRpcRow(row) {
  ...
  return {
    ...
    l3NetworkThb: round2(row.l3_network_thb ?? row.l3NetworkThb ?? 0),
    ...
    l1DistinctCount: Number(row.l1_distinct_count ?? row.l1DistinctCount) || 0,
    l2DistinctCount: Number(row.l2_distinct_count ?? row.l2DistinctCount) || 0,
    l3DistinctCount: Number(row.l3_distinct_count ?? row.l3DistinctCount) || 0,
  };
}
...
async function computeHostRetention(supabaseAdmin, referrerId) {
  ...
  const { data, error } = await supabaseAdmin.rpc('referral_host_retention_for_referrer', {
    p_referrer_id: uid,
  });

  if (error) {
    console.warn('[referral_host_retention_for_referrer]', error.message);
    return empty;
  }
  ...
  return {
    ratePercent: round2(row.rate_percent ?? row.ratePercent),
    numerator: Number(row.numerator) || 0,
    denominator: Number(row.denominator) || 0,
    definition: String(row.definition || empty.definition),
  };
}
...
export async function buildReferralTeamAnalytics(supabaseAdmin, profile, opts = {}) {
  ...
  const l3Period = periodRpc.l3NetworkThb;
  ...
  return {
    ...
    earnings: {
      ...
      breakdown: {
        ...
        l3NetworkThb: l3Period,
        l1DistinctCount: periodRpc.l1DistinctCount,
        l2DistinctCount: periodRpc.l2DistinctCount,
        l3DistinctCount: periodRpc.l3DistinctCount,
        byReferralType: {
