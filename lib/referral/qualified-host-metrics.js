...
function mergeQualificationMs(qualificationMsByReferee, refereeId, ms, nowMs) {
...
let warnedQualifiedHostBookingRpcMissing = false

/**
 * Legacy bookings row fetch for qualified-host qualification timestamps.
 * @param {import('@supabase/supabase-js').SupabaseClient} supabaseAdmin
 * @param {string[]} refereeIds
 */
export async function loadQualifiedHostCompletedBookingsLegacy(supabaseAdmin, refereeIds) {
 ...
 * @param {import('@supabase/supabase-js').SupabaseClient} supabaseAdmin
 * @param {string[]} refereeIds
 * @param {Record<string, number>} qualificationMsByReferee
 * @param {number} nowMs
 */
async function applyCompletedHostQualification(supabaseAdmin, refereeIds, qualificationMsByReferee, nowMs) {
  if (!refereeIds.length) return

  const { data, error } = await supabaseAdmin.rpc('qualified_host_first_completed_booking', {
    p_referee_ids: refereeIds,
  })

  if (!error) {
    for (const row of data || []) {
      mergeQualificationMs(
        qualificationMsByReferee,
        row?.referee_id,
        parseMs(row?.first_completed_at),
        nowMs,
      )
    }
    return
  ...
  if (isMissingRpcError(error, 'qualified_host_first_completed_booking')) {
    if (!warnedQualifiedHostBookingRpcMissing) {
      warnedQualifiedHostBookingRpcMissing = true
      console.warn(
        '[qualified-host-metrics] qualified_host_first_completed_booking RPC missing — legacy bookings fetch',
      )
    }
    const rows = await loadQualifiedHostCompletedBookingsLegacy(supabaseAdmin, refereeIds)
    for (const row of rows) {
      mergeQualificationMs(
        qualificationMsByReferee,
        row?.partner_id,
        parseMs(row?.updated_at) ?? parseMs(row?.created_at),
        nowMs,
      )
    }
    return
...
export async function loadQualifiedHostSets(supabaseAdmin, referrerId) {
  ...
  const [hostActivationRes] = await Promise.all([
    supabaseAdmin
    ...
    applyCompletedHostQualification(supabaseAdmin, refereeIds, qualificationMsByReferee, nowMs),
  ])
  ...
  for (const row of hostActivationRes.data || []) {
    ...
    mergeQualificationMs(
      qualificationMsByReferee,
      rid,
      parseMs(row?.earned_at) ?? parseMs(row?.created_at),
      nowMs,
    )
  }
