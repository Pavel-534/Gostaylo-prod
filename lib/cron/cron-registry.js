...
export const CRON_REGISTRY = Object.freeze([
  ...
  {
    path: '/api/cron/reconcile-yookassa-pending',
    ...
    notes:
      'Stage 202.7 — poll INITIATED MIR_RU intents via YooKassa GET (webhook lag); Vercel daily fallback; production ~every 10m via cron-job.org',
  },
  {
  ...
  },
  {
    ...
    notes: 'GET|POST run (Stage 202.10 — Vercel Cron is GET); PENDING_FISCAL / gateway / STALE_CRON',
  },
  {
    ...
    notes:
      'GET|POST run (202.10); ADR-203 Phase 1 shadow; ops_job_runs.zeroDrift; 30d gate before SoT flip; does not change getPartnerBalance',
  },
  {
