...
import { secretsTimingSafeEqual } from '@/lib/cron/secrets-timing-safe-equal.js'

export { secretsTimingSafeEqual }

/**
 * SSOT for `/api/cron/*` — trim `CRON_SECRET`, accept `Authorization: Bearer <token>` or `x-cron-secret`.
 * Comparison uses crypto.timingSafeEqual (AUDIT_03 W3.12).
...
export function verifyCronSecret(request) {
  ...
  if (cronHeader && secretsTimingSafeEqual(cronHeader, secret)) {
  ...
  if (auth) {
    ...
    if (bearer && secretsTimingSafeEqual(bearer, secret)) {
      return { ok: true, configured: true }
    }
  }
