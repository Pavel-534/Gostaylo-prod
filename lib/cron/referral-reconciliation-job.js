 ...
 * AUDIT_02 C2.5: также heal earned-без-wallet (идемпотентный creditWalletFromEarnedRows).
 */
...
const OPEN_LEDGER_STATUSES = ['pending', 'earned', 'earned_held']
...
export async function runReferralReconciliationJob(options = {}) {
  ...
  // Heal earned rows missing wallet credit (unlock crash window) — AUDIT_02 C2.5
  let walletHealBookingCount = 0
  let walletHealErrors = []
  if (!dryRun) {
    const { data: earnedOpen } = await supabaseAdmin
      .from('referral_ledger')
      .select('booking_id')
      .eq('status', 'earned')
      .limit(800)
    const healBookingIds = [
      ...new Set((earnedOpen || []).map((r) => String(r.booking_id || '')).filter(Boolean)),
    ].slice(0, Math.min(limit, 100))
    for (const bookingId of healBookingIds) {
      try {
        await ReferralLedgerService.creditWalletFromEarnedRows(bookingId)
        walletHealBookingCount += 1
      } catch (e) {
        walletHealErrors.push({ bookingId, error: e?.message || String(e) })
      }
    }
  }
  ...
  return {
    ...
    walletHealBookingCount,
    walletHealErrors: walletHealErrors.length ? walletHealErrors.slice(0, 20) : undefined,
