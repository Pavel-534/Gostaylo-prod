/**
 * Stage 201.112 / 201.113 — FX cron refresh: skip if DB is fresh, never upsert empty on upstream failure.
 * Soft HTTP 200 when existing rates kept (cron-job.org must not auto-disable on upstream 429).
 */
...
import { isFxUpstreamInCooldown } from '@/lib/services/fx-upstream-cooldown.js'
...
/**
 * Stage 201.113 — never return 429/5xx to external schedulers when DB rates were preserved.
 * cron-job.org treats non-2xx as failure and auto-disables after repeated failures.
 *
 * @param {{ keptExisting?: boolean, httpStatus?: number, success?: boolean }} result
 * @returns {number}
 */
export function httpStatusForExchangeRatesCronResult(result) {
  if (result?.keptExisting) return 200
  const n = Number(result?.httpStatus)
...
export async function runExchangeRatesCronRefresh() {
  ...
  if (isFxUpstreamInCooldown()) {
    return {
      ...
      skipped: true,
      keptExisting: true,
      message: 'Skipped, upstream rate-limited recently',
  ...
  if (!fetched.ok || !fetched.map) {
    return {
      // Soft success for scheduler: rates kept; ops still see errorMessage via route when needed
      success: true,
      httpStatus: 200,
      skipped: false,
      keptExisting: true,
