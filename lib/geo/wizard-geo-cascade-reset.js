...
export function applyWizardCityCascadeSelect(prev, opts = {}) {
  ...
  let next = {
    ...
    metadata: {
      ...
      geo_pin_country_conflict_dismissed: false,
      ...(clearAddress ? { street: '', house_number: '' } : {}),
    },
  }

  // Always reset region on city change — never keep a previous hub (e.g. Krasnodar after Chita).
  next.region =
    opts.regionCode != null && String(opts.regionCode).trim()
      ? String(opts.regionCode).trim()
      : ''
  if (next.metadata && typeof next.metadata === 'object') {
    if (opts.regionLabel) {
      next.metadata.region_label = String(opts.regionLabel).trim()
    } else if (!next.region) {
