...
import { matchLaunchCountryIsoFromName, findLaunchGeoByCode, launchGeoLabel, matchLaunchGeoByCoords, resolveLaunchCityCascade } from '@/lib/geo/launch-geo-index'
import {
  formatListingStreetAddress,
  normalizeNominatimLang,
} from '@/lib/geo/nominatim-lang'
import {
...
const HUB_BBOX = Object.freeze([
  ...
  { country: 'RU', region: 'RU-IRK', city: 'irkutsk', minLat: 52.15, maxLat: 52.4, minLon: 104.1, maxLon: 104.5 },
  { country: 'RU', region: 'RU-BU', city: 'ulan-ude', minLat: 51.7, maxLat: 51.95, minLon: 107.4, maxLon: 107.75 },
...
function matchRegionCity(countryCode, _haystack, lat, lon) {
  const hit = matchLaunchGeoByCoords(lat, lon, { countryCode })
  if (hit) {
    const cascade = resolveLaunchCityCascade(hit.code)
    if (cascade?.city_code && cascade.region_code) {
      return {
        region: { code: cascade.region_code },
        city: { code: cascade.city_code },
      }
    }
  }
...
export function mergeWizardFormGeoFromPin(prev, opts) {
  ...
  const uiLang = normalizeNominatimLang(opts.lang || geo.lang || 'en')
  ...
  // Stage 200.84 — short street line (not full OSM hierarchy / not forced EN dump)
  const streetLine =
    String(geo.streetAddress || '').trim() ||
    formatListingStreetAddress(geo.address, geo.displayName)
  if (streetLine) {
    next.address = streetLine
    const addrObj = geo.address && typeof geo.address === 'object' ? geo.address : null
    next.metadata = {
      ...next.metadata,
      street: String(addrObj?.road || addrObj?.pedestrian || addrObj?.street || '').trim() || streetLine,
      house_number: String(addrObj?.house_number || '').trim(),
    }
  }
  ...
  if (resolved.cityLabel) {
    // Prefer reverse/API city (already lang-aware) over EN seed fallback inside resolve
    const cityLabel = String(geo.city || resolved.cityLabel).trim()
    next.metadata.city_label = cityLabel
    next.metadata.city = cityLabel
  }

  // Stage 200.84 — region label by UI lang (seed) / Nominatim state / API regionLabel
  if (next.region) {
    const seedRegion = findLaunchGeoByCode(next.region)
    next.metadata.region_label =
      String(geo.regionLabel || geo.state || '').trim() ||
      launchGeoLabel(uiLang, seedRegion, next.region) ||
      String(next.metadata.region_label || '').trim()
  } else if (geo.regionLabel || geo.state) {
    next.metadata.region_label = String(geo.regionLabel || geo.state).trim()
