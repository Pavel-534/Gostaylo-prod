...
export function guessIanaTimezoneFromLatLon(lat, lon) {
  ...
  // European Russia / Urals (coarse); Far East still Moscow default for wizard SLA
  if (a >= 41 && a <= 82 && b >= 19 && b <= 180) return 'Europe/Moscow'
  if (a >= 36 && a <= 42.5 && b >= 26 && b <= 45) return 'Europe/Istanbul'
  return 'Asia/Bangkok'
}
