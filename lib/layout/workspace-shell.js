 ...
 * Bottom inset: ADR-100 `--app-bottom-nav-height` (PartnerMobileBottomNav on &lt;lg; 0 when hidden).
 */
export const WORKSPACE_FRAME_CLASS =
  'fixed inset-x-0 bottom-[var(--app-bottom-nav-height,0px)] top-[var(--app-header-height,64px)] flex min-h-0'
...
/** Scrollable page content inside main. `overflow-x-clip` — not `hidden` (breaks `position: sticky` in descendants). */
export const WORKSPACE_SCROLL_CLASS =
  'min-h-0 flex-1 overflow-x-clip overflow-y-auto overscroll-y-contain p-4 lg:p-6'

/** Bleed sticky subheaders to scrollport top (counteract WORKSPACE_SCROLL padding). */
export const WORKSPACE_SCROLL_STICKY_BLEED_CLASS =
  '-mx-4 -mt-4 px-4 lg:-mx-6 lg:-mt-6 lg:px-6'

/** DOM marker for workspace scrollport — SSOT for `useWorkspaceScrollTrigger`. */
export const WORKSPACE_SCROLL_ATTR = 'data-workspace-scroll'

/** Liquid compact header — sticky preview offset inside wizard scrollport (Stage 171.8). */
export const LISTING_WIZARD_STICKY_TOP_EXPANDED = '7.75rem'
export const LISTING_WIZARD_STICKY_TOP_COMPACT = '3.625rem'
 ...
 * Pair with `app-workspace-sidebar` (mobile: top=header, bottom=`--app-bottom-nav-height`).
 */
export const WORKSPACE_SIDEBAR_CLASS =
  'fixed z-50 flex w-[min(100%,17.5rem)] flex-col app-workspace-sidebar transition-all duration-300 ease-out lg:relative lg:z-auto lg:h-full lg:max-h-none lg:w-64 lg:shrink-0 lg:overflow-y-auto lg:translate-x-0'
