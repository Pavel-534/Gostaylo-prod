...
import { wizardFormProfileFromDb } from '@/lib/config/category-wizard-profile-db'
...
export const METADATA_KEYS_ALWAYS_ALLOWED = new Set([
  ...
  'region_label',
  'parent_location',
  /** Stage 200.87 — street/house for Location step (must survive normalize whitelist) */
  'street',
  'house_number',
  'geo_city_unmatched',
  'geo_source',
  'geo_pin_country',
  'geo_pin_country_conflict_dismissed',
  'base_price_asset',
  'check_in_instructions',
  'check_in_photos',
  /** Stage 201.86 — guest house rules (PDP «Good to know» + listing health) */
  'house_rules',
  /** Stage 200.99 — stay arrival hours (informational; PDP «Good to know») */
  'check_in_time',
  'check_out_time',
  'early_check_in_on_request',
  'late_check_out_on_request',
  'amenities',
 ...
 */
export function resolveWizardFormProfile(categorySlug, categoryNameFallback = '', wizardProfileFromDb) {
  const slug = String(categorySlug || '').toLowerCase().trim()
  const name = String(categoryNameFallback || '')

  const fromDb = wizardFormProfileFromDb(wizardProfileFromDb, categorySlug)
 ...
 * @param {string} categorySlug
 * @param {string} [categoryNameFallback]
 * @param {string | null | undefined} [wizardProfileFromDb]
 * @returns {WizardFormProfile}
 */
export function resolveWizardFormProfileWithLegacyName(
  categorySlug,
  categoryNameFallback = '',
  wizardProfileFromDb,
) {
  const base = resolveWizardFormProfile(categorySlug, categoryNameFallback, wizardProfileFromDb)
 ...
 */
export function getWizardStep1TransportFields(categorySlug, wizardProfileFromDb) {
  const profile = resolveWizardFormProfile(categorySlug, '', wizardProfileFromDb)
 ...
 * @returns {WizardFormFieldDef[]}
 */
export function getWizardSpecsSectionFields(categorySlug, categoryNameFallback = '', wizardProfileFromDb) {
  const profile = resolveWizardFormProfile(categorySlug, categoryNameFallback, wizardProfileFromDb)
  switch (profile) {
    case 'stay':
 ...
 * @returns {WizardFormFieldDef[]}
 */
export function getAllPartnerSearchMetadataFields(categorySlug, categoryNameFallback = '', wizardProfileFromDb) {
  return getAllPartnerSearchMetadataFieldsForProfile(
    profileFromSlugAndName(categorySlug, categoryNameFallback, wizardProfileFromDb),
  )
}
 ...
 * @returns {Set<string>}
 */
export function getAllowedWizardMetadataKeys(categorySlug, categoryNameFallback = '', wizardProfileFromDb) {
  const keys = new Set(METADATA_KEYS_ALWAYS_ALLOWED)
  const profile = resolveWizardFormProfileWithLegacyName(categorySlug, categoryNameFallback, wizardProfileFromDb)
...
function profileFromSlugAndName(categorySlug, categoryNameFallback, wizardProfileFromDb) {
  return resolveWizardFormProfileWithLegacyName(categorySlug, categoryNameFallback, wizardProfileFromDb)
}
