...
export const AUTO_HERO_TITLE_FALLBACK = 'Premium Rentals Worldwide';

/** Fallback для секции «топ» при AUTO и пустом словаре. */
export const AUTO_TOP_LISTINGS_TITLE_FALLBACK = 'Top Properties';
...
export function resolveHomeCopy(raw, resolveAuto, autoKey, autoEmptyFallback = null) {
  ...
  if (raw === HOME_COPY_AUTO_TOKEN) {
    if (typeof resolveAuto !== 'function' || !autoKey) {
      const fb = typeof autoEmptyFallback === 'string' ? autoEmptyFallback.trim() : '';
