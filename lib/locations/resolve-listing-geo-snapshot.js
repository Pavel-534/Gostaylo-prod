...
export function resolveListingGeoSnapshot(input = {}) {
  ...
  let districtIn = input.district

  if (!cic) {
    const place = matchLaunchGeoByCoords(input.latitude, input.longitude, { countryCode: cc })
    if (place) {
      ...
      if (cascade) {
        ...
        if (place.level === 'neighborhood') {
          const fromPlace = canonicalizeDistrict(place.label_en, cic)
          const existing = canonicalizeDistrict(districtIn, cic)
          if (!existing || !isPhuketDistrictName(existing)) {
            districtIn = fromPlace || districtIn
          }
        }
      }
    }
  ...
  const countryCode = String(input.countryCode || cc || '')
    ...
    .toUpperCase()
    .slice(0, 2)
  const district = canonicalizeDistrict(districtIn, cic || undefined)
  const meta =
...
export function inferGeoFromLegacyRow(row = {}) {
  ...
  const fromCoords = matchLaunchGeoByCoords(row.latitude, row.longitude)
  if (fromCoords) {
    const cascade = resolveLaunchCityCascade(fromCoords.code)
    if (cascade) {
      return resolveListingGeoSnapshot({
        countryCode: cascade.country_code,
        regionCode: cascade.region_code,
        cityCode: cascade.city_code,
        district:
          fromCoords.level === 'neighborhood'
            ? fromCoords.label_en
            : districtCanon || districtRaw,
