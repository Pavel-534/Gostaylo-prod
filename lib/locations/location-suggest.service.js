...
function popularCandidates(lang, inventory, geoRows) {
  ...
  // Cold start / empty inventory: do not advertise Bali/Dubai ghosts — fill from cities that have supply in geo.
  return inventoryBackedCityCandidates(lang, inventory, geoRows)
}

/**
 * Stage 201.82 — popular empty-q from cities that actually have ACTIVE listings.
 * @param {string} lang
 * @param {import('./location-inventory-cache.js').LocationInventoryIndex} inventory
 * @param {object[]} geoRows
 * @returns {LocationSuggestItem[]}
 */
function inventoryBackedCityCandidates(lang, inventory, geoRows) {
  const out = []
  for (const row of geoRows || []) {
    if (!row?.code) continue
    if (row.level !== 'city' && row.level !== 'neighborhood') continue
    const listing_count = inventory.cityCount(row.code)
    if (!(listing_count > 0)) continue
    out.push({
      value: row.code,
      level: 'city',
      type: 'city',
      label: geoLabel(lang, row, row.code),
      subtitle: countryLabelFromGeo(
        geoRows,
        row.iso_country || row.country_code || String(row.parent_code || '').slice(0, 2),
        lang,
      ),
      listing_count,
      city_code: row.code,
      region_code: row.parent_code || undefined,
      country_code: row.iso_country || row.country_code || undefined,
    })
  }
  return out.sort(
    (a, b) =>
      b.listing_count - a.listing_count || a.label.localeCompare(b.label),
  )
}
