...
export async function formatListingLocationLine(input = {}) {
  ...
  if (countryCode) {
    ...
    if (!countryName) {
      ...
      countryName =
        launchGeoLabel(lang, seed, '') ||
        getIsoCountryLabel(countryCode, lang) ||
        countryCode
    }
    if (
      countryName &&
...
export function resolveListingLocationPartsSync(listing, language = 'ru') {
  ...
  // Stage 200.47 — non-seed countries: ISO label, never invent Phuket
  const country =
    launchGeoLabel(language, seedCountry, '') ||
