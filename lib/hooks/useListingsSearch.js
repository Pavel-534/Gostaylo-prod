...
export function useListingsFetch({
  selectedCategory,
  /** Stage 67.0 — `categories.wizard_profile` для `selectedCategory` */
    ...
    async (isInitial = false) => {
      ...
      try {
        ...
        if (data.success) {
          ...
          // ─── Soft fallback (Airbnb-стиль) ─────────────────────────────────────────
          // Если строгий поиск (с датами/гостями) вернул 0 результатов —
          // автоматически делаем второй запрос без дат и показываем объекты
          // с флагом is_availability_mismatch=true чтобы не показывать пустой экран.
          if (newListings.length === 0 && hasDates && currentRequestId === requestIdRef.current) {
            const fallbackParams = new URLSearchParams()
            // Копируем все параметры кроме дат и гостей
            if (categoryForApi && categoryForApi !== 'all') fallbackParams.set('category', categoryForApi)
            if (debouncedWhere && debouncedWhere !== 'all') fallbackParams.set('where', debouncedWhere)
            if (qt.length >= 2) fallbackParams.set('q', qt)
            fallbackParams.set('limit', '100')
            appendExtraFiltersToParams(fallbackParams, extraFilters || {})

            try {
              const fallbackRes = await fetch(`${LISTINGS_SEARCH_API_PATH}?${fallbackParams.toString()}`)
              if (fallbackRes.ok) {
                const fallbackData = await fallbackRes.json()
                if (
                  currentRequestId === requestIdRef.current &&
                  fallbackData.success &&
                  (fallbackData.data.listings || []).length > 0
                ) {
                  const fallbackListings = fallbackData.data.listings.map((l) => ({
                    ...l,
                    is_availability_mismatch: true,
                  }))
                  setAllListings(fallbackListings)
                  setListings(fallbackListings.slice(0, itemsPerPage))
                  setDisplayedCount(itemsPerPage)
                  setMeta({ ...fallbackData.data.meta, isSoftFallback: true })
                  setError(null)
                  setLoading(false)
              ...
              // Fallback failed silently — показываем пустой экран как обычно
            }
          }
          // ────────────────────────────────────────────────────────────────────────────
          ...
          void trackProductEvent(ProductAnalyticsEvents.SEARCH, {
            ...
            where: debouncedWhere || undefined,
            results: newListings.length,
            semantic: semanticSiteEnabled || smartSearchOn || undefined,
          })
        } else {
