/**
 * Custom hooks for Listings Search functionality
 * Stage 128.1 — каталог на TanStack Query (без module-level searchCache).
 */

import { useState, useEffect, useCallback, useRef, useMemo } from 'react'
import { useQuery, keepPreviousData } from '@tanstack/react-query'
import { queryKeys } from '@/lib/query-keys'
import { buildCatalogSearchKeyParams } from '@/lib/catalog/build-catalog-search-params'
import { fetchCatalogSearch } from '@/lib/catalog/fetch-catalog-search'
import { mergeCatalogListingPages } from '@/lib/catalog/merge-catalog-listing-pages'
import { isCatalogCursorPaginationClientEnabled } from '@/lib/search/discovery-pipeline-flag-client'
import { CATALOG_SEARCH_SIMPLE_STALE_MS } from '@/lib/query-prefetch/catalog-query-constants'

const DEBOUNCE_DELAY = 300

// =========================================================
// DEBOUNCE HOOK
// =========================================================

export function useDebounce(value, delay = DEBOUNCE_DELAY) {
  const [debouncedValue, setDebouncedValue] = useState(value)

  useEffect(() => {
    const handler = setTimeout(() => setDebouncedValue(value), delay)
    return () => clearTimeout(handler)
  }, [value, delay])

  return debouncedValue
}

// =========================================================
// INTERSECTION OBSERVER HOOK
// =========================================================

export function useIntersectionObserver(callback, options = {}) {
  const targetRef = useRef(null)

  useEffect(() => {
    const target = targetRef.current
    if (!target) return

    const observer = new IntersectionObserver(([entry]) => {
      if (entry.isIntersecting) {
        callback()
      }
    }, { threshold: 0.1, ...options })

    observer.observe(target)
    return () => observer.disconnect()
  }, [callback, options])

  return targetRef
}

// =========================================================
// LISTINGS SEARCH (TanStack Query)
// =========================================================

export function useListingsFetch({
  selectedCategory,
  categoryWizardProfile = null,
  debouncedWhere,
  debouncedDateRange,
  debouncedGuests,
  checkInTime = '07:00',
  checkOutTime = '07:00',
  appliedMapBounds = null,
  appliedPolygon = null,
  extraFilters = null,
  debouncedTextQuery = '',
  liveTextQuery = '',
  smartSearchOn = false,
  semanticSiteEnabled = true,
  initialSemanticFromUrl = false,
  itemsPerPage = 12,
  /** Stage 167.0 — explicit catalog sort (`recommended` | `price_asc` | `price_desc` | `distance`). */
  catalogSort = 'recommended',
  /** Сегмент queryKey: смена валюты UI не ломает кэш поиска, но различает снимки при back-nav. */
  displayCurrency = 'THB',
  /** Stage 200.37 — centroid from geo_locations for distance sort. */
  whereSortCenter = null,
}) {
  const [displayedCount, setDisplayedCount] = useState(itemsPerPage)
  const pendingSemanticCommitRef = useRef(false)
  const [semanticCommitTick, setSemanticCommitTick] = useState(0)
  const isFirstFetchRef = useRef(true)
  const consumedUrlSemanticRef = useRef(false)
  const [loadingMore, setLoadingMore] = useState(false)
  const [cursorPages, setCursorPages] = useState([])
  const [nextCursor, setNextCursor] = useState(null)
  const [hasMoreCursor, setHasMoreCursor] = useState(false)

  const cursorPaginationActive = useMemo(
    () => isCatalogCursorPaginationClientEnabled(catalogSort),
    [catalogSort],
  )

  const searchKeyParams = useMemo(() => {
    let useSemantic = false
    if (pendingSemanticCommitRef.current && smartSearchOn && semanticSiteEnabled) {
      useSemantic = true
      pendingSemanticCommitRef.current = false
    } else if (
      isFirstFetchRef.current &&
      initialSemanticFromUrl &&
      !consumedUrlSemanticRef.current &&
      smartSearchOn &&
      semanticSiteEnabled
    ) {
      useSemantic = true
      consumedUrlSemanticRef.current = true
    }
    if (isFirstFetchRef.current) {
      isFirstFetchRef.current = false
    }

    const textQuery = useSemantic
      ? String(liveTextQuery || '').trim()
      : String(debouncedTextQuery || '').trim()

    return buildCatalogSearchKeyParams({
      selectedCategory,
      categoryWizardProfile,
      debouncedWhere,
      debouncedDateRange,
      debouncedGuests,
      checkInTime,
      checkOutTime,
      appliedMapBounds,
      appliedPolygon,
      extraFilters,
      textQuery,
      useSemantic,
      limit: cursorPaginationActive ? undefined : '100',
      catalogSort,
      whereSortCenter,
    })
  }, [
    selectedCategory,
    categoryWizardProfile,
    debouncedWhere,
    debouncedDateRange,
    debouncedGuests,
    checkInTime,
    checkOutTime,
    appliedMapBounds,
    appliedPolygon,
    extraFilters,
    debouncedTextQuery,
    liveTextQuery,
    smartSearchOn,
    semanticSiteEnabled,
    semanticCommitTick,
    displayCurrency,
    catalogSort,
    cursorPaginationActive,
    whereSortCenter,
  ])

  const searchIdentityKey = useMemo(
    () => JSON.stringify(searchKeyParams),
    [searchKeyParams],
  )

  const isSimpleSearch = !searchKeyParams._flags?.hasDates &&
    !searchKeyParams._flags?.hasBounds &&
    !searchKeyParams._flags?.hasPolygon &&
    !searchKeyParams._flags?.hasExtra

  const query = useQuery({
    queryKey: queryKeys.catalog.search({
      ...searchKeyParams,
      displayCurrency: String(displayCurrency || 'THB').toUpperCase(),
    }),
    queryFn: () => fetchCatalogSearch(searchKeyParams),
    placeholderData: keepPreviousData,
    staleTime: isSimpleSearch ? CATALOG_SEARCH_SIMPLE_STALE_MS : 0,
    gcTime: 30 * 60 * 1000,
  })

  useEffect(() => {
    if (!cursorPaginationActive) {
      setCursorPages([])
      setNextCursor(null)
      setHasMoreCursor(false)
      return
    }
    setCursorPages([])
    setNextCursor(null)
    setHasMoreCursor(false)
  }, [searchIdentityKey, cursorPaginationActive])

  useEffect(() => {
    if (!cursorPaginationActive || !query.data || query.isPlaceholderData) return
    const pagination = query.data.meta?.pagination
    setCursorPages([query.data.listings || []])
    setNextCursor(
      typeof pagination?.next_cursor === 'string' && pagination.next_cursor.trim()
        ? pagination.next_cursor.trim()
        : null,
    )
    setHasMoreCursor(Boolean(pagination?.hasMore))
  }, [
    cursorPaginationActive,
    query.data,
    query.dataUpdatedAt,
    query.isPlaceholderData,
    searchIdentityKey,
  ])

  const allListings = useMemo(() => {
    if (!cursorPaginationActive) return query.data?.listings ?? []
    if (cursorPages.length > 0) return mergeCatalogListingPages(cursorPages)
    return query.data?.listings ?? []
  }, [cursorPaginationActive, cursorPages, query.data?.listings])

  const meta = query.data?.meta ?? null

  useEffect(() => {
    if (cursorPaginationActive) return
    setDisplayedCount(itemsPerPage)
  }, [query.dataUpdatedAt, itemsPerPage, cursorPaginationActive])

  const listings = useMemo(() => {
    if (cursorPaginationActive) return allListings
    return allListings.slice(0, displayedCount)
  }, [allListings, displayedCount, cursorPaginationActive])

  const hasMore = cursorPaginationActive
    ? hasMoreCursor
    : displayedCount < allListings.length

  const effectiveDisplayedCount = cursorPaginationActive
    ? allListings.length
    : displayedCount

  const loading = query.isPending && !query.isPlaceholderData
  const isTransitioning = query.isFetching && !query.isPending

  const commitSemanticSearch = useCallback(() => {
    pendingSemanticCommitRef.current = true
    setSemanticCommitTick((t) => t + 1)
  }, [])

  const loadMore = useCallback(() => {
    if (cursorPaginationActive) {
      if (loadingMore || !hasMoreCursor || !nextCursor) return
      setLoadingMore(true)
      void (async () => {
        try {
          const result = await fetchCatalogSearch({
            ...searchKeyParams,
            cursor: nextCursor,
          })
          const pagination = result.meta?.pagination
          setCursorPages((prev) => [...prev, result.listings || []])
          setNextCursor(
            typeof pagination?.next_cursor === 'string' && pagination.next_cursor.trim()
              ? pagination.next_cursor.trim()
              : null,
          )
          setHasMoreCursor(Boolean(pagination?.hasMore))
        } catch {
          /* keep current pages; user can retry via scroll */
        } finally {
          setLoadingMore(false)
        }
      })()
      return
    }

    if (loadingMore || displayedCount >= allListings.length) return
    setLoadingMore(true)
    setTimeout(() => {
      setDisplayedCount((prev) => Math.min(prev + itemsPerPage, allListings.length))
      setLoadingMore(false)
    }, 300)
  }, [
    cursorPaginationActive,
    loadingMore,
    hasMoreCursor,
    nextCursor,
    searchKeyParams,
    displayedCount,
    allListings.length,
    itemsPerPage,
  ])

  const retry = useCallback(() => {
    void query.refetch()
  }, [query])

  const fetchListings = useCallback(() => {
    void query.refetch()
  }, [query])

  return {
    listings,
    allListings,
    displayedCount: effectiveDisplayedCount,
    hasMore,
    cursorPaginationActive,
    loading,
    loadingMore,
    meta,
    error: query.error?.message ?? null,
    isTransitioning,
    searchKeyParams,
    fetchListings,
    commitSemanticSearch,
    loadMore,
    retry,
  }
}
