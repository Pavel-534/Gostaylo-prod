...
async function fetchPartnerBookings({ partnerId, status }) {
  ...
  if (!res.ok) {
    const msg = data?.error || (res.status === 401 ? 'Auth required' : res.status === 403 ? 'Access denied' : 'Failed to fetch bookings')
    throw new Error(msg)
  }
...
export function useUpdateBookingStatus() {
  ...
  return useMutation({
    ...
    // Success toasts — caller-owned (localized in dashboard booking-actions / bookings page)
