...
export function usePublicSearchFilters({
  ...
  useEffect(() => {
    ...
    const id = schedule(() => {
      ...
      try {
        const keyParams = buildCatalogSearchKeyParams({
          selectedCategory: debouncedFilterSnapshot.selectedCategory,
          debouncedWhere: debouncedFilterSnapshot.where,
          debouncedDateRange: debouncedFilterSnapshot.dateRange,
          debouncedGuests: debouncedFilterSnapshot.guests,
          checkInTime: debouncedFilterSnapshot.checkInTime,
          checkOutTime: debouncedFilterSnapshot.checkOutTime,
          textQuery: debouncedFilterSnapshot.textQuery,
        })
        ...
        void qc.prefetchQuery({
          ...
          queryFn: () => fetchCatalogSearch(keyParams),
          staleTime: CATALOG_SEARCH_SIMPLE_STALE_MS,
        })
      } catch {
        /* ignore */
      }
