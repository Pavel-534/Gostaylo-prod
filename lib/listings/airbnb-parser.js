...
export async function fetchAirbnbViaApifyTask(canonicalUrl, token, taskId, extraInput = {}) {
  ...
  if (!res.ok) {
    ...
    console.error('[airbnb-parser] Full Task error response:', JSON.stringify(data).slice(0, 500))

    const err = new Error(`Apify Task error ${res.status} (task ${taskId}): ${msg}`)
    err.httpStatus = res.status

    if (res.status === 403 && /trial|rent|paid|payment|subscription|billing/i.test(msg)) {
      // Платный актор / истёк пробный период
      err.code = 'TASK_PAYMENT_REQUIRED'
      console.warn(`[airbnb-parser] Task ${taskId} requires payment — actor trial expired`)
    } else if (res.status === 404) {
      err.code = 'TASK_NOT_FOUND'
    } else if (res.status === 400) {
      const errType = data?.error?.type || ''
      err.code = errType === 'run-failed' ? 'TASK_RUN_FAILED' : 'TASK_BAD_INPUT'
    } else {
      err.code = 'TASK_HTTP_ERROR'
    }
    throw err
  }
  ...
  if (!first) {
    ...
    const err = new Error(`Apify Task ${taskId} returned empty dataset`)
    err.code = 'TASK_EMPTY_DATASET'
    throw err
  }
