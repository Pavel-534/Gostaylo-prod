...
/**
 * Акторы Apify, которые пробуем по очереди при 404 или пустом датасете.
...
const APIFY_ACTOR_CANDIDATES = [
  {
    // Официальный актор Apify — поддерживает прямые ссылки на листинги
...
async function tryApifyActor(canonicalUrl, token, actor, extraInput = {}) {
  ...
  try {
    ...
    console.error(`[airbnb-parser] Network error calling actor ${actor.id}:`, msg)
    const err = new Error(`Apify network error (actor ${actor.id}): ${msg}`)
  ...
  if (!res.ok) {
    ...
    console.error(
      `[airbnb-parser] Actor ${actor.id} → HTTP ${res.status}:`, msg,
      '\n[airbnb-parser] Full Apify error response:', JSON.stringify(data).slice(0, 1000)
    )

    const err = new Error(`Apify error ${res.status} (actor ${actor.id}): ${msg}`)
    err.code = 'APIFY_HTTP_ERROR'
    err.httpStatus = res.status
    // 400 = актор не поддерживает такой формат URL / input — цепочку продолжать
    // 422 = неверный input — цепочку продолжать
    // 401/403 = проблемы с токеном — прерываем сразу
...
export async function fetchAirbnbViaApify(canonicalUrl, token, primaryActorId, extraInput = {}) {
  ...
  /**
   * Коды, при которых продолжаем fallback-цепочку:
   *  ACTOR_NOT_FOUND         — 404, актора нет
   *  APIFY_EMPTY_DATASET     — актор запустился, но вернул пустой датасет
   *  APIFY_ACTOR_REJECTED_INPUT — HTTP 400/422: актор не поддерживает такой URL/input
   */
  const SKIPPABLE = new Set(['ACTOR_NOT_FOUND', 'APIFY_EMPTY_DATASET', 'APIFY_ACTOR_REJECTED_INPUT'])
  ...
  console.error(`[airbnb-parser] All Apify actors failed. Tried: ${triedIds.join(', ')}. Codes: ${[...skippedCodes].join(', ')}`)

  // Если хотя бы один актор вернул данные, но пустые — объявление недоступно
  ...
  // Все акторы либо не найдены, либо отклонили input
  const err = new Error(
