...
export function normalizeAirbnbPayloadForMapper(item, canonicalUrl) {
  // ── Лог входных ключей для диагностики ──────────────────────────────────
  if (item && typeof item === 'object') {
    console.log('[airbnb-parser] normalizeAirbnbPayloadForMapper keys:', Object.keys(item).slice(0, 40).join(', '))
  }
  ...
  // ── Текстовые поля из всех известных схем акторов ───────────────────────
  const name =
    asString(item.name) ||
    asString(item.title) ||
...
export async function fetchAirbnbViaApify(canonicalUrl, token, primaryActorId, extraInput = {}) {
  ...
  for (const actor of queue) {
    ...
    try {
      ...
      lastError = err
      if (err.code === 'ACTOR_NOT_FOUND' || err.code === 'APIFY_EMPTY_DATASET') {
        // 404 или пустой датасет — пробуем следующий актор
        skippedCodes.add(err.code)
        continue
      }
      // Любая другая ошибка (таймаут, HTTP 4xx/5xx кроме 404) — пробрасываем сразу
      throw err
    }
  }

  // Все акторы либо недоступны, либо не нашли данные
  const allNotFound = [...skippedCodes].every((c) => c === 'ACTOR_NOT_FOUND')
  const allEmpty = [...skippedCodes].every((c) => c === 'APIFY_EMPTY_DATASET')

  console.error(`[airbnb-parser] All Apify actors failed. Tried: ${triedIds.join(', ')}. Codes: ${[...skippedCodes].join(', ')}`)

  if (allEmpty || skippedCodes.has('APIFY_EMPTY_DATASET')) {
    const err = new Error(
