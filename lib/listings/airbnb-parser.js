...
const APIFY_ACTOR_CANDIDATES = [
  {
    ...
    // Использует Apify Proxy автоматически.
    id: 'apify~airbnb-scraper',
    buildInput: (url, extra) => ({
      startUrls: [{ url }],
      maxListings: 1,
      // Включаем Apify Proxy — без него официальный актор часто блокируется
...
async function tryApifyActor(canonicalUrl, token, actor, extraInput = {}) {
  ...
  if (!res.ok) {
    ...
    const runId = data?.error?.message?.match(/run ID: ([^,)]+)/)?.[1] || ''
    ...
    if (res.status === 400) {
      if (errType === 'run-failed') {
        // Актор ЗАПУСТИЛСЯ, но провалился во время выполнения (Airbnb заблокировал, прокси не помогли)
        // Пробуем следующий актор в цепочке
        err.code = 'APIFY_RUN_FAILED'
      } else {
        // Актор отклонил input (не поддерживает такой формат URL / поля)
        err.code = 'APIFY_ACTOR_REJECTED_INPUT'
      }
    } else if (res.status === 422) {
      err.code = 'APIFY_ACTOR_REJECTED_INPUT'
    }
    // 401/403 = проблемы с токеном — код остаётся APIFY_HTTP_ERROR, цепочка прерывается
    throw err
  }
...
export async function fetchAirbnbViaApify(canonicalUrl, token, primaryActorId, extraInput = {}) {
  ...
  if (skippedCodes.has('APIFY_RUN_FAILED')) {
    const err = new Error(
      ...
      `Возможно, требуется Residential Proxy. Установите APIFY_AIRBNB_ACTOR_ID и убедитесь, что в аккаунте Apify есть доступ к прокси.`
    )
