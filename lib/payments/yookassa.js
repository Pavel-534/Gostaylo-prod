...
/** Stage 200.75 — bound hung PSP calls so serverless invocations are not pinned. */
const YOOKASSA_FETCH_TIMEOUT_MS = 10_000
 ...
 * Stage 202.7 — include guest `user_id` when known (YooKassa registry ↔ bookings.renter_id).
 */
...
/**
 * Stage 202.12 — deterministic UUID v4 from seed (YooKassa Idempotence-Key must be UUID).
 * Same intent id → same key across concurrent initiates (unlike randomUUID()).
 * @param {string} seed
 * @returns {string}
 */
export function deterministicUuidV4FromSeed(seed) {
  const hash = createHash('sha256').update(String(seed || ''), 'utf8').digest()
...
/**
 * Stable Idempotence-Key for create payment (Stage 202.12).
 * Prefer persisted metadata; else deterministic UUID from intent.id (no random race).
 *
...
export function resolveIdempotenceKey(intent) {
  ...
  if (intentId) {
    return {
      key: deterministicUuidV4FromSeed(`yookassa-idempotence:${intentId}`),
      generated: true,
    }
  }
 ...
 * @param {string} [input.userId] — guest profile id (`bookings.renter_id`)
...
export async function createPayment(input) {
  ...
  try {
    ...
    if (isFetchAbortError(err)) {
      return { ok: false, code: 'YOOKASSA_TIMEOUT', error: err?.message || 'timeout' }
    }
    return { ok: false, code: 'YOOKASSA_NETWORK_ERROR', error: err?.message || 'network_error' }
  }
...
export async function getPayment(paymentId) {
  ...
  let res
  try {
    res = await fetch(`${apiBase}/payments/${encodeURIComponent(id)}`, {
      method: 'GET',
