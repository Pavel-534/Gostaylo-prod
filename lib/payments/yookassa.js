...
/** @see Stage 130.2 — Vercel branding, no hardcoded product name */
export function getBrandName() {
  const name =
    process.env.NEXT_PUBLIC_APP_NAME || process.env.NEXT_PUBLIC_SITE_NAME || 'Booking'
...
/**
 * Payment description for YooKassa dashboard / cardholder (Stage 130.5 KYC).
 * @param {{ brandName?: string, bookingId: string, categorySlug?: string }} params
 */
export function buildYookassaPaymentDescription({ brandName, bookingId, categorySlug }) {
  ...
  const text = `${brand}: предоплата бронирования №${displayId}${categoryPart}`
