...
export function createLeafletPricePillDivIcon(L, priceText, options = {}) {
  ...
  // Stage 201.76 — do not bake selected into HTML; ListingPriceMarker toggles class in DOM
  // so hover/select never calls Leaflet setIcon (that blinks the open popup).
  void options
  return L.divIcon({
    className: 'gostaylo-price-pill-icon-root',
    html: `<div class="gostaylo-price-pill-marker-slot"><div class="gostaylo-price-pill">${safe}</div></div>`,
