 ...
 * @typedef {{ id: string, lat: number, lng: number, priceLabel: string, selected?: boolean }} MapPricedPin
 */
...
export function createLeafletPricePillDivIcon(L, priceText, options = {}) {
  ...
  // Stage 201.79 — dark slate default (matches Android force-dark); selected = mint + amber ring via DOM.
  void options
  return L.divIcon({
    className: 'gostaylo-price-pill-icon-root',
    html: `<div class="gostaylo-price-pill-marker-slot"><div class="gostaylo-price-pill" style="background:#0f172a;color:#ffffff;border:1px solid #1e293b;-webkit-text-fill-color:#ffffff">${safe}</div></div>`,
