...
export function filterCatalogRailListingsForMapViewport(listings, opts = {}) {
  ...
  const selectedListing = selectedId
    ? source.find((l) => String(l?.id || '').trim() === selectedId) || null
    : null
  ...
  if (pinIds.size > 0) {
    ...
    if (byPin.length > 0) {
      if (!selectedListing) return byPin
      const hasSelected = byPin.some((l) => String(l?.id || '').trim() === selectedId)
      return hasSelected ? byPin : [selectedListing, ...byPin]
    }
  }
  ...
  if (bbox) {
    ...
    if (byBbox.length > 0) {
      if (!selectedListing) return byBbox
      const hasSelected = byBbox.some((l) => String(l?.id || '').trim() === selectedId)
      return hasSelected ? byBbox : [selectedListing, ...byBbox]
    }
  }
