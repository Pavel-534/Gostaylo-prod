...
export function buildCatalogSearchKeyParams(input) {
  const {
    ...
    cursor = null,
    /** Optional pre-resolved centroid from /api/v2/geo/resolve-where (Stage 200.37). */
  ...
  const polygon =
    appliedPolygon != null && String(appliedPolygon).trim()
      ? String(appliedPolygon).trim()
      : null
  const hasPolygon = Boolean(polygon)
  const hasBounds = !hasPolygon && boundsParamsReady(appliedMapBounds)
  const hasExtra = hasActiveExtraFilters(extraFilters)
  const sortCenter =
    (!hasPolygon && bboxCenter(appliedMapBounds)) ||
    (whereSortCenter &&
    Number.isFinite(Number(whereSortCenter.lat)) &&
    Number.isFinite(Number(whereSortCenter.lng))
      ? { lat: Number(whereSortCenter.lat), lng: Number(whereSortCenter.lng) }
      : null)
  ...
  return {
    ...
    _flags: { hasDates, hasBounds, hasPolygon, hasExtra, intervalMode, cursorPaginationActive },
  }
}
...
export function catalogSearchKeyParamsToUrlSearchParams(keyParams) {
  ...
  if (polygon) {
    params.set('polygon', String(polygon))
  } else if (boundsParamsReady(bounds)) {
