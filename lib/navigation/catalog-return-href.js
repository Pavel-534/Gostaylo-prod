 ...
 * Stage 201.81 — also clears map viewport memory when not returning to `#map`.
 */

import { clearCatalogMapViewport, peekCatalogMapViewport } from './catalog-map-viewport-memory.js'
import {
  readCatalogMobileMapOpenFromLocation,
  writeCatalogMobileMapHash,
} from './catalog-mobile-map-hash.js'
 ...
 * Saves catalog query (+ `#map` when map/camera was active) for soft-back.
 */
export function captureCatalogReturnBeforePdp() {
  if (typeof window === 'undefined') return
  const path = String(window.location.pathname || '').replace(/\/+$/, '') || '/'
  if (path === '/listings') {
    const mapOpen = readCatalogMobileMapOpenFromLocation()
    const hasViewport = Boolean(peekCatalogMapViewport())
    // Guarantee `#map` in return href so soft-back reopens the sheet + camera restore.
    if (mapOpen || hasViewport) {
