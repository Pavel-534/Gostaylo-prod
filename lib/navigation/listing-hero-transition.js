/**
 * Catalog → PDP navigation (Stage 171.22 / 200.15 / 201.100).
 * Do **not** wrap App Router `router.push` in `document.startViewTransition`:
 * the callback returns before RSC commits, so Chromium (Samsung Internet)
 * freezes the old snapshot (teal pending ring) then flashes an empty shell.
 */

import { dispatchOptimisticNavPending } from './optimistic-nav-href.js'
import { persistLiveRouteScroll } from './route-scroll-memory.js'
import { captureCatalogReturnBeforePdp } from './catalog-return-href.js'
...
/**
 * Persist catalog return URL + optimistic pending. Use on `<Link>` click
 * **without** preventDefault so Next can navigate from the prefetched RSC.
 * @param {string | null | undefined} [href]
 */
export function prepareListingPdpNavigation(href) {
...
/**
 * Map popup / non-Link open. Immediate `navigate()` — no View Transition.
 * @param {() => void} navigate
 * @param {string | number | null | undefined} listingId
...
export function navigateWithListingHeroTransition(navigate, listingId, href) {
  void listingId
  prepareListingPdpNavigation(href)
  navigate()
}
