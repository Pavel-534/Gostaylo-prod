 ...
 * @param {{ anchorHref?: string, anchorTop?: number } | null} [extra]
 */
...
export function isRouteScrollLayoutReady(entry, maxScroll) {
  ...
  return Number(maxScroll) >= Math.max(0, y - 24)
}

/**
 * When to apply / wait / commit a pending restore (Stage 201.109).
 * Do not apply a footer/card anchor before the page is tall enough — that
 * parks the viewport in the middle after later sections mount.
 *
 * @param {{
 *   layoutReady: boolean,
...
export function resolveRouteScrollRestoreStep(opts) {
  ...
  if (layoutReady) {
    return { wait: false, applyMode: anchorStable ? 'anchor' : null, commit: true }
  }
  if (budgetExceeded) {
    return { wait: false, applyMode: hasY ? 'y' : (anchorStable ? 'anchor' : null), commit: true }
  }
  return { wait: true, applyMode: hasY ? 'y' : null, commit: false }
}
...
export function applyRouteScrollEntry(entry, opts = {}) {
  ...
  if (entry.y > 0) {
    ...
    window.scrollTo(0, Math.min(entry.y, maxScroll || entry.y))
