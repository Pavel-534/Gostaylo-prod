 ...
 */
export async function loadTreasuryRailsSummary(opts = {}) {
  ...
  const { data: readyRows } = await supabaseAdmin
    .from('bookings')
    .select(
      'id, partner_id, partner_earnings_thb, status, metadata, listing_id, guest_name, special_requests, renter_id',
    )
  ...
  const keepBooking = (b) => !excludeTest || !isFintechTestBookingRow(b)

  for (const b of readyRows || []) {
    if (!keepBooking(b)) continue
