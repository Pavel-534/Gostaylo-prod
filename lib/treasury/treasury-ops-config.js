/**
 * Stage 105 / 125.5 — SSOT режима казначейства: ручной Concierge, emergency pause, пороги алертов.
 * Операционные алерты — только `critical_signal_events` (см. treasury-monitoring-alerts.js).
 * system_settings.general — глобальные toggles (pause, manual mode), не журнал алертов.
 */
 ...
 *   controlledLive: { active: boolean, startedAt?: string, startedBy?: string, reason?: string, firstPaymentAt?: string, firstPaymentBookingId?: string } | null,
...
export async function loadTreasuryOpsSettings() {
  ...
  if (v.controlled_live && typeof v.controlled_live === 'object') {
    base.controlledLive = {
      active: Boolean(v.controlled_live.active),
      startedAt: v.controlled_live.startedAt || null,
      startedBy: v.controlled_live.startedBy || null,
      reason: v.controlled_live.reason || '',
      firstPaymentAt: v.controlled_live.firstPaymentAt || null,
      firstPaymentBookingId: v.controlled_live.firstPaymentBookingId || null,
    }
  }
