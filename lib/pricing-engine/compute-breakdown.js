 ...
 *
 * FX markup (Stage 200.88): when payment_currency ≠ listing_base_currency.
 * - Non-THB pay: worse customer rate → guest pays more in pay currency; mid THB payable unchanged.
 * - THB pay × foreign base: guest pays mid + fx_markup_thb in THB (brutto); partner netto unchanged.
 */
...
export function computeFinalBreakdown(input) {
  ...
  if (applyFxMarkup) {
    if (payCur === 'THB') {
      // Guest pays THB for a non-THB listing: surcharge in THB (real cash on ledger).
      fxRawRate = 1
      fxCustomerRate = 1 / safeMult
      // Integer THB surcharge so brutto − mid == fx_markup_thb (ledger identity).
      fxMarkupThb = Math.round(guestPayableRoundedThb * (safeMult - 1))
    } else if (Number.isFinite(fxRawRate) && fxRawRate > 0) {
      // Guest pays foreign currency: worse customer rate; mid THB payable unchanged.
  ...
  if (payCur !== 'THB' && Number.isFinite(fxCustomerRate) && fxCustomerRate > 0) {
    ...
    if (applyFxMarkup && fxMarkupThb > 0) {
      breakdown.fx_raw_rate_to_thb = fxRawRate
      breakdown.fx_customer_rate_to_thb = fxCustomerRate
      breakdown.total_guest_brutto = {
        amount: guestPayableRoundedThb + fxMarkupThb,
        currency: 'THB',
      }
    } else {
      breakdown.total_guest_brutto = {
