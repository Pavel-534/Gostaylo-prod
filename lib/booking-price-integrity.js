 ...
 * When guest pays THB for a non-THB listing, attestation/charge use brutto (mid + FX).
 * @param {object | null | undefined} pricingSnapshot
 * @returns {number | null}
 */
export function guestRoundedTotalFromSnapshotV2(pricingSnapshot) {
  ...
  const mid = Math.round(Number(midRaw))
  if (!Number.isFinite(mid) || mid < 0) return null

  const brutto = fb.total_guest_brutto
  if (brutto && typeof brutto === 'object' && String(brutto.currency || '').toUpperCase() === 'THB') {
    const charge = Math.round(Number(brutto.amount))
    if (Number.isFinite(charge) && charge > mid) return charge
  }
  return mid
}
...
export function guestPayableRoundedThbFromBooking(booking) {
  ...
  // Chat Special Offer quote is capture SSOT over stale catalog / v2 breakdown (AUDIT_02 C2.6).
  const inv = snap.chat_invoice_quote
  if (inv && typeof inv === 'object') {
    const quoted = Math.round(Number(inv.amount_thb))
    if (Number.isFinite(quoted) && quoted > 0) return quoted
  }
  const meta =
    booking.metadata && typeof booking.metadata === 'object' ? booking.metadata : null
  if (meta) {
    const metaQuoted = Math.round(Number(meta.chat_invoice_amount_thb))
    if (Number.isFinite(metaQuoted) && metaQuoted > 0) return metaQuoted
  }
