...
async function resolveGuestTermsVersion() {
  try {
    return await LegalVersionsService.getGuestTermsVersion()
  } catch {
    return CURRENT_LEGAL_TERMS_VERSION
  }
}

async function resolvePartnerTermsVersion() {
  try {
    return await LegalVersionsService.getPartnerTermsVersion()
  } catch {
    return CURRENT_PARTNER_TERMS_VERSION
  }
}
...
export async function recordGuestLegalConsent({ userId, bookingId = null }) {
  ...
  const termsVersion = await resolveGuestTermsVersion()
...
export async function recordPartnerLegalConsent(userId) {
  ...
  const iso = new Date().toISOString()
  const version = await resolvePartnerTermsVersion()
