...
import { resolveListingTimeZoneFromMetadata, isValidIanaTimeZone } from '@/lib/geo/listing-timezone-ssot';
import {
  BANGKOK_IANA,
...
/**
 * @param {{ listingTimeZone?: string | null, listingMetadata?: object | null, listingId?: string | null }} [ctx]
 * @returns {string}
 */
export function resolveEscrowThawListingTimeZone(ctx = {}) {
  const direct = String(ctx.listingTimeZone || '').trim();
  if (direct) {
    if (isValidIanaTimeZone(direct)) {
      return direct;
    }
    notifyInvalidListingTimezone(ctx.listingId, direct);
    return BANGKOK_IANA;
  }
  return resolveListingTimeZoneFromMetadata(ctx.listingMetadata, { listingId: ctx.listingId });
}
...
export function computeEscrowThawAt({
  ...
  const tz = resolveEscrowThawListingTimeZone({ listingTimeZone, listingMetadata, listingId });
