...
export function getOrderTypeLabel(type, language) {
  ...
  if (lang === 'en') {
    ...
    if (normalized === 'activity') return 'Tours'
    if (normalized === 'service') return 'Services'
    return 'Stays'
  }
  if (lang === 'th') {
    if (normalized === 'transport') return 'การเดินทาง'
    if (normalized === 'activity') return 'ทัวร์'
    if (normalized === 'service') return 'บริการ'
    return 'ที่พัก'
  }
  if (lang === 'zh') {
    if (normalized === 'transport') return '交通'
    if (normalized === 'activity') return '旅游'
    if (normalized === 'service') return '服务'
    return '住宿'
  }
  if (normalized === 'transport') return 'Транспорт'
  if (normalized === 'activity') return 'Туры'
  if (normalized === 'service') return 'Услуги'
  return 'Жильё'
}
