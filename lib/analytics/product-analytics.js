 ...
 * Stage 202.44 — autocapture off + session recording off (console rate-limit spam / CPU).
 */
      ...
      .then(({ default: posthog }) => {
        ...
        if (!posthog.__loaded) {
          posthog.init(key, {
            ...
            // Stage 202.44 — explicit product events only; default autocapture flooded client rate limits.
            autocapture: false,
 ...
 */
function appendAnalyticsTap(eventName, properties = {}) {
  if (typeof window === 'undefined') return
  try {
    const tap = globalThis.__GSL_ANALYTICS_TAP__
    if (Array.isArray(tap)) {
      tap.push({ event: eventName, properties: { ...properties } })
    }
  } catch {
    /* ignore */
  }
...
export async function trackProductEvent(eventName, properties = {}) {
  if (typeof window === 'undefined') return
  appendAnalyticsTap(eventName, properties)
