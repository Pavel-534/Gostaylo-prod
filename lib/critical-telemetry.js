...
import { scrubPiiDetailLines, scrubPiiValue } from '@/lib/logging/pii-scrub.js'
...
async function persistCriticalSignalRow(key, opts) {
  try {
    ...
    const { error } = await supabaseAdmin.from('critical_signal_events').insert({
      ...
      detail: scrubPiiValue({
        correlationId: opts.correlationId || null,
        severity,
        detailLines: scrubPiiDetailLines(detailLines).slice(0, 30),
...
export function recordCriticalSignal(key, opts = {}) {
  const severityRaw = String(opts.severity || 'WARN').toUpperCase()
  ...
  void notifySystemAlert(html, {
    ...(reply_markup ? { reply_markup } : {}),
    severity: isCritical ? 'CRITICAL' : severity === 'INFO' ? 'INFO' : 'WARN',
  })
}
