/**
 * Stage 167.2 — «Популярно рядом» personalization v1 (non-ML, ADR-167 §2.8).
 *
...
export async function getGuestPersonalizedForYouRecommendations(guestViewItems, opts = {}) {
  ...
  const centroid =
    ctx.viewCentroid ?? (await resolveWhereSortCenter({ where: opts.where }))
...
async function fetchCandidateListings(opts = {}) {
  ...
  const centroid = opts.centroid ?? (await resolveWhereSortCenter({ where: opts.where }))
