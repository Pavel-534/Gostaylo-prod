...
/**
 * Stage 171.41 — use LISTINGS_SELECT_LITE only.
 * Do not re-embed `categories` (PostgREST: `listings_categories_1` twice).
 */
const ANCHOR_SELECT = LISTINGS_SELECT_LITE
 ...
 * @param {Set<string>} [guestCategoryIds]
 */
function sortSimilarCandidates(rows, distanceKmById, guestCategoryIds = null) {
  const guestBoostKm = 2
  return [...rows].sort((a, b) => {
    let aDist = distanceKmById.get(String(a.id))
    let bDist = distanceKmById.get(String(b.id))
    aDist = Number.isFinite(aDist) ? aDist : Number.POSITIVE_INFINITY
    bDist = Number.isFinite(bDist) ? bDist : Number.POSITIVE_INFINITY

    if (guestCategoryIds?.size) {
      if (guestCategoryIds.has(String(a.category_id))) aDist = Math.max(0, aDist - guestBoostKm)
      if (guestCategoryIds.has(String(b.category_id))) bDist = Math.max(0, bDist - guestBoostKm)
    }
 ...
 */
async function loadGuestCategoryIdsForSimilar(guestViewItems) {
  const ids = pruneGuestViewedItems(guestViewItems).map((row) => row.id)
  ...
  if (error) {
    console.error('[similar-listings] guest category boost failed', error.message || error)
    return new Set()
  }
 ...
 * @param {{ limit?: number, guestViewItems?: Array<{ id: string, t: number }> }} [opts]
...
export async function findSimilarListings(anchorListingId, opts = {}) {
  ...
  const guestCategoryIds = opts.guestViewItems?.length
    ? await loadGuestCategoryIdsForSimilar(opts.guestViewItems)
    : null
  ...
  const ranked = sortSimilarCandidates(
    filtered,
    distanceKmById,
    guestCategoryIds?.size ? guestCategoryIds : null,
  ).slice(0, limit)
  ...
  return {
    ...
    meta: {
      ...
      engine,
      guest_category_boost: Boolean(guestCategoryIds?.size),
    },
  }
}
