...
export function mergeAirbnbPreviewWizard(prevFormData, preview) {
  ...
  const nextFormData = {
    ...
    metadata: {
      ...prevMeta,
      // flat legacy keys (used by form inputs)
      ...
      // structured blocks — сохраняем для БД (не теряем при wizard-мерже)
      property_info: { ...(prevMeta.property_info || {}), ...pi },
      amenities_detail: md.amenities || prevMeta.amenities_detail,
      rules: md.rules || prevMeta.rules,
      // presets-список для чипов в UI
