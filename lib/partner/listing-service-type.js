...
import { canonicalizeHousingPropertyTypeSlug } from '@/lib/listing/housing-property-type.js'

/** @typedef {'stay' | 'transport' | 'service' | 'tour'} ListingServiceType */

/**
 * @param {unknown} slug — канонический `categories.slug`
 * @param {string | null | undefined} [wizardProfileFromDb] — `categories.wizard_profile`
 * @returns {ListingServiceType}
 */
export function inferListingServiceTypeFromCategorySlug(slug, wizardProfileFromDb) {
  return inferListingServiceTypeFromRegistry(slug, wizardProfileFromDb)
}
 ...
 */
export function categorySlugMatchesListingServiceType(slug, serviceType, wizardProfileFromDb) {
  return inferListingServiceTypeFromCategorySlug(slug, wizardProfileFromDb) === serviceType
}
...
export function defaultMetadataForListingServiceType(serviceType, existingMeta = {}) {
  ...
  const stayType = canonicalizeHousingPropertyTypeSlug(base.property_type)
  return {
    ...base,
    timezone: tz,
    // Never invent «Villa» — type comes from the housing category (Stage 201.92).
    ...(stayType ? { property_type: stayType } : { property_type: '' }),
