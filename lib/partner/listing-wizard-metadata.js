...
export function normalizePartnerListingMetadata(
  ...
  if (housingLike) {
    ...
    const synced = applyHousingPropertyTypeFromCategorySlug(out, slug)
    if (synced.property_type) {
      out.property_type = synced.property_type
  ...
  const street = String(out.street ?? '').trim().slice(0, 200)
  if (street) out.street = street
  else delete out.street
  const houseNo = String(out.house_number ?? '').trim().slice(0, 32)
  if (houseNo) out.house_number = houseNo
  else delete out.house_number
