 ...
 */
export function normalizePartnerListingMetadata(metadata = {}, categorySlug = '', categoryNameFallback = '') {
  const out = { ...metadata }
  const slug = String(categorySlug || '').toLowerCase()
  const nameFb = String(categoryNameFallback || '')

  const svcType = inferListingServiceTypeFromRegistry(slug)
  const housingLike =
  ...
  if (isTransportListingCategory(slug)) {
    ...
    const isHeli = slug === 'helicopter' || slug === 'helicopters'
    if (isHeli) normalizePremiumAirSeaFields(out)
  }

  if (isYachtLikeCategory(slug)) {
    delete out.bedrooms
    delete out.bathrooms
    delete out.max_guests
    delete out.area
  ...
  const wizardProfile = resolveWizardFormProfile(slug, nameFb)
  if (
    wizardProfile === 'nanny' ||
  ...
  const allowed = getAllowedWizardMetadataKeys(slug, nameFb)
