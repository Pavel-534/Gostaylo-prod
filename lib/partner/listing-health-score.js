...
export const LISTING_HEALTH_WEIGHT_PHOTOS = 40
export const LISTING_HEALTH_WEIGHT_DESCRIPTION = 25
export const LISTING_HEALTH_WEIGHT_AMENITIES = 20
/** Soft tip only — below photos/description so partners are not stalled at 70/100. Stage 201.34 */
export const LISTING_HEALTH_WEIGHT_RULES = 15

/**
 * Wizard step (1–6) for incomplete health parts — Basics=1, Photos=3.
 * @param {string} partKey
 * @returns {number | null}
 */
export function listingHealthWizardStepForPart(partKey) {
  switch (String(partKey || '')) {
    case 'photos':
      return 3
    case 'description':
    case 'amenities':
...
/**
 * DOM `data-listing-health-anchor` value for a health part (step jump scroll target).
 * @param {string} partKey
 * @returns {string | null}
 */
export function listingHealthAnchorForPart(partKey) {
  switch (String(partKey || '')) {
    case 'photos':
      return 'photos'
    case 'description':
...
/**
 * After setCurrentStep — scroll/focus the field for `partKey` (retries while step remounts).
 * @param {string | null | undefined} partKey
 * @param {{ attempts?: number, delayMs?: number }} [opts]
 */
export function scrollToListingHealthAnchor(partKey, opts = {}) {
  if (typeof document === 'undefined') return
  const anchor = listingHealthAnchorForPart(partKey)
  if (!anchor) return
  const maxAttempts = opts.attempts ?? 10
  const delayMs = opts.delayMs ?? 50

  const tryScroll = (attempt) => {
    const el = document.querySelector(`[data-listing-health-anchor="${anchor}"]`)
    if (!el) {
    ...
    const focusable =
      el.matches('textarea, input, button, select, [tabindex]:not([tabindex="-1"])')
        ? el
        : el.querySelector('textarea, input, button, select, [tabindex]:not([tabindex="-1"])')
...
export function calculateListingHealthScore(input = {}) {
  ...
  /** @type {Array<{ key: string, labelKey: string, weight: number, ok: boolean, earned: number, current?: number, min?: number, tipKey: string, tipParams?: Record<string, number>, wizardStep: number | null }>} */
  let parts
  ...
  if (mode === 'stay') {
    ...
    parts = [
      {
        ...
        wizardStep: listingHealthWizardStepForPart('photos'),
      },
      {
        key: 'description',
        ...
        wizardStep: listingHealthWizardStepForPart('description'),
      },
      {
        key: 'amenities',
        ...
        wizardStep: listingHealthWizardStepForPart('amenities'),
      },
      {
        key: 'rules',
        ...
        wizardStep: listingHealthWizardStepForPart('rules'),
      },
    ]
    ...
    parts = [
      ...
      },
      {
        ...
        wizardStep: listingHealthWizardStepForPart('features'),
      },
      {
        key: 'pickup',
        ...
        wizardStep: listingHealthWizardStepForPart('pickup'),
      },
    ]
  } else {
    ...
    parts = [
      ...
      },
      {
        ...
        wizardStep: listingHealthWizardStepForPart('details'),
      },
    ]
  }
