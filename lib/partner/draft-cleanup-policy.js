 ...
 * ADR-210 Slice 1: Concierge Supply drafts are never GC'd (see isConciergeProtectedDraft).
 */
...
/**
 * ADR-210 — Concierge Supply drafts must not be deleted by cleanup-drafts.
 * Skip when metadata.concierge_protected is true/'true', or import_platform starts with "concierge".
 *
 * @param {{ metadata?: object, import_platform?: string|null }} listing
 */
export function isConciergeProtectedDraft(listing) {
  if (!listing || typeof listing !== 'object') return false
  const meta = listing.metadata
  if (meta && typeof meta === 'object') {
    if (meta.concierge_protected === true || meta.concierge_protected === 'true') {
      return true
    }
  }
  const platform = String(listing.import_platform || '')
    .trim()
    .toLowerCase()
  return platform.startsWith('concierge')
}
...
export function shouldDeleteExpiredDraft(listing, opts = {}) {
  if (!isListingDraftMetadata(listing?.metadata)) return false
  if (isConciergeProtectedDraft(listing)) return false
