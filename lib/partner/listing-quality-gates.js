...
/** Soft publish (PENDING only) — Stage 200.23 / H-P1-1 */
export const LISTING_SOFT_MIN_PHOTOS = 1
export const LISTING_SOFT_MIN_DESCRIPTION = 40

/** Full publish photos — Stage 200.29 (aligned with soft: 1). */
export const LISTING_QUALITY_MIN_PHOTOS = LISTING_SOFT_MIN_PHOTOS
/** Soft floor only — Stage 200.28 dropped the old 120-char hard gate. */
export const LISTING_QUALITY_MIN_DESCRIPTION = LISTING_SOFT_MIN_DESCRIPTION
...
/** Жильё, транспорт, яхты, туры — координаты для карты и spatial search (Stage 116.1 → 170.11). */
const GEO_REQUIRED_PROFILES = new Set([
  'stay',
  'transport',
...
export function validateListingPublishQuality(input = {}) {
  ...
  // Stage 200.83 — district optional when city + coords exist (micro-label / reverse fill)
...
/**
 * Soft publish: lower bar for PENDING moderation (not ACTIVE).
 * Full {@link validateListingPublishQuality} still required for ACTIVE / Featured.
 * @param {Parameters<typeof validateListingPublishQuality>[0]} input
 * @returns {{ ok: boolean, errors: string[], codes: string[] }}
 */
export function validateListingSoftPublishQuality(input = {}) {
  const errors = []
  const codes = []
  ...
  const description = String(input.description || '').trim()
  if (description.length < LISTING_SOFT_MIN_DESCRIPTION) {
    errors.push(
      `Description must be at least ${LISTING_SOFT_MIN_DESCRIPTION} characters for soft publish`,
    )
    codes.push('LISTING_DESCRIPTION_TOO_SHORT_SOFT')
  }

  const images = Array.isArray(input.images) ? input.images.filter(Boolean) : []
  if (images.length < LISTING_SOFT_MIN_PHOTOS) {
    errors.push(`At least ${LISTING_SOFT_MIN_PHOTOS} photo is required for soft publish`)
    codes.push('LISTING_PHOTOS_INSUFFICIENT_SOFT')
  }
...
export function buildListingPublishQualityChecklist(input = {}) {
  ...
  const items = [
    ...
    {
      code: 'LISTING_DISTRICT_OPTIONAL',
      ok: true,
      i18nKey: 'listingQuality_districtOptional',
      params: {
        filled: Boolean(String(input.district || '').trim()) ? 1 : 0,
      },
    },
    {
...
/**
 * Required checklist progress (skips optional district row).
 * @param {{ items?: Array<{ code?: string, ok?: boolean }> } | null | undefined} checklist
 * @returns {{ done: number, total: number, ok: boolean }}
 */
export function listingPublishQualityProgress(checklist) {
  const items = Array.isArray(checklist?.items)
    ? checklist.items.filter((item) => item?.code !== 'LISTING_DISTRICT_OPTIONAL')
    : []
  const total = items.length
  const done = items.filter((item) => item?.ok).length
  return { done, total, ok: Boolean(checklist?.ok) && total > 0 && done === total }
}

/** Metadata keys → partner wizard labelKey (human labels in checklist). */
const PUBLISH_METADATA_FIELD_LABEL_KEY = {
  bedrooms: 'fieldBedrooms',
  bathrooms: 'fieldBathrooms',
  max_guests: 'fieldMaxGuests',
  vehicle_year: 'fieldVehicleYear',
  seats: 'fieldVehicleSeats',
  passengers: 'fieldPassengers',
  duration: 'fieldDuration',
  experience_years: 'fieldExperienceYears',
  languages: 'fieldLanguages',
  cuisine_types: 'fieldCuisineTypes',
}
...
export function formatListingQualityChecklistLabel(item, t) {
  ...
  if (item.i18nKey === 'listingQuality_metadataField' && params.field) {
    const labelKey = PUBLISH_METADATA_FIELD_LABEL_KEY[String(params.field)]
    if (labelKey) {
      const human = t(labelKey, String(params.field))
      if (human && human !== labelKey) params.field = human
    }
  }
  const fb = item.i18nKey === 'listingQuality_metadataField'
    ? `Fill required field: ${params.field || ''}`
    : item.i18nKey
