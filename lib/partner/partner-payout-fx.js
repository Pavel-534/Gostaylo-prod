...
export async function sumPendingPartnerPayoutReserveThb(supabaseAdmin, partnerId) {
  ...
  const { data, error } = await supabaseAdmin
    .from('payouts')
    .select('gross_amount, final_amount, amount, payout_rail, metadata')
  ...
  let sum = 0
  for (const row of (data || []).filter(isOpenPartnerHostPayoutRow)) {
    const thb =
...
export async function listPartnerIdsWithOpenPayoutRequests(supabaseAdmin) {
  ...
  const { data, error } = await supabaseAdmin
    .from('payouts')
    .select('partner_id, payout_rail, metadata')
    .in('status', OPEN_PAYOUT_RESERVE_STATUSES)
  ...
  return new Set(
    (data || []).filter(isOpenPartnerHostPayoutRow).map((r) => r.partner_id).filter(Boolean),
  )
}
