...
export const RUB_PAYOUT_METHOD_IDS = new Set(['pm-bank-ru', 'pm-card-ru'])
export const USDT_PAYOUT_METHOD_ID = 'pm-usdt-trc20'
 ...
 * @param {{ channel?: string, currency?: string } | null} [method]
 */
export function resolvePayoutCurrency(payoutMethodId, method = null) {
  ...
  if (id === USDT_PAYOUT_METHOD_ID || channel === 'CRYPTO' || methodCurrency === 'USDT') {
    return 'USDT'
  }
  if (RUB_PAYOUT_METHOD_IDS.has(id) || (channel === 'BANK' && methodCurrency === 'RUB') || (channel === 'CARD' && methodCurrency === 'RUB')) {
    return 'RUB'
  }
