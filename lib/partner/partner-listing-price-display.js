...
export function resolvePartnerListingPriceParts(row = {}) {
  ...
  // Stale L1 `{amount:0,currency:USD}` from draft-before-country — prefer ledger when present.
  if (Number.isFinite(assetAmount) && assetAmount === 0 && Number.isFinite(ledgerThb) && ledgerThb > 0) {
    return {
      ledgerThb,
      primaryAmount: ledgerThb,
      primaryCurrency: baseCurrency && baseCurrency !== 'USD' ? baseCurrency : 'THB',
