 ...
 * Stage 200.20 — no silent copy-fill across locales; storefront falls back via getListingText.
 */
...
/**
 * Persist only the UI-language slot (+ any locales already present, e.g. from AI).
 * Does **not** copy RU→EN/ZH/TH — missing locales stay empty for honest storefront fallback.
 *
...
export function mergeDescriptionTranslationsForSave(formData, uiLang) {
  ...
  const dt = { ...(typeof prev === 'object' && prev ? prev : {}) }
