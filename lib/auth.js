...
export async function signUp({ email, password, name, role = 'RENTER' }) {
  try {
    ...
    const apiUrl = `${baseUrl}/api/v2/auth/register`;
    console.log('[AUTH] SignUp attempt for:', email);
    console.log('[AUTH] API URL:', apiUrl);
    console.log('[AUTH] Request payload:', { email: email.toLowerCase(), password: '***', firstName: name, role });
    ...
    if (!response.ok) {
      ...
      try {
        ...
        return { success: false, error: errJson.error || `Server error: ${response.status}` };
      } catch {
        return { success: false, error: `Server error: ${response.status}` };
      }
    }
