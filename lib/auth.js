/**
 * FunnyRent 2.1 - Supabase Auth Client
...
export async function signIn(email, password) {
  ...
  localStorage.setItem('funnyrent_user', JSON.stringify(userInfo));
  localStorage.setItem('funnyrent_auth_token', data.session?.access_token || '');
...
export async function signOut() {
  ...
  localStorage.removeItem('funnyrent_impersonating');
  localStorage.removeItem('funnyrent_original_user');
  return { success: true };
}
...
export function impersonateUser(targetUser) {
  ...
  if (currentUser) {
    ...
    if (user.role === 'ADMIN') {
      localStorage.setItem('funnyrent_original_user', currentUser);
      localStorage.setItem('funnyrent_impersonating', 'true');
      localStorage.setItem('funnyrent_user', JSON.stringify(targetUser));
      return true;
    }
  }
  return false;
...
export function returnFromImpersonation() {
  const originalUser = localStorage.getItem('funnyrent_original_user');
  if (originalUser) {
    localStorage.setItem('funnyrent_user', originalUser);
    localStorage.removeItem('funnyrent_original_user');
    localStorage.removeItem('funnyrent_impersonating');
...
export function isImpersonating() {
  return localStorage.getItem('funnyrent_impersonating') === 'true';
}
