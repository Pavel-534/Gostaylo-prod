...
import {
  classifyPartnerCancelledRow,
  cancelledRowTimestamp,
} from './partner-cancel-classify.js'
...
export async function fetchFinancialReliabilityInputs(partnerId) {
  ...
  let partnerDeclinedCount = 0
  let partnerInitiatedCancelCount = 0
  let partnerCancelWeighted = 0
  for (const b of cancelRows || []) {
    const kind = classifyPartnerCancelledRow(b, pid)
    if (!kind) continue
    const ts = cancelledRowTimestamp(b)
    const w = incidentRecencyWeight(ts)
    if (kind === 'declined') {
      partnerDeclinedCount += 1
      partnerDeclinedWeighted += w
    } else {
      partnerInitiatedCancelCount += 1
      partnerCancelWeighted += w
    }
  }
...
export async function merge12mTooltipBatch(partnerIds, trustMap) {
  ...
  const { data: canRows, error: c12Err } = await supabaseAdmin
    .from('bookings')
    .select('partner_id, status, metadata, updated_at, created_at')
    .in('partner_id', unique)
    .eq('status', 'CANCELLED')
  ...
  for (const r of canRows || []) {
    ...
    const kind = classifyPartnerCancelledRow(r, pid)
    if (!kind) continue
    if (kind === 'declined') {
      declCount[pid] = (declCount[pid] || 0) + 1
    } else {
      const ca = cancelledRowTimestamp(r)
