 ...
 * Stage 79.0 — Google/Apple; Stage 78.0 legal_terms_accepted_at.
 */
...
export async function upsertOAuthProfile({ supabaseAdmin, authUser, legalAcceptedFromRegisterFlow, request }) {
  ...
  if (profile) {
    ...
    if (!staff && !profile.avatar && avatarUrl) {
    ...
    let legalAcceptedAt = profile.legal_terms_accepted_at;
    if (
    ...
    const outLegal = fresh?.legal_terms_accepted_at ?? updates.legal_terms_accepted_at ?? profile.legal_terms_accepted_at;

    return {
      ok: true,
      profile: fresh || { ...profile, ...updates },
      created: false,
      needsLegalCompletion: !outLegal,
    };
  }
      ...
      if (race) {
        const { data: mergedRow, error: mergeErr } = await supabaseAdmin
          ...
          .update({
            ...
            is_verified: true,
            updated_at: new Date().toISOString(),
            ...(legalAcceptedFromRegisterFlow && !race.legal_terms_accepted_at
        ...
        return {
          ...
          profile: out,
          created: false,
          needsLegalCompletion: !out.legal_terms_accepted_at,
        };
      }
    }
  ...
  if (mergedReferral && prevalidatedReferral?.referrerId) {
    try {
  ...
  const needsLegalCompletion = !legalAcceptedFromRegisterFlow;
