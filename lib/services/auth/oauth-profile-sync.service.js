 ...
 * Stage 189.1 — verified-email merge only; AUTH_PROFILE_AUTH_CONFLICT → link-conflict token.
 */
...
import {
  createAuthLinkConflict,
  isAuthUserEmailVerified,
  resolveOAuthProviderId,
  upsertProfileAuthIdentity,
} from '@/lib/auth/account-linking.service.js';
...
async function recordOAuthIdentity(profileId, authUser) {
  if (!profileId || !authUser?.id) return;
  const provider = resolveOAuthProviderId(authUser);
  await upsertProfileAuthIdentity(String(profileId), {
    provider,
    providerSubject: String(authUser.id),
    authUserId: String(authUser.id),
    metadata: { email: authUser.email || null },
  });
}

async function openAuthProfileConflict(
  supabaseAdmin,
  profile,
  authUser,
  normalizedEmail,
  emailVerified,
  challengerProfileId = null,
) {
  const providerId = resolveOAuthProviderId(authUser);
  const authUid = String(authUser.id);
  const conflict = await createAuthLinkConflict({
    provider: providerId,
    occupiedProfileId: profile.id,
    challengerProfileId: challengerProfileId || null,
    authUserId: authUid,
    providerEmail: normalizedEmail,
    providerSubject: authUid,
    metadata: { reason: 'AUTH_PROFILE_AUTH_CONFLICT', emailVerified },
  });
  return {
    ok: false,
    error: 'AUTH_PROFILE_AUTH_CONFLICT',
    profile: null,
    created: false,
    conflictToken: conflict.ok ? conflict.token : null,
    conflictProvider: providerId,
    occupiedProfileId: profile.id,
  };
}
 ...
 * @param {string|null} [opts.challengerProfileId] — logged-in profile attempting to link OAuth (Stage 189.1)
 */
export async function upsertOAuthProfile({
  ...
  const providerId = resolveOAuthProviderId(authUser);
  const emailVerified = isAuthUserEmailVerified(authUser);
  if (!authUid) {
  ...
  // Stage 189.1: merge by email only when provider email is verified.
  ...
  if (profile) {
    ...
    if (existingAuth && existingAuth !== authUid) {
      return openAuthProfileConflict(
        supabaseAdmin,
        profile,
        authUser,
        normalizedEmail,
  ...
  if (profile) {
    ...
    await recordOAuthIdentity(fresh?.id || profile.id, authUser);
  ...
  if (insErr) {
    if (insErr.code === '23505' || String(insErr.message || '').includes('unique')) {
      ...
      if (race) {
        ...
        await recordOAuthIdentity(out.id, authUser);
        return {
          ok: true,
          profile: out,
          created: false,
  ...
  await recordOAuthIdentity(created.id, authUser);
