...
/**
 * Active (non-expired) calendar_blocks rows for one iCal source URL.
 * @param {*} supabase
 * @param {string} listingId
 * @param {string} sourceUrl
 */
export async function countActiveCalendarBlocksForSource(supabase, listingId, sourceUrl) {
  if (!supabase || !listingId || !sourceUrl) return 0
  ...
  if (error) {
    console.warn('[ical-calendar-blocks-sync] countActiveCalendarBlocksForSource:', error.message)
    return 0
  }
...
export async function syncIcalSourceToCalendarBlocks(supabase, listingId, sourceConfig, options = {}) {
  ...
  const existingActive = await countActiveCalendarBlocksForSource(supabase, listingId, url)
  if (ranges.length === 0 && existingActive > 0) {
    const msg = `SUSPICIOUS_EMPTY_FEED: parsed 0 events but ${existingActive} active block(s) exist`
    logStructured({
      module: 'ical-calendar-blocks-sync',
      stage: '173.1',
      level: 'warn',
      error: 'SUSPICIOUS_EMPTY_FEED',
      listingId,
      sourceUrl: url,
      existingActive,
    })
    void notifySystemAlert(
      `⏰ <b>iCal: SUSPICIOUS_EMPTY_FEED</b>\n` +
        `listing: <code>${escapeSystemAlertHtml(listingId)}</code>\n` +
        `source: <code>${escapeSystemAlertHtml(url)}</code>\n` +
        `active_blocks=${existingActive} — replace skipped`,
    )
    return {
      status: 'skipped',
      events_count: 0,
...
export default {
  ...
  countActiveCalendarBlocksForSource,
