...
/**
 * Удалить старые блоки источника и вставить новые.
...
export async function replaceCalendarBlocksForSource(
  ...
  const { error: insErr } = await supabase.from('calendar_blocks').insert(blocks)
  if (insErr) {
...
export async function syncIcalSourceToCalendarBlocks(supabase, listingId, sourceConfig, options = {}) {
  ...
  let ranges = parseIcalToOccupancyRanges(fetchResult.text)
