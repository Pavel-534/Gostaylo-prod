/**
 * FunnyRent 2.1 - Dynamic Forex Engine
 * Real-time currency conversion with hidden 3.5% markup ("FunnyRate")
 * 
...
const EXCHANGE_API_KEY = process.env.EXCHANGE_API_KEY || '9098becb6ccc3ca934ab5884';
const EXCHANGE_API_URL = `https://v6.exchangerate-api.com/v6/${EXCHANGE_API_KEY}`;

// FunnyRate markup (3.5% hidden margin)
...
export class ForexService {
  ...
  static async fetchLiveRates(baseCurrency = 'THB') {
    try {
      const response = await fetch(
        `${EXCHANGE_API_URL}/latest/${baseCurrency}`,
        {
  ...
  /**
   * Convert THB to target currency with FunnyRate markup
  ...
  static async convertFromThb(amountThb, targetCurrency, applyMarkup = true) {
    ...
    // Apply FunnyRate markup (user pays 3.5% more)
