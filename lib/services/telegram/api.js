...
import { withTransientRetry } from '@/lib/ops/with-transient-retry.js'

/**
 * Подмешать главное inline-меню к sendTelegram.
 * @param {'ru' | 'en' | 'zh' | 'th'} lang
 * @param {object} [options]
 * @param {'partner' | 'partner_guest' | 'renter' | 'guest'} [options.menuVariant] — если задано, БД не дергаем
...
export async function sendTelegram(chatId, text, options = {}) {
  ...
  try {
    ...
    // Stage 202.42 — one retry on Abort/timeout only (outbound Bot API, not inbound webhook).
    const data = await withTransientRetry(
      async () => {
        ...
        if (!json.ok && response.status >= 500) {
          throw new Error(`Telegram HTTP ${response.status}: ${json.description || response.statusText}`)
        }
        return json
      },
      {
        ...
        isRetryable: (err) => isTransientOpsError(err),
      },
    )
    if (!data.ok) {
      console.error('[TELEGRAM sendMessage]', data.error_code, data.description || 'not ok')
    }
