...
import { insertAiUsageLog, TASK_TELEGRAM_PARSER } from '@/lib/ai/usage-log'
import { getSiteDisplayName } from '@/lib/site-url'

const OPENAI_MODEL = 'gpt-4o-mini'

const LISTING_PARSE_SYSTEM = `You are the listing copywriter for ${getSiteDisplayName()} (Phuket rentals). Parse the user's Telegram message/caption and return ONLY valid JSON with exactly these keys:
...
/**
 * Одна запись в ai_usage_logs на каждый HTTP-вызов парсера (успех или ошибка после ответа API).
 * @param {string|null|undefined} userId — profiles.id партнёра
 * @param {string} model
 * @param {Record<string, unknown>} usage
 * @param {Record<string, unknown>} metadata
 */
async function logTelegramParserUsage(userId, model, usage, metadata) {
  await insertAiUsageLog({
    userId: userId != null ? String(userId).trim() : undefined,
    listingId: null,
    taskType: TASK_TELEGRAM_PARSER,
 ...
 */
async function callOpenAiListingParse(caption, lang, apiKey, userId) {
  ...
  const model = (data && data.model) || OPENAI_MODEL
  ...
  if (!res.ok) {
    ...
    await logTelegramParserUsage(userId, model, usage, {
      lang,
      outcome: 'http_error',
 ...
 * @param {string} caption
 * @param {{ lang?: 'ru'|'en', openaiApiKey?: string|null, userId?: string|null }} opts
 * @returns {Promise<
...
export async function parseListingCaption(caption, opts = {}) {
  ...
  try {
    const norm = await callOpenAiListingParse(text, lang, String(apiKey).trim(), opts.userId)
