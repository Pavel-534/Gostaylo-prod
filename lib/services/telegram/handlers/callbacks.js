...
export async function handleCallbackQuery(callbackQuery) {
  ...
  try {
    ...
    const bookingRes = await fetch(
      `${supabaseUrl}/rest/v1/bookings?id=eq.${bookingId}&select=*,listing:listings(title,owner_id),partner:profiles!bookings_partner_id_fkey(telegram_id,first_name,language,preferred_language)`,
      {
        headers: {
          apikey: serviceKey,
          Authorization: `Bearer ${serviceKey}`,
