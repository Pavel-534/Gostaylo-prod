...
async function finalizeDraftListing({
  ...
  const { commissionRate } = await PricingService.calculateCommission(price || 1000, ownerId)
...
async function processLazyRealtorPhotos(chatId, messages, firstName, lang) {
  ...
  try {
    ...
    const parsed = await parseListingCaption(caption, { lang, openaiApiKey, userId: ownerId })
    if (!parsed.ok) {
