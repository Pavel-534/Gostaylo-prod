...
export async function handlePhotoUpload(chatId, message, firstName, lang) {
  ...
  try {
    ...
    if (!partner) {
      await sendTelegram(chatId, t.lazyNotLinked(lang), withMainMenu(lang))
      return
    }

    if (!['PARTNER', 'ADMIN'].includes(partner.role)) {
      await sendTelegram(chatId, t.lazyNoRights(lang), withMainMenu(lang))
      return
    }
    ...
    if (listingRes.ok) {
      ...
      await sendTelegram(chatId, t.lazyDraftCreateError(), withMainMenu(lang))
    }
  } catch (e) {
    console.error('[PHOTO ERROR]', e)
    await sendTelegram(chatId, t.lazyPhotoError(), withMainMenu(lang))
  }
}
