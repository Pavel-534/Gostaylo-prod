...
import { sendTelegram, withMainMenu } from '../api.js'
...
export async function handleStatusCheck(chatId, lang) {
  ...
  try {
    ...
    if (profile) {
      await sendTelegram(chatId, t.statusOk(profile, lang), withMainMenu(lang))
    } else {
      await sendTelegram(chatId, t.statusUnlinked(), withMainMenu(lang))
    }
  } catch (e) {
    console.error('[STATUS ERROR]', e)
    await sendTelegram(chatId, t.statusError(), withMainMenu(lang))
  }
}
...
export async function handleDeepLink(chatId, userId, firstName, username, telegramId, lang) {
  ...
  try {
    ...
    if (!profile) {
      await sendTelegram(chatId, t.deepLinkUserNotFound(userId), withMainMenu(lang))
      return
    }

    if (profile.telegram_id && profile.telegram_id !== chatId.toString()) {
      await sendTelegram(chatId, t.deepLinkAlreadyLinked(), withMainMenu(lang))
      return
    }
    ...
    await sendTelegram(chatId, t.deepLinkError(), withMainMenu(lang))
  }
}
...
export async function handleLinkAccount(chatId, email, firstName, username, lang) {
  ...
  try {
    ...
    if (!profile) {
      await sendTelegram(chatId, t.linkEmailNotFound(email), withMainMenu(lang))
      return
    }

    if (!['RENTER', 'PARTNER', 'ADMIN', 'MODERATOR'].includes(profile.role)) {
      await sendTelegram(chatId, t.linkNotPartner(), withMainMenu(lang))
      return
    }
    ...
    await sendTelegram(chatId, t.linkSuccess(profile.first_name, profile.last_name, roleLabel), withMainMenu(lang))
  } catch (e) {
    console.error('[LINK ERROR]', e)
    await sendTelegram(chatId, t.linkError(), withMainMenu(lang))
  }
}
