...
export async function handleStatusCheck(chatId, lang) {
  ...
  try {
    ...
    if (profile) {
      await sendTelegram(chatId, t.statusOk(profile, lang))
    } else {
      await sendTelegram(chatId, t.statusUnlinked())
    }
  } catch (e) {
    console.error('[STATUS ERROR]', e)
    await sendTelegram(chatId, t.statusError())
  }
}
...
export async function handleDeepLink(chatId, userId, firstName, username, telegramId, lang) {
  ...
  try {
    ...
    if (!profile) {
      await sendTelegram(chatId, t.deepLinkUserNotFound(userId))
      return
    }

    if (profile.telegram_id && profile.telegram_id !== chatId.toString()) {
      await sendTelegram(chatId, t.deepLinkAlreadyLinked())
      return
    }
    ...
    await sendTelegram(chatId, t.deepLinkError())
  }
}
...
export async function handleLinkAccount(chatId, email, firstName, username, lang) {
  ...
  try {
    ...
    if (!profile) {
      await sendTelegram(chatId, t.linkEmailNotFound(email))
      return
    }

    if (!['PARTNER', 'ADMIN', 'MODERATOR'].includes(profile.role)) {
      await sendTelegram(chatId, t.linkNotPartner())
      return
    }
    ...
    await sendTelegram(chatId, t.linkSuccess(profile.first_name, profile.last_name, roleLabel))
  } catch (e) {
    console.error('[LINK ERROR]', e)
    await sendTelegram(chatId, t.linkError())
  }
}
