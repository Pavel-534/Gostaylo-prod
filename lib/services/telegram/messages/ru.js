...
export const telegramRu = {
  ...
  help_partner: (lang) => {
    ...
    return (
      `<b>📖 Справка ${esc(getSiteDisplayName())}</b>\n\n` +
  ...
  help_renter: (lang) => {
    ...
    return (
      ...
      `если вы привязали Telegram к аккаунту на ${esc(getSiteDisplayName())}.\n\n` +
      ...
      `<b>Сайт</b>\n<a href="${esc(homeUrl)}">${esc(getSiteDisplayName())} — главная →</a>`
    )
  },
  ...
  start_partner: (firstName, lang) => {
    ...
    return (
      ...
      `Добро пожаловать в <b>${esc(getSiteDisplayName())}</b> — аренда на Пхукете.\n\n` +
  ...
  statusOk: (profile, lang, aiEnabled) => {
    ...
    const aiLine = aiEnabled
      ? `<b>Интеллект ${esc(getSiteDisplayName())}:</b> ✅ ВКЛЮЧЕН`
      : `<b>Интеллект ${esc(getSiteDisplayName())}:</b> ❌ ВЫКЛЮЧЕН (нет ключа API)`
    return (
