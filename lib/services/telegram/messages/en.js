 ...
 */
import { buildLocalizedSiteUrl, getSiteDisplayName } from '../../../site-url.js'
...
export const telegramEn = {
  ...
  help_partner: (lang) => {
    ...
    return (
      `<b>📖 ${esc(getSiteDisplayName())} quick guide</b>\n\n` +
      ...
      '<b>Chat commands</b>\n' +
      '<code>/my</code> — drafts\n' +
      '<code>/promo</code> — active Flash sales &amp; stats\n' +
  ...
  help_renter: (lang) => {
    ...
    return (
      `<b>📖 ${esc(getSiteDisplayName())} guide</b>\n\n` +
      '<b>Booking status</b>\n' +
      `Tap <b>«Status»</b> below to see whether your Telegram is linked to your ${esc(getSiteDisplayName())} account. ` +
      ...
      `<b>Website</b>\n<a href="${esc(homeUrl)}">${esc(getSiteDisplayName())} — home →</a>`
    )
  },
  ...
  start_partner: (firstName, lang) => {
    ...
    return (
      ...
      `Welcome to <b>${esc(getSiteDisplayName())}</b> — rentals in Phuket.\n\n` +
  ...
  statusOk: (profile, lang, aiEnabled) => {
    ...
    const aiLine = aiEnabled
      ? `🧠 <b>${esc(getSiteDisplayName())} intelligence:</b> ✅ ON`
      : `🧠 <b>${esc(getSiteDisplayName())} intelligence:</b> ❌ OFF (no API key)`
    return (
      '✅ <b>Account linked</b>\n\n' +
  ...
  promo_flash_header: () => '🔥 Active Flash Sales',
  promo_flash_empty: () => 'No active Flash sales right now.',
  promo_flash_partner_only: () => '<code>/promo</code> is for partners only.',
  promo_flash_link_first: () => 'Link your account first: <code>/link you@email.com</code>',
  promo_flash_db_error: () => '⚠️ Could not load data. Try again later.',
  promo_flash_line_remaining: (hm) => `${hm} left`,
  promo_flash_line_bookings: (n) => `Bookings: ${n}`,
  promo_flash_coach_high: () => '🚀 Strong traction — your Flash sale is performing well.',
  promo_flash_coach_zero: () =>
    '💡 Tip: try a slightly deeper discount to attract the first bookings.',
