...
export function payoutRowToTbankRegistryLine(row, partnerNameFallback = '') {
  ...
  const payoutCur = String(row.payout_currency || row.currency || '').toUpperCase();
  const rubAmount =
    payoutCur === 'RUB' && row.amount_in_payout_currency != null
      ? row.amount_in_payout_currency
      : row.metadata?.payout_fx?.amount_in_payout_currency ?? row.final_amount ?? row.amount;
  const amountRub = formatAmountRub(rubAmount);
  const payoutType = String(row.metadata?.payout_type || '').trim();
  const purpose =
    payoutType === 'referral_withdrawal'
      ? `Выплата амбассадору ${getSiteDisplayName()} referral ${row.id}`
      : `Выплата партнеру ${getSiteDisplayName()} payout ${row.id}${row.booking_id ? ` booking ${row.booking_id}` : ''}`;
  return {
...
export class TbankPayoutRegistryService {
  static isReferralPayoutRow(p) {
    const rail = String(p?.payout_rail || '').toUpperCase();
  ...
  /**
   * Preview registry lines without mutating payout status.
   * @param {{ referralOnly?: boolean }} [opts]
   */
  static async buildRegistryPreview({ referralOnly = false } = {}) {
    const { exportable, skippedUnverified } = await this.listPendingRuBankPayoutsForRegistry();
    const pool = referralOnly ? exportable.filter((p) => this.isReferralPayoutRow(p)) : exportable;
    const stillSkipped = [...skippedUnverified];
    if (!pool.length) {
      return { preview: [], skippedUnverified: stillSkipped, exportableCount: 0 };
    }
    const names = await this.enrichPartnerNames(pool);
    const preview = [];
    for (const p of pool) {
      const parsed = payoutRowToTbankRegistryLine(p, names.get(p.partner_id) || '');
    ...
    return { preview, skippedUnverified: stillSkipped, exportableCount: preview.length };
  }
   ...
   * @param {{ referralOnly?: boolean }} [opts]
   */
  static async exportRegistryAndMarkProcessing({ referralOnly = false } = {}) {
    ...
    const { exportable, skippedUnverified } = await this.listPendingRuBankPayoutsForRegistry();
    const exportableFiltered = referralOnly
      ? exportable.filter((p) => this.isReferralPayoutRow(p))
      : exportable;
    const stillSkipped = [...skippedUnverified];

    if (!exportableFiltered.length) {
      return { csv: buildTbankRegistryCsv([]), exportedIds: [], skippedUnverified: stillSkipped };
    }

    const names = await this.enrichPartnerNames(exportableFiltered);
    const lines = [];
    for (const p of exportableFiltered) {
    ...
    const candidateIds = lines.map((L) => L.payoutId).filter(Boolean);
    const { data: claimedRows, error: claimErr } = await supabaseAdmin.rpc(
      'claim_payouts_for_tbank_registry',
      { p_ids: candidateIds },
    );
    if (claimErr) {
      return {
        csv: buildTbankRegistryCsv([]),
        exportedIds: [],
        skippedUnverified: [
          ...stillSkipped,
          { payoutId: null, reason: `CLAIM_FAILED:${claimErr.message}` },
        ],
      };
    }
    const claimedSet = new Set(
      (Array.isArray(claimedRows) ? claimedRows : claimedRows ? [claimedRows] : [])
        .map((r) => (r && typeof r === 'object' ? r.claimed_id || r.id : r))
        .filter(Boolean)
        .map(String),
    );
    ...
    for (const L of lines) {
      if (!claimedSet.has(String(L.payoutId))) {
        stillSkipped.push({
          payoutId: L.payoutId,
          partnerId: L.partnerId,
          reason: 'CONCURRENT_CLAIM_MISS',
        });
        continue;
      }
      exportedIds.push(L.payoutId);
      successfulLines.push(L);
      const p = exportableFiltered.find((r) => r.id === L.payoutId);
      if (isReferralWithdrawalPayout(p)) {
        const meta = p.metadata && typeof p.metadata === 'object' ? p.metadata : {};
        void notifyReferralWithdrawalStatus(p.partner_id, 'registry_sent', {
          payoutId: p.id,
          grossThb: meta.gross_thb,
          netThb: meta.net_thb,
          netRub: p.amount_in_payout_currency ?? meta.net_rub,
        });
      }
    }
