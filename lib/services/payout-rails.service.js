...
function normalizeFeeType(value) {
  ...
  return PAYOUT_FEE_TYPES.includes(normalized) ? normalized : 'fixed'
}
...
export class PayoutRailsService {
  ...
  static normalizeMethodPayload(payload = {}) {
    ...
    return {
      ...
      fee_type: normalizeFeeType(payload.feeType || payload.fee_type),
  ...
  static calculatePayoutFee(basePayoutAmount, payoutMethod) {
    ...
    const feeType = normalizeFeeType(method.fee_type)
