...
import { normalizePayoutFeeType, resolvePayoutFeeTypeFromMethodRow } from '@/lib/finance/payout-method-fee'
...
function normalizeEmbeddedMethod(method) {
  ...
  return {
    ...method,
    fee_type: resolvePayoutFeeTypeFromMethodRow(method),
  }
}
...
export class PayoutRailsService {
  ...
  static async listPayoutMethods({ activeOnly = false } = {}) {
    ...
    return rows.map((row) => ({
      ...row,
      fee_type: resolvePayoutFeeTypeFromMethodRow(row),
    }))
  }
  ...
  static async getPayoutMethodById(methodId) {
    ...
    return {
      ...data,
      fee_type: resolvePayoutFeeTypeFromMethodRow(data),
    }
  }
  ...
  static calculatePayoutFee(basePayoutAmount, payoutMethod) {
    ...
    const feeType = resolvePayoutFeeTypeFromMethodRow(method)
