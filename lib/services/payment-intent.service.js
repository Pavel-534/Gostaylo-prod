  ...
  static async resolveOrCreateForCheckout({ booking, invoice = null, createdBy = null }) {
    ...
    if (existing.intent) {
      ...
      const expectedAmountThb = invoice
        ? Math.round(Number(existing.intent.amountThb || 0))
        : expectedPaymentIntentAmountThbFromBooking(booking)
      const amountMatches =
        Boolean(invoice) || Math.round(Number(existing.intent.amountThb || 0)) === expectedAmountThb
      if ((invoiceId || expectedWalletDiscount === existingWalletDiscount) && amountMatches) {
        return existing
      }
    }
