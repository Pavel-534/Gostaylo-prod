import {
  ...
  listingYmdAtStartOfDayIso,
  listingYmdLocalWallTimeToUtcIso,
 ...
 */
export function normalizeVehicleIntervalBounds(checkIn, checkOut, options = {}) {
  const listingTimeZone =
    String(options.listingTimeZone || '').trim() || getListingDateTimeZone()
  ...
  const startRaw = hasStartTime
    ? String(checkIn)
    : listingYmdAtStartOfDayIso(startDateKey, listingTimeZone)
  // Day-only protection: treat requested day as fully occupied (wall time in listing TZ).
  const endRaw = hasEndTime
    ? String(checkOut)
    : listingYmdLocalWallTimeToUtcIso(endDateKey, 23, 59, listingTimeZone)
