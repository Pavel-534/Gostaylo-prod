 ...
 * Stage 200.88 — intent amount includes THB FX surcharge when brutto is THB above mid payable.
 */

import { fxMarkupExtraThbFromFinalBreakdown } from '@/lib/pricing-engine/guest-fx-charge.js'
 ...
 * Adds FX markup extra when guest pays THB for a non-THB listing (brutto THB > mid payable).
 * @param {object} booking
 */
export function expectedPaymentIntentAmountThbFromBooking(booking) {
  const columnSum = Math.round(
  ...
  const fxExtra = fxMarkupExtraThbFromFinalBreakdown(fb)
  return columnSum + Math.max(0, Math.round(fxExtra))
}
