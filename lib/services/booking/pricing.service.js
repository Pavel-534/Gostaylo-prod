/**
 * Booking-scoped pricing_snapshot / settlement (не путать с lib/services/pricing.service.js).
 * SSOT settlement на оплате — этот файл; SSOT дневной цены — PricingService.
 * Карта: `lib/pricing/PRICING_SERVICES.md`
...
/**
 * Raw FX locked at booking create (pricing_snapshot.final_breakdown.fx_raw_rate_to_thb).
 * Snapshot overrides live rates for the payment currency; other currencies fall back to live mid.
 * @param {object} snapshot
 * @param {string | null} [bookingCurrency]
 */
export function buildSnapshotLockedRawRateMap(snapshot, bookingCurrency = null) {
  const map = { THB: 1 };
  const fb = snapshot?.final_breakdown;
  const raw = Number(fb?.fx_raw_rate_to_thb);
  const payCur = String(fb?.total_guest_brutto?.currency || bookingCurrency || '')
    .toUpperCase()
    .trim();
  if (Number.isFinite(raw) && raw > 0 && payCur && payCur !== 'THB') {
    map[payCur] = raw;
  }
  return map;
}

export async function mergeSnapshotLockedRawRateMap(snapshot, bookingCurrency = null) {
  const snapLocked = buildSnapshotLockedRawRateMap(snapshot, bookingCurrency);
  const live = await PricingService.getRawRateMap();
  return { ...live, ...snapLocked };
}
...
export function readFeeSplitFromSnapshot(snapshot) {
  ...
  return {
    ...
    /** Резервный фонд: доля % от валовой маржи платформы (имя поля в JSON — legacy `insurance_*`). */
...
export async function attachSettlementSnapshotForBooking(bookingId) {
  const { data: booking, error } = await supabaseAdmin
    .from('bookings')
    .select(
      'id, partner_id, price_thb, price_paid, currency, exchange_rate, commission_thb, commission_rate, partner_earnings_thb, applied_commission_rate, listing_currency, pricing_snapshot, taxable_margin_amount, rounding_diff_pot',
    )
    .eq('id', bookingId)
    .single();
  ...
  const rawRateMap = await mergeSnapshotLockedRawRateMap(snapshot, booking.currency);
  ...
  const snapFxRaw = Number(snapshot?.final_breakdown?.fx_raw_rate_to_thb);
  snapshot.settlement_v3 = {
    ...
    fx_raw_rate_to_thb: Number.isFinite(snapFxRaw) && snapFxRaw > 0 ? snapFxRaw : null,
    partner_net: {
    ...
    /** Резервный (страховой) фонд по брони; `insurance_fund_percent` — % от маржи платформы, не от субтотала и не комиссия хоста. */
...
/** P1-8: явный namespace — settlement / pricing_snapshot на брони. */
export const BookingSettlementPricing = {
  normalizeListingCurrency,
  cloneBookingPricingSnapshot,
  buildSnapshotLockedRawRateMap,
  mergeSnapshotLockedRawRateMap,
  readFeeSplitFromSnapshot,
  attachSettlementSnapshotForBooking,
}
