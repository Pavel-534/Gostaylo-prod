...
async function attachActiveDisputeSla(admin, bookings) {
  ...
  const { data, error } = await admin
    .from('disputes')
    .select('id, booking_id, status, current_deadline_at')
    .in('booking_id', ids)
    .in('status', ['OPEN', 'IN_REVIEW'])
    .order('updated_at', { ascending: false })
  if (error) {
    console.warn('[BookingQuery] attachActiveDisputeSla', error.message)
    return bookings
  }
  const firstByBooking = new Map()
  for (const row of data || []) {
    const bid = String(row?.booking_id || '')
    if (bid && !firstByBooking.has(bid)) firstByBooking.set(bid, row)
  }
  return bookings.map((b) => {
    const d = firstByBooking.get(String(b?.id || ''))
    if (!d) return b
    return {
      ...b,
      active_dispute: {
        id: d.id,
        status: d.status,
        current_deadline_at: d.current_deadline_at || null,
      },
      active_dispute_id: d.id,
      active_dispute_status: d.status,
      active_dispute_current_deadline_at: d.current_deadline_at || null,
    }
  })
}
