...
export function mapBookingListingsJoin(booking) {
  ...
  const { categories: _categories, ...listingRest } = row;
  const status = normalizeOrderStatus(booking.status);
  const revealAddress = CONTACT_REVEALED_BOOKING_STATUSES.has(status);
  return {
    ...booking,
    listings: {
      ...listingRest,
      ...(category_slug ? { category_slug } : {}),
      // Stage 196.0-A — exact street only after paid/reveal statuses (privacy SSOT).
      address: revealAddress ? listingRest.address ?? null : null,
    },
  };
}
...
async function attachActiveDisputeSla(admin, bookings) {
  return attachDisputeToBookings(admin, bookings)
}

export async function resolveListingCategorySlug(categoryId) {
  const ctx = await resolveListingCategoryContext(categoryId);
  return ctx.slug;
}

/**
 * Stage 149 — slug + wizard_profile for escrow thaw / payout copy SSOT.
 * @param {string | number | null | undefined} categoryId
 * @returns {Promise<{ slug: string, wizardProfile: string | null }>}
 */
export async function resolveListingCategoryContext(categoryId) {
  if (!categoryId) return { slug: '', wizardProfile: null };
  const { data } = await supabaseAdmin
    .from('categories')
    .select('slug, wizard_profile')
    .eq('id', categoryId)
    .maybeSingle();
  return {
    slug: String(data?.slug || '').toLowerCase(),
    wizardProfile: data?.wizard_profile != null ? String(data.wizard_profile) : null,
  };
}
...
export async function getBookings(filters = {}) {
  let query = supabaseAdmin
    ...
    .select(`
        *,
        listings (
          id, title, district, address, country_code, region_code, city_code, images, base_price_thb, category_id, metadata, owner_id,
          categories ( slug )
        ),
  ...
  const withDispute = await attachActiveDisputeSla(supabaseAdmin, withConv);
  const mapped = withDispute.map(mapBookingListingsJoin);
...
export async function getBookingById(bookingId, options = {}) {
  const { data, error } = await supabaseAdmin
    ...
    .select(
      `
        *,
        listings (
          id, title, district, address, country_code, region_code, city_code, cover_image, images, base_price_thb, owner_id, metadata, cancellation_policy, category_id,
          categories ( slug )
        ),
  ...
  const [withDispute] = await attachActiveDisputeSla(supabaseAdmin, [withConv]);
  const [withTrust] = await attachPartnerTrustToBookings([withDispute]);
