...
export async function resolveBookingPricingWithEngine(params) {
  ...
  try {
    ...
    if (v2Enabled) {
      ...
      pricingSnapshot = {
        ...v2Result.snapshot,
        fiscal_kassa_preview: toFiscalKassaPayload(v2Breakdown),
    ...
    const errMessage = v2Err?.message || String(v2Err)
    logStructured({
    ...
    if (v2Enabled) {
      void notifySystemAlert(
        `⚠️ <b>Pricing Engine v2 fallback</b>\n` +
          `listing: <code>${escapeSystemAlertHtml(listingId)}</code>\n` +
          `<code>${escapeSystemAlertHtml(errMessage)}</code>\n` +
          `Используется legacy fee split для брони.`,
      )
      pricingEngineV2Active = false
      activeFeeSplit = feeSplit
      roundedGuestTotalThb = legacyRounded.roundedGuestTotalThb
      roundingDiffPotThb = legacyRounded.roundingPotThb
    }
  }
