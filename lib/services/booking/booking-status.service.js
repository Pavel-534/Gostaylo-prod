...
import {
  ...
  DIRECT_PAID_ESCROW_TRANSITION_ERROR,
  validatePaidEscrowFsmGuard,
...
export async function transitionBookingStatus(bookingId, newStatus, options = {}) {
  ...
  const meta = {
    ...
    viaEscrowRpc: options.viaEscrowRpc === true,
  }
  ...
  // Stage 125.3 — block direct FSM writes to PAID_ESCROW (RPC is SSOT for status + ledger).
  if (toStatus === 'PAID_ESCROW' && !downstreamOnly && !options.skipDbUpdate) {
    const guard = validatePaidEscrowFsmGuard(toStatus, meta)
    if (!guard.ok) {
      return {
        success: false,
        error: guard.error || DIRECT_PAID_ESCROW_TRANSITION_ERROR,
