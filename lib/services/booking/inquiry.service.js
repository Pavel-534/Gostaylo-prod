...
import { resolveBookingPricePaidFields } from '@/lib/pricing-engine/guest-fx-charge.js';
...
export async function verifyInventoryBeforePartnerConfirm(bookingId) {
  ...
  const isWholeUnit = isWholeUnitCalendarInventory(listingCategorySlug, listingRow?.metadata);
  ...
  const cal = await CalendarService.checkAvailability(booking.listing_id, cin, cout, {
    guestsCount: isTransportCalendarUnit || isWholeUnit ? 1 : gc,
...
export async function createInquiryBooking(bookingData) {
  ...
  const needsPriceAttestation = !privateTrip && !negotiationRequest && !contactInquiry;
  ...
  const skipTransactionalMin = privateTrip === true || negotiationRequest === true || contactInquiry === true;
  ...
  const checkoutRate = await PricingService.getCheckoutRateToThb(currency, listingCurrency);
  const paidFields = resolveBookingPricePaidFields({
    paymentCurrency: currency,
    roundedGuestTotalThb,
    exchangeRateToThb: checkoutRate,
    finalBreakdown: pricingSnapshot?.final_breakdown || pricing.v2Breakdown || null,
  });
  const exchangeRate = paidFields.exchangeRate;
  const pricePaid = paidFields.pricePaid;
  const taxableMarginAmount = Math.max(0, paidFields.guestChargeThb - feeSplit.partnerEarningsThb);
  ...
  const tag = privateTrip
    ...
    : negotiationRequest
      ...
      : contactInquiry
        ? 'Тип заявки: уточнение у хозяина (даты, гости, условия).'
  ...
  const { data: booking, error: bookingError } = await supabaseAdmin
    ...
    .insert({
      ...
      metadata: {
        ...pickCheckInInstructionsForBookingMetadata(listing, { uiLocale }),
        // AUDIT_03 W3.1 — skip only at inquiry create; payable paths must re-gate
        ...(!needsPriceAttestation || skipTransactionalMin
          ? {
              price_attestation_skipped: true,
              price_attestation_skip_flags: {
                privateTrip: Boolean(privateTrip),
                negotiationRequest: Boolean(negotiationRequest),
                contactInquiry: Boolean(contactInquiry),
                skippedMinTotal: Boolean(skipTransactionalMin),
              },
            }
          : {}),
      },
