...
export async function createInquiryBooking(bookingData) {
  ...
  const holdHours = resolveInquirySoftHoldHours();
  let inquirySoftHold = null;
  if (shouldSoftHold) {
    const hold = await createInquirySoftHold({
      listingId,
    ...
    if (!hold.ok) {
      console.warn('[Inquiry] soft-hold failed for booking', booking.id, hold.error);
    } else {
      inquirySoftHold = {
        holdHours,
        expiresAt: hold.expiresAt,
        partnerNotice: getInquirySoftHoldPartnerNotice('ru', holdHours),
      };
    }
  }
