...
import { resolveGuestLedgerTotalThbFromFinalBreakdown } from '@/lib/pricing-engine/guest-fx-charge.js'
...
export function resolveCaptureGuestTotalThb(booking, ctx = null) {
  ...
  // Mid payable + THB FX surcharge when pay=THB × base≠THB (Stage 200.88).
  let guestTotalThb = resolveGuestLedgerTotalThbFromFinalBreakdown(fb, fs)
  if (!guestTotalThb) {
    guestTotalThb = round2(fb.total_guest_payable_rounded_thb ?? fs.guest_payable_rounded_thb ?? 0)
  }
  if (!guestTotalThb) {
...
/**
 * AUDIT_03 W3.7 — align JS legs with RPC `move_to_escrow_and_post_ledger_v1` (v_insurance := 0).
 * Insurance is folded into the platform credit so partner/guest totals match the journal the RPC posts.
 * @param {object} legs
 */
export function alignLedgerLegsToEscrowRpcCaptureSsot(legs) {
  if (!legs || typeof legs !== 'object') return legs
  const insurance = round2(legs.insuranceThb || 0)
  if (!(insurance > 0)) {
    return { ...legs, insuranceThb: 0 }
  }
  if (legs.ledgerV2) {
    return {
      ...legs,
      insuranceThb: 0,
      platformHostFeeThb: round2(Math.max(0, (legs.platformHostFeeThb || 0) + insurance)),
    }
  }
  return {
    ...legs,
    insuranceThb: 0,
    platformFeeThb: round2(Math.max(0, (legs.platformFeeThb || 0) + insurance)),
  }
}
