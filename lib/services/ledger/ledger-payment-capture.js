/**
 * Stage 110.3 — BOOKING_PAYMENT_CAPTURED journal.
 * Prod: posted inside RPC `move_to_escrow_and_post_ledger_v1` with PAID_ESCROW.
 * This module is the JS SSOT for the same legs (replay, admin reconcile, smoke); idempotent per booking.
 */
...
import {
  journalNeedsEntryHeal,
  skipCompensatingJournalDelete,
} from '@/lib/services/ledger/ledger-append-only.js'
...
export async function postPaymentCaptureFromBooking(booking) {
  ...
  let healEmptyJournal = false
  if (existing?.id) {
    healEmptyJournal = await journalNeedsEntryHeal(supabaseAdmin, existing.id)
    if (!healEmptyJournal) {
  ...
  const journalId = healEmptyJournal && existing?.id ? existing.id : `lj-cap-${bookingId}`
    ...
    if (jErr) {
      if (String(jErr.message || '').includes('duplicate') || jErr.code === '23505') {
        const { data: raced } = await supabaseAdmin
          ...
          .eq('idempotency_key', idempotencyKey)
          .maybeSingle()
        if (raced?.id && (await journalNeedsEntryHeal(supabaseAdmin, raced.id))) {
  ...
  if (eErr) {
    ...
    await skipCompensatingJournalDelete(journalId, eErr.message)
    return { success: false, error: eErr.message, orphanJournalId: journalId }
  }
