...
export function classifySystemAlert(htmlMessage) {
  ...
  if (isTransientOpsError(s)) return 'TRANSIENT_INFRA'
...
export async function notifySystemAlert(htmlMessage, opts = {}) {
  try {
    // Stage 202.41 — Gateway Timeout / AbortError blips: log only (STALE_CRON still catches real outages).
    if (opts.allowTransient !== true && isTransientOpsError(htmlMessage)) {
      console.warn('[system-alert] skipped transient infra noise:', previewMessage(htmlMessage))
      return
    }
    ...
    const maxPerWindow =
      alertClass === 'TRANSIENT_INFRA' ? 1 : maxForSeverity(severity)
    ...
    const { severity: _severity, allowTransient: _allowTransient, ...tgOpts } = opts || {}
    void _severity
    void _allowTransient
