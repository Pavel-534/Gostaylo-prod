...
export class NotificationService {
  static async dispatch(event, data) {
    ...
    const registryEntry = NOTIFICATION_REGISTRY[event]
    if (registryEntry?.intentionallyDead) {
      console.warn(
        `[NOTIFICATION] intentionally-dead event "${event}"` +
          (registryEntry.supersededBy ? ` (use ${registryEntry.supersededBy})` : ''),
      )
    }
  ...
  static async runPartnerHostOnboardingRetention() {
    wireHandlerDepsInner()
    return MarketingEvents.runPartnerHostOnboardingRetention()
  }

  static async runReferralTeamWeeklyDigest() {
    wireHandlerDepsInner()
    return MarketingEvents.runReferralTeamWeeklyDigest()
  }
}
