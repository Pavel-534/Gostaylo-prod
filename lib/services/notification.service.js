 ...
 * 
 * PRIVACY & SECURITY:
 * - Admin Topics (15-18) → ADMIN GROUP ONLY (not accessible by partners/renters)
 * - Partner Notifications → PRIVATE MESSAGES to partner's telegram_id
 * - Renter Notifications → PRIVATE EMAIL only (no telegram)
 * - Sensitive data (guest info, commissions) → ADMIN ONLY
 */
...
function bookingSpecialRequestsSnippet(raw, maxLen = 120) {
  if (raw == null || raw === '') return '';
  const text = typeof raw === 'string' ? raw : JSON.stringify(raw);
  return text.length > maxLen ? `${text.slice(0, maxLen)}…` : text;
}

function escapeTelegramHtmlText(s) {
  return String(s)
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;');
...
function formatBookingAmountForNotify(booking, totalPriceThb) {
...
export class NotificationService {
  ...
  /**
   * Единая отправка в Telegram: decodeTelegramText + HTML, без дублирования.
   * @param {Record<string, unknown>} payload — chat_id, text (сырой), опционально reply_markup, message_thread_id, disable_web_page_preview
   */
  ...
  /** Личное сообщение партнёру / гостю */
  static async sendTelegram(chatId, message) {
    return this.sendTelegramMessagePayload({
      chat_id: chatId,
      text: message,
    });
  }
  ...
  static async sendToAdminTopic(topicType, message, reply_markup) {
    ...
    const result = await this.sendTelegramMessagePayload({
  ...
  /**
   * Личное сообщение админу (например Pavel) или fallback в топик FINANCE группы.
   * Приоритет: `TELEGRAM_ADMIN_DM_CHAT_ID` → `ADMIN_TELEGRAM_ID` (как в iCal-админке) → топик FINANCE.
   */
  static async sendToAdmin(message, reply_markup) {
    ...
    return this.sendToAdminTopic('FINANCE', message, reply_markup)
  }

  /**
   * Критические операционные алерты → топик «System» в админ-группе.
   * Задайте TELEGRAM_SYSTEM_ALERTS_TOPIC_ID (message_thread_id форума).
   * Если не задан — fallback на sendToAdmin (личка или FINANCE).
   * @param {string} message — HTML (как в остальных Telegram-уведомлениях)
   * @param {{ omitBanner?: boolean, reply_markup?: Record<string, unknown> }} [opts]
   */
  static async sendSystemAlert(message, opts = {}) {
    const banner = opts.omitBanner
      ? ''
      : '🚨 <b>SYSTEM</b>\n'
    ...
    return this.sendToAdmin(text, opts.reply_markup)
  }
  ...
  static async handleNewBookingRequest(data) {
    ...
    // 2. Professional HTML Email to Partner (New Lead Alert) with commission info
    if (partner?.email) {
      try {
    ...
    // 3. Telegram to Partner (if linked) with inline buttons - PRIVATE MESSAGE
    if (partner?.telegram_id) {
      await this.sendTelegramBookingRequest(partner.telegram_id, {
        booking,
        listing,
        totalPrice,
        commissionRate,
    ...
    // 4. Admin Topic: BOOKINGS - ADMIN ONLY (NOT visible to partners)
    // This goes to the admin group's BOOKINGS topic (thread 15)
    // Only admins have access to this group
  ...
  /**
   * Send Telegram booking request with inline buttons
   */
  static async sendTelegramBookingRequest(chatId, data) {
    ...
    // Inline keyboard with Approve/Decline buttons
    const keyboard = {
      inline_keyboard: [
        [
        ...
        [
          { text: '📱 Открыть в приложении', url: `${BASE_URL}/my-bookings/${booking.id}` }
        ]
      ]
    };
        ...
        await EmailService.sendPaymentSuccessGuest(
          {
            ...
            escrowText: escrowEmailLine(emailLang),
            bookingsUrl: `${BASE_URL}/renter/bookings/`,
            chatUrl,
            profileUrl: `${BASE_URL}/renter/profile/`,
          },
          booking.guest_email,
  ...
  static async handleReviewReminder(data) {
    ...
    if (guestTelegram) {
      await this.sendTelegram(
        ...
        `⭐ <b>Оставьте отзыв</b>\n\n` +
          `📍 ${listing?.title || 'Объект'}\n` +
          `Расскажите, как прошло проживание — это помогает другим гостям.\n\n` +
          `📱 <a href="${BASE_URL}/renter/bookings">Мои бронирования</a>`,
      );
    }
  }
  ...
  static async handleCheckInReminder(data) {
    ...
    if (guestTelegram) {
      await this.sendTelegram(guestTelegram,
        ...
        `📱 <a href="${BASE_URL}/my-bookings/${booking.id}">Открыть бронирование</a>`
      );
    }
