...
export class NotificationService {
  ...
  static async handleNewBookingRequest(data) {
    ...
    const commissionRate = parseFloat(booking.commission_rate) || 15;  // Fallback only if missing
    ...
    await this.sendToAdminTopic('BOOKINGS',
      ...
      `💰 <b>Сумма:</b> ฿${totalPrice.toLocaleString()}\n` +
  ...
  static async handlePayoutProcessed(data) {
    ...
    const commissionRate = parseFloat(payout?.commissionRate) || parseFloat(booking?.commission_rate) || 15;
  ...
  static async sendPartnerDraftDigestReminder({ telegramId, draftCount, lang = 'ru' }) {
    ...
    const draftsUrl = buildLocalizedSiteUrl(lang, '/partner/listings?filter=draft');
    ...
    return this.sendTelegramMessagePayload({
      chat_id: telegramId,
      text,
      reply_markup: { inline_keyboard: [[{ text: btn, url: draftsUrl }]] },
    });
  }

  /**
   * Cron: раз в сутки — всем партнёрам с telegram_id и хотя бы одним черновиком.
  ...
  static async runDailyDraftDigestReminders() {
    ...
    let sent = 0;
    for (const [ownerId, draftCount] of byOwner) {
      const { data: profile, error: pErr } = await supabaseAdmin
        .from('profiles')
        .select('telegram_id, language, role')
        .eq('id', ownerId)
        .maybeSingle();
      if (pErr || !profile?.telegram_id) continue;
      const role = String(profile.role || '').toUpperCase();
      if (!['PARTNER', 'ADMIN'].includes(role)) continue;
    ...
    return { sent, partnersWithDrafts: byOwner.size };
  }
}
