...
export class NotificationService {
  ...
  static async handleNewBookingRequest(data) {
    ...
    const totalPrice = booking.price_thb || 0;
    const commissionRate = booking.commission_rate || 15;
    const commissionAmount = booking.commission_thb || Math.round(totalPrice * (commissionRate / 100));
    const partnerEarnings = booking.partner_earnings_thb || (totalPrice - commissionAmount);
    ...
    if (partner?.email) {
      try {
        ...
        await this.sendEmail(
          ...
          `${booking.special_requests ? `💬 Сообщение от гостя: "${booking.special_requests}"\n\n` : ''}` +
    ...
    await this.sendToAdminTopic('BOOKINGS',
      ...
      `${booking.special_requests ? `💬 <b>Сообщение:</b> "${booking.special_requests.substring(0, 100)}"\n` : ''}` +
  ...
  static async sendTelegramBookingRequest(chatId, data) {
    ...
    try {
      ...
      console.error(`[TELEGRAM BOOKING ERROR] ${error.message}`);
      return { success: false, error: error.message };
    }
  }
