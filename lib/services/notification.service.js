...
export class NotificationService {
  ...
  static async handleUserWelcome(data) {
    ...
    // If partner registration, notify admin group
  ...
  static async handlePaymentReceived(data) {
    ...
    // Send to admin group FINANCE topic
  ...
  static async handlePartnerVerified(data) {
    ...
    // Send to admin group NEW_PARTNERS topic
    await this.sendToAdminTopic('NEW_PARTNERS',
      `✅ <b>ПАРТНЁР ВЕРИФИЦИРОВАН</b>\n\n👤 <b>Имя:</b> ${partner?.first_name || ''} ${partner?.last_name || ''}\n📧 <b>Email:</b> ${partner?.email || 'N/A'}\n📞 <b>Телефон:</b> ${partner?.phone || 'N/A'}\n\n📊 <b>Статус:</b> VERIFIED`
    );
  }
