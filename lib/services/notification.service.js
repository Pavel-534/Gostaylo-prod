...
export const NotificationEvents = {
  ...
  PAYMENT_SUBMITTED: 'PAYMENT_SUBMITTED',  // NEW: When user submits TXID
  PAYMENT_RECEIVED: 'PAYMENT_RECEIVED',
  PAYMENT_SUCCESS: 'PAYMENT_SUCCESS',
  PAYMENT_CONFIRMED: 'PAYMENT_CONFIRMED',  // NEW: When admin confirms payment
...
export class NotificationService {
  ...
  static async handlePaymentSubmitted(data) {
    ...
    // Admin Topic: FINANCE (Thread 16)
    await this.sendToAdminTopic('FINANCE',
      `💰 <b>NEW [${paymentMethodLabel}] PAYMENT!</b>\n\n` +
      `📍 <b>Объект:</b> ${listing?.title || 'N/A'}\n` +
      `👤 <b>Гость:</b> ${booking?.guest_name || 'N/A'}\n` +
      `💵 <b>Сумма:</b> ฿${booking?.price_thb?.toLocaleString() || payment?.amount?.toLocaleString() || 0}\n` +
