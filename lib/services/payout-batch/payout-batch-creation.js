...
import {
  getBatchWithItems,
  getBookingIdsBlockedFromNewPayoutPools,
...
export async function promoteThawedToReadyForPayout(limit = 500) {
  ...
  const { data: thawed, error } = await supabaseAdmin
    .from('bookings')
    .select('id, partner_id, partner_earnings_thb, status, metadata, updated_at')
    .eq('status', 'THAWED')
  ...
  // FAIL-CLOSED: freeze lookup down → do not promote anyone.
  if (isFrozenBookingLookupFailed(frozenSet)) {
    emitFreezeFailIfNeeded(frozenSet, 'promote-ready-for-payout')
    return {
      promoted: 0,
      skipped_hold: thawed.length,
      error: 'freeze_lookup_failed',
      message: 'getFrozenBookingIdSet failed — promote blocked until disputes readable',
    }
  }
  ...
  const inBatch = await getBookingIdsBlockedFromNewPayoutPools(ids)
  let promoted = 0
  for (const b of eligible) {
    if (inBatch.has(b.id)) continue
    const meta = b.metadata && typeof b.metadata === 'object' ? { ...b.metadata } : {}
    const now = new Date().toISOString()
    const statusRes = await transitionBookingStatus(b.id, 'READY_FOR_PAYOUT', {
      scope: 'system',
      actorContext: { trigger: 'promote_ready_for_payout' },
      metadata: { updatedAt: now },
      extraPatch: {
        metadata: { ...meta, ready_for_payout_at: now },
      },
    })
    if (statusRes.success && !statusRes.skipped) promoted += 1
  }
...
export async function createDraftPoolForToday(opts = {}) {
  ...
  // FAIL-CLOSED: freeze lookup down → do not create a payout batch.
  if (isFrozenBookingLookupFailed(frozenSet)) {
    emitFreezeFailIfNeeded(frozenSet, 'payout-batch-pools')
    return {
      error: 'freeze_lookup_failed',
      message: 'getFrozenBookingIdSet failed — payout batch blocked until disputes readable',
      batchId: null,
      itemCount: 0,
      rail,
  ...
  const taken = await getBookingIdsBlockedFromNewPayoutPools(bookingIds)
...
export async function listBatchesForAdmin({ limit = 50, status = null } = {}) {
  ...
  if (allPartnerIds.size) {
    ...
    for (const row of (openPayouts || []).filter(isOpenPartnerHostPayoutRow)) {
...
export async function lockBatch(batchId, userId = null) {
  ...
  const now = new Date().toISOString()
  const { data: lockedRow, error: batchUpErr } = await supabaseAdmin
    .from('payout_batches')
  ...
  if (!lockedRow) {
    return {
      success: false,
      error: 'lock_race',
      message: 'Пул уже зафиксирован или статус изменился. Обновите страницу.',
      status: batch.status,
    }
  }
