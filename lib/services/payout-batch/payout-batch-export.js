...
export async function exportBatchRegistry(batchId, format = 'json') {
  ...
  const batchSt = String(pack.batch.status || '')
    .trim()
    .toUpperCase()
  // Concierge: Lock → CSV → bank. Do not export DRAFT as bank registry (AUDIT_02 W2.9).
  if (!['LOCKED', 'EXPORTED', 'SETTLED'].includes(batchSt)) {
    return {
      error: 'invalid_status',
      status: pack.batch.status,
      message: 'Сначала зафиксируйте пул (Lock), затем скачайте CSV для банка.',
    }
  }
