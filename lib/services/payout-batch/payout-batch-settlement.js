  ...
  for (const item of pack.items || []) {
    ...
    if (booking && ['READY_FOR_PAYOUT', 'THAWED'].includes(String(booking.status))) {
      ...
      if (!upErr) {
        bookingsCompleted += 1
        try {
          await runReferralPayoutOnBookingCompleted(bookingId, {
            trigger: 'payout_batch_settled',
          })
        } catch (e) {
          console.warn('[PayoutBatch] referral completion trigger', bookingId, e?.message || e)
        }
      }
    }
