...
import { getFrozenBookingIdSet as getFrozenBookingIdSetFromEligibility } from '@/lib/partner/partner-payout-eligibility.js'
...
/** @see lib/partner/partner-payout-eligibility.js — SSOT (includes PENDING_MEDIATION soft block). */
export async function getFrozenBookingIdSet(bookingIds) {
  return getFrozenBookingIdSetFromEligibility(bookingIds)
}
      ...
      if (remainingMs <= 0) {
        try {
          ...
          const refundResult = await executeDisputeResolution({
            ...
            disputeId: row.id,
            resolutionReason,
            trigger: 'dispute_sla_auto_resolve',
            actorRole: 'SYSTEM',
          })
          if (!refundResult.success) {
            throw new Error(refundResult.error || 'DISPUTE_GUEST_REFUND_FAILED')
          }
          resolved += 1
        } catch (e) {
          resolveErrors += 1
