...
/**
 * Ops TG for stale display FX. Stage 202.3 — call only from cron writer (not guest hot path).
 * @param {{ stale: boolean, staleCodes: string[], oldestStaleIso?: string | null, lastUpdateLabel?: string | null }} health
 */
export async function maybeAlertStaleDisplayRatesFromHealth(health) {
  ...
  const dateLabel =
    health.lastUpdateLabel ||
    (health.oldestStaleIso
...
/** @deprecated Prefer {@link maybeAlertStaleDisplayRatesFromHealth} from cron only. */
export async function maybeAlertStaleDisplayRates(updatedAtByCode, map) {
  return maybeAlertStaleDisplayRatesFromHealth(evaluateDisplayFxStale(updatedAtByCode, map))
}
...
export async function resolveThbPerUsdt() {
  if (supabaseAdmin) {
    ...
    if (!error && data) {
      ...
      // Stage 202.3 — accept stale DB row; cron is the only ExchangeRate-API writer.
 ...
 * 5) USDT fallback: `resolveThbPerUsdt()` (DB/env only — Stage 202.3); USD из USDT при пустом USD.
 * 6) **Розничная надбавка:** на возвращаемую карту применяется {@link resolveChatInvoiceRateMultiplier}.
 *    Stale FX Telegram — только cron (`maybeAlertStaleDisplayRatesFromHealth`), не guest hot path.
 *
...
export async function getDisplayRateMap(options = {}) {
  ...
  // Stage 202.3 — do not Telegram from guest/API hot path (serverless cooldown is per-isolate spam).
  // Stale FX ops alert is emitted from the cron writer only.
