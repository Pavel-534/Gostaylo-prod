...
export async function handleReferralFriendBooked(data) {
  ...
  await sendEmail(profile.email, `${copy.subject} — ${site}`, `${copy.body}\n\n/profile/referral`)

  if (profile.telegram_id) {
    await sendTelegram(
      profile.telegram_id,
      `📅 <b>${lang === 'en' ? 'Friend booked' : lang === 'zh' ? '好友已预订' : lang === 'th' ? 'เพื่อนจองแล้ว' : 'Друг забронировал'}</b>\n\n${copy.headline}\n<a href="/profile/referral">Referral hub</a>`,
    )
  }

  await PushService.sendToUser(ambassadorId, 'REFERRAL_FRIEND_BOOKED', {
    friendName,
    listingTitle,
    amountLabel,
    amountThb: String(amountThb),
...
/** Stage 131.A2 — referral friend paid. */
export async function handleReferralFriendPaid(data) {
  const { sendEmail, sendTelegram } = getNotifyDeps()
  const ambassadorId = String(data?.ambassadorId || '').trim()
  const amountThb = round2(data?.paidAmountThb)
  if (!ambassadorId || amountThb <= 0) return
  ...
  const amountLabel = formatAmountThb(amountThb, lang)
  const friendName = await resolveFriendDisplayNameForNotify(data)
  const copy = buildReferralFriendPaidCopy({ friendName, amountLabel, lang })
  ...
  if (profile.telegram_id) {
    await sendTelegram(
      profile.telegram_id,
      `💳 <b>${lang === 'en' ? 'Friend paid' : lang === 'zh' ? '好友已付款' : lang === 'th' ? 'เพื่อนชำระแล้ว' : 'Друг оплатил'}</b>\n\n${copy.headline}\n<a href="/profile/referral">Referral hub</a>`,
    )
  }

  await PushService.sendToUser(ambassadorId, 'REFERRAL_FRIEND_PAID', {
    friendName,
    amountLabel,
    amountThb: String(amountThb),
    bookingId: String(data?.bookingId || ''),
...
export async function handleReferralFriendCompleted(data) {
  ...
  await PushService.sendToUser(ambassadorId, 'REFERRAL_FRIEND_COMPLETED', {
    friendName,
    listingTitle,
    amountLabel,
    amountThb: String(amountThb),
