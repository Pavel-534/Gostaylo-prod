...
import { notifyCopyLang } from '@/lib/i18n/resolve-notify-locale.js'
...
} from '@/lib/services/notifications/referral-friend-notify-copy.js'
...
async function loadProfileNotifyFields(userId) {
  ...
  const lang = notifyCopyLang(resolveUserLocale(data))
...
export async function handleReferralAdminAlert(data) {
  ...
  if (amountThb <= 0 && !data?.monthly && !data?.monthlyApproaching) return
  ...
  if (data?.monthly) {
    reasons.push(
      `Early warning · referral spend growing ≥ ${formatAmountThb(data.monthly?.thresholdThb || policyThb)} (${formatAmountThb(data.monthlySpendThb || data.monthly?.monthlySpendThb)})`,
    )
  }
  if (data?.monthlyApproaching) {
    reasons.push(
      `Approaching program cap: ${formatAmountThb(data.monthlyApproachSpendThb || data.monthlySpendThb)} (80% ≥ ${formatAmountThb(data.monthlyApproachWarnThb)})`,
    )
  }
...
async function dispatchReferralWithdrawalUserNotice(data, stage) {
  ...
  const rejectReason = String(data?.reason ?? data?.admin_comment ?? data?.adminComment ?? '').trim()
  if (!userId) return
  ...
  const copy = {
    ...
    rejected: {
      ...
      bodyRu:
        ...
        `\nСредства остались на кошельке — вы можете подать заявку снова.\n\n` +
        `Кошелёк: /profile/wallet`,
      bodyEn:
        `Hello${profile.first_name ? `, ${profile.first_name}` : ''}!\n\n` +
      ...
      headlineRu: rejectReason
        ? `Заявка на вывод отклонена. Причина: ${rejectReason}. Средства остались на кошельке.`
        : rubLabel
          ? `Заявка отклонена: ≈ ${rubLabel} остаётся на кошельке`
          : `Заявка на вывод отклонена. Средства остались на кошельке.`,
      headlineEn: rejectReason
        ? `Withdrawal rejected. Reason: ${rejectReason}. Funds remain in your wallet.`
        : rubLabel
          ? `Withdrawal rejected: ≈ ${rubLabel} remains in wallet`
          : `Withdrawal request rejected. Funds remain in your wallet.`,
    },
    expired: {
...
export async function handleReferralTeamWeeklyDigest(data) {
  ...
  const l3Thb = round2(data?.l3Thb)
  const totalThb = round2(data?.totalThb ?? l2Thb + l3Thb)
  ...
  const { profile, lang } = loaded
  const site = getSiteDisplayName()
  const referralHubUrl = buildPublicUrl('/profile/referral')
  ...
  const l3Label = formatAmountThb(l3Thb, lang)
  const totalRub = await resolveRubEquivalent(totalThb, data)
  const rubPart = totalRub != null ? ` (~${formatAmountRub(totalRub, lang)})` : ''
  const l2l3Mode = String(data?.networkLevelMode || '') === 'l2_l3'

  const headlineRu = l2l3Mode
    ? `За неделю твоя сеть L2/L3 заработала ${totalLabel}${rubPart}, твоя доля ~${totalLabel}.`
    : `Ваша команда заработала ${totalLabel}${rubPart} на этой неделе`
  const headlineEn = l2l3Mode
    ? `Your L2/L3 network generated ${totalLabel}${rubPart} this week, your share is ~${totalLabel}.`
    : `Your team earned ${totalLabel}${rubPart} this week`
  ...
  const textBody =
    lang === 'en'
      ? `Hello${profile.first_name ? `, ${profile.first_name}` : ''}!\n\n` +
        `${headlineEn}\n` +
        (l2l3Mode
          ? `L2: ${l2Label}\nL3: ${l3Label}\n\nWallet ETA: up to 14 days\n\nReferral hub: ${referralHubUrl}\n\nTeam ${site}`
          : `L1 (direct): ${l1Label}\nL2 (network): ${l2Label}\n\nReferral hub: ${referralHubUrl}\n\nTeam ${site}`)
      : `Здравствуйте${profile.first_name ? `, ${profile.first_name}` : ''}!\n\n` +
        `${headlineRu}\n` +
        (l2l3Mode
          ? `L2: ${l2Label}\nL3: ${l3Label}\n\nПоступление на кошелёк: до 14 дней\n\nРефералка: ${referralHubUrl}\n\nКоманда ${site}`
          : `L1 (прямые): ${l1Label}\nL2 (сеть): ${l2Label}\n\nРефералка: ${referralHubUrl}\n\nКоманда ${site}`)
  ...
  if (profile.telegram_id) {
    await sendTelegram(
      profile.telegram_id,
      lang === 'en'
        ? `📊 <b>Weekly team digest</b>\n\n${headlineEn}\nL2: ${l2Label} · L3: ${l3Label}\n${referralHubUrl}`
        : `📊 <b>Итоги недели</b>\n\n${headlineRu}\nL2: ${l2Label} · L3: ${l3Label}\n${referralHubUrl}`,
    )
    return
  }

  const pushResult = await PushService.sendToUser(userId, 'REFERRAL_TEAM_WEEKLY_DIGEST', {
...
async function resolveFriendDisplayNameForNotify(data) {
  let displayName = String(data?.friendName || '').trim()
  const friendUserId = String(data?.friendUserId || '').trim()
  if (!displayName && friendUserId) {
    const refLoaded = await loadProfileNotifyFields(friendUserId)
    if (refLoaded?.profile) {
      displayName = formatPrivacyDisplayNameForParticipant(
        refLoaded.profile.first_name,
        refLoaded.profile.last_name,
...
/** Stage 131.A2 — referral friend booked. */
export async function handleReferralFriendBooked(data) {
  const { sendEmail, sendTelegram } = getNotifyDeps()
  const ambassadorId = String(data?.ambassadorId || '').trim()
  const amountThb = round2(data?.estimatedL1Thb)
  if (!ambassadorId || amountThb <= 0) return

  const loaded = await loadProfileNotifyFields(ambassadorId)
  if (!loaded?.profile?.id) return
  const { profile, lang } = loaded
  const site = getSiteDisplayName()
  const referralHubUrl = buildPublicUrl('/profile/referral')
  const amountLabel = formatAmountThb(amountThb, lang)
  const friendName = await resolveFriendDisplayNameForNotify(data)
  const listingTitle = String(data?.listingTitle || '').trim()
  const copy = buildReferralFriendBookedCopy({ friendName, listingTitle, amountLabel, lang })
  const tasks = []
  if (profile.email) {
    tasks.push(sendEmail(profile.email, `${copy.subject} — ${site}`, `${copy.body}\n\n${referralHubUrl}`))
  }
  if (profile.telegram_id) {
    tasks.push(
      sendTelegram(
        profile.telegram_id,
        `📅 <b>${lang === 'en' ? 'Friend booked' : lang === 'zh' ? '好友已预订' : lang === 'th' ? 'เพื่อนจองแล้ว' : 'Друг забронировал'}</b>\n\n${copy.headline}\n${referralHubUrl}`,
      ),
    )
  }
  tasks.push(
    PushService.sendToUser(ambassadorId, 'REFERRAL_FRIEND_BOOKED', {
      friendName,
      listingTitle,
      amountLabel,
      amountThb: String(amountThb),
...
/** Stage 131.A2 — referral friend completed trip (before bonus earned notify). */
export async function handleReferralFriendCompleted(data) {
  const { sendEmail, sendTelegram } = getNotifyDeps()
  const ambassadorId = String(data?.ambassadorId || '').trim()
  const amountThb = round2(data?.estimatedTotalThb ?? data?.estimatedL1Thb)
  if (!ambassadorId || amountThb <= 0) return
  ...
  const friendName = await resolveFriendDisplayNameForNotify(data)
  const listingTitle = String(data?.listingTitle || '').trim()
  const copy = buildReferralFriendCompletedCopy({ friendName, listingTitle, amountLabel, lang })
  const tasks = []
  if (profile.email) {
    tasks.push(sendEmail(profile.email, `${copy.subject} — ${site}`, `${copy.body}\n\n${referralHubUrl}`))
  }
  if (profile.telegram_id) {
    tasks.push(
      sendTelegram(
        profile.telegram_id,
        `✈️ <b>${lang === 'en' ? 'Trip completed' : lang === 'zh' ? '行程已完成' : lang === 'th' ? 'เดินทางเสร็จสิ้น' : 'Поездка завершена'}</b>\n\n${copy.headline}\n${referralHubUrl}`,
      ),
    )
  }
  tasks.push(
    PushService.sendToUser(ambassadorId, 'REFERRAL_FRIEND_COMPLETED', {
      friendName,
      listingTitle,
      amountLabel,
      amountThb: String(amountThb),
      estimatedL1Thb: String(round2(data?.estimatedL1Thb)),
      estimatedL2Thb: String(round2(data?.estimatedL2Thb)),
      estimatedL3Thb: String(round2(data?.estimatedL3Thb)),
      bookingId: String(data?.bookingId || ''),
      headline: copy.headline,
      link: '/profile/referral',
      siteName: site,
...
/** Stage 131.A2.1 — referral friend cancelled booking. */
export async function handleReferralFriendCancelled(data) {
  const ambassadorId = String(data?.ambassadorId || '').trim()
  ...
  const friendName = await resolveFriendDisplayNameForNotify(data)
  const listingTitle = String(data?.listingTitle || '').trim()
  const copy = buildReferralFriendCancelledCopy({ friendName, listingTitle, lang })

  // Cancellation is low-noise: in-app + push only.
  await PushService.sendToUser(ambassadorId, 'REFERRAL_FRIEND_CANCELLED', {
    friendName,
    listingTitle,
    bookingId: String(data?.bookingId || ''),
    headline: copy.headline,
