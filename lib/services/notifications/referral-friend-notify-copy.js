...
export function buildReferralFriendBookedCopy(p) {
  ...
  if (lang === 'en') {
    const headline = `${friend} booked ${listing} — your ~${amount} bonus is coming (payment is tracked automatically)`
    return { subject: `Friend booked — ~${amount} bonus pending`, headline, body: headline }
  }
  if (lang === 'zh') {
    const headline = `${friend} 预订了 ${listing} — 您的 ~${amount} 奖金即将到账（付款会自动跟踪）`
    return { subject: `好友已预订 — ~${amount} 奖金待入账`, headline, body: headline }
  }
  if (lang === 'th') {
    const headline = `${friend} จอง ${listing} — โบนัส ~${amount} ของคุณกำลังจะมา (ระบบติดตามการชำระเงินอัตโนมัติ)`
    return { subject: `เพื่อนจองแล้ว — โบนัส ~${amount}`, headline, body: headline }
  }
  const headline = `${friend} забронировал(а) ${listing} — ваш бонус ~${amount} скоро появится (оплата отслеживается автоматически)`
...
/**
 * @param {{ friendName: string, listingTitle: string, lang?: string }} p
 */
export function buildReferralFriendCancelledCopy(p) {
  const lang = normalizeLang(p.lang)
  const friend =
    String(p.friendName || '').trim() ||
    (lang === 'en' ? 'Friend' : lang === 'zh' ? '好友' : lang === 'th' ? 'เพื่อน' : 'Друг')
  const listing =
    String(p.listingTitle || '').trim() ||
    (lang === 'en' ? 'listing' : lang === 'zh' ? '房源' : lang === 'th' ? 'ที่พัก' : 'объект')

  if (lang === 'en') {
    const headline = `${friend} cancelled their booking at ${listing}. No bonus this time — totally normal.`
    return { subject: 'Booking cancelled', headline, body: headline }
  }
  if (lang === 'zh') {
    const headline = `${friend} 取消了 ${listing} 的预订。这次没有奖金 — 正常情况。`
    return { subject: '预订已取消', headline, body: headline }
  }
  if (lang === 'th') {
    const headline = `${friend} ยกเลิกการจอง ${listing} ไม่มีโบนัสครั้งนี้ — เป็นเรื่องปกติ`
    return { subject: 'ยกเลิกการจอง', headline, body: headline }
  }
  const headline = `${friend} отменил(а) бронирование в ${listing}. Бонуса не будет. Это нормально — такое бывает.`
  return { subject: 'Бронирование отменено', headline, body: headline }
}
