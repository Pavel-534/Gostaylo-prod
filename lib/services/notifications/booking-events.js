...
import { formatThbAmountAsCode } from '@/lib/currency.js'
...
export async function handleNewBookingRequest(data) {
  const { sendEmail, sendTelegram, sendToAdminTopic, calculateNights, buildGuestChatUrlForBooking, resolveGuestEmailLang, sendTelegramBookingRequest } =
    ...
    const guestPriceLine = formatBookingAmountForNotify(booking, totalPrice, guestLang)
    const ledgerPriceLine = formatThbAmountAsCode(totalPrice, partnerLang)
    ...
    await safeNotifyChannel('newBooking:partnerPush', async () => {
      ...
      await PushService.sendToUser(
        ...
        buildPartnerLifecyclePushData({
          booking,
          listing,
          guestName: booking.guest_name || guest?.name,
        }),
      );
    });
...
export async function handleBookingConfirmed(data) {
  const { sendEmail, sendToAdminTopic, buildGuestChatUrlForBooking, resolveGuestEmailLang } =
    getNotifyDeps()
    ...
    const checkoutPath = bookingId ? `/checkout/${encodeURIComponent(bookingId)}` : null
    const checkoutUrl = checkoutPath ? `${BASE_URL}${checkoutPath}` : `${BASE_URL}/my-bookings`
    const payable = isBookingPayable(booking?.status)
    const payHint = payable
      ? notifyTelegramCopy('notifyTg_bookingConfirmedPayHint', guestLang, { checkoutUrl })
      : notifyTelegramCopy('notifyTg_bookingConfirmedNoPayHint', guestLang, {
          bookingsUrl: `${BASE_URL}/my-bookings`,
        })
          ...
          EmailService.sendBookingConfirmedGuest(
            {
              ...
              checkoutUrl: payable ? checkoutUrl : `${BASE_URL}/my-bookings`,
              chatUrl,
    ...
    if (renterTg) {
      ...
      const payBtn = notifyTelegramCopy('notifyTg_bookingConfirmedPayBtn', guestLang)
      await safeNotifyChannel('bookingConfirmed:guestTelegram', async () => {
        await Tg.sendTelegramMessagePayload({
          ...
          ...(payable && checkoutUrl
            ? {
                reply_markup: {
                  inline_keyboard: [[{ text: payBtn, url: checkoutUrl }]],
                },
              }
            : {}),
        })
    ...
    const guestId = guest?.id || booking?.renter_id || null
    if (guestId && payable && checkoutPath) {
      await safeNotifyChannel('bookingConfirmed:guestPush', async () => {
        await PushService.sendToUser(String(guestId), 'BOOKING_CONFIRMED', {
          listing: listing?.title || '',
          bookingId: bookingId,
          link: checkoutPath,
        })
...
export async function handleBookingCancelled(data) {
  const { sendEmail, sendTelegram, sendToAdminTopic, calculateNights, buildGuestChatUrlForBooking, resolveGuestEmailLang, sendTelegramBookingRequest } =
    ...
    const { booking, listing, guest: guestField, renter, partner, reason, notifyPartner } = data;
    const guest = guestField ?? renter;
    const guestLang = await resolveGuestNotifyLocale(booking, guest);
    const listingTitle = listing?.title || notifyTelegramCopy('notifyTg_listingFallback', guestLang);
    const reasonText = reason || notifyTelegramCopy('notifyTg_reasonUnknown', guestLang);
    const partnerId = partner?.id || booking?.partner_id || null
    const cancelledBy = booking?.metadata?.cancelled_by_user_id
    const partnerInitiated =
      partnerId != null &&
      cancelledBy != null &&
      String(cancelledBy) === String(partnerId)
    const shouldNotifyPartner = notifyPartner !== false && partnerId && !partnerInitiated
    ...
    // Wave H5 — FCM (+ optional TG) when guest/system cancels; skip partner self-cancel / partner API.
    if (shouldNotifyPartner) {
      await safeNotifyChannel('bookingCancelled:partnerPush', async () => {
        await PushService.sendToUser(
          String(partnerId),
          'CANCEL_REQUESTED',
          buildPartnerLifecyclePushData({
      ...
      await safeNotifyChannel('bookingCancelled:partnerTelegram', async () => {
        const tg = partner?.telegram_id
        if (!tg) return
        const partnerLang = resolveNotifyLocaleFromProfile(partner)
        const pushData = buildPartnerLifecyclePushData({
          booking,
          listing,
          guestName: booking?.guest_name || guest?.name,
        })
        const deep = `${BASE_URL}${pushData.link}`
        await sendTelegram(
          tg,
          `❌ <b>${partnerLang === 'en' ? 'Booking cancel requested' : 'Запрос на отмену бронирования'}</b>\n\n` +
            `📍 ${escapeTelegramHtmlText(pushData.listing)}\n` +
            `📅 ${escapeTelegramHtmlText(pushData.dates)}\n` +
            `👤 ${escapeTelegramHtmlText(pushData.guest)}\n\n` +
            `<a href="${deep}">${partnerLang === 'en' ? 'Open booking' : 'Открыть бронь'}</a>`,
        )
      })
    }
  }
...
export async function handleNewMessage(data) {
  const { sendEmail, sendTelegram, sendToAdminTopic, calculateNights, buildGuestChatUrlForBooking, resolveGuestEmailLang, sendTelegramBookingRequest } =
    ...
    const fromName = escapeTelegramHtmlText(
      String(sender?.name || message?.sender_name || 'User'),
    )
    const regarding = escapeTelegramHtmlText(
      String(listing?.title || conversation?.listing_title || 'Listing'),
    )
    const previewRaw = String(message?.message || '')
    const preview100 = escapeTelegramHtmlText(
      previewRaw.substring(0, 100) + (previewRaw.length > 100 ? '...' : ''),
    )
    ...
    await sendToAdminTopic('MESSAGES',
      `💬 <b>New message</b>\n\n` +
      `👤 <b>From:</b> ${fromName}\n` +
      `📍 <b>Regarding:</b> ${regarding}\n` +
      `💬 <i>"${preview100}"</i>\n\n` +
      `📱 Reply on site`
    );
    
    // Email to recipient (plain text → textToHtml escapes HTML)
    if (recipient?.email) {
...
export async function handleCheckInConfirmed(data) {
  const { sendEmail, sendTelegram, sendToAdminTopic, calculateNights, buildGuestChatUrlForBooking, resolveGuestEmailLang, sendTelegramBookingRequest } =
      ...
      await sendEmail(
        ...
        [
          ...
          notifyEmailFallbackCopy('emailFb_amountLine', partnerLang, {
            amount: formatThbAmountAsCode(parseFloat(booking.price_thb) || 0, partnerLang),
          }),
          '',
    ...
    await sendToAdminTopic('FINANCE',
      ...
      `📍 ${listing?.title}\n` +
      `💰 ฿${booking.price_thb?.toLocaleString()}\n\n` +
      `🔒 Эскроу ещё удерживает средства (thaw/payout — отдельно)`
    );
  }
...
export async function handleCheckInReminder(data) {
  const { sendTelegram, sendToAdminTopic } = getNotifyDeps()
