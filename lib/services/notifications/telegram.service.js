...
/**
 * Resolve HQ forum thread id for system / user-feedback alerts.
 * Stage 202.0 — `userFeedbackTopic` prefers TELEGRAM_USER_FEEDBACK_TOPIC_ID,
 * then falls back to TELEGRAM_SYSTEM_ALERTS_TOPIC_ID.
 * @param {{ userFeedbackTopic?: boolean, message_thread_id?: number }} [opts]
 */
export function resolveSystemAlertThreadId(opts = {}) {
  if (opts.message_thread_id != null) {
    const n = Number(opts.message_thread_id)
    if (Number.isFinite(n) && n > 0) return n
  }
  const feedbackRaw = (process.env.TELEGRAM_USER_FEEDBACK_TOPIC_ID || '').trim()
  const systemRaw = (process.env.TELEGRAM_SYSTEM_ALERTS_TOPIC_ID || '').trim()
  const raw = opts.userFeedbackTopic
    ? feedbackRaw || systemRaw
    : systemRaw
...
/**
 * Критические алерты → топик System (или User Feedback) / sendToAdmin.
...
export async function sendSystemAlertTelegram(message, opts = {}) {
  ...
  const threadId = resolveSystemAlertThreadId(opts)
  if (TELEGRAM_BOT_TOKEN && TELEGRAM_ADMIN_GROUP_ID && threadId != null) {
    return sendTelegramMessagePayload({
      chat_id: TELEGRAM_ADMIN_GROUP_ID,
      message_thread_id: threadId,
      ...payloadBase,
  ...
  console.warn(
    opts.userFeedbackTopic
      ? '[USER FEEDBACK] TELEGRAM_USER_FEEDBACK_TOPIC_ID / SYSTEM_ALERTS missing; using sendToAdmin fallback'
