...
import { buildPartnerLifecyclePushData } from '@/lib/services/push/partner-lifecycle-push.js'
...
import { getPayoutReleaseConfig, formatPayoutNotificationCopy } from '@/lib/booking/payout-release-config.js'
...
import {
  formatGuestPaymentEscrowLine,
  getGuestBookingLabelPlaceholders,
} from '@/lib/i18n/guest-booking-labels.js'
import {
...
import { wasPaymentReceivedNotified } from '@/lib/services/escrow/move-to-escrow-side-effects.js'

const BASE_URL = getPublicSiteUrl()

/**
 * Load guest profile for multichannel payment notifications.
 * @param {object} booking
 * @param {object | null | undefined} guest
 */
async function resolveGuestProfileForPayment(booking, guest) {
  if (guest?.id) return guest
  const renterId = booking?.renter_id
  if (!renterId || !supabaseAdmin) return null
  try {
    const { data } = await supabaseAdmin
      .from('profiles')
      .select('id, email, telegram_id, language, first_name, name')
      .eq('id', String(renterId))
      .maybeSingle()
    return data || null
  } catch (e) {
    console.warn('[NOTIFICATION] resolveGuestProfileForPayment', e?.message || e)
    return null
  }
}

/**
 * Guest multichannel notify: payment captured into escrow (email + push + Telegram).
 * @param {object} params
 * @param {object} params.booking
 * @param {object} [params.listing]
 * @param {object} [params.payment]
 * @param {object} [params.guest]
 * @param {ReturnType<typeof getNotifyDeps>} params.deps
 */
async function notifyGuestPaymentEscrowReceived({ booking, listing, payment, guest, deps }) {
  const {
    sendEmail,
    sendTelegram,
  ...
  if (wasPaymentReceivedNotified(booking?.metadata)) {
    return
  }

  const guestProfile = await resolveGuestProfileForPayment(booking, guest)
  ...
  const emailLang = await resolveGuestEmailLang(booking, guestProfile)
  const guestLang = emailLang
  const listingTitle =
    listing?.title || notifyEmailFallbackCopy('emailFb_fallbackListing', guestLang)
  const guestEmail = booking?.guest_email || guestProfile?.email
  const listingCategorySlug =
    listing?.category_slug || listing?.categorySlug || booking?.category_slug || null
  const wizardProfile = listing?.wizard_profile || listing?.wizardProfile || null
  const guestLabelPh = getGuestBookingLabelPlaceholders({
    categorySlug: listingCategorySlug,
    wizardProfile,
    language: guestLang,
  })

  await safeNotifyChannel('paymentReceived:guestEmail', async () => {
    if (!guestEmail) return
    const chatUrl = await buildGuestChatUrlForBooking(booking.id)
    await deliverPremiumEmailWithPlainFallback({
      channelLabel: 'paymentReceived:guestEmail',
      to: guestEmail,
      premiumFn: () =>
        EmailService.sendPaymentSuccessGuest(
          {
            ...
            escrowText: formatGuestPaymentEscrowLine('escrow', {
              categorySlug: listingCategorySlug,
              wizardProfile,
              language: emailLang,
            }),
      ...
      buildFallback: () =>
        buildGuestPaymentFallback(guestLang, {
          variant: 'confirmed',
          guestName: booking?.guest_name || guestProfile?.first_name,
          listing: listing?.title,
          checkIn: booking?.check_in,
          checkOut: booking?.check_out,
          amount: payment?.amount?.toLocaleString() || String(totalThb || 0),
        }),
    })
  })

  await safeNotifyChannel('paymentReceived:guestPush', async () => {
    const guestId = booking?.renter_id || guestProfile?.id
    if (!guestId) return
    const pushBody = formatGuestPaymentEscrowLine('pushBody', {
      categorySlug: listingCategorySlug,
      wizardProfile,
      language: guestLang,
    })
    await PushService.sendToUser(String(guestId), 'PAYMENT_ESCROW_GUEST', {
      amount: String(totalThb || payment?.amount || 0),
      listing: listingTitle,
      link: renterBookingsListPath(booking.id),
      bookingId: String(booking.id),
      siteName: getSiteDisplayName(),
      providerDative: guestLabelPh.providerDative,
      providerInstrumental: guestLabelPh.providerInstrumental,
      pushBodyOverride: pushBody,
    })
  })

  const guestTg = guestProfile?.telegram_id
  if (guestTg) {
    await safeNotifyChannel('paymentReceived:guestTelegram', async () => {
      const escrowLine = formatGuestPaymentEscrowLine('escrow', {
        categorySlug: listingCategorySlug,
        wizardProfile,
        language: guestLang,
      })
      await sendTelegram(
        guestTg,
        `${notifyTelegramCopy('notifyTg_paymentReceivedTitle', guestLang)}\n\n` +
          `📍 ${escapeTelegramHtmlText(listingTitle)}\n` +
          `💵 ฿${(payment?.amount || totalThb || 0).toLocaleString()}\n\n` +
          `${escapeTelegramHtmlText(escrowLine)}`,
      )
    })
  }
}
...
export async function handlePaymentReceived(data) {
  const { sendEmail, sendTelegram, sendToAdminTopic, calculateNights, buildGuestChatUrlForBooking, resolveGuestEmailLang, sendTelegramBookingRequest } =
    ...
    await safeNotifyChannel('paymentReceived:partnerPush', async () => {
      ...
      if (!partnerId) return
      await PushService.sendToUser(String(partnerId), 'PAYMENT_COMPLETED', {
    ...
    await notifyGuestPaymentEscrowReceived({
      booking,
      listing,
      payment,
      guest: data?.guest || data?.renter,
      deps: {
        sendEmail,
        sendTelegram,
        buildGuestChatUrlForBooking,
...
export async function handlePaymentSubmitted(data) {
  const { sendEmail, sendTelegram, sendToAdminTopic, calculateNights, buildGuestChatUrlForBooking, resolveGuestEmailLang, sendTelegramBookingRequest } =
    ...
    await sendToAdminTopic('FINANCE',
      `💰 <b>NEW [${escapeTelegramHtmlText(String(paymentMethodLabel))}] PAYMENT!</b>\n\n` +
      `📍 <b>Объект:</b> ${escapeTelegramHtmlText(String(listing?.title || 'N/A'))}\n` +
      `👤 <b>Гость:</b> ${escapeTelegramHtmlText(String(booking?.guest_name || 'N/A'))}\n` +
      `💵 <b>Amount:</b> ${usdtAmount} USDT\n` +
      `💴 <b>THB:</b> ฿${parseFloat(thbAmount).toLocaleString()}\n` +
      `📋 <b>TXID:</b> <code>${escapeTelegramHtmlText(String(payment?.txid || 'N/A'))}</code>\n\n` +
      `🔗 <a href="https://tronscan.org/#/transaction/${encodeURIComponent(String(payment?.txid || ''))}">Verify on TronScan</a>\n\n` +
...
export async function handlePaymentSuccess(data) {
  const deps = getNotifyDeps()
  const { sendToAdminTopic } = deps
...
export async function handlePartnerFundsThawedAvailable(data) {
  const { sendEmail, sendTelegram, sendToAdminTopic, calculateNights, buildGuestChatUrlForBooking, resolveGuestEmailLang, sendTelegramBookingRequest } =
    ...
    const releaseConfig = getPayoutReleaseConfig({
      ...booking,
      listing,
      metadata: booking?.metadata,
    })
    const thawCopy = formatPayoutNotificationCopy(releaseConfig, emailLang, 'thaw', {
      amount: amountStr,
      listing: listingTitle,
    })
          ...
          EmailService.sendPartnerFundsThawedEmail(
            {
              ...
              listingTitle,
              bookingId: String(booking.id),
              categorySlug: releaseConfig.categorySlug,
              wizardProfile: releaseConfig.wizardProfile,
            },
            to,
            emailLang,
          ),
