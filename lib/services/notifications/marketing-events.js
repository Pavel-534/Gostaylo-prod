...
export async function handleUserWelcome(data) {
  const { sendEmail, sendTelegram, sendToAdminTopic, calculateNights, buildGuestChatUrlForBooking, resolveGuestEmailLang, sendTelegramBookingRequest } =
    ...
    const notifyLang = lang || resolveNotifyLocaleFromProfile(user);
    
    await safeNotifyChannel('userWelcome:email', async () => {
      if (!user?.email) return;
      await deliverPremiumEmailWithPlainFallback({
        channelLabel: 'userWelcome:email',
        to: user.email,
        premiumFn: () =>
        ...
        buildFallback: () => ({
          subject: notifyEmailFallbackCopy('emailFb_welcomeSubj', notifyLang),
          body: [
            notifyEmailFallbackCopy('emailFb_greetingGuest', notifyLang, {
              name: user.first_name || notifyEmailFallbackCopy('emailFb_fallbackFriend', notifyLang),
            }),
            '',
            notifyEmailFallbackCopy('emailFb_welcomeLead', notifyLang),
            '',
            emailFallbackSignoff(notifyLang),
          ].join('\n'),
        }),
      });
    });
...
export async function handlePartnerVerified(data) {
  const { sendEmail, sendTelegram, sendToAdminTopic, calculateNights, buildGuestChatUrlForBooking, resolveGuestEmailLang, sendTelegramBookingRequest } =
    ...
    const { partner } = data;
    const notifyLang = resolveNotifyLocaleFromProfile(partner);

    await safeNotifyChannel('partnerVerified:email', async () => {
      if (!partner?.email) return;
      await deliverPremiumEmailWithPlainFallback({
        channelLabel: 'partnerVerified:email',
        to: partner.email,
        premiumFn: () =>
          EmailService.sendPartnerApproved(
            {
              name: partner.first_name || partner.name || notifyEmailFallbackCopy('emailFb_fallbackPartner', notifyLang),
              email: partner.email,
            },
            notifyLang,
          ),
        sendPlainEmail: sendEmail,
        buildFallback: () => ({
          subject: notifyEmailFallbackCopy('emailFb_partnerVerifiedSubj', notifyLang),
          body: [
            notifyEmailFallbackCopy('emailFb_greetingPartner', notifyLang, {
              name: partner.first_name || notifyEmailFallbackCopy('emailFb_fallbackPartner', notifyLang),
            }),
            '',
            notifyEmailFallbackCopy('emailFb_partnerVerifiedLead', notifyLang),
            '',
            notifyEmailFallbackCopy('emailFb_partnerVerifiedBullets', notifyLang),
            '',
            notifyEmailFallbackCopy('emailFb_partnerVerifiedTail', notifyLang),
            '',
            emailFallbackSignoff(notifyLang),
          ].join('\n'),
        }),
...
export async function handleListingApproved(data) {
  ...
  await safeNotifyChannel('listingApproved:email', async () => {
    if (!partner?.email) return
    await deliverPremiumEmailWithPlainFallback({
      channelLabel: 'listingApproved:email',
      to: partner.email,
      premiumFn: () => EmailService.sendListingApproved(listing, partner.email, notifyLang),
      sendPlainEmail: sendEmail,
      buildFallback: () => ({
        subject: notifyEmailFallbackCopy('emailFb_listingApprovedSubj', notifyLang, { title }),
        body: [
          notifyEmailFallbackCopy('emailFb_listingApprovedLead', notifyLang, { title, brand }),
          '',
          emailFallbackSignoff(notifyLang),
        ].join('\n'),
      }),
  ...
  if (partner?.telegram_id) {
    const tgText =
      notifyLang === 'en'
        ? `✅ <b>Listing approved</b> · ${brand}\n\n📍 ${title}\n\n🎉 It’s now available for booking`
        : notifyLang === 'zh'
          ? `✅ <b>上架已通过</b> · ${brand}\n\n📍 ${title}\n\n🎉 现在可以接受预订`
          : notifyLang === 'th'
            ? `✅ <b>อนุมัติรายการแล้ว</b> · ${brand}\n\n📍 ${title}\n\n🎉 พร้อมให้จองแล้ว`
            : `✅ <b>Объявление одобрено</b> · ${brand}\n\n📍 ${title}\n\n🎉 Теперь доступно для бронирования`
    await sendTelegram(partner.telegram_id, tgText)
  }
}
...
export async function handleListingRejected(data) {
  ...
  const brand = getSiteDisplayName()
  const title = listing?.title || '—'
  const reasonText = reason || notifyEmailFallbackCopy('emailFb_listingRejectedReasonFallback', notifyLang)

  await safeNotifyChannel('listingRejected:email', async () => {
    if (!partner?.email) return
    await deliverPremiumEmailWithPlainFallback({
      channelLabel: 'listingRejected:email',
      to: partner.email,
      premiumFn: () => EmailService.sendListingRejected(listing, partner.email, reason, notifyLang),
      sendPlainEmail: sendEmail,
      buildFallback: () => ({
        subject: notifyEmailFallbackCopy('emailFb_listingRejectedSubj', notifyLang, { title }),
        body: [
          notifyEmailFallbackCopy('emailFb_listingRejectedLead', notifyLang, { title, brand }),
          '',
          notifyEmailFallbackCopy('emailFb_listingRejectedReason', notifyLang, { reason: reasonText }),
          '',
          notifyEmailFallbackCopy('emailFb_listingRejectedHint', notifyLang),
          '',
          emailFallbackSignoff(notifyLang),
        ].join('\n'),
      }),
  ...
  if (partner?.telegram_id) {
    const tgText =
      notifyLang === 'en'
        ? `❌ <b>Listing rejected</b> · ${brand}\n\n📍 ${title}\n\n📝 Reason: ${reasonText}`
        : notifyLang === 'zh'
          ? `❌ <b>上架未通过</b> · ${brand}\n\n📍 ${title}\n\n📝 原因：${reasonText}`
          : notifyLang === 'th'
            ? `❌ <b>รายการไม่ผ่าน</b> · ${brand}\n\n📍 ${title}\n\n📝 เหตุผล: ${reasonText}`
            : `❌ <b>Объявление отклонено</b> · ${brand}\n\n📍 ${title}\n\n📝 Причина: ${reasonText}`
    await sendTelegram(partner.telegram_id, tgText)
  }
}
...
export async function runDailyDraftDigestReminders() {
  const { sendEmail, sendTelegram, sendToAdminTopic, calculateNights, buildGuestChatUrlForBooking, resolveGuestEmailLang, sendTelegramBookingRequest } =
    ...
    for (const profile of partnerProfiles) {
      ...
      // Stage 200.74 — per-partner via registry (outbox-aware when NOTIFICATION_OUTBOX=1)
      await NotificationService.dispatch('DRAFT_DIGEST_REMINDER', {
...
/** Stage 135 — D+1 / D+3 nudge for verified partners without published listings. */
export async function handlePartnerHostOnboardingNudge(data) {
  const { sendEmail, sendTelegram } = getNotifyDeps()
  const userId = String(data?.userId || '').trim()
  const dayOffset = Number(data?.dayOffset)
  if (!userId || (dayOffset !== 1 && dayOffset !== 3)) return

  const { data: profile, error } = await supabaseAdmin
    .from('profiles')
    .select('email,first_name,telegram_id,language,preferred_language,metadata')
    .eq('id', userId)
    .maybeSingle()
  if (error || !profile?.email) return

  const lang = String(profile.preferred_language || profile.language || 'ru').toLowerCase().startsWith('en')
    ? 'en'
    : 'ru'
  const site = getSiteDisplayName()
  const wizardUrl = buildLocalizedSiteUrl(lang, '/partner/listings/new')
  const dashUrl = buildLocalizedSiteUrl(lang, '/partner/dashboard')

  const headlineRu =
    dayOffset === 1
      ? 'Как создать первое объявление за 10 минут — мастер уже ждёт в кабинете'
      : 'Напоминание: опубликуйте первое объявление и активируйте реферальный бонус хоста'
  const headlineEn =
    dayOffset === 1
      ? 'Create your first listing in ~10 minutes — the wizard is ready in your dashboard'
      : 'Reminder: publish your first listing and unlock the host referral bonus'

  const subject =
    lang === 'en'
      ? dayOffset === 1
        ? `Your first listing in 10 minutes — ${site}`
        : `Publish your first listing — ${site}`
      : dayOffset === 1
        ? `Первое объявление за 10 минут — ${site}`
        : `Опубликуйте первое объявление — ${site}`

  const textBody =
    lang === 'en'
      ? `Hello${profile.first_name ? `, ${profile.first_name}` : ''}!\n\n${headlineEn}\n\n` +
        `• Open listing wizard: ${wizardUrl}\n` +
        `• Partner dashboard (referral welcome strip): ${dashUrl}\n\n` +
        `Team ${site}`
      : `Здравствуйте${profile.first_name ? `, ${profile.first_name}` : ''}!\n\n${headlineRu}\n\n` +
        `• Мастер объявления: ${wizardUrl}\n` +
        `• Кабинет партнёра (полоска «Вас пригласили»): ${dashUrl}\n\n` +
        `Команда ${site}`

  await sendEmail(profile.email, subject, textBody)

  if (profile.telegram_id) {
    const btn = lang === 'en' ? '📝 Open wizard' : '📝 Открыть мастер'
    await Tg.sendTelegramMessagePayload({
      ...
      text:
        lang === 'en'
          ...
          : `🏠 <b>Старт хоста</b>\n\n${headlineRu}`,
      reply_markup: {
        inline_keyboard: [[{ text: btn, url: wizardUrl }]],
      },
    })
  }

  const { PushService } = await import('@/lib/services/push.service.js')
  await PushService.sendToUser(userId, 'PARTNER_HOST_ONBOARDING_NUDGE', {
    dayOffset: String(dayOffset),
    headline: lang === 'en' ? headlineEn : headlineRu,
    link: '/partner/listings/new',
    siteName: site,
  })
}

const HOST_RETENTION_META_D1 = 'host_retention_d1_sent_at'
const HOST_RETENTION_META_D3 = 'host_retention_d3_sent_at'
const WEEKLY_DIGEST_META = 'referral_weekly_digest_sent_at'

function partnerCreatedInWindow(createdAtIso, minHours, maxHours) {
  const createdMs = Date.parse(String(createdAtIso || ''))
  if (!Number.isFinite(createdMs)) return false
  const ageMs = Date.now() - createdMs
  return ageMs >= minHours * 3600000 && ageMs < maxHours * 3600000
}

/** Stage 135 — cron: D+1 (24–48h) and D+3 (72–96h) for partners without ACTIVE listings. */
export async function runPartnerHostOnboardingRetention() {
  if (!supabaseAdmin) return { sent: 0, scanned: 0, error: 'no_db' }

  const { data: partners, error } = await supabaseAdmin
    .from('profiles')
    .select('id,created_at,metadata,role,verification_status')
    .eq('role', 'PARTNER')
    .eq('verification_status', 'VERIFIED')
  if (error) throw error

  const partnerIds = (partners || []).map((p) => p.id).filter(Boolean)
  if (!partnerIds.length) return { sent: 0, scanned: 0 }
  ...
  const hasActive = new Set((activeListings || []).map((r) => r.owner_id))

  let sent = 0
  for (const p of partners || []) {
    if (hasActive.has(p.id)) continue
    const meta = p.metadata && typeof p.metadata === 'object' ? p.metadata : {}
    let dayOffset = null
    if (!meta[HOST_RETENTION_META_D1] && partnerCreatedInWindow(p.created_at, 24, 48)) {
      dayOffset = 1
    } else if (!meta[HOST_RETENTION_META_D3] && partnerCreatedInWindow(p.created_at, 72, 96)) {
      dayOffset = 3
    }
    if (!dayOffset) continue

    const { NotificationService, NotificationEvents } = await import(
      '@/lib/services/notification.service.js'
    )
    await NotificationService.dispatch(NotificationEvents.PARTNER_HOST_ONBOARDING_NUDGE, {
      userId: p.id,
      dayOffset,
    })

    const stampKey = dayOffset === 1 ? HOST_RETENTION_META_D1 : HOST_RETENTION_META_D3
    await supabaseAdmin
      .from('profiles')
      .update({ metadata: { ...meta, [stampKey]: new Date().toISOString() } })
      .eq('id', p.id)
    sent += 1
  }

  return { sent, scanned: partners.length }
}

/** Stage 135 — cron: Monday weekly digest for referrers with team earnings > 0. */
export async function runReferralTeamWeeklyDigest() {
  if (!supabaseAdmin) return { sent: 0, error: 'no_db' }

  const periodEnd = new Date()
  periodEnd.setUTCHours(0, 0, 0, 0)
  const periodStart = new Date(periodEnd.getTime() - 7 * 24 * 3600000)
  ...
  const { data: referrers, error } = await supabaseAdmin
    ...
    .select('referrer_id')
  if (error) throw error
  const uniqueReferrers = [...new Set((referrers || []).map((r) => r.referrer_id).filter(Boolean))]
  if (!uniqueReferrers.length) return { sent: 0, referrers: 0 }

  const { notifyReferralTeamWeeklyDigest } = await import(
  ...
  for (const referrerId of uniqueReferrers) {
    ...
    if (meta[WEEKLY_DIGEST_META] === weekKey) continue
    ...
    const { data: rows, error: rpcErr } = await supabaseAdmin.rpc(
      'referral_team_analytics_for_referrer',
      {
        ...
        p_period_end_exclusive: periodEnd.toISOString(),
      },
    )
    if (rpcErr) continue
    const row = Array.isArray(rows) ? rows[0] : rows
    const l1Thb = Number(row?.l1_direct_thb) || 0
    const l2Thb = Number(row?.l2_network_thb) || 0
    const totalThb = Math.round((l1Thb + l2Thb) * 100) / 100
    if (totalThb <= 0) continue

    await notifyReferralTeamWeeklyDigest({ userId: referrerId, totalThb, l1Thb, l2Thb })
    await supabaseAdmin
      .from('profiles')
      .update({ metadata: { ...meta, [WEEKLY_DIGEST_META]: weekKey } })
      .eq('id', referrerId)
    sent += 1
  }

  return { sent, referrers: uniqueReferrers.length, weekKey }
}
