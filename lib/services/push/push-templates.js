/**
 * Push notification copy — templates, i18n, interpolation (Stage 70.6).
 * Canonical templates live here; `notification-templates.js` re-exports for backward compatibility.
 */

import { getPublicSiteUrl, getSiteDisplayName } from '@/lib/site-url.js'
import { interpolatePushTemplate } from '@/lib/services/push/push-interpolate.js'

export function normalizePushUiLang(raw) {
  const s = String(raw || 'ru').toLowerCase().replace(/_/g, '-')
  if (s.startsWith('zh')) return 'zh'
  if (s.startsWith('th')) return 'th'
  if (s.startsWith('en')) return 'en'
  return 'ru'
}

export function pickLocalizedTemplateStrings(template, lang) {
  const L = normalizePushUiLang(lang)
  if (L === 'en') {
    return { title: template.titleEn ?? template.title, body: template.bodyEn ?? template.body }
  }
  if (L === 'zh') {
    return {
      title: template.titleZh ?? template.titleEn ?? template.title,
      body: template.bodyZh ?? template.bodyEn ?? template.body,
    }
  }
  if (L === 'th') {
    return {
      title: template.titleTh ?? template.titleEn ?? template.title,
      body: template.bodyTh ?? template.bodyEn ?? template.body,
    }
  }
  return { title: template.title, body: template.body }
}

/** @type {Record<string, object>} */
export const NOTIFICATION_TEMPLATES = {
  NEW_MESSAGE: {
    title: '{sender}',
    titleEn: '{sender}',
    body: '{message}',
    bodyEn: '{message}',
    icon: '/icons/icon-192x192.png',
    badge: '/icons/badge-72x72.png',
    tag: 'message',
    requireInteraction: false,
  },
  BOOKING_REQUEST: {
    title: 'Новый запрос на бронирование',
    titleEn: 'New booking request',
    titleZh: '新的预订请求',
    titleTh: 'คำขอจองใหม่',
    body: '{listing} · {dates} · {guest}',
    bodyEn: '{listing} · {dates} · {guest}',
    bodyZh: '{listing} · {dates} · {guest}',
    bodyTh: '{listing} · {dates} · {guest}',
    icon: '/icons/icon-192x192.png',
    tag: 'booking',
    requireInteraction: true,
  },
  BOOKING_INSTANT_PARTNER: {
    title: '✅ Новое подтверждённое бронирование',
    titleEn: '✅ New confirmed booking',
    titleZh: '✅ 新的已确认预订',
    titleTh: '✅ การจองที่ยืนยันแล้ว',
    body: '{listing} · {dates} · {guest}',
    bodyEn: '{listing} · {dates} · {guest}',
    bodyZh: '{listing} · {dates} · {guest}',
    bodyTh: '{listing} · {dates} · {guest}',
    icon: '/icons/icon-192x192.png',
    tag: 'booking',
    requireInteraction: true,
  },
  /** Wave H5 — paid escrow; partner must prepare / confirm check-in */
  PAYMENT_COMPLETED: {
    title: 'Срочно: подтвердите заезд',
    titleEn: 'Urgent: confirm check-in',
    titleZh: '紧急：请确认入住',
    titleTh: 'ด่วน: ยืนยันเช็คอิน',
    body: '{listing} · {dates} · {guest}',
    bodyEn: '{listing} · {dates} · {guest}',
    bodyZh: '{listing} · {dates} · {guest}',
    bodyTh: '{listing} · {dates} · {guest}',
    icon: '/icons/icon-192x192.png',
    tag: 'payment',
    requireInteraction: true,
  },
  /** Wave H5 — guest (or system) cancel; partner alert */
  CANCEL_REQUESTED: {
    title: 'Запрос на отмену бронирования',
    titleEn: 'Booking cancel requested',
    titleZh: '取消预订请求',
    titleTh: 'คำขอยกเลิกการจอง',
    body: '{listing} · {dates} · {guest}',
    bodyEn: '{listing} · {dates} · {guest}',
    bodyZh: '{listing} · {dates} · {guest}',
    bodyTh: '{listing} · {dates} · {guest}',
    icon: '/icons/icon-192x192.png',
    tag: 'booking_cancel',
    requireInteraction: true,
  },
  /** Wave H5 — chat reply SLA >30m (cron partner-sla-telegram-nudge) */
  SLA_EXPIRING_SOON: {
    title: 'SLA под угрозой',
    titleEn: 'SLA at risk',
    titleZh: 'SLA 即将超时',
    titleTh: 'SLA ใกล้หมดเวลา',
    body: 'Гость ждёт ответа более 30 минут · {listing} · {guest}',
    bodyEn: 'Guest waiting over 30 minutes · {listing} · {guest}',
    bodyZh: '客人等待回复已超过 30 分钟 · {listing} · {guest}',
    bodyTh: 'แขกรอตอบมากกว่า 30 นาที · {listing} · {guest}',
    icon: '/icons/icon-192x192.png',
    tag: 'partner_sla',
    requireInteraction: true,
  },
  FUNDS_THAWED_PARTNER: {
    title: '🔓 Эскроу разморожен',
    titleEn: '🔓 Escrow released',
    titleZh: '🔓 托管已解冻',
    titleTh: '🔓 ปลดเอสโครว์แล้ว',
    body: '«{listing}»: ฿{amount} — 24 ч ожидания перед выводом. · {siteName}',
    bodyEn: '«{listing}»: ฿{amount} — 24h security hold before withdrawal. · {siteName}',
    bodyZh: '「{listing}」：฿{amount} — 提现前 24 小时安全等待。 · {siteName}',
    bodyTh: '«{listing}»: ฿{amount} — รอความปลอดภัย 24 ชม. ก่อนถอน · {siteName}',
    icon: '/icons/icon-192x192.png',
    tag: 'payout',
    requireInteraction: true,
  },
  BOOKING_CONFIRMED: {
    title: '✅ Бронирование подтверждено',
    titleEn: '✅ Booking Confirmed',
    titleZh: '✅ 预订已确认',
    titleTh: '✅ การจองได้รับการยืนยัน',
    body: '«{listing}» подтверждено — перейдите к оплате',
    bodyEn: '«{listing}» is confirmed — continue to payment',
    bodyZh: '「{listing}」已确认 — 请前往付款',
    bodyTh: '«{listing}» ยืนยันแล้ว — ไปชำระเงิน',
    icon: '/icons/icon-192x192.png',
    tag: 'booking',
    requireInteraction: true,
  },
  PAYMENT_RECEIVED: {
    title: '💰 Платёж получен',
    titleEn: '💰 Payment Received',
    body: 'Получен платёж ฿{amount} за {listing}',
    bodyEn: 'Payment received ฿{amount} for {listing}',
    icon: '/icons/icon-192x192.png',
    tag: 'payment',
  },
  /** Wave H1 — soft abandonment nudge while AWAITING_PAYMENT hold is active */
  CHECKOUT_ABANDONED: {
    title: 'Бронирование ждет подтверждения',
    titleEn: 'Your stay is reserved',
    titleZh: '预订等待确认',
    titleTh: 'การจองรอการยืนยัน',
    body: 'Завершите оплату, чтобы зафиксировать даты: {listing}',
    bodyEn: 'Complete payment to lock your dates: {listing}',
    bodyZh: '完成付款以锁定日期：{listing}',
    bodyTh: 'ชำระเงินเพื่อล็อกวันที่: {listing}',
    icon: '/icons/icon-192x192.png',
    tag: 'checkout_abandon',
    requireInteraction: true,
  },
  PAYMENT_ESCROW_GUEST: {
    title: '✅ Оплата получена',
    titleEn: '✅ Payment received',
    titleZh: '✅ 付款成功',
    titleTh: '✅ ชำระเงินสำเร็จ',
    body: 'Средства защищены эскроу {siteName}. Напишите {providerDative} в чате · {listing}',
    bodyEn: 'Funds protected by {siteName} escrow. Message {providerDative} · {listing}',
    bodyZh: '资金由 {siteName} 托管保护。联系{providerDative} · {listing}',
    bodyTh: 'เงินใน escrow {siteName}. ติดต่อ{providerDative} · {listing}',
    icon: '/icons/icon-192x192.png',
    tag: 'payment_guest',
  },
  PAYMENT_PENDING_PARTNER: {
    title: '⏳ Оплата на проверке',
    titleEn: '⏳ Payment under verification',
    body: 'Платёж ฿{amount} за {listing} проходит верификацию',
    bodyEn: 'Payment ฿{amount} for {listing} is being verified',
    icon: '/icons/icon-192x192.png',
    tag: 'payment',
  },
  CHECKIN_REMINDER: {
    title: '🔑 Подтвердите бронирование',
    titleEn: '🔑 Confirm your booking',
    body: 'Добро пожаловать! Подтвердите начало бронирования «{listing}»',
    bodyEn: 'Welcome! Please confirm your booking for "{listing}"',
    icon: '/icons/icon-192x192.png',
    tag: 'checkin',
    requireInteraction: true,
    actions: [
      { action: 'confirm', title: 'Подтвердить' },
      { action: 'help', title: 'Нужна помощь' },
    ],
  },
  REVIEW_REMINDER: {
    title: '⭐ Как прошло бронирование?',
    titleEn: '⭐ How was your booking?',
    body: 'Оставьте отзыв о «{listing}» — это помогает другим пользователям.',
    bodyEn: 'Leave a review for "{listing}" — it helps other travelers.',
    icon: '/icons/icon-192x192.png',
    tag: 'review',
    requireInteraction: false,
  },
  PAYOUT_READY: {
    title: '💸 Выплата готова',
    titleEn: '💸 Payout Ready',
    body: 'Ваши средства ฿{amount} разморожены и готовы к выплате',
    bodyEn: 'Your funds ฿{amount} are thawed and ready for payout',
    icon: '/icons/icon-192x192.png',
    tag: 'payout',
  },
  PARTNER_GUEST_REVIEW: {
    title: '⭐ Отзыв о клиенте',
    titleEn: '⭐ Review your client',
    titleZh: '⭐ 请评价客户',
    titleTh: '⭐ ให้คะแนนลูกค้า',
    body: '«{listing}» завершено. Клиент: {client}. · {siteName}',
    bodyEn: '"{listing}" has ended. Client: {client}. · {siteName}',
    bodyZh: '「{listing}」已结束。客户：{client}。· {siteName}',
    bodyTh: '«{listing}» สิ้นสุดแล้ว ลูกค้า: {client} · {siteName}',
    icon: '/icons/icon-192x192.png',
    tag: 'guest_review',
    requireInteraction: false,
  },
  SUPPORT_REQUESTED: {
    title: '🆘 Нужна помощь в чате',
    titleEn: '🆘 Support needed',
    body: 'Запрос поддержки в диалоге {conversationId}',
    bodyEn: 'Support requested in conversation {conversationId}',
    icon: '/icons/icon-192x192.png',
    tag: 'support_escalation',
    requireInteraction: true,
  },
  DISPUTE_OPENED: {
    title: '⚖️ Открыт спор по бронированию',
    titleEn: '⚖️ Dispute opened for booking',
    body: 'По вашему заказу открыт спор. Оплата приостановлена до выяснения обстоятельств.',
    bodyEn: 'A dispute has been opened for your booking. Payment is paused while we investigate.',
    icon: '/icons/icon-192x192.png',
    tag: 'booking_dispute',
    requireInteraction: true,
  },
  DISPUTE_RESOLVED: {
    title: '✅ Спор по заказу {bookingId} закрыт',
    titleEn: '✅ Dispute closed — booking {bookingId}',
    body: 'Поддержка зафиксировала решение: {summary}. Откройте приложение для деталей.',
    bodyEn: 'Support recorded a resolution: {summary}. Open the app for details.',
    icon: '/icons/icon-192x192.png',
    tag: 'booking_dispute_resolved',
    requireInteraction: true,
  },
  DISPUTE_SLA_REMINDER: {
    title: '⏳ Спор {bookingId}: дедлайн через {hoursLeft}ч',
    titleEn: '⏳ Dispute {bookingId}: deadline in {hoursLeft}h',
    body: '{summary}',
    bodyEn: '{summary}',
    icon: '/icons/icon-192x192.png',
    tag: 'booking_dispute_sla_reminder',
    requireInteraction: true,
  },
  DISPUTE_AUTO_RESOLVED: {
    title: '⚖️ Спор {bookingId} завершён по SLA',
    titleEn: '⚖️ Dispute {bookingId} resolved by SLA',
    body: '{summary}',
    bodyEn: '{summary}',
    icon: '/icons/icon-192x192.png',
    tag: 'booking_dispute_sla_auto_resolved',
    requireInteraction: true,
  },
  PARTNER_GUEST_HELP_NUDGE: {
    title: 'Клиент запрашивает помощь',
    titleEn: 'Client needs help',
    body: 'Решите вопрос в чате сейчас, чтобы избежать официального спора. {listing}',
    bodyEn: 'Resolve in chat now to avoid an official dispute. {listing}',
    icon: '/icons/icon-192x192.png',
    tag: 'guest_help_nudge',
    requireInteraction: true,
  },
  RENTER_EMERGENCY_CONTACT: {
    title: '🚨 Экстренная связь от арендатора',
    titleEn: '🚨 Emergency contact from renter',
    body: 'Арендатор нажал «Экстренная связь» по «{listing}». Проверьте чат и заказ.',
    bodyEn: 'Renter used Emergency contact for «{listing}». Check chat and booking.',
    icon: '/icons/icon-192x192.png',
    tag: 'emergency_partner_contact',
    requireInteraction: true,
  },
  REFERRAL_BONUS_EARNED: {
    title: '🎁 Реферальный бонус',
    titleEn: '🎁 Referral reward',
    titleTh: '🎁 โบนัสแนะนำ',
    body: '{headline}',
    bodyEn: '{headline}',
    bodyTh: '{headline}',
    icon: '/icons/icon-192x192.png',
    tag: 'referral_bonus',
    requireInteraction: true,
  },
  REFERRAL_BONUS_HELD: {
    title: '⏳ Бонус на проверке',
    titleEn: '⏳ Bonus on hold',
    titleTh: '⏳ โบนัสรอตรวจสอบ',
    body: '{headline}',
    bodyEn: '{headline}',
    bodyTh: '{headline}',
    icon: '/icons/icon-192x192.png',
    tag: 'referral_bonus_held',
    requireInteraction: true,
  },
  REFERRAL_TEAMMATE_JOINED: {
    title: '👥 Новый в команде',
    titleEn: '👥 New teammate',
    titleTh: '👥 สมาชิกใหม่ในทีม',
    body: '{displayName} присоединился по вашей ссылке. · {siteName}',
    bodyEn: '{displayName} joined via your referral link. · {siteName}',
    bodyTh: '{displayName} เข้าร่วมทีมผ่านลิงก์ของคุณ · {siteName}',
    icon: '/icons/icon-192x192.png',
    tag: 'referral_team',
    requireInteraction: true,
  },
  REFERRAL_WITHDRAWAL_APPROVED: {
    title: '✅ Вывод одобрен',
    titleEn: '✅ Withdrawal approved',
    titleTh: '✅ Withdrawal approved',
    body: '{headline}',
    bodyEn: '{headline}',
    bodyTh: '{headline}',
    icon: '/icons/icon-192x192.png',
    tag: 'referral_withdrawal',
    requireInteraction: true,
  },
  REFERRAL_WITHDRAWAL_REGISTRY_SENT: {
    title: '🏦 Реестр в банке',
    titleEn: '🏦 Registry at bank',
    titleTh: '🏦 Registry at bank',
    body: '{headline}',
    bodyEn: '{headline}',
    bodyTh: '{headline}',
    icon: '/icons/icon-192x192.png',
    tag: 'referral_withdrawal',
    requireInteraction: true,
  },
  REFERRAL_WITHDRAWAL_PAID: {
    title: '💸 Выплата выполнена',
    titleEn: '💸 Payout completed',
    titleTh: '💸 Payout completed',
    body: '{headline}',
    bodyEn: '{headline}',
    bodyTh: '{headline}',
    icon: '/icons/icon-192x192.png',
    tag: 'referral_withdrawal',
    requireInteraction: true,
  },
  REFERRAL_WITHDRAWAL_REJECTED: {
    title: '❌ Вывод отклонён',
    titleEn: '❌ Withdrawal rejected',
    titleTh: '❌ Withdrawal rejected',
    body: '{headline}',
    bodyEn: '{headline}',
    bodyTh: '{headline}',
    icon: '/icons/icon-192x192.png',
    tag: 'referral_withdrawal',
    requireInteraction: true,
  },
  REFERRAL_WITHDRAWAL_EXPIRED: {
    title: '⏱ Курс истёк',
    titleEn: '⏱ FX lock expired',
    titleTh: '⏱ FX lock expired',
    body: '{headline}',
    bodyEn: '{headline}',
    bodyTh: '{headline}',
    icon: '/icons/icon-192x192.png',
    tag: 'referral_withdrawal',
    requireInteraction: true,
  },
  REFERRAL_TEAM_WEEKLY_DIGEST: {
    title: '📊 Итоги недели',
    titleEn: '📊 Weekly team digest',
    titleTh: '📊 Weekly team digest',
    body: '{headline}',
    bodyEn: '{headline}',
    bodyTh: '{headline}',
    icon: '/icons/icon-192x192.png',
    tag: 'referral_team_digest',
    requireInteraction: false,
  },
  REFERRAL_FRIEND_BOOKED: {
    title: '📅 Друг забронировал',
    titleEn: '📅 Friend booked',
    titleZh: '📅 好友已预订',
    titleTh: '📅 เพื่อนจองแล้ว',
    body: '{headline}',
    bodyEn: '{headline}',
    bodyZh: '{headline}',
    bodyTh: '{headline}',
    icon: '/icons/icon-192x192.png',
    tag: 'referral_friend_booked',
    requireInteraction: true,
  },
  REFERRAL_FRIEND_COMPLETED: {
    title: '✈️ Поездка завершена',
    titleEn: '✈️ Trip completed',
    titleZh: '✈️ 行程已完成',
    titleTh: '✈️ เดินทางเสร็จสิ้น',
    body: '{headline}',
    bodyEn: '{headline}',
    bodyZh: '{headline}',
    bodyTh: '{headline}',
    icon: '/icons/icon-192x192.png',
    tag: 'referral_friend_completed',
    requireInteraction: true,
  },
  REFERRAL_FRIEND_CANCELLED: {
    title: '❌ Бронирование отменено',
    titleEn: '❌ Booking cancelled',
    titleZh: '❌ 预订已取消',
    titleTh: '❌ ยกเลิกการจอง',
    body: '{headline}',
    bodyEn: '{headline}',
    bodyZh: '{headline}',
    bodyTh: '{headline}',
    icon: '/icons/icon-192x192.png',
    tag: 'referral_friend_cancelled',
    requireInteraction: false,
  },
  PARTNER_HOST_ONBOARDING_NUDGE: {
    title: '🏠 Первое объявление',
    titleEn: '🏠 Your first listing',
    titleTh: '🏠 Your first listing',
    body: '{headline}',
    bodyEn: '{headline}',
    bodyTh: '{headline}',
    icon: '/icons/icon-192x192.png',
    tag: 'host_onboarding',
    requireInteraction: false,
  },
}

/** FCM WebPush требует абсолютный URL; относительные пути (/messages/...) дополняем origin из getPublicSiteUrl(). */
export function absolutizePushLink(link) {
  const base = (getPublicSiteUrl() || '').replace(/\/$/, '')
  if (!link || typeof link !== 'string') return `${base}/`
  if (/^https?:\/\//i.test(link.trim())) return link.trim()
  const path = link.startsWith('/') ? link : `/${link}`
  return `${base}${path}`
}

/**
 * @param {string} templateKey
 * @param {Record<string, unknown>} data
 * @param {string} lang
 * @returns {{ ok: true, title: string, body: string, template: object } | { ok: false, error: string }}
 */
export function buildRenderedPushNotification(templateKey, data, lang) {
  const template = NOTIFICATION_TEMPLATES[templateKey]
  if (!template) {
    return { ok: false, error: `Unknown template: ${templateKey}` }
  }

  const batchN = Number(data.messageBatchCount || 0)
  const uiLang = normalizePushUiLang(lang)
  const defaultSender =
    uiLang === 'en'
      ? 'Someone'
      : uiLang === 'zh'
        ? '联系人'
        : uiLang === 'th'
          ? 'ผู้ติดต่อ'
          : 'Собеседник'
  const defaultNewMessageBody =
    uiLang === 'en'
      ? 'New message'
      : uiLang === 'zh'
        ? '新消息'
        : uiLang === 'th'
          ? 'ข้อความใหม่'
          : 'Новое сообщение'
  const siteName = getSiteDisplayName()
  const dataForTemplate =
    templateKey === 'NEW_MESSAGE'
      ? {
          ...data,
          siteName,
          sender: (data.sender && String(data.sender).trim()) || defaultSender,
          message: (() => {
            const m = data.message != null ? String(data.message).trim() : ''
            return m || defaultNewMessageBody
          })(),
        }
      : { ...data, siteName }

  const { title: titleTpl, body: bodyTpl } = pickLocalizedTemplateStrings(template, lang)
  const title = interpolatePushTemplate(titleTpl, dataForTemplate)
  let body
  if (templateKey === 'NEW_MESSAGE' && batchN > 1) {
    const s =
      dataForTemplate.sender ||
      (uiLang === 'en' ? 'someone' : uiLang === 'zh' ? '对方' : uiLang === 'th' ? 'ผู้ติดต่อ' : 'собеседника')
    body =
      uiLang === 'en'
        ? `You have new messages from ${s}`
        : uiLang === 'zh'
          ? `您有多条来自 ${s} 的新消息`
          : uiLang === 'th'
            ? `มีข้อความใหม่หลายข้อความจาก ${s}`
            : `У вас новых сообщений от ${s}`
  } else if (templateKey === 'PAYMENT_ESCROW_GUEST' && data.pushBodyOverride) {
    body = String(data.pushBodyOverride)
  } else {
    body = interpolatePushTemplate(bodyTpl, dataForTemplate)
  }

  return { ok: true, title, body, template }
}

/** Data payload keys for FCM `data` map (string values). */
export function buildPushDataStrings(templateKey, data, silent) {
  const linkAbsolute = absolutizePushLink(data.link || data.url || data.deepLink || '/')
  const dataPayload = {
    ...data,
    link: linkAbsolute,
    url: absolutizePushLink(data.url || data.link || data.deepLink || linkAbsolute),
    deepLink: absolutizePushLink(data.deepLink || data.link || data.url || linkAbsolute),
  }
  if (silent) {
    dataPayload.silent = '1'
  }
  return Object.fromEntries(Object.entries(dataPayload).map(([k, v]) => [k, String(v)]))
}

export function buildSilentBadgeFcmPayload(fcmToken, unreadCount) {
  return {
    message: {
      token: fcmToken,
      data: {
        type: 'badge_update',
        unread_count: String(unreadCount),
      },
      webpush: {
        headers: {
          'Content-Available': '1',
        },
      },
      android: {
        priority: 'normal',
      },
      apns: {
        headers: { 'apns-priority': '5' },
        payload: {
          aps: { 'content-available': 1, badge: unreadCount },
        },
      },
    },
  }
}
