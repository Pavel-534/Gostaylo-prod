...
} from '@/lib/email/listing-moderation-email.js'
...
import {
  ...
  getGuestPaymentEscrowNotifyCopy,
  ...
  welcome: (user, lang = 'en') => {
    ...
    const texts = {
      ru: {
        ...
        body: `Спасибо за регистрацию на ${brand} — платформе аренды жилья, транспорта и туров.`,
        featuresHead: 'Что вас ждёт',
        features: [
          'Жильё, виллы и апартаменты',
      ...
      en: {
        ...
        body: `Thank you for joining ${brand} — your rental marketplace for homes, transport, and tours.`,
      ...
      zh: {
        ...
        body: `感谢您注册 ${brand} — 住房、交通与旅游的租赁平台。`,
        featuresHead: '您可以享受',
        features: ['住房与别墅', '车辆、游艇、行程与服务', '安全支付与客服支持'],
        cta: '登录并开始',
        closing: `此致敬礼,\n${brand} 团队`,
        loginPath: '/login',
      },
      th: {
        ...
        body: `ขอบคุณที่เข้าร่วม ${brand} — แพลตฟอร์มเช่าที่พัก ยานพาหนะ และทัวร์`,
        featuresHead: 'สิ่งที่รอคุณ',
        features: ['ที่พักและวิลล่า', 'รถ เรือ ทัวร์ และบริการ', 'การชำระเงินที่ปลอดภัย'],
        cta: 'เข้าสู่ระบบ',
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid ${colors.border};border-radius:${borderRadius};overflow:hidden;${theme.shadows.soft}">
  ...
  <tr><td style="padding:18px 20px;border-bottom:1px solid ${colors.divider};">
    ...
    <p style="margin:6px 0 0;font-size:20px;font-weight:700;color:${colors.primary};">${escapeHtml(String(booking.priceLine || booking.totalPrice?.toLocaleString?.() || booking.totalPrice || '0'))}</p>
  </td></tr>
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid ${colors.border};border-radius:${borderRadius};overflow:hidden;${theme.shadows.soft}">
  ...
  <tr><td style="padding:18px 20px;">
    ...
    <p style="margin:6px 0 0;font-size:22px;font-weight:700;color:${colors.primary};">${escapeHtml(String(booking.ledgerPriceLine || booking.priceLine || booking.totalPrice?.toLocaleString?.() || booking.totalPrice || '0'))}</p>
  </td></tr>
</table>`
    const bodyRows =
      emailTitleRow(t.title) +
  ...
  /** Эскроу THAWED: 24h hold before withdraw (Stage 149). */
  partnerFundsThawed: (payload, lang = 'en') => {
    ...
    const releaseConfig = getPayoutReleaseConfig({
      ...
      wizardProfile: payload?.wizardProfile,
    })
    const thawCopy = formatPayoutNotificationCopy(releaseConfig, L, 'thaw', {
      amount,
    ...
    const texts = {
      ru: {
        subject: `${thawCopy.title} — ฿${amount} · ${brand}`,
        preheader: `Эскроу разморожен — ${PAYOUT_HOLD_HOURS} ч до вывода`,
        title: thawCopy.title,
        body: `${thawCopy.body} Net по брони: ฿${amount}. Сумма появится в «Доступно к выводу» после ${PAYOUT_HOLD_HOURS}-часового ожидания.`,
        cta: 'Открыть финансы',
        note: `Это не моментальная выплата на реквизиты — сначала период безопасности ${PAYOUT_HOLD_HOURS} ч, затем пул пн/чт.`,
      },
      en: {
        subject: `${thawCopy.title} — ฿${amount} · ${brand}`,
        preheader: `Escrow released — ${PAYOUT_HOLD_HOURS}h hold before withdrawal`,
        title: thawCopy.title,
        body: `${thawCopy.body} Booking net: ฿${amount}. It moves to “Available to withdraw” after the ${PAYOUT_HOLD_HOURS}-hour hold.`,
        cta: 'Open Finances',
        note: `This is not an instant bank transfer — ${PAYOUT_HOLD_HOURS}h security hold, then Mon/Thu pools.`,
      },
      zh: {
        subject: `${thawCopy.title} — ฿${amount} · ${brand}`,
        preheader: `托管已解冻 — 提现前 ${PAYOUT_HOLD_HOURS} 小时`,
        title: thawCopy.title,
        body: `${thawCopy.body} 本单净额 ฿${amount}。${PAYOUT_HOLD_HOURS} 小时等待后进入「可提现」。`,
        cta: '打开财务',
        note: `非即时到账 — ${PAYOUT_HOLD_HOURS} 小时安全等待，随后周一/周四池。`,
      },
      th: {
        subject: `${thawCopy.title} — ฿${amount} · ${brand}`,
        preheader: `ปลดเอสโครว์แล้ว — รอ ${PAYOUT_HOLD_HOURS} ชม. ก่อนถอน`,
        title: thawCopy.title,
        body: `${thawCopy.body} Net การจอง ฿${amount} จะเข้า «พร้อมถอน» หลังรอ ${PAYOUT_HOLD_HOURS} ชม.`,
        cta: 'เปิดการเงิน',
        note: `ไม่ใช่โอนทันที — รอความปลอดภัย ${PAYOUT_HOLD_HOURS} ชม. แล้วคิวจ.-พ.`,
      },
    }
  ...
  paymentSuccessGuest: (payload, lang = 'ru') => {
    ...
    const vertical = getGuestPaymentEscrowNotifyCopy({
      categorySlug: payload.listingCategorySlug,
      wizardProfile: payload.wizardProfile,
      language: L,
    })
    ...
    const lead =
      payload.leadLine ||
      formatGuestPaymentEscrowLine('lead', {
        guestName: guest,
        categorySlug: payload.listingCategorySlug,
        wizardProfile: payload.wizardProfile,
        language: L,
    ...
    const escrowText =
      payload.escrowText ||
      formatGuestPaymentEscrowLine('escrow', {
        categorySlug: payload.listingCategorySlug,
        wizardProfile: payload.wizardProfile,
        language: L,
      }) ||
      copy.escrowDefault
    const chatLabel = payload.chatLabel || vertical.chatLabel || ({ ru: 'Открыть чат', en: 'Open chat', zh: '打开聊天', th: 'เปิดแชท' }[L] || 'Open chat')
    const methodRow =
      payload.methodLine ?
    ...
    const bodyRows =
      ...
      emailCtaStack({
        ...
        secondary: [
          { href: payload.chatUrl, label: chatLabel },
          { href: payload.profileUrl, label: copy.profile },
        ],
      })
...
async function sendEmail(to, template) {
  ...
  if (shouldMockResendDelivery(to)) {
    return mockResendDeliveryResult(to, template?.subject, 'premium_transport_guard')
  }
...
export const EmailService = {
  ...
  async sendListingApproved(listing, partnerEmail, lang = 'ru') {
    const template = buildListingApprovedEmailTemplate(listing, lang)
    return sendEmail(partnerEmail, template)
  },

  async sendListingRejected(listing, partnerEmail, reason, lang = 'ru') {
    const template = buildListingRejectedEmailTemplate(listing, reason, lang)
    return sendEmail(partnerEmail, template)
  },
