  ...
  static async getPartnerBalance(partnerId) {
    try {
      ...
      const { data: bookings } = await supabaseAdmin
        .from('bookings')
        .select('price_thb, commission_thb, net_amount_thb, commission_rate, status')
      ...
      (bookings || []).forEach(b => {
        ...
        const comm = parseFloat(b.commission_thb) || price * ratio;
        const net = parseFloat(b.net_amount_thb) || price - comm;
