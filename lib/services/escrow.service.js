 ...
 *
 * Money flow (prod):
 * - moveToEscrow (PAID_ESCROW) → schedulePostPaymentLedgerCapture → LedgerService.postPaymentCaptureFromBooking
 * - thaw cron → THAWED → promote-ready-for-payout → READY_FOR_PAYOUT
 * - PayoutBatchService.markBatchSettled → LedgerService.postPartnerBatchBookingPayoutSettled (not legacy processPayout)
 *
 * Thaw: `lib/services/escrow/thaw.service.js`
 * Legacy payout: `lib/services/escrow/payout.service.js` (blocked by legacy-payout-guard on prod)
