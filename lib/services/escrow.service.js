...
export class EscrowService {
  ...
  static async moveToEscrow(bookingId, paymentData = {}) {
    try {
      ...
      let commission;
      let netAmount;
      /** Guest-facing capture for notifications when paying by intent/invoice */
  ...
  /**
   * Persist profiles.frozen_balance_thb / available_balance_thb from booking sums.
   */
  static async syncPartnerBalanceColumns(partnerId) {
    if (!partnerId) return { success: false, error: 'missing partner' };
    const { success, balance } = await this.getPartnerBalance(partnerId);
  ...
  /**
   * PAID_ESCROW → THAWED when now >= escrow_thaw_at (category rules).
   */
  static async thawBookingToThawed(bookingId) {
    const { data: booking, error: fetchError } = await supabaseAdmin
      .from('bookings')
      .select(
        `
  ...
  static async backfillMissingEscrowThawAt(limit = 300) {
    ...
    for (const b of rows) {
      ...
      const escrowAt =
        (b.metadata && b.metadata.escrow_started) || new Date().toISOString();
  ...
  /**
   * Cron: thaw bookings whose escrow_thaw_at has passed.
   */
  static async processDueEscrowThaws() {
    await this.backfillMissingEscrowThawAt(500);
    ...
    for (const item of due) {
      const r = await this.thawBookingToThawed(item.id);
      if (r.success && item.partnerId) {
        partners.add(item.partnerId);
      }
    }
    for (const pid of partners) {
      await this.syncPartnerBalanceColumns(pid);
    }
    return { success: true, processed: due.length };
  }

  /**
   * Partner balance split by listing category (THB net).
   */
  static async getPartnerBalanceByCategory(partnerId) {
    const fallbackPct = await resolveDefaultCommissionPercent();
    const fallbackRatio = fallbackPct / 100;
