...
import {
  ...
  alignLedgerLegsToEscrowRpcCaptureSsot,
...
import { ESCROW_THAW_SOURCE_STATUSES } from '@/lib/booking/status-sets.js'
...
function emitEscrowRpcFailedSignal(bookingId, error, source = 'moveToEscrow') {
  recordCriticalSignal('ESCROW_RPC_FAILED', {
    severity: 'CRITICAL',
    tag: '[FINANCE]',
    threshold: 1,
    windowMs: 60 * 60 * 1000,
    detailLines: [
      `[ESCROW_RPC_FAILED] moveToEscrow failed`,
...
export class EscrowService {
  ...
  static async moveToEscrow(bookingId, paymentData = {}) {
    try {
      ...
      if (Number.isFinite(captureGuestTotalThb) && captureGuestTotalThb > 0) {
        const legs0 = alignLedgerLegsToEscrowRpcCaptureSsot(computeBookingPaymentLedgerLegs(booking))
      ...
      const escrowThawAt = computeEscrowThawAt({
        ...
        listingId: booking.listing_id || booking.listing?.id || null,
      })
      ...
      if (!wasPaidEscrowBeforeRpc) {
        ...
        if (rpcError) {
          ...
          emitEscrowRpcFailedSignal(bookingId, rpcError.message || 'atomic_escrow_rpc_failed')
          return { success: false, error: rpcError.message || 'atomic_escrow_rpc_failed' }
        }
        rpc = Array.isArray(rpcRows) ? rpcRows[0] : rpcRows
        if (!rpc?.success) {
          emitEscrowRpcFailedSignal(bookingId, rpc?.error_code || 'atomic_escrow_failed')
      ...
      emitEscrowRpcFailedSignal(bookingId, error?.message || error)
  ...
  static async processDueEscrowThaws() {
    ...
    // AUDIT_03 C3.10: cursor over all PAID_ESCROW ordered by id (stable), filter due in JS.
    // Avoids starvation of due rows outside a fixed .limit(800) window.
    for (;;) {
      let q = supabaseAdmin
        .from('bookings')
        .select('id, partner_id, check_in, escrow_thaw_at, metadata, listing:listings(category_id, metadata)')
        .in('status', [...ESCROW_THAW_SOURCE_STATUSES])
      ...
      if (error) {
        return { success: false, error: error.message, processed }
      }
      if (!rows?.length) break

      lastId = rows[rows.length - 1].id
      const frozenByDispute = await DisputeService.getFrozenBookingIdSet(rows.map((b) => b.id))
      // FAIL-CLOSED: freeze lookup down → do not thaw any row in this page (or further).
      if (isFrozenBookingLookupFailed(frozenByDispute)) {
        emitFreezeFailIfNeeded(frozenByDispute, 'escrow-thaw')
        return {
          success: false,
          error: 'freeze_lookup_failed',
          processed,
          message: 'getFrozenBookingIdSet failed — thaw blocked until disputes readable',
        }
      }
      const due = []
      for (const b of rows) {
        ...
        if (!thawIso) {
          ...
          thawIso = computeEscrowThawAt({
            ...
            listingMetadata: b.listing?.metadata,
          })
      ...
      for (const item of due) {
        ...
        if (r.success) {
          processed += 1
          if (item.partnerId) partners.add(item.partnerId)
        }
      }
