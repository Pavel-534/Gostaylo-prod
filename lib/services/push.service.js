...
/** UI язык пуша из `profiles.language` (SSOT для копирайта шаблонов). */
function normalizePushUiLang(raw) {
...
function pickLocalizedTemplateStrings(template, lang) {
...
const NOTIFICATION_TEMPLATES = {
  ...
  /** Партнёр → отзыв о клиенте после завершения брони (check_out), Stage 47.2. */
...
export class PushService {
  ...
  static async sendPush(fcmToken, templateKey, data = {}, lang = 'ru', logContext = {}) {
    ...
    try {
      ...
      const title = this.interpolate(titleTpl, dataForTemplate)
      let body
      if (templateKey === 'NEW_MESSAGE' && batchN > 1) {
        ...
        body = this.interpolate(bodyTpl, dataForTemplate)
      }
