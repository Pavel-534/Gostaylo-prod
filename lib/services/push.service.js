...
/** Временная отладка (Vercel): `FCM_INSTANT_PUSH_DEBUG=1` — все NEW_MESSAGE уходят сразу, без last_seen / 45 с. */
...
export class PushService {
  ...
  static async sendNewMessageWithSmartDelivery(userId, data, lang, legacyToken, quietConfig = null) {
    ...
    if (FCM_INSTANT_PUSH_DEBUG) {
      console.log('[FCM] FCM_INSTANT_PUSH_DEBUG=1 — instant NEW_MESSAGE for all tokens (no last_seen / delayed batch)')
