...
/** Окно «веб-вкладка активна» для Smart Delivery (ping ~30 с); пуш всё равно откладывается — см. Premium Quiet Policy. */
const WEB_ACTIVE_WINDOW_MS = 60_000
/**
 * Premium Quiet Policy (v3): единое окно отложенной доставки NEW_MESSAGE (FCM после паузы).
 * Перед отправкой проверяется `messages.is_read` — прочитано → пуш не шлём.
 * Значение в диапазоне 30–45 с (центр ~40 с).
 */
const PREMIUM_CHAT_PUSH_DELAY_MS = 40_000

/** Временная отладка (Vercel): `FCM_INSTANT_PUSH_DEBUG=1` — все NEW_MESSAGE уходят сразу, без last_seen / задержки. */
const FCM_INSTANT_PUSH_DEBUG = process.env.FCM_INSTANT_PUSH_DEBUG === '1'
...
const NOTIFICATION_TEMPLATES = {
  ...
  REVIEW_REMINDER: {
    ...
    titleEn: '⭐ How was your stay?',
    body: 'Оставьте отзыв о «{listing}» — это помогает другим гостям.',
    bodyEn: 'Leave a review for "{listing}" — it helps other guests.',
    icon: '/icons/icon-192x192.png',
    tag: 'review',
    requireInteraction: false,
  },
  ...
  PARTNER_GUEST_REVIEW: {
    title: '⭐ Оцените гостя',
    titleEn: '⭐ Rate your guest',
    body: 'Средства по «{listing}» доступны. Отзыв о госте: {guest}.',
    bodyEn: 'Funds for "{listing}" are available. Review guest {guest}.',
    icon: '/icons/icon-192x192.png',
    tag: 'guest_review',
    requireInteraction: false,
  },
  ...
  PARTNER_GUEST_HELP_NUDGE: {
    title: 'Гость запрашивает помощь',
    titleEn: 'Guest needs help',
  ...
  RENTER_EMERGENCY_CONTACT: {
    title: '🚨 Экстренная связь от гостя',
    titleEn: '🚨 Emergency contact from guest',
    body: 'Гость нажал «Экстренная связь» по «{listing}». Проверьте чат и заказ.',
    bodyEn: 'Guest used Emergency contact for «{listing}». Check chat and booking.',
    icon: '/icons/icon-192x192.png',
    tag: 'emergency_host_contact',
    requireInteraction: true,
  },
}
...
export class PushService {
  ...
  /**
   * Stage 21.0 — «тихий» FCM для NEW_MESSAGE: то же окно, что fair SLA (**AvailabilityService.resolvePartnerQuietContext**).
   * @param {string} recipientId
   * @param {Array<{ token: string, device_info?: object }>} tokenRows
   * @param {{ bookingId?: string|null, listingId?: string|null, emergencyBypass?: boolean }} [opts]
   */
  static async resolveSilentForPushDelivery(recipientId, _tokenRows, opts = {}) {
  ...
  static async scheduleSimpleDelayedPush(_recipientId, tokensDelayed, data, lang) {
    ...
    await scheduleBackgroundWork(async () => {
      ...
      const silent = await PushService.resolveSilentForPushDelivery(_recipientId, rows, {
  ...
  static async deliverChatBatchSnapshot({ snap, recipientId, senderId, lang, source = 'leader' }) {
    ...
    const silent = await PushService.resolveSilentForPushDelivery(recipientId, tokenRows, {
      bookingId,
      listingId,
  ...
  static async signJwt(header, payload, privateKey) {
    ...
    // PKCS#8 DER (base64): убираем заголовки и любые пробелы/переносы (частая причина «Invalid character» в atob).
  ...
  static async sendPush(fcmToken, templateKey, data = {}, lang = 'ru', logContext = {}) {
    ...
    try {
      ...
      if (silent) {
        ...
        // Трей + data для SW (клик); notification — для системного баннера Android / совместимости.
      ...
      console.log('[FCM Debug] Firebase HTTP:', response.status, response.statusText)
      console.log(
      ...
      if (!response.ok) {
        const { message: errMsg, stale } = PushService.parseFcmHttpError(rawText, response.status)
  ...
  static async sendNewMessageWithSmartDelivery(userId, data, lang, legacyToken) {
    ...
    let fail = 0
    if (instant.length) {
      const silentInstant = await this.resolveSilentForPushDelivery(userId, enriched, {
        bookingId: data.bookingId || null,
        listingId: data.listingId || null,
        emergencyBypass: data.emergencyBypass === true,
      })
  ...
  static async sendSilentBadgeUpdate(fcmToken, unreadCount) {
    ...
    try {
      ...
      if (!res.ok) {
        const { message: errMsg, stale } = PushService.parseFcmHttpError(rawText, res.status)
        if (stale) await PushService.deleteInvalidPushToken(fcmToken)
