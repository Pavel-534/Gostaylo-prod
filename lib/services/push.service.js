...
export class PushService {
  ...
  /** Локальное время получателя 22:00–08:00 — тихий пуш (без звука/вибрации на клиенте). */
  static isRecipientLocalSilentHours(ianaTimeZone) {
  ...
  static resolveSilentForRecipientRows(tokenRows) {
    return this.isRecipientLocalSilentHours(this.pickRecipientTimezone(tokenRows))
  }
  ...
  static async scheduleSimpleDelayedPush(_recipientId, tokensDelayed, data, lang) {
    ...
    await scheduleBackgroundWork(async () => {
      ...
      const silent = PushService.resolveSilentForRecipientRows(rows)
  ...
  static async runChatPushBatchLeader(recipientId, senderId, lang) {
    ...
    if (!stillSend) {
      console.log('[FCM] Batched NEW_MESSAGE skipped (read or missing):', lastId)
      return
    }
    ...
    const silent = PushService.resolveSilentForRecipientRows(tokenRows)
    ...
    await Promise.allSettled(
      pending
        ...
        .map((token) => PushService.sendPush(token, 'NEW_MESSAGE', payload, lang)),
    )
  }
  ...
  static async sendPush(fcmToken, templateKey, data = {}, lang = 'ru') {
    ...
    try {
      ...
      if (templateKey === 'NEW_MESSAGE' && batchN > 1) {
        body =
          lang === 'en'
            ? `You have new messages from ${data.sender || 'someone'}`
            : `У вас новых сообщений от ${data.sender || 'собеседника'}`
      } else {
        body = this.interpolate(lang === 'en' ? template.bodyEn : template.body, data)
      }
  ...
  static async sendNewMessageWithSmartDelivery(userId, data, lang, legacyToken) {
    ...
    const silentInstant = this.resolveSilentForRecipientRows(enriched)
    let ok = 0
    let fail = 0
    if (instant.length) {
      const settled = await Promise.allSettled(
