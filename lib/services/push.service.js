  ...
  static async sendToUser(userId, templateKey, data = {}) {
    try {
      ...
      const { data: profile, error } = await supabaseAdmin
        .from('profiles')
        .select('language, role, quiet_mode_enabled, quiet_hour_start, quiet_hour_end')
        .eq('id', userId)
        .single()
      ...
      if (templateKey === 'NEW_MESSAGE' && data.messageId) {
        const result = await this.sendNewMessageWithSmartDelivery(userId, data, lang, legacyToken, quietConfig)
        console.log(
