...
export class CalendarService {
  ...
  static async getCalendar(listingId, daysAhead = 180) {
    ...
    // 2. Fetch active bookings (PENDING, CONFIRMED, PAID)
    ...
    const bookingsUrl = `${supabaseUrl}/rest/v1/bookings?select=id,check_in,check_out,status,guest_name&listing_id=eq.${listingId}&status=in.(PENDING,CONFIRMED,PAID)&check_out=gte.${rangeStart}&check_in=lte.${rangeEnd}&order=check_in.asc`;
