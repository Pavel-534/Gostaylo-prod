...
/**
 * Stage 202.10 — `Number(undefined) ?? default` is NaN (?? does not catch NaN),
 * which made `|drift| <= NaN` always false → false GATEWAY_LEDGER_DRIFT on zero window.
 * @param {{ driftToleranceThb?: number }} [opts]
 * @returns {number}
 */
export function resolveGatewayDriftToleranceThb(opts = {}) {
  const n = Number(opts?.driftToleranceThb)
  if (Number.isFinite(n) && n >= 0) return n
  return DRIFT_TOLERANCE_THB
}
...
export async function runGatewayLedgerReconciliation(opts = {}) {
  ...
  const driftTolerance = resolveGatewayDriftToleranceThb(opts)
