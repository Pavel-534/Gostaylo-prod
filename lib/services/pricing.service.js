...
export class PricingService {
  ...
  /**
   * Net-profit guard baseline for growth programs (e.g. referral budget):
   * platform gross margin minus insurance reserve.
   *
   * @param {{ platformGrossRevenueThb?: number, insuranceReserveThb?: number }} feeSplitLike
   * @returns {{ platformGrossRevenueThb: number, insuranceReserveThb: number, netProfitOrderThb: number }}
   */
  static calculateNetProfitOrder(feeSplitLike = {}) {
    ...
    const netProfitOrderThb = Math.max(
      0,
      Math.round((platformGrossRevenueThb - insuranceReserveThb) * 100) / 100,
    );
    ...
    if (!applicability.ok) {
      switch (applicability.reason) {
        case 'LISTING_REQUIRED_FOR_ALLOWLIST':
          return {
            valid: false,
            error_code: PromoErrorCode.PROMO_LISTING_REQUIRED_FOR_ALLOWLIST,
          };
        case 'LISTING_OWNER_REQUIRED':
          return {
            valid: false,
            error_code: PromoErrorCode.PROMO_LISTING_OWNER_REQUIRED,
          };
        case 'EXPIRED':
