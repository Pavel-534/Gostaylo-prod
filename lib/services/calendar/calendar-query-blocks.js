...
import { deriveListingMaxCapacityColumn } from '@/lib/listing-guest-capacity';
import {
  getSupabase,
  getDateRange,
  parseMaxCapacity,
...
/** Partner grid origin label — no `bookings.source` column in live schema (Stage 194.0-D). */
function resolvePartnerBookingOrigin(booking) {
  const meta = booking?.metadata && typeof booking.metadata === 'object' ? booking.metadata : null
  const fromMeta = meta?.booking_source ?? meta?.source ?? meta?.origin
...
export async function getOccupyingBookings({
  ...
  // Stage 194.0-D: `bookings` has no `source` column — origin lives in metadata / UI default.
...
export function buildCalendar({
  ...
  const maxCapacity =
    deriveListingMaxCapacityColumn({
      ...
      bedroomsCount: listing?.bedrooms_count,
    }) || parseMaxCapacity(listing);
  ...
  const isWholeUnit = isWholeUnitCalendarInventory(listingCategorySlug, listing?.metadata);
  const g = maxCapacity <= 1 || isVehicleCategory || isWholeUnit ? 1 : rawRequested;
  ...
  for (const booking of bookings) {
    ...
    let night = cin;
    while (night < cout) {
      if (isVehicleCategory || isWholeUnit) {
  ...
  for (const block of blocks) {
    ...
    const blocksAvailability = countsTowardCalendarAvailability(block.source);
...
export async function getCalendarForDateRange(listingId, rangeStart, rangeEnd, options = {}) {
  ...
  const supabase = getSupabase();
  /** Stage 200.53.2 — partner bulk path may pass listingRow to skip a duplicate listings fetch. */
  const listingFromOptions =
    options.listingRow && String(options.listingRow.id) === String(listingId)
      ? options.listingRow
      : null;
  let listing = listingFromOptions;
  if (!listing) {
  ...
  const listingCategorySlugOverride = options.listingCategorySlugOverride
    ? String(options.listingCategorySlugOverride).toLowerCase().trim()
    : '';
  let listingCategorySlug = listingCategorySlugOverride;
  if (!listingCategorySlug && listing.category_id) {
    const { data: catRow } = await supabase
      .from('categories')
      .select('slug')
      .eq('id', listing.category_id)
  ...
  // Overlap filter — avoid loading all-time seasonal rows for a short partner window.
  const { data: seasonalPrices, error: seasonalError } = await supabase
    .from('seasonal_prices')
    .select('*')
    .eq('listing_id', listingId)
...
export function mapBuildCalendarToPartnerAvailability({ calendar, listingTimeZone, bookings }) {
  ...
  for (const day of calendar || []) {
    ...
    const pblocks = (Array.isArray(day.partner_blocks) ? day.partner_blocks : []).filter((block) =>
      countsTowardCalendarAvailability(block?.source),
    );
    ...
    if (pb.length > 0) {
      ...
      availability[date] = {
        ...
        source: resolvePartnerBookingOrigin(booking),
    ...
    if (pblocks.length > 0) {
      ...
      const src = String(block.source || MANUAL_BLOCK_SOURCE);
      availability[date] = {
        status: 'BLOCKED',
        blockId: block.id,
        reason: block.reason,
        ...
        blockSource: src,
        blockKind: resolveBlockDisplayKind(src),
        blockExpiresAt: block.expires_at || null,
      };
      continue;
    }
    ...
    if (day.status === 'BLOCKED') {
      availability[date] = {
        ...
        blockType: 'INVENTORY',
        blockKind: BLOCK_DISPLAY_KIND.INVENTORY,
      };
      continue;
    }
