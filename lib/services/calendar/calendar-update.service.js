...
import { getServerGuestRoundingMode } from '@/lib/booking-guest-rounding.js';
...
export class CalendarUpdateLayer {
  ...
  static async checkBatchAvailability(listingIds, checkIn, checkOut, options = {}) {
    ...
    if (!checkInStr || !checkOutStr || checkInStr >= checkOutStr) {
      console.error('[CALENDAR][BATCH] INVALID_DATE_RANGE', {
        rawCheckIn: checkIn,
        rawCheckOut: checkOut,
        normalizedCheckIn: checkInStr,
        normalizedCheckOut: checkOutStr,
      });
      return {
        success: false,
    ...
    console.info('[CALENDAR][BATCH] RPC params', {
      listingIdsCount: ids.length,
      checkIn: checkInStr,
      checkOut: checkOutStr,
      guestsCount,
      occupyingStatuses: statuses,
    });
    ...
    if (error) {
      console.error('[CALENDAR][BATCH] RPC error', {
        message: error.message || 'BATCH_AVAILABILITY_RPC_FAILED',
    ...
    const guestRoundingMode = await getServerGuestRoundingMode();
      ...
      if (row?.available === true) {
        ...
        if (listing) {
          ...
          if (!priceCalc.error) {
            ...
            const rounded = computeRoundedGuestTotal(feeSplit.guestPayableThb, guestRoundingMode);
            ...
            pricing = {
              ...
              roundingDiffPotThb: rounded?.roundingPotThb ?? 0,
              roundingMode: guestRoundingMode,
              averagePerNight,
