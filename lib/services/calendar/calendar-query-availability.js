...
export async function checkAvailability(listingId, checkIn, checkOut, options = {}) {
  ...
  const listingMeta = result.data.listingMetadata || {};
  const isVehicleCategory = isTransportListingCategory(catSlug);
  const isWholeUnit = isWholeUnitCalendarInventory(catSlug, listingMeta);
  ...
  while (night < checkOutStr) {
    ...
    if (!dayInfo) {
      ...
      const needRem =
        maxCap <= 1 || isTransportListingCategory(catSlug) || isWholeUnit ? 1 : guestsCount;
      if (rem < needRem) {
        conflicts.push({
          date: night,
