...
import { MIN_BOOKING_GUEST_TOTAL_THB } from '@/lib/booking-price-integrity.js';
...
export class BookingService {
  ...
  static async createBooking(bookingData) {
    ...
    if (!Number.isFinite(clientSubtotalThb) || serverSubtotalThb !== clientSubtotalThb) {
      ...
      void notifySystemAlert(
        `[SECURITY_ALERT] <b>PRICE_MISMATCH</b> createBooking (subtotal)\n` +
      ...
      recordCriticalSignal('PRICE_MISMATCH', {
        tag: '[FRAUD_DETECTION]',
    ...
    const guestPayableThb =
      Math.round(Number(priceThb)) + Math.round(Number(commission.commissionThb));
    ...
    if (quotedGuestTotalRaw !== undefined && quotedGuestTotalRaw !== null) {
      ...
      if (!Number.isFinite(qg) || qg !== guestPayableThb) {
        const fraudBanMarkupGuest = buildFraudBanReplyMarkup(finalRenterId);
        void notifySystemAlert(
          `[SECURITY_ALERT] <b>PRICE_MISMATCH</b> createBooking (guest total)\n` +
            `[FRAUD_DETECTION] ⚠️ <b>ATTEMPTED PRICE MANIPULATION</b>\n` +
            `ожидалось итог THB (субтотал+сбор): <b>${guestPayableThb}</b>, пришло: <b>${escapeSystemAlertHtml(String(quotedGuestTotalRaw))}</b>\n` +
            `listing: <code>${escapeSystemAlertHtml(listingId)}</code>\n` +
            `renter: <code>${escapeSystemAlertHtml(finalRenterId || '—')}</code>`,
        ...
        recordCriticalSignal('PRICE_MISMATCH', {
          ...
          detailLines: [
            ...
            `listing: ${listingId}`,
            `server payable THB: ${guestPayableThb}`,
            `client THB: ${qg}`,
          ],
        });
        return { error: 'Price verification failed', code: 'PRICE_MISMATCH' };
    ...
    if (guestPayableThb < MIN_BOOKING_GUEST_TOTAL_THB) {
      void notifySystemAlert(
        `[SECURITY_ALERT] <b>BOOKING_MIN_TOTAL_THB</b>\n` +
          `path: createBooking\n` +
          `listing: <code>${escapeSystemAlertHtml(listingId)}</code>\n` +
          `guest payable THB: <b>${guestPayableThb}</b> (min <b>${MIN_BOOKING_GUEST_TOTAL_THB}</b>)\n` +
  ...
  static async createInquiryBooking(bookingData) {
    ...
    if (needsPriceAttestation) {
      ...
      if (!Number.isFinite(clientSubtotalThb) || serverSubtotalThb !== clientSubtotalThb) {
        ...
        void notifySystemAlert(
          `[SECURITY_ALERT] <b>PRICE_MISMATCH</b> createInquiryBooking (subtotal)\n` +
            `[FRAUD_DETECTION] ⚠️ <b>ATTEMPTED PRICE MANIPULATION</b>\n` +
    ...
    if (!skipTransactionalMin) {
      ...
      if (quotedGuestTotalRawInq !== undefined && quotedGuestTotalRawInq !== null) {
        ...
        if (!Number.isFinite(qg) || qg !== guestPayableThb) {
          ...
          void notifySystemAlert(
            `[SECURITY_ALERT] <b>PRICE_MISMATCH</b> createInquiryBooking (guest total)\n` +
              `[FRAUD_DETECTION] ⚠️ <b>ATTEMPTED PRICE MANIPULATION</b>\n` +
              `ожидалось итог THB: <b>${guestPayableThb}</b>, пришло: <b>${escapeSystemAlertHtml(String(quotedGuestTotalRawInq))}</b>\n` +
              `listing: <code>${escapeSystemAlertHtml(listingId)}</code>\n` +
              `renter: <code>${escapeSystemAlertHtml(finalRenterId || '—')}</code>`,
  ...
  static async getBookingById(bookingId) {
    const { data, error } = await supabaseAdmin
      ...
      .select(`
        *,
        listings (id, title, district, cover_image, images, base_price_thb, owner_id, metadata),
