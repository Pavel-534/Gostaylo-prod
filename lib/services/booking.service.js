...
async function ensureInquiryConversation({
  ...
  try {
    ...
    const cin = checkIn ? new Date(checkIn).toLocaleDateString('ru-RU') : '—';
    const cout = checkOut ? new Date(checkOut).toLocaleDateString('ru-RU') : '—';
    const price = priceThb ? ` · ориентир ฿${Math.round(priceThb).toLocaleString('ru-RU')}` : '';

    const text = privateTrip
      ? `🔔 Inquiry for Private Trip: «${listing?.title || 'listing'}» · ${guestsCount} guests · ${cin} – ${cout}${price}. Please send a custom offer in this chat.`
      : `🔔 Price / capacity inquiry: «${listing?.title || 'listing'}» · ${guestsCount} guests · ${cin} – ${cout}${price}. (Current min. available spots on these dates: ${minRemainingSpots ?? '—'}).`;
    ...
    await supabaseAdmin.from('messages').insert({
      ...
      type: 'system',
      metadata: {
        system_key: privateTrip ? 'private_trip_inquiry' : 'capacity_price_inquiry',
...
export class BookingService {
  ...
  static async createInquiryBooking(bookingData) {
    ...
    const tag = privateTrip ? '[PRIVATE_TRIP_INQUIRY]' : '[CAPACITY_PRICE_INQUIRY]';
