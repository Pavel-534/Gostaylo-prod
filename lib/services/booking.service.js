...
export class BookingService {
  ...
  static async attachSettlementSnapshotForBooking(bookingId) {
    const { data: booking, error } = await supabaseAdmin
      .from('bookings')
      .select(
        'id, partner_id, price_thb, commission_thb, commission_rate, partner_earnings_thb, applied_commission_rate, listing_currency, pricing_snapshot'
      )
      .eq('id', bookingId)
      .single();
    ...
    const [partnerNetPreferred, platformMarginListing, insuranceReserveListing] = await Promise.all([
  ...
  static async createBooking(bookingData) {
    ...
    const pricePaid = guestPayableThb / exchangeRate;
