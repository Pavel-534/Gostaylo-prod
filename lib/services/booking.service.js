...
import { normalizeBookingInstantForDb, getListingDateTimeZone } from '@/lib/listing-date';
...
async function ensureInquiryConversation({
  ...
  try {
    ...
    if (privateTrip || negotiationRequest) {
      ...
      if (showDetailedTimes) {
        inquiryBodyRu = `${timeBlockRu}\n\nГость интересуется индивидуальным предложением (${guestsCount} гостей). Ответьте, выставив счёт в чате.`;
        inquiryBodyEn = `${timeBlockEn}\n\nThe guest is interested in a private/special deal (${guestsCount} guests). Please send a quote via Invoice.`;
      } else {
        inquiryBodyRu = `Гость интересуется индивидуальным предложением на период ${dateRangeRu} (${guestsCount} гостей). Ответьте, выставив счёт в чате.`;
        inquiryBodyEn = `The guest is interested in a private/special deal for ${dateRangeEn} (${guestsCount} guests). Please send a quote via Invoice.`;
      }
      text = inquiryBodyRu;
    } else {
      ...
      if (showDetailedTimes) {
        inquiryBodyRu = `${timeBlockRu}\n\nГостей: ${guestsCount}.${priceRu} Минимум свободных мест на эти даты: ${spotsRu}.`;
        inquiryBodyEn = `${timeBlockEn}\n\nGuests: ${guestsCount}.${priceEn} Min. spots available on these dates: ${spotsEn}.`;
      } else {
        inquiryBodyRu = `Гостей: ${guestsCount}. Период: ${dateRangeRu}.${priceRu} Минимум свободных мест на эти даты: ${spotsRu}.`;
        inquiryBodyEn = `Guests: ${guestsCount}. Period: ${dateRangeEn}.${priceEn} Min. spots available on these dates: ${spotsEn}.`;
      }
      text = inquiryBodyRu;
    }
