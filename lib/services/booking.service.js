...
async function ensureInquiryConversation({
  ...
  try {
    ...
    if (privateTrip || negotiationRequest) {
      ...
      if (showDetailedTimes) {
        inquiryBodyRu = `${timeBlockRu}\n\n${formatInquiryPartyLineRu(categorySlug, guestsCount)}${priceRu} Минимум свободных мест на эти даты: ${spotsRu}.`;
        inquiryBodyEn = `${timeBlockEn}\n\n${formatInquiryPartyLineEn(categorySlug, guestsCount)}${priceEn} Min. spots available on these dates: ${spotsEn}.`;
      } else {
        inquiryBodyRu = `${formatInquiryPartyLineRu(categorySlug, guestsCount)} Период: ${dateRangeRu}.${priceRu} Минимум свободных мест на эти даты: ${spotsRu}.`;
        inquiryBodyEn = `${formatInquiryPartyLineEn(categorySlug, guestsCount)} Period: ${dateRangeEn}.${priceEn} Min. spots available on these dates: ${spotsEn}.`;
      }
      text = inquiryBodyRu;
    }
