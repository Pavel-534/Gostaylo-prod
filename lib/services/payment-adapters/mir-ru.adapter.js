...
export class MirRuAdapter {
   ...
   */
  /**
   * @returns {Promise<boolean>} false if persist failed (caller must fail closed before PSP POST)
   */
   ...
   *   intent: object,
   *   bookingId: string,
   *   charge?: import('./acquirer-charge-amount').ResolvedCharge,
   *   booking?: object | null,
   * }} params
   */
  static async createSession({ intent, bookingId, charge, booking = null }) {
    ...
    const userId = String(booking?.renter_id || booking?.renterId || '').trim()
    ...
    if (generated) {
      const persisted = await MirRuAdapter.persistIdempotenceKey(
        intent.id,
        idempotenceKey,
        intent.metadata || {},
      )
      if (!persisted) {
        return {
          ...
          adapterPayload: {
            adapter_key: 'MIR_RU',
            mode: 'error',
            error: 'YOOKASSA_IDEMPOTENCE_PERSIST_FAILED',
            user_message: 'Не удалось подготовить платёж. Попробуйте ещё раз.',
            ...chargeMeta,
          },
        }
      }
