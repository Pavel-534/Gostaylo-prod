 ...
 * Stage 200.70 — paid events fail-closed when amount missing (`requireAmount`, default true).
 *
 * @param {{
 ...
 * }} args
 * @returns {{ ok: true, shadowLegacy?: boolean, legacyWebhookThb?: boolean } | { ok: false, error: string, expected: object | null, received: object }}
 */
export function verifyWebhookPaidAmount({
  receivedAmount,
  receivedCurrency,
  booking,
  intent,
  ...
  if (!Number.isFinite(received) || received <= 0) {
    if (requireAmount) {
      return {
        ...
        expected: null,
        received: { amount: receivedAmount, currency: rc },
      }
    }
