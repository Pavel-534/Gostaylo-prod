...
/** Stage 198 — PSP canceled/failed/expired (not a capture). */
export function isIntentTerminalNonPaidStatus(status) {
  const st = String(status || '').toUpperCase()
  return (
    st === INTERNAL_INTENT_STATUSES.FAILED ||
    st === INTERNAL_INTENT_STATUSES.CANCELLED ||
    st === INTERNAL_INTENT_STATUSES.EXPIRED
  )
}
