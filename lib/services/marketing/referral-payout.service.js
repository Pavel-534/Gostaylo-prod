...
export async function distributeHostPartnerActivation(bookingId) {
    ...
    // Stage 202.40 — host activation pays only levels with configuredPercent > 0
    // (canon: L1=100, L2=0). Guest pool L2/L3 is a separate path.
    const payouts = [];
    if (level1ReferrerId && mlmL1 > 0) {
      payouts.push({
    ...
    if (mlmL2 > 0 && level2Candidate && level2Candidate !== level1ReferrerId) {
      payouts.push({
