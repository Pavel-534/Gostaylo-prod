...
export async function getBookingForDistribution(bookingId) {
    ...
    const { data, error } = await supabaseAdmin
      .from('bookings')
      .select(
        'id,status,renter_id,price_thb,commission_thb,commission_rate,applied_commission_rate,pricing_snapshot,completed_at,updated_at',
      )
      .eq('id', id)
      .maybeSingle();
...
export async function distribute(bookingId, options = {}) {
    ...
    let attributionId = await ReferralAttributionService.getAttributionIdForBooking(booking.id);
    if (!attributionId && renterId) {
      attributionId = await ReferralAttributionService.getAttributionIdForRenter(renterId);
    }
    ...
    const holdDays = Math.max(0, Math.floor(Number(policy.referralHoldDays) || 0));
...
export async function distributeHostPartnerActivation(bookingId) {
    ...
    const { notifyReferralBonusEarned } = await import(
