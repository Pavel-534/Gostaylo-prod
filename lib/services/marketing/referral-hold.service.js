...
export async function adjustHeldReferralBalanceThb(userId, deltaThb) {
  ...
  // Prefer atomic RPC (stage201_01); CAS fallback if RPC missing.
  try {
    const { data, error } = await supabaseAdmin.rpc('adjust_held_referral_balance_thb', {
      p_user_id: uid,
      p_delta_thb: delta,
    });
    const row = Array.isArray(data) ? data[0] : data;
    if (!error && row?.applied === true) {
      return {
        success: true,
        data: {
          heldReferralBalanceThb: round2(row.held_referral_balance_thb),
          previousThb: round2(row.previous_thb),
        },
      };
    }
    if (error && !String(error.message || '').includes('does not exist')) {
      return { success: false, error: error.message || 'HELD_BALANCE_RPC_FAILED' };
    }
  } catch {
    /* fall through to CAS */
  }
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    const fresh = attempt === 0 ? wallet : await WalletService.getOrCreateWallet(uid);
    if (!fresh.success) return fresh;
    const prev = round2(fresh.data?.held_referral_balance_thb ?? 0);
    const next = round2(Math.max(0, prev + delta));
    const { data, error } = await supabaseAdmin
      .from('user_wallets')
      .update({ held_referral_balance_thb: next, updated_at: new Date().toISOString() })
      .eq('user_id', uid)
      .eq('held_referral_balance_thb', prev)
      .select('held_referral_balance_thb')
      .maybeSingle();
    if (error) return { success: false, error: error.message || 'HELD_BALANCE_UPDATE_FAILED' };
    if (data) {
      return { success: true, data: { heldReferralBalanceThb: round2(data.held_referral_balance_thb), previousThb: prev } };
    }
  }
  return { success: false, error: 'HELD_BALANCE_CAS_CONFLICT' };
}
