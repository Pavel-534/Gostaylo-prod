...
async function maybeNotifyMonthlySpendAlert(policy) {
  ...
  const monthlySpendThb = await sumMonthlyReservedSpendThb()
  ...
  try {
    await supabaseAdmin.from('critical_signal_events').insert({
      ...
      severity: 'info',
      message: `Early warning · referral spend growing: ${monthlySpendThb} THB >= ${policy.monthlySpendAlertThb}`,
...
/**
 * Monthly reserved spend: pending + earned + earned_held, by created_at, guest_booking only.
 * Aligned with program cap gate (referral-program-cap.service.js).
 */
export async function sumMonthlyReservedSpendThb() {
  const monthStartIso = currentUtcMonthStartIso()
...
/**
 * Actual payouts: only earned rows that have been credited to wallets.
 */
export async function sumMonthlyPaidActualThb() {
  const monthStartIso = currentUtcMonthStartIso()
  const { data: rows, error } = await supabaseAdmin
    .from('referral_ledger')
    .select('amount_thb')
...
/** Stage 131.A1.3 — independent 80% of program cap (does not suppress when 150k early warning also fires). */
async function maybeNotifyMonthlySpendApproaching(policy) {
  const warnThb = round2(policy.programCapWarnThb ?? policy.programCapThb * 0.8)
  const monthlySpendThb = await sumMonthlyReservedSpendThb()
  if (monthlySpendThb == null || warnThb <= 0 || monthlySpendThb < warnThb) return null

  const monthStartIso = currentUtcMonthStartIso()
  const signalKey = 'REFERRAL_PROGRAM_CAP_APPROACHING'
  try {
    const { data: existing } = await supabaseAdmin
      .from('critical_signal_events')
      .select('id')
  ...
  try {
    await supabaseAdmin.from('critical_signal_events').insert({
      signal_key: signalKey,
      severity: 'warning',
      message: `Approaching program cap: ${monthlySpendThb} THB (warn ${warnThb} = 80% of ${policy.programCapThb})`,
      metadata: {
        monthlySpendThb,
        warnThb,
        warnPercent: policy.programCapWarnPercent ?? 80,
        programCapThb: policy.programCapThb,
        monthStartIso,
      },
    })
  } catch {
  ...
  return { monthlySpendThb, warnThb, thresholdThb: policy.programCapThb }
}
...
export async function notifyReferralBonusHeld(payload) {
  if (!payload?.beneficiaryId || !(Number(payload.amountThb) > 0)) return
  try {
    let enriched = { ...payload, fraudGateHold: payload.fraudGateHold === true }
    try {
      const fx = await convertReferralPayoutThbToCurrency(Number(payload.amountThb), 'RUB')
      enriched = { ...enriched, amountRub: fx.amountInPayoutCurrency }
    } catch {
      /* optional */
    }
...
/** Stage 135 — weekly L1+L2 team earnings digest. */
export async function notifyReferralTeamWeeklyDigest(payload) {
  const userId = String(payload?.userId || '').trim()
  const totalThb = round2(payload?.totalThb)
  if (!userId || totalThb <= 0) return
  try {
    let enriched = { ...payload, userId, totalThb }
    try {
      const fx = await convertReferralPayoutThbToCurrency(totalThb, 'RUB')
      enriched = { ...enriched, amountRub: fx.amountInPayoutCurrency }
    } catch {
      /* optional */
    }
    await NotificationService.dispatch(NotificationEvents.REFERRAL_TEAM_WEEKLY_DIGEST, enriched)
  } catch (e) {
    console.warn('[REFERRAL_NOTIFY] weekly digest', e?.message || e)
  }
}

/** Stage 131.A2 — friend booked (referral tree). */
export async function notifyReferralFriendBooked(payload) {
  const ambassadorId = String(payload?.ambassadorId || '').trim()
  if (!ambassadorId || !payload?.bookingId) return
  try {
    await NotificationService.dispatch(NotificationEvents.REFERRAL_FRIEND_BOOKED, payload)
  } catch (e) {
    console.warn('[REFERRAL_NOTIFY] friend booked', e?.message || e)
  }
}

/** Stage 131.A2 — friend trip completed (before wallet credit notify). */
export async function notifyReferralFriendCompleted(payload) {
  const ambassadorId = String(payload?.ambassadorId || '').trim()
  if (!ambassadorId || !payload?.bookingId) return
  try {
    await NotificationService.dispatch(NotificationEvents.REFERRAL_FRIEND_COMPLETED, payload)
  } catch (e) {
    console.warn('[REFERRAL_NOTIFY] friend completed', e?.message || e)
  }
}

/** Stage 131.A2.1 — friend cancelled booking. */
export async function notifyReferralFriendCancelled(payload) {
  const ambassadorId = String(payload?.ambassadorId || '').trim()
  if (!ambassadorId || !payload?.bookingId) return
  try {
    await NotificationService.dispatch(NotificationEvents.REFERRAL_FRIEND_CANCELLED, payload)
  } catch (e) {
    console.warn('[REFERRAL_NOTIFY] friend cancelled', e?.message || e)
  }
}
