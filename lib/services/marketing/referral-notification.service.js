...
/** Stage 131.A2 — friend paid escrow. */
export async function notifyReferralFriendPaid(payload) {
  const ambassadorId = String(payload?.ambassadorId || '').trim()
  if (!ambassadorId || !payload?.bookingId) return
  try {
    await NotificationService.dispatch(NotificationEvents.REFERRAL_FRIEND_PAID, payload)
  } catch (e) {
    console.warn('[REFERRAL_NOTIFY] friend paid', e?.message || e)
  }
}
