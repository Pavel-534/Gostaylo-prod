...
export function buildCohortRoiSeries({
    ...
    const now = new Date();
    const horizonStart = addMonthsUtc(now, -36);
    const cohortEntries = [...cohortMap.values()].filter((c) => {
      const a = cohortMonthAnchorUtc(c.cohortMonth);
    ...
    for (const c of cohortEntries) {
      const anchor = cohortMonthAnchorUtc(c.cohortMonth);
      if (!anchor) continue;
      const cumulativeCommissionThb = {};
      for (const { key, months } of milestones) {
        const end = addMonthsUtc(anchor, months);
        let sum = 0;
        for (const rid of c.refereeIds) {
...
export async function getAnalyticsStats() {
    ...
    const bookings = [];
    for (const chunk of chunkArray(refereeIdsList, 80)) {
    ...
    const cohortRoi = buildCohortRoiSeries({
      relations,
      refereeProfilesById,
      bookingsByRenter,
      ledgerRowsByReferee,
