...
const META_PAID = 'referral_friend_paid_notified_at'
...
/**
 * @param {{ booking: object, payment?: object, listing?: object }} params
 */
export async function maybeNotifyReferralFriendPaid(params) {
  const booking = params?.booking
  const bookingId = String(booking?.id || '').trim()
  if (!bookingId || !supabaseAdmin) return

  const claim = await claimFriendNotifyOnce(bookingId, META_PAID)
  if (!claim.claimed) return
  ...
  const paidAmountThb = round2(
    params?.payment?.amount_thb ??
      params?.payment?.guestCaptureAmountThb ??
      booking?.price_thb ??
      booking?.guest_total_thb,
  )
  if (paidAmountThb <= 0) return

  const listingTitle = String(params?.listing?.title || booking?.listing?.title || '').trim()
  const displayName = await loadFriendDisplayName(renterId)

  void notifyReferralFriendPaid({
    ambassadorId,
    friendUserId: renterId,
    bookingId,
    paidAmountThb,
