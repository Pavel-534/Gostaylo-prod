...
import { distributeReferralLedgerCreditAtomic, assertReferralAtomicCreditResult } from '@/lib/services/marketing/referral-distribute-atomic.service.js'
...
async function syncRewardRuleColumnsByIds(ledgerIds, rewardRuleMetadata) {
  const ids = Array.isArray(ledgerIds) ? ledgerIds.map((x) => String(x || '')).filter(Boolean) : []
  if (!ids.length) return
  const meta =
    rewardRuleMetadata && typeof rewardRuleMetadata === 'object' ? rewardRuleMetadata : {}
  const ruleVersion =
    meta.reward_rule_version != null && Number.isFinite(Number(meta.reward_rule_version))
      ? Number(meta.reward_rule_version)
      : null
  const rewardRuleId = meta.reward_rule_id ? String(meta.reward_rule_id) : null
  if (ruleVersion == null && !rewardRuleId) return
  try {
    await supabaseAdmin
      .from('referral_ledger')
      .update({
        ...(ruleVersion != null ? { rule_version: ruleVersion } : {}),
        ...(rewardRuleId ? { reward_rule_id: rewardRuleId } : {}),
      })
      .in('id', ids)
  } catch {
    // migration stage123_1 may not be applied yet
  }
}
...
export class ReferralLedgerService {
  static async createPendingLedgerRows({
    ...
    const attrMeta =
      attributionId != null && String(attributionId).trim() !== ''
        ...
        : {};
    const campaignMeta =
      campaignSlug != null && String(campaignSlug).trim() !== ''
        ? { campaign_slug: String(campaignSlug).trim() }
        : {};
    const ruleMeta =
      rewardRuleMetadata && typeof rewardRuleMetadata === 'object' ? rewardRuleMetadata : {};
    const relMeta = relation?.metadata && typeof relation.metadata === 'object' ? relation.metadata : {}
    const fraudMeta = relMeta.fraud_suspicious
      ? {
          fraud_suspicious: true,
          fraud_severity: String(relMeta.fraud_severity || 'review'),
          fraud_rule_codes: Array.isArray(relMeta.fraud_rule_codes) ? relMeta.fraud_rule_codes : [],
        }
      : {};
    ...
    await syncRewardRuleColumnsByIds(rows.map((r) => r.id), rewardRuleMetadata)
  }
  ...
  static async markPendingAsEarned(bookingId, options = {}) {
    ...
    const { data: pendingRows, error: readErr } = await supabaseAdmin
      .from('referral_ledger')
      .select('id,metadata,amount_thb,type,referrer_id,referee_id,ledger_depth,referral_type')
      .eq('booking_id', bid)
    ...
    if (!toEarn.length) return { held: holdDays > 0, count: 0, fraudHeldCount: 0 }

    const fraudHeld = []
    const cleanEarn = []
    const listingCity = await resolveBookingListingCity(bid)
    for (const row of toEarn) {
      const gate = await ReferralFraudGate.evaluateAccrualForLedgerRow({ row, bookingId: bid })
      if (gate.hold) {
        fraudHeld.push({ row, gate })
      } else {
        cleanEarn.push(row)
      }
    }

    for (const { row, gate } of fraudHeld) {
      const prevMeta = row?.metadata && typeof row.metadata === 'object' ? row.metadata : {}
      const fraudMeta = ReferralFraudGate.buildAccrualHoldMetadata(prevMeta, gate, nowIso)
      const { error: fraudErr } = await supabaseAdmin
        .from('referral_ledger')
        .update({
        ...
        .eq('id', String(row.id))
        .eq('status', REFERRAL_STATUSES.PENDING)
      if (fraudErr) throw new Error(fraudErr.message || 'REFERRAL_LEDGER_FRAUD_HOLD_FAILED')
      await ReferralFraudGate.enqueueAccrualFraudHold({ row, gate, bookingId: bid })
      void notifyReferralBonusHeld({
        beneficiaryId: gate.beneficiaryId,
    ...
    if (!cleanEarn.length) {
      return {
        held: true,
        count: 0,
        fraudHeldCount: fraudHeld.length,
      }
    }

    const earnIds = cleanEarn.map((row) => String(row.id || '')).filter(Boolean)
    const ruleMeta =
      options.rewardRuleMetadata && typeof options.rewardRuleMetadata === 'object'
        ? options.rewardRuleMetadata
        : null

    if (nextStatus === REFERRAL_STATUSES.EARNED) {
      for (const row of cleanEarn) {
        const rowMetaPatch =
          ruleMeta && Object.keys(ruleMeta).length
            ? {
            ...
            : row?.metadata && typeof row.metadata === 'object'
              ? row.metadata
              : {}
        const atomic = await distributeReferralLedgerCreditAtomic({
          row,
          bookingId: bid,
          earnedAt: nowIso,
          unlockAt: null,
          metadataPatch: rowMetaPatch,
          creditOnly: false,
        })
        if (!atomic.success) {
          throw new Error(atomic.error || `REFERRAL_ATOMIC_FAILED:${String(row.id)}`)
        }
        if (atomic.walletApplied) {
          const beneficiaryId = beneficiaryIdForLedgerRow(row)
          const amount = round2(row?.amount_thb)
    ...
    } else if (ruleMeta && earnIds.length) {
      for (const row of cleanEarn) {
        const prevMeta = row?.metadata && typeof row.metadata === 'object' ? row.metadata : {}
        const { error: rowErr } = await supabaseAdmin
          ...
          .update({
            ...
            updated_at: nowIso,
            unlock_at: unlockAt,
            metadata: { ...prevMeta, ...ruleMeta },
          })
          .eq('id', String(row.id))
          .eq('status', REFERRAL_STATUSES.PENDING)
        if (rowErr) throw new Error(rowErr.message || 'REFERRAL_LEDGER_EARNED_UPDATE_FAILED')
      }
    } else if (nextStatus === REFERRAL_STATUSES.EARNED_HELD) {
    ...
    await syncRewardRuleColumnsByIds(earnIds, ruleMeta)

    return {
      held: nextStatus === REFERRAL_STATUSES.EARNED_HELD,
      count: earnIds.length,
      fraudHeldCount: fraudHeld.length,
    }
  }
   ...
   * AUDIT_02 C2.5: flip one row at a time, debit held, credit immediately (creditOnly);
   * final heal pass via creditWalletFromEarnedRows.
  ...
  static async unlockHeldRowsForBooking(bookingId, options = {}) {
    ...
    let q = supabaseAdmin
      .from('referral_ledger')
      .select('id, booking_id, referrer_id, referee_id, amount_thb, type, status, unlock_at, metadata, ledger_depth, referral_type')
      .eq('booking_id', id)
      .eq('status', REFERRAL_STATUSES.EARNED_HELD)
    ...
    if (error) return { success: false, error: error.message || 'REFERRAL_LEDGER_HELD_READ_FAILED' }
    const eligible = (rows || []).filter((row) => !ReferralFraudGate.isFraudGateHeldRow(row))
    if (!eligible.length) {
      // Still heal any already-earned rows missing wallet (prior crash window).
      try {
    ...
    for (const row of eligible) {
      ...
      const { data: flipped, error: upErr } = await supabaseAdmin
        .from('referral_ledger')
        .update({
        ...
        .maybeSingle()
      if (upErr) {
        creditErrors.push({ ledgerId: rowId, error: upErr.message || 'UNLOCK_UPDATE_FAILED' })
        continue
      }
      // Lost race to another unlock worker — do not debit held / credit again.
      if (!flipped?.id) continue
      ...
      if (beneficiaryId && amount > 0) {
        try {
          ...
          creditErrors.push({
            ledgerId: rowId,
            error: heldErr?.message || 'HELD_BALANCE_DEBIT_FAILED',
          })
        }
      }
      ...
      try {
        const credit = await distributeReferralLedgerCreditAtomic({
          row: { ...row, status: REFERRAL_STATUSES.EARNED },
          bookingId: id,
          creditOnly: true,
        })
        assertReferralAtomicCreditResult(credit, {
          bookingId: id,
          ledgerId: rowId,
          operation: 'unlockHeldRowsForBooking',
        })
      } catch (creditErr) {
        creditErrors.push({
          ledgerId: rowId,
          error: creditErr?.message || 'WALLET_CREDIT_FAILED',
        })
      }

      unlockedCount += 1
      unlockedAmountThb = round2(unlockedAmountThb + amount)
    }

    try {
      await this.creditWalletFromEarnedRows(id)
    } catch (healErr) {
      creditErrors.push({ ledgerId: null, error: healErr?.message || 'HEAL_CREDIT_FAILED' })
    }

    return {
      success: creditErrors.length === 0,
      unlockedCount,
      unlockedAmountThb,
      bookingId: id,
      creditErrors: creditErrors.length ? creditErrors : undefined,
    }
  }
  ...
  static async creditWalletFromEarnedRows(bookingId) {
    const { data, error } = await supabaseAdmin
      .from('referral_ledger')
      .select(
        'id,referrer_id,referee_id,amount_thb,type,status,metadata,ledger_depth,referral_type',
      )
      .eq('booking_id', String(bookingId))
      .eq('status', REFERRAL_STATUSES.EARNED)
    if (error) throw new Error(error.message || 'REFERRAL_LEDGER_EARNED_READ_FAILED')

    const listingCity = await resolveBookingListingCity(String(bookingId))

    for (const row of data || []) {
      if (ReferralFraudGate.isFraudGateHeldRow(row)) continue

      const rowMeta = row?.metadata && typeof row.metadata === 'object' ? row.metadata : {}
      const adminFraudApproved = rowMeta.fraud_gate_resolved === 'approved'

      if (!adminFraudApproved) {
        const gate = await ReferralFraudGate.evaluateAccrualForLedgerRow({
          row,
        ...
        if (gate.hold) {
          ...
          await supabaseAdmin
            ...
            .update({
              ...
              metadata: ReferralFraudGate.buildAccrualHoldMetadata(prevMeta, gate, new Date().toISOString()),
            })
            .eq('id', String(row.id))
            .eq('status', REFERRAL_STATUSES.EARNED)
          await ReferralFraudGate.enqueueAccrualFraudHold({ row, gate, bookingId: String(bookingId) })
          continue
        }
      }
      ...
      assertReferralAtomicCreditResult(credit, {
        bookingId: String(bookingId),
        ledgerId: String(row.id),
        operation: 'creditWalletFromEarnedRows',
      })
...
async function resolveBookingListingCity(bookingId) {
  const bid = String(bookingId || '').trim()
  if (!bid) return null
  try {
    const { data } = await supabaseAdmin
      .from('bookings')
      .select('listing_id, listings(district,title)')
      .eq('id', bid)
      .maybeSingle()
