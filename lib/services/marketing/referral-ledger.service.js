...
export class ReferralLedgerService {
  ...
  static async unlockHeldRowsForBooking(bookingId, options = {}) {
    ...
    const ids = eligible.map((r) => String(r.id)).filter(Boolean)
    const { error: upErr } = await supabaseAdmin
      .from('referral_ledger')
      .update({
    ...
    const unlockedAmountThb = eligible.reduce((sum, row) => sum + round2(row?.amount_thb), 0)
    return { success: true, unlockedCount: eligible.length, unlockedAmountThb, bookingId: id }
  }
