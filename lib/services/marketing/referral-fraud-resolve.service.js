...
export class ReferralFraudResolveService {
  ...
  static async resolveFraudItem(params = {}) {
    ...
    let ledgerOutcome = { approvedRows: [], rejectedRows: [], creditedBookingIds: [] }
    ...
    if (action === 'approved') {
      ...
      await this._syncPayoutBlockedAfterApprove(rows, queueId)
  ...
  /** @param {object[]} rows @param {string} excludeQueueId */
