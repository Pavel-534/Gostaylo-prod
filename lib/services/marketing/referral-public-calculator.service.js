...
function clampGuestFeePercent(raw) {
  const n = Number(raw)
  if (!Number.isFinite(n)) return PLATFORM_SPLIT_FEE_DEFAULTS.guestServiceFeePercent
...
function splitPoolForMarketing(poolThb, config) {
  ...
  const l1Pct = Number(
    config.ambassadorGuestPoolL1Percent ?? FINTECH_JS_DEFAULTS.ambassadorGuestPoolL1Percent,
  )
  const l2Pct = Number(
    config.ambassadorGuestPoolL2Percent ?? FINTECH_JS_DEFAULTS.ambassadorGuestPoolL2Percent,
  )
  const l3Pct = l3Enabled
    ? Number(config.ambassadorGuestPoolL3Percent ?? FINTECH_JS_DEFAULTS.ambassadorGuestPoolL3Percent)
    : 0
  const l1 = round2((pool * l1Pct) / 100)
  const l2 = round2((pool * l2Pct) / 100)
  const l3 = round2((pool * l3Pct) / 100)
  const guest = round2(pool - l1 - l2 - l3)
  return {
    l1AmountThb: l1,
    l2AmountThb: l2,
    l3AmountThb: l3Enabled ? l3 : 0,
    l3WithheldThb: 0,
    refereeAmountThb: guest,
  }
}
...
export async function computePublicReferralCalculatorEstimate({
  ...
  /**
   * A5 inline hero: count assumptions for multi-level bookings.
   * Does NOT change existing per-booking fields (l1AmountThb/l2AmountThb/...)
   * Only adds additional *TotalThb fields for UI breakdown.
   */
  l1BookingsCount = 1,
  l2ConversionRate = 1 / 3,
} = {}) {
  ...
  const safeL1Bookings = Math.max(0, Number.isFinite(l1BookingsCount) ? Number(l1BookingsCount) : 1)
  // For UI-only estimates: "1 L2 booking per 3 L1 bookings" default (33%).
  const safeL2ConversionRate = Number.isFinite(l2ConversionRate) ? Number(l2ConversionRate) : 1 / 3
  const l1BookingsN = Math.max(0, Math.floor(safeL1Bookings))
  const l2BookingsN = Math.max(0, l1BookingsN * safeL2ConversionRate)
  // "Analogously for L3" (derived as another conversion step).
  const l3BookingsN = Math.max(0, l2BookingsN * safeL2ConversionRate)
  ...
  const result = {
    ...
    // Additive display fields for calculator v2 funnel (no formula change).
    guestServiceFeeThb: round2(preview.feeBase?.guestServiceFeeThb ?? preview.guestPaymentThb - subtotal),
    platformGrossThb: round2(preview.platformGrossRevenueThb ?? 0),
    adjustedNetThb: round2(preview.adjustedNetThb ?? 0),
    referralPoolThb: round2(preview.referralPoolThb ?? 0),
    referralReinvestmentPercent: Number(
      config.referralReinvestmentPercent ?? FINTECH_JS_DEFAULTS.referralReinvestmentPercent ?? 45,
    ),
    l1AmountThb: split.l1AmountThb,
    l2AmountThb: split.l2AmountThb,
    l3AmountThb: split.l3AmountThb,
    l3WithheldThb: split.l3WithheldThb,
    refereeAmountThb: split.refereeAmountThb,
    l2LiveEnabled: l2Enabled,
    l2ShownAs: l2Enabled ? 'live' : 'preview',
    l3LiveEnabled: l3Enabled,
    l3MinDirectPartners: Number(
      config.ambassadorGuestL3MinDirectPartners ?? FINTECH_JS_DEFAULTS.ambassadorGuestL3MinDirectPartners ?? 10,
    ),
    l2CapPerBookingThb: Number(
      config.ambassadorGuestL2MaxThbPerBooking ?? FINTECH_JS_DEFAULTS.ambassadorGuestL2MaxThbPerBooking ?? 500,
    ),
    l2CapPerMonthThb: Number(
      config.ambassadorGuestL2MaxThbPerMonth ?? FINTECH_JS_DEFAULTS.ambassadorGuestL2MaxThbPerMonth ?? 50_000,
    ),
    // Network depth assumption used for L2/L3 totals (UI activity slider → l2ConversionRate).
    l2ConversionRate: safeL2ConversionRate,
    l1BookingsCount: l1BookingsN,
    l2BookingsCount: round2(l2BookingsN),
    l3BookingsCount: round2(l3BookingsN),
    // Totals for multi-level assumptions (UI only).
    l1TotalThb: round2(split.l1AmountThb * l1BookingsN),
    l2TotalThb: round2(split.l2AmountThb * l2BookingsN),
    l3TotalThb: round2(split.l3AmountThb * l3BookingsN),
    // Guest cashback is the referee share for every completed booking at each depth assumption.
    guestCashbackTotalThb: round2(split.refereeAmountThb * (l1BookingsN + l2BookingsN + l3BookingsN)),
    splitPercents: {
      l1: config.ambassadorGuestPoolL1Percent,
      l2: config.ambassadorGuestPoolL2Percent,
      l3: l3Enabled ? config.ambassadorGuestPoolL3Percent : 0,
