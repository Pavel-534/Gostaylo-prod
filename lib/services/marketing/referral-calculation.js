...
import { resolveFintechPolicyForBooking, FinSnapshotMissingError } from '@/lib/services/finance/fintech-snapshot.service.js';
...
export async function getReferralSettings({ booking } = {}) {
  const general = await PricingService.getGeneralPricingSettings();
  let fintech;
  try {
    fintech = await resolveFintechPolicyForBooking(booking);
  } catch (err) {
    if (err instanceof FinSnapshotMissingError) {
      void recordCriticalSignal('FIN_SNAPSHOT_MISSING_FOR_NEW_BOOKING', {
        severity: 'CRITICAL',
        tag: '[FINTECH][CRITICAL]',
        detailLines: [`bookingId=${err.bookingId || 'unknown'}`],
        persistDetail: { bookingId: err.bookingId },
      });
    }
    throw err;
  }
  ...
  return {
    ...
    operationalReservePercent: clamp(fintech.operationalReservePercent, 0, 100),
