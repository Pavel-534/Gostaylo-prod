...
export class ReferralPnlService {
  ...
  static async distributeHostPartnerActivation(bookingId) {
    ...
    if (!promoDebit.applied) {
      ...
      const bal = promoDebit.newBalanceThb;
      ...
      meta.host_activation_promo_tank = {
        ...
        balance_thb_at_failure: Number.isFinite(Number(bal)) ? round2(Number(bal)) : null,
        reason,
        updated_at: new Date().toISOString(),
      };
      ...
      void notifySystemAlert(
        `🧯 <b>Promo tank: host activation blocked</b>\n` +
          ...
          `Нужно <b>${escapeSystemAlertHtml(String(bonusThb))}</b> THB, в баке ≈ <b>${escapeSystemAlertHtml(
            Number.isFinite(Number(bal)) ? String(round2(Number(bal))) : '—',
          )}</b> THB\n` +
          `Причина: <code>${escapeSystemAlertHtml(reason)}</code>\n` +
      ...
      return {
        ...
        data: { bookingId: id, promoTankReason: reason, requiredThb: bonusThb, balanceThb: bal },
      };
    }
