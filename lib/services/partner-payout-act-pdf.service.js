...
export function renderPartnerPayoutActPdf(opts) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const font = fontReg.lgc ? 'PdfNotoLGC' : 'Helvetica'
    ...
    doc.fontSize(11).fillColor('#000').text('Исполнитель (агент платформы)')
    doc.fontSize(10).fillColor('#333')
    drawPdfUnicodeLine(doc, publisher.companyName, 48, doc.y, { fontSize: 10 })
    doc.y += 14
    doc.text(`ИНН ${publisher.inn}`)
    doc.moveDown(0.6)

    doc.fontSize(11).fillColor('#000').text('Получатель (партнёр)')
    drawPdfUnicodeLine(doc, String(opts.partnerLabel || '—').slice(0, 200), 48, doc.y, { fontSize: 10 })
    doc.y += 14
    if (opts.partnerInn) {
      doc.fontSize(10).text(`ИНН / идентификатор: ${String(opts.partnerInn).slice(0, 32)}`)
    }
    doc.moveDown(0.8)

    doc.fontSize(11).text('Предмет')
    doc.fontSize(10).fillColor('#333')
    doc.text(
    ...
    doc.fontSize(11).fillColor('#000').text('Сумма к выплате')
    doc.fontSize(12).text(`${amountThb.toLocaleString('ru-RU')} THB`, { continued: false })
    if (payoutCur && amountFx != null && payoutCur !== 'THB') {
      doc.fontSize(10).fillColor('#555').text(`Эквивалент к перечислению: ${amountFx.toLocaleString('ru-RU')} ${payoutCur}`)
    }
    doc.moveDown(0.5)

    if (opts.batchId) {
      doc.fontSize(9).fillColor('#666').text(`Пул выплат: ${opts.batchId}`)
    }
    if (opts.payoutId) {
      doc.fontSize(9).text(`Заявка на вывод: ${opts.payoutId}`)
    }

    if (bookingIds.length) {
      doc.moveDown(0.5)
      doc.fontSize(10).fillColor('#000').text('Связанные бронирования (ID):')
