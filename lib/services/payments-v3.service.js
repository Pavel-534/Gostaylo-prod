...
import { ensureEscrowForConfirmedPayment } from '@/lib/payment/reconcile-confirmed-without-escrow.js'
...
export class PaymentsV3Service {
  ...
  static async confirmPayment(paymentId, verificationData = {}) {
    ...
    if (SUBMIT_TXID_FINALIZED_PAYMENT_STATUSES.has(payStatus)) {
      // AUDIT_03 C3.4: CONFIRMED without escrow must retry moveToEscrow
      const ensure = await ensureEscrowForConfirmedPayment(payment, verificationData)
      if (!ensure.success && !ensure.alreadyEscrowed) {
        return {
          success: false,
          alreadyConfirmed: true,
          error: ensure.error || 'Escrow transition failed',
          code: 'CONFIRMED_WITHOUT_ESCROW',
      ...
      return {
        ...
        alreadyConfirmed: true,
        payment: mapPaymentRow(payment),
        escrow: ensure.escrow || null,
        escrowHealed: Boolean(ensure.success && !ensure.alreadyEscrowed),
      }
    }
    ...
    const txid = verificationData?.txid || verificationData?.txId || payment.tx_id || null
    const idempotencyKey =
      verificationData?.idempotencyKey ||
      (txid && payment.booking_id ? `crypto_payment:${txid}:${payment.booking_id}` : null)
    const { data: updated, error } = await supabaseAdmin
      .from('payments')
      .update({
        // Stage 201.67 — DB enum has COMPLETED, not CONFIRMED
    ...
    if (error) {
      ...
      if (msg.includes('payments_tx_id_unique') || error.code === '23505') {
        return { success: false, error: 'already_processed', code: 'ALREADY_PROCESSED' }
      }
    ...
    if (!escrow?.success) {
      // Mid-flight: payment CONFIRMED, escrow pending — stamp for reconcile cron (C3.4)
      const { stampPaymentEscrowAttempt } = await import(
        '@/lib/payment/reconcile-confirmed-without-escrow.js'
      )
      await stampPaymentEscrowAttempt(updated, { error: escrow?.error || 'Escrow transition failed' })
