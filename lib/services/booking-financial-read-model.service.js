...
/**
 * SSOT order: booking.metadata.listing_category_slug (escrow snapshot) → listing.categories.slug.
 * @param {Record<string, unknown> | null | undefined} booking
 * @returns {string | null}
 */
export function categorySlugFromBookingFinancialRow(booking) {
  if (!booking || typeof booking !== 'object') return null
  const meta = booking.metadata && typeof booking.metadata === 'object' ? booking.metadata : {}
  const snap = meta.listing_category_slug
  if (typeof snap === 'string') {
    const t = snap.trim()
    if (t) return t
  }
  const listing = booking.listing
  const cats = listing && typeof listing === 'object' ? listing.categories : null
  if (cats && typeof cats === 'object') {
    const row = Array.isArray(cats) ? cats[0] : cats
...
export function buildBookingFinancialSnapshotFromRow(booking) {
  ...
  /** Partner-facing line (SSOT): listing subtotal vs platform take vs net to partner. */
  const gross = subtotalThb
  const net = partnerPayoutThb
  const fee = round2(gross - net)
  const category_slug = categorySlugFromBookingFinancialRow(booking)
