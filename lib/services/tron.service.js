...
/** @deprecated Prefer getCryptoReceiveWallet() */
export function getGostayloWallet() {
 ...
 */
export async function thbToUsdt(thbAmount) {
  const rate = await resolveThbPerUsdt();
  return Math.round((thbAmount / rate) * 100) / 100;
}
...
export async function verifyTronTransaction(txid, expectedAmountUsdt = null) {
  ...
  try {
    ...
    // Extract transaction details (amount null until explicitly parsed — AUDIT_03 C3.3)
    const txInfo = {
      hash: data.hash,
    ...
    if (data.contractType === 31 && data.trigger_info) {
      ...
      // Get transfer amount — only set when parse yields a finite number
      if (data.trigger_info.parameter?._value != null && data.trigger_info.parameter?._value !== '') {
        try {
          const rawAmount = BigInt(data.trigger_info.parameter._value);
          const parsed = Number(rawAmount) / Math.pow(10, txInfo.tokenInfo.decimals);
          txInfo.amount = Number.isFinite(parsed) ? parsed : null;
        } catch {
          txInfo.amount = null;
        }
      }
    ...
    const wallet = getCryptoReceiveWallet();
    const isToCorrectWallet = txInfo.toAddress?.toLowerCase() === wallet.toLowerCase();
    ...
    const amountResolved = Number.isFinite(txInfo.amount) && txInfo.amount > 0;

    // FULL AMOUNT VERIFICATION — fail-closed when expected is set but amount unresolved (incl. 0)
    let amountStatus = null;
    let amountDifference = 0;
    let amountPercentage = 100;

    if (hasExpected && !amountResolved) {
      amountStatus = 'AMOUNT_UNRESOLVED';
    } else if (hasExpected && amountResolved) {
      amountDifference = txInfo.amount - expected;
      amountPercentage = (txInfo.amount / expected) * 100;
      
      const minAcceptable = expected * (1 - AMOUNT_TOLERANCE);
      
      if (txInfo.amount < minAcceptable) {
        amountStatus = 'UNDERPAID';
      } else if (txInfo.amount >= expected) {
        amountStatus = 'FULL';
      } else {
    ...
    if (!isConfirmed) {
      ...
      status = 'WRONG_TOKEN';
    } else if (amountStatus === 'AMOUNT_UNRESOLVED') {
      status = 'AMOUNT_UNRESOLVED';
    ...
    return {
      ...
      status,
      ...(status === 'AMOUNT_UNRESOLVED' ? { error: 'AMOUNT_UNRESOLVED', reason: 'AMOUNT_UNRESOLVED' } : {}),
      data: {
        txid: txInfo.hash,
        ...
        expectedAmount: hasExpected ? expected : expectedAmountUsdt,
        ...
        isAmountSufficient: amountStatus !== 'UNDERPAID' && amountStatus !== 'AMOUNT_UNRESOLVED',
...
export async function verifyTransactionWithBooking(txid, bookingAmountThb) {
  const expectedUsdt = await thbToUsdt(bookingAmountThb);
...
export function generateTronPaymentUri(amountUsdt = null) {
  ...
  let uri = `tron:${getCryptoReceiveWallet()}`;
