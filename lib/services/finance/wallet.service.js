...
export class WalletService {
  ...
  static async getReferralWithdrawalPreview(userId, payoutCurrency = 'RUB', grossThbOverride = null) {
    ...
    if (gross < 0) gross = 0;
    const preview = await buildReferralWithdrawalPreview(gross, {
      payoutCurrency: 'RUB',
    });
    return {
      success: true,
  ...
  static async requestReferralWithdrawal(userId) {
    ...
    const withdrawalPreview = await buildReferralWithdrawalPreview(amountThb, { payoutCurrency: 'RUB' });
    ...
    const { data: updated, error } = await supabaseAdmin
      ...
      .eq('user_id', uid)
      .select(
        'id,user_id,withdrawable_balance_thb,referral_withdrawal_status,referral_withdrawal_requested_at,referral_withdrawal_amount_thb',
      )
      .maybeSingle();
    if (error) {
    ...
    return {
      ...
      data: {
        ...
        netInPayoutCurrency: withdrawalPreview.netInPayoutCurrency,
  ...
  static async attachReferralWithdrawalFields(walletRow) {
    ...
    const { data, error } = await supabaseAdmin
      .from('user_wallets')
      .select('referral_withdrawal_status,referral_withdrawal_requested_at,referral_withdrawal_amount_thb')
