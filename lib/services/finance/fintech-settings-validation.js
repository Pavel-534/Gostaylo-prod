...
import { validateGuestPoolSplit } from '@/lib/services/finance/guest-pool-split-validation.js'

export { validateGuestPoolSplit }
...
export const fintechSettingsPutSchema = z
  .object({
    ...
    insurance_fund_percent: percent0to10.optional(),
    ...
    ambassador_guest_pool_l3_percent: percent0to100.optional(),
    ...
    ambassador_guest_l3_enabled: z.boolean().optional(),
    ambassador_guest_l3_min_direct_partners: z.coerce.number().int().min(0).max(10_000).optional(),
    ambassador_guest_l3_max_thb_per_booking: thbNonNeg.optional(),
    ambassador_guest_l3_max_thb_per_month: thbNonNeg.optional(),
...
export function mergeFintechPatch(current, patch) {
  const map = {
    ...
    ambassador_guest_pool_l3_percent: 'ambassadorGuestPoolL3Percent',
    ...
    ambassador_guest_l3_enabled: 'ambassadorGuestL3Enabled',
    ambassador_guest_l3_min_direct_partners: 'ambassadorGuestL3MinDirectPartners',
    ambassador_guest_l3_max_thb_per_booking: 'ambassadorGuestL3MaxThbPerBooking',
    ambassador_guest_l3_max_thb_per_month: 'ambassadorGuestL3MaxThbPerMonth',
...
export function validateWaterfallEnvelope(policy) {
  ...
  const pool = validateGuestPoolSplit(policy)
...
export function validateFintechSettingsUpdate(body, currentConfig) {
  ...
  const acknowledgeDangerousAcquiringZero = raw.acknowledgeDangerousAcquiringZero === true
  const acknowledgeHighReinvestment = raw.acknowledgeHighReinvestment === true
  const { acknowledgeDangerousAcquiringZero: _a, acknowledgeHighReinvestment: _b, ...patchBody } = raw

  const parsed = fintechSettingsPutSchema.safeParse(patchBody)
  if (!parsed.success) {
    return {
      ok: false,
  ...
  if (
    parsed.data.acquiring_fee_percent !== undefined &&
    Number(merged.acquiringFeePercent) === 0 &&
    merged.ambassador3WaterfallEnabled !== false
  ) {
    if (!acknowledgeDangerousAcquiringZero) {
      return {
        ok: false,
        error: 'DANGEROUS_ACQUIRING_ZERO',
        message:
          'Эквайринг 0% при включённом Ambassador waterfall завышает adjusted net. Подтвердите acknowledgeDangerousAcquiringZero или верните ≥ 4.3%.',
  ...
  if (
    parsed.data.referral_reinvestment_percent !== undefined &&
    Number(merged.referralReinvestmentPercent) > 90 &&
    !acknowledgeHighReinvestment
  ) {
    return {
      ok: false,
      error: 'HIGH_REINVESTMENT',
      message:
        'Reinvestment >90% оставит платформе <10% net. Подтвердите acknowledgeHighReinvestment или снизьте процент.',
      requiresAcknowledgement: true,
    }
  }
...
export function policyToDbRow(policy) {
  return {
    ...
    insurance_fund_percent: policy.insuranceFundPercent,
    ...
    ambassador_guest_pool_l3_percent: policy.ambassadorGuestPoolL3Percent,
    ...
    ambassador_guest_l3_enabled: policy.ambassadorGuestL3Enabled,
    ambassador_guest_l3_min_direct_partners: policy.ambassadorGuestL3MinDirectPartners,
    ambassador_guest_l3_max_thb_per_booking: policy.ambassadorGuestL3MaxThbPerBooking,
    ambassador_guest_l3_max_thb_per_month: policy.ambassadorGuestL3MaxThbPerMonth,
