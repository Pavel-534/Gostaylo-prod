...
import { resolveLatestPaymentCapturedAtForBooking } from '@/lib/services/booking/payment-captured-at.js';

/**
 * @param {string} bookingId
 * @param {Date} [cancelledAt]
 * @returns {Promise<
 *   | { ok: true; bookingId: string; refundGuestThb: number; percent: number; hoursBefore: number; policy: string; guestTotalThb: number; listingId: string | null; refundReason: string | null; gracePeriodActive: boolean }
...
export async function computeRefundEstimateForBooking(bookingId, cancelledAt = new Date()) {
  ...
  const paymentCapturedAt = await resolveLatestPaymentCapturedAtForBooking(booking.id);

  const { refundGuestThb, percent, hoursBefore, policy, refundReason, gracePeriodActive } =
