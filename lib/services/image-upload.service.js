/**
 * FunnyRent 2.1 - Image Upload Service
 * 
 * Features:
    ...
    try {
      ...
      const compressedFile = await compressImage(file, (p) => {
        if (onProgress) {
          ...
          const stepProgress = (p / 2) / total; // Compression is 50% of each file's progress
      ...
      const result = await uploadToStorage(compressedFile, listingId, (p) => {
        if (onProgress) {
          const baseProgress = (completed / total) * 100;
          const stepProgress = (50 + p / 2) / total; // Upload is the other 50%
          onProgress(Math.round(baseProgress + stepProgress));
        }
      });
