 ...
 *
 * Клиентское сжатие — SSOT: `lib/services/media/compress-image-browser.js` (профиль listing_photo).
 * Загрузка только через POST /api/v2/upload (service role на сервере).
 */

import { compressImageForBrowser } from '@/lib/services/media/compress-image-browser'
import { uploadViaApi, deleteViaApi } from '@/lib/storage/storage-upload.client'
import { STORAGE_BUCKETS } from '@/lib/storage/storage-buckets'

const BUCKET_NAME = STORAGE_BUCKETS.LISTING_IMAGES
export const REVIEW_IMAGES_BUCKET = 'review-images'

/** @deprecated Используйте `compressImageForBrowser` из `@/lib/services/media/compress-image-browser` с нужным профилем. */
...
export async function uploadToStorage(file, listingId, onProgress) {
  ...
  try {
    ...
    const json = await uploadViaApi(formData, { onProgress });
    ...
    return {
      ...
      thumbFileName: json.thumbPath || null,
      size: file.size,
    };
  } catch (error) {
...
export async function deleteFromStorage(fileUrl) {
  try {
    ...
    await deleteViaApi(BUCKET_NAME, path);
    return true;
  } catch (error) {
