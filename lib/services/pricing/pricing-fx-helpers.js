...
export async function getRawRateMap() {
  const { getMidMarketDisplayRateMap } = await import('@/lib/pricing/fx-display.js')
  return getMidMarketDisplayRateMap()
}

/**
 * THB-per-unit rate for bookings.price_paid / exchange_rate when pay ≠ THB.
 * pay=THB always returns 1 — cross-currency surcharge for THB pay lives in PricingEngine
 * (`fx_markup_thb` + `total_guest_brutto`), not in this rate (Stage 200.88).
 */
...
export async function convertThbToCurrencyRaw(amountThb, targetCurrency = 'THB', rawRateMap = null) {
  ...
  const rate = normalizeThbPerUnitRate(target, Number(map?.[target]))
  if (rate == null || rate <= 0) return thb
  return thb / rate
}
