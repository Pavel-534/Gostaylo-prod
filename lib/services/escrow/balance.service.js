...
import { sumPendingPartnerPayoutReserveThb } from '@/lib/partner/partner-payout-fx.js'
...
export async function getPartnerBalance(partnerId) {
  try {
    ...
    for (const b of bookingList) {
      ...
      } else if (st === BookingStatus.PAID_ESCROW || st === BookingStatus.CHECKED_IN) {
        totalEarnings += net
        totalCommission += comm
        escrowBalance += net
    ...
    const grossAvailableThb = Math.round(availableBalance * 100) / 100
    const pendingPayoutReserveThb = await sumPendingPartnerPayoutReserveThb(supabaseAdmin, partnerId)
    const netAvailableThb = Math.max(0, Math.round((grossAvailableThb - pendingPayoutReserveThb) * 100) / 100)
    ...
    return {
      ...
      balance: {
        ...
        grossAvailableBalanceThb: grossAvailableThb,
        pendingPayoutReserveThb,
        availableBalance: netAvailableThb,
        availableBalanceThb: netAvailableThb,
        /** Payout requests in flight — not escrow. Escrow lives in frozenBalanceThb only (Stage 201.06). */
        pendingPayouts: pendingPayoutReserveThb,
...
export async function getPartnerBalanceByCategory(partnerId) {
  ...
  for (const b of bookings || []) {
    ...
    const disputeFrozen = isBookingDisputePaymentFrozen(b, frozenSet)
    if (b.status === BookingStatus.PAID_ESCROW || b.status === BookingStatus.CHECKED_IN) {
