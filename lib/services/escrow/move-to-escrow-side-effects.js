 ...
 * Stage 200.77 — PAYMENT_RECEIVED: claim → dispatch → mark; clear claim on hard fail.
 */
import { randomUUID } from 'node:crypto'
import { supabaseAdmin } from '@/lib/supabase'
import { NotificationService, NotificationEvents } from '../notification.service.js'
import { maybeNotifyReferralFriendPaid } from '@/lib/services/marketing/referral-friend-notify-triggers.js'
...
/** Stale in-flight claim may be re-taken (crash mid-dispatch). */
const PAYMENT_RECEIVED_CLAIM_TTL_MS = 10 * 60 * 1000

function readBookingMeta(rowOrMeta) {
  const meta =
    rowOrMeta?.metadata && typeof rowOrMeta.metadata === 'object' && !Array.isArray(rowOrMeta.metadata)
      ? rowOrMeta.metadata
      : rowOrMeta && typeof rowOrMeta === 'object' && !Array.isArray(rowOrMeta) && !rowOrMeta.metadata
        ? rowOrMeta
        : {}
  return meta && typeof meta === 'object' && !Array.isArray(meta) ? meta : {}
}

function readEscrowEffects(meta) {
  const effects = meta?.escrow_side_effects
  return effects && typeof effects === 'object' && !Array.isArray(effects) ? effects : {}
}
...
export function wasPaymentReceivedNotified(meta) {
  const effects = readEscrowEffects(readBookingMeta(meta))
  return Boolean(effects.payment_received_at)
}

/**
 * Soft CAS: write claim id, then re-read to confirm we own the slot.
 * @param {string} bookingId
 * @returns {Promise<{ claimed: boolean, claimId?: string, meta?: object, reason?: string }>}
 */
export async function claimPaymentReceivedNotify(bookingId) {
  if (!supabaseAdmin || !bookingId) return { claimed: false, reason: 'no_admin' }
  ...
  const { data: row } = await supabaseAdmin
    ...
    .eq('id', bookingId)
    .maybeSingle()
  const meta = readBookingMeta(row)
  if (wasPaymentReceivedNotified(meta)) return { claimed: false, reason: 'already_notified' }

  const effects = readEscrowEffects(meta)
  const existingClaimId = effects.payment_received_claim_id
  const claimedAtMs = effects.payment_received_claimed_at
    ? Date.parse(String(effects.payment_received_claimed_at))
    : NaN
  if (
    existingClaimId &&
    Number.isFinite(claimedAtMs) &&
    Date.now() - claimedAtMs < PAYMENT_RECEIVED_CLAIM_TTL_MS
  ) {
  ...
  const nextEffects = {
    ...effects,
    payment_received_claim_id: claimId,
    payment_received_claimed_at: new Date().toISOString(),
  }
  delete nextEffects.payment_received_claim_error

  await supabaseAdmin
    .from('bookings')
    .update({
      metadata: { ...meta, escrow_side_effects: nextEffects },
  ...
  const { data: again } = await supabaseAdmin
    ...
    .eq('id', bookingId)
    .maybeSingle()
  const liveMeta = readBookingMeta(again)
  if (wasPaymentReceivedNotified(liveMeta)) return { claimed: false, reason: 'already_notified' }
  const liveClaim = readEscrowEffects(liveMeta).payment_received_claim_id
  if (liveClaim !== claimId) return { claimed: false, reason: 'lost_race' }
  return { claimed: true, claimId, meta: liveMeta }
}

/**
 * Allow reconcile retry after hard dispatch failure.
 * @param {string} bookingId
 * @param {string} claimId
 */
export async function clearPaymentReceivedClaim(bookingId, claimId) {
  if (!supabaseAdmin || !bookingId || !claimId) return
  const { data: row } = await supabaseAdmin
    .from('bookings')
    .select('metadata')
    .eq('id', bookingId)
    .maybeSingle()
  const meta = readBookingMeta(row)
  if (wasPaymentReceivedNotified(meta)) return
  const effects = readEscrowEffects(meta)
  if (effects.payment_received_claim_id !== claimId) return
  const nextEffects = { ...effects }
  delete nextEffects.payment_received_claim_id
  delete nextEffects.payment_received_claimed_at
  nextEffects.payment_received_claim_error = new Date().toISOString()
  await supabaseAdmin
    .from('bookings')
    .update({
      metadata: { ...meta, escrow_side_effects: nextEffects },
...
export async function markPaymentReceivedNotified(bookingId, priorMeta = {}) {
  ...
  delete effects.payment_received_claim_id
  delete effects.payment_received_claimed_at
  delete effects.payment_received_claim_error
  await supabaseAdmin
    .from('bookings')
    .update({
...
export async function ensureEscrowDownstreamEffects(ctx) {
  ...
  if (!reconcileOnly || !wasPaymentReceivedNotified(meta)) {
    let claimId = null
    try {
      const claim = await claimPaymentReceivedNotify(bookingId)
      if (!claim.claimed) {
        if (claim.reason === 'already_notified') {
          /* idempotent skip */
        } else if (claim.reason === 'in_flight' || claim.reason === 'lost_race') {
          console.warn('[ESCROW] PAYMENT_RECEIVED claim skipped', bookingId, claim.reason)
        }
      } else {
        ...
        await markPaymentReceivedNotified(bookingId, claim.meta || meta)
        ran.push('payment_received')
        void maybeNotifyReferralFriendPaid({ booking, listing, payment })
      }
    } catch (e) {
      console.error('[ESCROW] PAYMENT_RECEIVED dispatch', e)
      try {
        await clearPaymentReceivedClaim(bookingId, claimId)
      } catch (clearErr) {
        console.error('[ESCROW] PAYMENT_RECEIVED clear claim', clearErr)
      }
    }
  }
