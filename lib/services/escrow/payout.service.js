...
export async function requestPayout(partnerId, amount, details) {
  try {
    ...
    const grossAvailableThb = balance?.grossAvailableBalanceThb ?? 0
    ...
    const methodId = payoutProfile?.method_id || payoutMethod?.id || null
    // AUDIT_03 W3.10 — ON DELETE SET NULL must not create fee=0 / null-rail payouts
    if (!methodId) {
      return {
        success: false,
        error: 'Payout method required',
        code: 'PAYOUT_METHOD_REQUIRED',
        i18nKey: 'partnerPayout_methodRequired',
      }
    }
    ...
    // AUDIT_03 C3.5: atomic claim under advisory lock (gross available − open reserves).
    // Do not debit profiles.available_balance_thb — that column is a sync cache only.
    const { data: claimRows, error: claimErr } = await supabaseAdmin.rpc(
      'insert_partner_host_payout_if_available',
      {
        p_partner_id: partnerId,
        p_gross_available_thb: grossAvailableThb,
        p_request_gross_thb: payoutMath.baseAmountThb,
        p_row: {
      ...
      return {
        ...
        error:
          reason === 'INSUFFICIENT_BALANCE'
            ? `Insufficient balance. Available: ฿${claim?.available_thb ?? availableThb}`
            : reason,
        code: reason === 'INSUFFICIENT_BALANCE' ? 'INSUFFICIENT_BALANCE' : reason,
        availableThb: claim?.available_thb ?? availableThb,
      }
    }

    const payout = claim.payout && typeof claim.payout === 'object' ? claim.payout : { id: claim.payout_id }
