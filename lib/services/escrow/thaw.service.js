...
import { isEscrowThawSourceStatus, ESCROW_THAW_SOURCE_STATUSES } from '@/lib/booking/status-sets.js'
import { readBookingFinancialSnapshot } from '@/lib/services/booking-financial-read-model.service'

export { isEscrowThawSourceStatus }
...
export async function thawBookingToThawed(bookingId) {
  const { data: booking, error: fetchError } = await supabaseAdmin
    ...
    .eq('id', bookingId)
    .maybeSingle()
  if (fetchError || !booking || !isEscrowThawSourceStatus(booking.status)) {
    return { success: false, error: 'not_eligible' }
  }
  const now = new Date().toISOString()
  const meta = booking.metadata && typeof booking.metadata === 'object' ? booking.metadata : {}
  const statusRes = await transitionBookingStatus(bookingId, BookingStatus.THAWED, {
    scope: 'system',
    actorContext: { trigger: 'escrow_thaw' },
    metadata: { updatedAt: now },
    extraPatch: { metadata: { ...meta, escrow_thawed_at: now } },
  })
...
export async function backfillMissingEscrowThawAt(limit = 300) {
  const { data: rows, error } = await supabaseAdmin
    .from('bookings')
    .select('id, check_in, metadata, listing:listings(category_id, metadata)')
    .in('status', [...ESCROW_THAW_SOURCE_STATUSES])
  ...
  for (const b of rows) {
    ...
    const thawAt = computeEscrowThawAt({
      ...
      listingId: b.listing_id || b.listing?.id || null,
    })
