 ...
 */
/**
 * Stage 201.86 — map Concierge amenities (boolean map or string[]) → guest `metadata.amenities` slugs.
 * @param {unknown} raw
 * @returns {string[]}
 */
export function conciergeAmenitiesToSlugList(raw) {
...
export function validateConciergeListingItem(item) {
  ...
  const countryCode = String(geo.countryCode || geo.country_code || '')
    .trim()
    .toUpperCase()
  const cityCode = String(geo.cityCode || geo.city_code || '')
    .trim()
    .toLowerCase()
  if (countryCode) geo.countryCode = countryCode
  if (cityCode) geo.cityCode = cityCode

  const amenities = conciergeAmenitiesToSlugList(item.amenities)
  ...
  try {
    ...
    for (const item of validated) {
      ...
      const cityCode = String(geo.cityCode || geo.city_code || '')
        .trim()
        .toLowerCase()
      const amenitySlugs = Array.isArray(item.amenities)
        ? item.amenities.map((x) => String(x || '').trim()).filter(Boolean)
        : conciergeAmenitiesToSlugList(item.amenities)
      ...
      const metadata = {
        ...
        ...(amenitySlugs.length ? { amenities: amenitySlugs } : {}),
        ...(countryCode ? { geo_pin_country: countryCode } : {}),
