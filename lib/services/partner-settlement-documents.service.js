 ...
 */
export async function listPartnerSettlementDocuments(partnerId, opts = {}) {
  if (!supabaseAdmin || !partnerId) {
    return { success: false, error: 'invalid_input', rows: [] }
  }

  const scopeBatchId = String(opts.batchId || '').trim() || null
  const rows = []
  const seen = new Set()
  ...
  if (scopeBatchId) batchIds.add(scopeBatchId)

  const { data: batchItems, error: itemsErr } = await supabaseAdmin
  ...
  if (itemsErr) {
    return { success: false, error: itemsErr.message, rows: [] }
  }
  ...
  if (batchIds.size) {
    ...
    if (bErr) {
      return { success: false, error: bErr.message, rows: [] }
    }
