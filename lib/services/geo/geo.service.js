...
async function suggestFromCatalog(query, countryCode) {
  ...
  const matched = (rows || []).filter((row) => {
    const hay = [row.label_en, row.label_ru, row.label_th, row.label_zh, row.code]
      .filter(Boolean)
      .join(' ')
    return haystackMatchesGeoQuery(hay, q)
  })
  ...
  try {
    ...
    const synHit = (synRows || []).find((r) => {
      const alias = String(r.alias_term || '')
      return haystackMatchesGeoQuery(alias, q)
    })
 ...
 * @param {{ countryCode?: string|null, state?: string|null, city?: string|null, lang?: string }} p
 */
async function matchCatalogCodes({ countryCode, state, city, lang }) {
  const iso = String(countryCode || '').toUpperCase()
  const uiLang = normalizeNominatimLang(lang)
  let region_code = null
  let city_code = null
  let city_name = city || null
  let region_label = null

  if (!supabaseAdmin || !iso) {
    return { country_code: iso || null, region_code, city_code, city_name, region_label }
  }

  const { data: cities } = await supabaseAdmin
    .from('geo_locations')
    .select('code, label_en, label_ru, label_th, label_zh, parent_code, level')
    .eq('country_code', iso)
    .eq('is_active', true)
  ...
  if (needle && cities?.length) {
    ...
    if (hit) {
      ...
      // Stage 200.84 — display label follows UI lang (not hard label_en)
      city_name = geoRowLabelForLang(hit, uiLang) || city_name
    }
  }
  ...
  if (!region_code && state) {
    ...
    const { data: regions } = await supabaseAdmin
      .from('geo_locations')
      .select('code, label_en, label_ru, label_th, label_zh')
      .eq('country_code', iso)
      .eq('level', 'region')
      .eq('is_active', true)
    ...
    if (rh) {
      region_code = rh.code
      region_label = geoRowLabelForLang(rh, uiLang)
    }
  }

  if (region_code && !region_label) {
    const { data: regionRow } = await supabaseAdmin
      ...
      .eq('level', 'region')
      .maybeSingle()
    if (regionRow) region_label = geoRowLabelForLang(regionRow, uiLang)
  }
...
export const GeoService = {
   ...
   * Stage 200.84 — `opts.lang` drives Nominatim Accept-Language + cache key.
   * @param {number} lat
   * @param {number} lng
   * @param {{ lang?: string }} [opts]
   */
  async resolveFromPin(lat, lng, opts = {}) {
    ...
    const lang = normalizeNominatimLang(opts.lang)
    const qHash = hashKey('reverse', `${lang}|${latN.toFixed(5)},${lngN.toFixed(5)}`)
    ...
    return {
      ...
      region_label: matched.region_label || mapped.state || null,
   ...
   * Stage 200.84 — lang + optional city viewbox (bounded street suggest).
   * @param {string} query
   * @param {string} [countryCode]
   * @param {{ lang?: string, viewbox?: string|null, bounded?: boolean }} [opts]
   */
  async resolveFromQuery(query, countryCode, opts = {}) {
    const q = String(query || '').trim()
    if (q.length < 3) {
    ...
    const cc = countryCode ? String(countryCode).toUpperCase().slice(0, 2) : ''
    const lang = normalizeNominatimLang(opts.lang)
    const viewbox = String(opts.viewbox || '').trim() || ''
    const bounded = opts.bounded === true && Boolean(viewbox)

    const catalog = await suggestFromCatalog(q, cc || undefined)
    const searchQ = pickGeocodeSearchQuery(q, lang)

    const qHash = hashKey(
      'search',
      `${cc}|${lang}|${viewbox}|${bounded ? '1' : '0'}|${searchQ.toLowerCase()}`,
    )
    let raw = await readCache(qHash)
    let nominatim = []
