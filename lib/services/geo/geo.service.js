...
function mapReverseResult(data) {
  ...
  const district =
    ...
    addr.county ||
    ''
  const city = addr.city || addr.municipality || addr.town || addr.village || addr.state || ''
...
export const GeoService = {
  ...
  async upsertProvisionalLocation({
    ...
    const base = slugifyCode(name) || `place-${Date.now()}`
    let code = base
    ...
    const row = {
      ...
      level,
      parent_code,
      label_en: String(name || code).slice(0, 120),
      label_ru: String(name || code).slice(0, 120),
      iso_country: iso,
      country_code: iso,
      centroid_lat: Number.isFinite(Number(lat)) ? Number(lat) : null,
      centroid_lng: Number.isFinite(Number(lng)) ? Number(lng) : null,
      timezone: timezone || ct.timezone,
  ...
  async getCurrencyAndTimezone(countryCode) {
    ...
    const fallback = COUNTRY_CURRENCY_TZ[iso] || { currency: 'THB', timezone: 'Asia/Bangkok' }
