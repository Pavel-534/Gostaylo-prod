...
export async function sendEmail({ to, subject, template, data }) {
  ...
  if (template) {
    if (template === 'booking-request-email') {
      const pct = Number.isFinite(Number(data.commissionRate))
        ? Number(data.commissionRate)
        : await resolveDefaultCommissionPercent()
      const ratio = pct / 100
      const total = Number(data.totalPrice) || 0
      const commission =
        data.commission != null ? data.commission : Math.round(total * ratio)
      const partnerEarnings =
        data.partnerEarnings != null ? data.partnerEarnings : Math.round(total * (1 - ratio))
  ...
  try {
    ...
    if (IS_PRODUCTION) {
      ...
      void notifySystemAlert(
        `📧 <b>Resend: сбой отправки</b> (lib/mail)\n` +
...
async function sendEmailInternal({ to, subject, react }) {
  ...
  try {
    ...
    if (IS_PRODUCTION) {
      const toHint = Array.isArray(to) ? to.join(', ') : String(to)
      void notifySystemAlert(
        `📧 <b>Resend: сбой</b> (lib/mail internal)\n` +
          `subject: <code>${escapeSystemAlertHtml(subject)}</code>\n` +
          `to: <code>${escapeSystemAlertHtml(toHint.slice(0, 120))}</code>\n` +
          `<code>${escapeSystemAlertHtml(error?.message || error)}</code>`,
      )
