...
/**
 * Sync read of cached rateMap for TanStack Query placeholder / ambassador bootstrap.
 * Mid query may fall back to retail bundle until mid bundle is warm.
 */
export function readExchangeRatesCacheSnapshot(options = {}) {
  const retail = options.retail !== false
  const allowRetailFallbackForMid = options.allowRetailFallbackForMid === true
  if (typeof window === 'undefined') return null

  const readBundle = (bundleKey) => {
    const cached = getCache(bundleKey)
    if (cached?.rateMap && isCompleteDisplayRateMap(cached.rateMap)) {
      return { THB: 1, ...cached.rateMap }
    }
    return null
  }

  const primaryKey = retail ? 'exchange_rates_retail_bundle' : 'exchange_rates_mid_bundle'
  const primary = readBundle(primaryKey)
  if (primary) return primary

  if (!retail && allowRetailFallbackForMid) {
    return readBundle('exchange_rates_retail_bundle')
  }
