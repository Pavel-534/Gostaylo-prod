/**
 * Stage 124.20 — MIR / ЮKassa smoke: initiate (RUB) → mock webhook → PAID_ESCROW + ledger checks.
 */
...
/** ЮKassa credentials required (no real charge — mock webhook only). */
export function isYookassaMirSmokeConfigured() {
  ...
  return Boolean(shopId && secretKey && webhookSecret)
}

export function yookassaMirSmokeSkipReason() {
  return 'SKIP: задайте YOOKASSA_SHOP_ID, YOOKASSA_SECRET_KEY и YOOKASSA_WEBHOOK_SECRET (или PAYMENT_ACQUIRING_WEBHOOK_SECRET)'
}
...
export async function runCheckoutMirEscrowStep({
  ...
  if (!isYookassaMirSmokeConfigured()) {
