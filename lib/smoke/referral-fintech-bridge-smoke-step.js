/**
 * Stage 131.6 — smoke: referral FinTech Bridge (withdrawal request → bulk approve → payouts + T-Bank registry).
 */
...
export async function runReferralFintechBridgeSmokeStep() {
  ...
  try {
    ...
    const request = await WalletService.requestReferralWithdrawal(userId)
    if (!request.success) {
      await cleanup(userId, null, profileId)
      return fail(s, `request: ${request.error || 'REQUEST_FAILED'}`, t0)
    }

    const preview = await buildReferralWithdrawalPreview(GROSS_THB, { payoutCurrency: 'RUB' })
    ...
    const debit = await WalletService.debitReferralWithdrawalPayout(userId, GROSS_THB, {
      ...
      net_paid_rub: bridge.data?.netRub ?? preview.netInPayoutCurrency,
      payout_currency: 'RUB',
    ...
    const expectedRub = round2(bridge.data?.netRub ?? preview.netInPayoutCurrency)
    ...
    return pass(
      s,
      `PENDING referral_withdrawal rail=${PAYOUT_RAIL.REFERRAL_RUB_CARD} rub=${actualRub} registry=OK`,
      t0,
    )
  } catch (e) {
    await cleanup(userId, payoutId, profileId)
