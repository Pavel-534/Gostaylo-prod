...
async function insertSmokeBooking({ bookingId, renterId, partnerId, completedAt, listingId }) {
...
/**
 * @param {{ renterId?: string, partnerId?: string, listingId?: string }} [options]
...
export async function runReferralHoldSmokeStep(options = {}) {
  ...
  const t0 = Date.now();
  const listingId = String(options.listingId || STAGE72_REUSE_LISTING_ID).trim() || STAGE72_REUSE_LISTING_ID;
  ...
  try {
    ...
    await insertSmokeBooking({ bookingId, renterId: refereeId, partnerId: referrerId, completedAt, listingId });
    ...
    const heldRow = await withSmokeRetry(
      async () => {
        const { data, error } = await supabaseAdmin
          ...
          .eq('id', ledgerId)
          .maybeSingle();
        if (error) throw new Error(error.message || 'held row read failed');
        if (!data) throw new Error('fetch failed: held row not visible yet');
        return data;
      },
      { label: '12d-held-row-read' },
    );
    ...
    const earnedRow = await withSmokeRetry(
      async () => {
        const { data, error } = await supabaseAdmin
          .from('referral_ledger')
          .select('status')
          .eq('id', ledgerId)
          .maybeSingle();
        if (error) throw new Error(error.message || 'earned row read failed');
        if (!data) throw new Error('fetch failed: earned row not visible yet');
        return data;
      },
      { label: '12d-earned-row-read' },
    );
