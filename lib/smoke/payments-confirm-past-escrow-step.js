...
export async function runPaymentsConfirmPastEscrowStep({ bookingId, guestTotalThb }) {
  ...
  const doneRes = await smokeTransitionBookingStatus(id, 'COMPLETED', {
    scope: 'system',
    trigger: 'smoke_payments_confirm_past_escrow_completed',
    metadata: { updatedAt: completedAt },
    extraPatch: { completed_at: completedAt },
  })
  if (!doneRes.success) return { ok: false, error: doneRes.error || 'COMPLETED transition failed' }
