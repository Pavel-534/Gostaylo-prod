...
import { ensureSmokeListingExists } from '@/lib/smoke/ensure-smoke-listing.js'
...
/**
 * @param {{ listingId?: string, partnerId?: string, categoryId?: string | null }} [options]
 * @returns {Promise<{ name: string, ok: boolean, detail: string, durationMs: number }>}
 */
export async function runReferralFraudGateSmokeStep(options = {}) {
  ...
  const listingId =
    String(options.listingId || '').trim() || makeId('lst-smoke-1317')
  ...
  try {
    ...
    await withSmokeRetry(
      () =>
        ensureSmokeListingExists({
          listingId,
          partnerId: String(options.partnerId || '').trim() || referrerId,
          categoryId: options.categoryId ?? null,
          tag: `${E2E_TEST_DATA_TAG} smoke_131_7_fraud`,
        }),
      { label: '12g listing ensure' },
    )
    await withSmokeRetry(
      () =>
        insertSmokeBooking({
