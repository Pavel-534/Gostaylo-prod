...
export async function runReferralFraudGateSmokeStep() {
  ...
  try {
    await insertSmokeProfiles(referrerId, refereeId)
    await insertSmokeBooking({ bookingId, renterId: refereeId, partnerId: referrerId })
    ...
    const marked = await ReferralLedgerService.markPendingAsEarned(bookingId, { referralHoldDays: 0 })
    ...
    const walletSummary = await WalletService.getWalletSummary(referrerId)
