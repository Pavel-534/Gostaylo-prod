...
export async function runAudit02RegressionSmoke(opts = {}) {
  ...
  try {
    ...
    if (!opts.skipCleanup) {
      ...
      // Orphan referral rows (booking cleanup above misses earned rows when booking already gone).
      await supabaseAdmin.from('referral_ledger').delete().ilike('referrer_id', 'user-smoke-a02-%')
      await supabaseAdmin.from('referral_ledger').delete().ilike('referee_id', 'user-smoke-a02-%')
    }
    pass(sClean, 'done', t0)
    ...
    const audit02BookingIds = new Set([bookingInvoice, bookingOk2, bookingFail])
    const inBatch = new Set((pack0?.items || []).map((i) => i.booking_id))
    for (const bid of audit02BookingIds) {
    ...
    // Isolate batch to audit02 fixtures only (staging pool may contain unrelated READY rows).
    const { data: extraItems } = await supabaseAdmin
      .from('payout_batch_items')
      .select('id, booking_id')
      .eq('batch_id', batchId)
    for (const row of extraItems || []) {
      if (audit02BookingIds.has(row.booking_id)) continue
      await supabaseAdmin.from('payout_batch_items').delete().eq('id', row.id)
    }
    ...
    t0 = Date.now()
    const sPartial = step('7. Partial settle (break item #3 amount_thb → ledger fail)')
    steps.push(sPartial)
    const { data: failItem } = await supabaseAdmin
      .from('payout_batch_items')
      .select('id, partner_id, amount_thb')
      .eq('batch_id', batchId)
    ...
    const { error: breakErr } = await supabaseAdmin
      .from('payout_batch_items')
      .update({ amount_thb: 0, updated_at: new Date().toISOString() })
      .eq('id', failItem.id)
    if (breakErr) {
      fail(sPartial, `break item update: ${breakErr.message}`, t0)
      return finalize(steps, context, runStartedAt)
    }
    ...
    t0 = Date.now()
    const sRepair = step('8. Repair settle (restore amount_thb; idempotent journals)')
    steps.push(sRepair)
    const { error: restoreErr } = await supabaseAdmin
      .from('payout_batch_items')
      .update({ amount_thb: partnerNet, updated_at: new Date().toISOString() })
      .eq('id', failItem.id)
    if (restoreErr) {
      fail(sRepair, `restore item update: ${restoreErr.message}`, t0)
      return finalize(steps, context, runStartedAt)
    }
    ...
    if (!repaired?.success) {
      fail(
        sRepair,
        `${repaired?.error || 'repair_failed'}: ${repaired?.message || ''} ${
          repaired?.ledgerErrors ? JSON.stringify(repaired.ledgerErrors).slice(0, 200) : ''
        } ${repaired?.completedErrors ? JSON.stringify(repaired.completedErrors).slice(0, 200) : ''}`.trim(),
        t0,
      )
      return finalize(steps, context, runStartedAt)
    }
