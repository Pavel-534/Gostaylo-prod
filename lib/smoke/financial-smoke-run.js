...
const STAGE152_CAP_LISTING_ID = 'lst-stage152-cap-smoke'
...
export async function runFinancialSmoke(opts = {}) {
  ...
  try {
    ...
    steps.push(sClean)
    if (!opts.skipCleanup) {
      for (const lid of [LISTING_ID, LEGACY_LISTING_ID, STAGE149_RPC_LISTING_ID, STAGE152_CAP_LISTING_ID]) {
    ...
    // SMOKE: legal system FSM PAID_ESCROW → THAWED (prod: escrow-thaw cron / thaw.service)
    const thawRes = await smokeTransitionBookingStatus(bookingId, 'THAWED', {
      scope: 'system',
      trigger: 'smoke_stage104_force_thaw',
      extraPatch: {
    ...
    if (!thawRes.success) {
      fail(sThaw, thawRes.error || 'THAWED transition failed', t0)
      return finalize(steps, context, runStartedAt)
    }
    ...
    const reThaw = await smokeForceBookingStatusNegativeTest(
      bookingId,
      'THAWED',
      'negative_test:restore_thawed_after_completed_webhook_probe',
    )
    if (!reThaw.success) {
      fail(sPastEscrowWebhook, `restore THAWED after COMPLETED probe: ${reThaw.error}`, t0)
      return finalize(steps, context, runStartedAt)
    }
    ...
    if (bReady?.status !== 'READY_FOR_PAYOUT') {
      // Fallback via FSM if promote cron path missed — prod: promote-ready-for-payout cron
      const readyRes = await smokeTransitionBookingStatus(bookingId, 'READY_FOR_PAYOUT', {
        scope: 'system',
        trigger: 'smoke_stage104_force_ready',
        extraPatch: {
          metadata: { ready_for_payout_at: new Date().toISOString(), smoke_stage103: true },
        },
      })
      if (!readyRes.success) {
        fail(sReady, readyRes.error || 'READY_FOR_PAYOUT transition failed', t0)
        return finalize(steps, context, runStartedAt)
      }
    }
    ...
    t0 = Date.now()
    const sStage152Cap = step('12h. Stage 152 calendar capacity + checkout-hold TTL')
    steps.push(sStage152Cap)
    try {
      const { runStage152CalendarConcurrencySmokeStep } = await import(
        '@/lib/smoke/stage152-calendar-concurrency-smoke-step.js'
      )
      const capStep = await runStage152CalendarConcurrencySmokeStep({
        partnerId,
        guestId,
        categoryId: context.categoryId,
        tag: TAG,
      ...
      if (capStep.ok) {
        pass(sStage152Cap, capStep.detail, t0)
      } else {
        fail(sStage152Cap, capStep.detail, t0)
      }
    } catch (e) {
      fail(sStage152Cap, e?.message || String(e), t0)
    }

    t0 = Date.now()
    const sStage153Audit = step('12i. Stage 153 admin audit idempotency intercept')
    steps.push(sStage153Audit)
    try {
      const { runStage153AdminAuditIdempotencySmokeStep } = await import(
        '@/lib/smoke/stage153-admin-audit-idempotency-smoke-step.js'
      )
      const auditStep = await runStage153AdminAuditIdempotencySmokeStep()
      if (auditStep.ok) {
        pass(sStage153Audit, auditStep.detail, t0)
      } else {
        fail(sStage153Audit, auditStep.detail, t0)
      }
    } catch (e) {
      fail(sStage153Audit, e?.message || String(e), t0)
    }

    t0 = Date.now()
    const sStage154Profile = step('12j. Stage 154 profile API anonymous guard')
    steps.push(sStage154Profile)
    try {
      const { runStage154ProfileAuthGuardSmokeStep } = await import(
        '@/lib/smoke/stage154-profile-auth-guard-smoke-step.js'
      )
      const profileGuard = await runStage154ProfileAuthGuardSmokeStep()
      if (profileGuard.ok) {
        pass(sStage154Profile, profileGuard.detail, t0)
      } else {
        fail(sStage154Profile, profileGuard.detail, t0)
      }
    } catch (e) {
      fail(sStage154Profile, e?.message || String(e), t0)
    }
    ...
    try {
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        attrStep = await runReferralAttributionSmokeStep({ categoryId: context.categoryId })
    ...
    const docs = await listPartnerSettlementDocuments(partnerId, { batchId })
    const count = docs.rows?.length || 0
    if (!docs.success || count < 1) {
      fail(sPartnerDocs, `найдено: ${count}${docs.error ? ` (${docs.error})` : ''}`, t0)
    } else {
