...
function applyCtxStringPlaceholders(str, ctx) {
  ...
  if (ctx.minAmount != null && String(ctx.minAmount).length) {
    out = out.replace(/\{minAmount\}/g, String(ctx.minAmount))
  }
  if (ctx.currency != null && String(ctx.currency).length) {
    out = out.replace(/\{currency\}/g, String(ctx.currency))
  }
  if (ctx.code != null) {
  ...
  if (ctx.count != null) {
    out = out.replace(/\{count\}/g, String(ctx.count))
  }
  if (ctx.hours != null) {
    out = out.replace(/\{hours\}/g, String(ctx.hours))
  }
  if (ctx.used != null) {
    out = out.replace(/\{used\}/g, String(ctx.used))
  }
  // Generic fallback: replace any remaining {key} placeholders from ctx.
  if (out.includes('{')) {
...
function applyGuestBookingLabelPlaceholders(str, lang, ctx) {
  if (typeof str !== 'string' || !ctx || typeof ctx !== 'object') {
    return str
  }
  if (!ctx.listingCategorySlug && !ctx.wizardProfile) {
    return str
  }
 ...
 * @param {string} key
 * @param {string} [lang]
 * @param {{ listingCategorySlug?: string, welcomeThb?: string|number, welcomeApprox?: string, count?: string|number, total?: string|number }} [ctx] — vertical override или плейсхолдеры `{welcomeThb}` / `{count}` / `{total}` и т.д.
 */
