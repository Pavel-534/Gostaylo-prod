...
export function formatChatListingTimeFootnote(lang, district) {
  ...
  if (L === 'zh') {
    return place
      ? `时间为房源所在地（${place}，${tzShort}），非您设备本地时间。`
      : `时间为房源所在地（${tzShort}），非您设备本地时间。`
  }
  if (L === 'th') {
    return place
      ? `เวลาตามสถานที่ให้เช่า (${place}, ${tzShort}) ไม่ใช่เวลาบนอุปกรณ์ของคุณ`
      : `เวลาตามสถานที่ให้เช่า (${tzShort}) ไม่ใช่เวลาบนอุปกรณ์ของคุณ`
  }
  return place
    ? `Times are for the listing location (${place}, ${tzShort}), not your device clock.`
...
export function buildBookingEmailTimeZoneNote(lang, district) {
  ...
  if (L === 'zh') {
    return place
      ? `日期与时间均为房源所在地时间（${place}，${tzShort}），与您设备本地时间可能不同。`
      : `日期与时间为房源所在地时间（${tzShort}），与您设备本地时间可能不同。`
  }
  if (L === 'th') {
    return place
      ? `วันและเวลาตามสถานที่ให้เช่า (${place}, ${tzShort}) อาจไม่ตรงกับเวลาบนอุปกรณ์ของคุณ`
      : `วันและเวลาตามสถานที่ให้เช่า (${tzShort}) อาจไม่ตรงกับเวลาบนอุปกรณ์ของคุณ`
  }
  return place
