...
/** Подстановка белого лейбла в шаблоны с плейсхолдером `{brand}`. */
function brandIn(str) {
  return String(str).replace(/\{brand\}/g, getSiteDisplayName())
}
...
const ALT_PREFIX = {
  ru: {
    ...
    fallback: 'Фото жилья на {brand} для вашей поездки',
  },
  en: {
    withBoth: 'A welcoming photo of your stay —',
    withTitle: 'Photo of your stay —',
    fallback: 'Your {brand} stay photo — we’re glad you’re here',
  },
  zh: {
    withBoth: '您住宿的实景照片 —',
    withTitle: '住宿照片 —',
    fallback: '{brand} 为您准备的温馨住宿照片',
  },
  th: {
    withBoth: 'ภาพที่พักของคุณ —',
    withTitle: 'ภาพที่พัก —',
    fallback: 'ภาพที่พักจาก {brand} ขอให้ทริปสนุกนะคะ/ครับ',
  },
}
...
export function buildStayCalendarDescriptionI18n(payload, lang, absUrl) {
  const L = normalizeEmailLang(lang)
  const lines = [brandIn(CAL_DESC_LINE1[L] || CAL_DESC_LINE1.en)]
...
const CAL_DESC_LINE1 = {
  ru: 'Бронирование подтверждено в {brand}.',
  en: 'Your booking is confirmed on {brand}.',
  zh: '您的订单已在 {brand} 确认。',
  th: 'การจองของคุณได้รับการยืนยันบน {brand} แล้ว',
}
...
export function bookingConfirmedCopy(lang) {
  const L = normalizeEmailLang(lang)
  const row = BOOKING_CONFIRMED[L] || BOOKING_CONFIRMED.en
  return {
    ...row,
    escrowDefault: brandIn(row.escrowDefault),
  }
}
...
export function escrowEmailLine(lang) {
  const L = normalizeEmailLang(lang)
  const lines = {
    ru: '🔒 Ваши средства защищены эскроу {brand} до выполнения условий бронирования.',
    en: '🔒 Your funds are protected by {brand} escrow until the booking is fulfilled as agreed.',
    zh: '🔒 您的款项由 {brand} 托管，按订单约定完成后才会结算给合作伙伴。',
    th: '🔒 เงินของคุณได้รับการคุ้มครองด้วย {brand} Escrow จนกว่าการจองจะเป็นไปตามข้อตกลง',
  }
  return brandIn(lines[L] || lines.en)
}
...
const BOOKING_CONFIRMED = {
  ru: {
    ...
    escrowDefault: 'Средства удерживаются в эскроу {brand} до выполнения условий бронирования.',
  ...
  en: {
    ...
    escrowDefault: 'Funds are held in {brand} escrow until the booking is fulfilled.',
  ...
  zh: {
    ...
    escrowDefault: '款项由 {brand} 托管，按约定完成后才会结算给合作伙伴。',
    checkout: '去支付',
    chat: '打开聊天',
    profile: '个人资料',
  ...
  th: {
    ...
    escrowDefault: 'เงินจะถูกเก็บใน {brand} Escrow จนกว่าการจองจะเป็นไปตามข้อตกลง',
    checkout: 'ไปชำระเงิน',
    chat: 'เปิดแชท',
    profile: 'โปรไฟล์',
...
export function paymentSuccessCopy(lang) {
  const L = normalizeEmailLang(lang)
  const row = PAYMENT_SUCCESS[L] || PAYMENT_SUCCESS.en
  return {
    ...row,
    escrowDefault: brandIn(row.escrowDefault),
  }
...
const PAYMENT_SUCCESS = {
  ru: {
    ...
    escrowDefault: 'Средства защищены эскроу {brand}.',
  ...
  en: {
    ...
    escrowDefault: 'Your funds are protected by {brand} escrow.',
  ...
  zh: {
    ...
    escrowDefault: '您的款项由 {brand} 托管保护。',
    method: '支付方式',
    bookings: '我的预订',
    chat: '联系合作伙伴',
    profile: '个人资料',
  ...
  th: {
    ...
    escrowDefault: 'เงินของคุณได้รับการคุ้มครองด้วย {brand} Escrow',
    method: 'วิธีชำระเงิน',
