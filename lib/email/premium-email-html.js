 ...
 * SSOT chrome: lib/theme/constants.js + lib/email/email-brand-assets.js
 * Display name: getSiteDisplayName() (white-label).
 */

import { theme } from '@/lib/theme/constants'
import { getPublicSiteUrl, getSiteDisplayName } from '@/lib/site-url'
import {
  EMAIL_BRAND_LOCKUP_PUBLIC_PATH,
  EMAIL_BRAND_LOCKUP_WIDTH_PX,
} from '@/lib/email/email-brand-assets.js'
...
/** Absolute URL for the email header lockup (PNG). */
export function emailBrandLockupUrl() {
  return absUrl(EMAIL_BRAND_LOCKUP_PUBLIC_PATH)
}
...
export function emailHeaderRow() {
  ...
  const logoSrc = escapeHtml(emailBrandLockupUrl())
  const home = escapeHtml(absUrl('/'))
  const w = EMAIL_BRAND_LOCKUP_WIDTH_PX
  return `
<tr>
  <td style="padding:28px 32px 20px;text-align:center;border-bottom:1px solid ${colors.divider};">
    <a href="${home}" style="text-decoration:none;border:0;display:inline-block;">
      <img src="${logoSrc}" width="${w}" alt="${brand}"
        style="display:block;margin:0 auto;border:0;outline:none;text-decoration:none;width:${w}px;max-width:70%;height:auto;" />
    </a>
  </td>
</tr>`
}
...
export function emailFooterInsideCard() {
  ...
  const brand = escapeHtml(getSiteDisplayName())
  const home = escapeHtml(absUrl('/'))
  const help = escapeHtml(absUrl('/help'))
  return `
<tr>
  <td style="padding:24px 32px 28px;text-align:center;border-top:1px solid ${colors.divider};">
    <p style="margin:0 0 8px;font-size:12px;line-height:1.5;font-family:${fontFamilyEmail};">
      <a href="${home}" style="color:${colors.primary};text-decoration:none;font-weight:600;">${brand}</a>
      <span style="color:${colors.subtle};"> · </span>
      <a href="${help}" style="color:${colors.muted};text-decoration:none;">Help</a>
    </p>
    <p style="margin:0;color:${colors.subtle};font-size:12px;line-height:1.5;font-family:${fontFamilyEmail};">© ${y} ${brand}</p>
  </td>
</tr>`
}
...
export function emailListingCardBlock({
  ...
  const brandFallback = `${getSiteDisplayName()} — listing`
  const alt = escapeHtml(imageAlt || title || brandFallback)
  const imgRow = absImg
...
/** Caller must HTML-escape plain dynamic text; markup spans are allowed when inners are escaped. */
export function emailContentParagraph(htmlSafe) {
  return `
<tr>
  <td style="padding:0 32px 8px;">
    <p style="margin:0;font-size:16px;line-height:1.65;color:${colors.text};font-family:${fontFamilyEmail};">${htmlSafe}</p>
  </td>
</tr>`
}
