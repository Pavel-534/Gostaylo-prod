...
import { buildInvoicePaymentWindowSystemMessage } from '@/lib/booking/payment-window-system-message.js'
...
export async function executePostChatInvoice(request) {
  try {
    ...
    // AUDIT_03 W3.2 — platform min guest payable on chat invoice
    const adminOverrideMin =
      (body.adminOverrideMinTotal === true || body.admin_override_min_total === true) &&
      (accessRole === 'ADMIN' || accessRole === 'MODERATOR')
    if (!adminOverrideMin && amountThb < MIN_BOOKING_GUEST_TOTAL_THB) {
      return chatApiJson(
        {
          success: false,
          error: `Minimum invoice total is ${MIN_BOOKING_GUEST_TOTAL_THB} THB`,
          code: 'INVOICE_BELOW_MINIMUM',
          minThb: MIN_BOOKING_GUEST_TOTAL_THB,
        },
        422,
      )
    }
    ...
    const commission =
      partnerIdForFee && amountThb > 0
        ? await PricingService.calculateCommissionFromGuestPayable(amountThb, partnerIdForFee)
        : {
            commissionRate: 0,
            commissionThb: 0,
    ...
    if (intent !== 'extension') {
      ...
      if (!syncResult.ok) {
        logChatInvoiceGateEvent({
          level: 'error',
          status: syncResult.code === 'BOOKING_ALREADY_PAID' ? 409 : 500,
        ...
        if (syncResult.code === 'BOOKING_ALREADY_PAID') {
          return chatApiJson(
            {
              success: false,
              error: 'BOOKING_ALREADY_PAID',
              code: 'BOOKING_ALREADY_PAID',
              message: 'Cannot rewrite booking amounts after payment or while checkout is active',
            },
            409,
          )
        }
