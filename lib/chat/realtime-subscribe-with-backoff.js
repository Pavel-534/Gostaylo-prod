 ...
 * @param {number} [opts.minBackoffDelayMs] — нижняя граница паузы перед переподпиской (напр. 2000 для треда чата).
 */
    ...
    void (async () => {
      ...
      ch.subscribe((status) => {
        ...
        const delay = Math.max(
          Number(minBackoffDelayMs) || 0,
          realtimeBackoffDelayMs(attempt),
        )
        attempt += 1
