...
import { resolveShouldMaskMessageContacts } from '@/lib/chat/resolve-message-contact-mask.js'
...
export function mapApiMessageToRow(raw, opts = {}) {
  ...
  const shouldMask =
    !isStaff && !isOwnMessage && !noTextTypes.has(type)
      ? resolveShouldMaskMessageContacts({
          viewerRole,
          viewerUserId,
          senderId,
          bookingStatus,
...
export function mergeRealtimeMessage(prev, rawIncoming, opts = {}) {
  ...
  const next = [...prev]
  next[idx] = preservePaidInvoiceOnMerge(prev[idx], incoming)
  return next
}

/**
 * Не даём reload/resync откатить оптимистичный invoice PAID → PENDING.
 * @param {import('@/lib/chat/map-api-message').ChatMessage} priorMsg
 * @param {import('@/lib/chat/map-api-message').ChatMessage} incomingMsg
 */
function preservePaidInvoiceOnMerge(priorMsg, incomingMsg) {
  if (!priorMsg?.id || priorMsg.id !== incomingMsg?.id) {
    return { ...priorMsg, ...incomingMsg }
  }
  const priorPaid = String(priorMsg?.metadata?.invoice?.status || '').toUpperCase() === 'PAID'
  const incomingPaid = String(incomingMsg?.metadata?.invoice?.status || '').toUpperCase() === 'PAID'
  if (priorPaid && !incomingPaid) {
    return {
      ...incomingMsg,
      metadata: {
        ...incomingMsg.metadata,
        invoice: {
          ...(incomingMsg.metadata?.invoice || {}),
          ...(priorMsg.metadata?.invoice || {}),
          status: 'PAID',
        },
      },
    }
...
/**
 * Merge fetched thread rows with prior optimistic invoice PAID states.
 * @param {import('@/lib/chat/map-api-message').ChatMessage[]} prior
 * @param {import('@/lib/chat/map-api-message').ChatMessage[]} incoming
 * @returns {import('@/lib/chat/map-api-message').ChatMessage[]}
 */
export function mergeMessagesPreservingPaidInvoices(prior, incoming) {
  if (!Array.isArray(incoming) || incoming.length === 0) return prior || []
  const paidById = new Map()
  for (const m of prior || []) {
    if (String(m?.metadata?.invoice?.status || '').toUpperCase() === 'PAID' && m.id) {
      paidById.set(m.id, m.metadata.invoice)
    }
  }
  if (paidById.size === 0) return incoming
  return incoming.map((m) => {
    const preserved = paidById.get(m.id)
    if (!preserved) return m
    const serverSt = String(m.metadata?.invoice?.status || 'PENDING').toUpperCase()
    if (serverSt === 'PAID') return m
    return {
      ...m,
      metadata: {
        ...m.metadata,
        invoice: { ...(m.metadata?.invoice || {}), ...preserved, status: 'PAID' },
      },
    }
  })
}
