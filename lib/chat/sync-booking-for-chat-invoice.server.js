 ...
 * AUDIT_03 C3.9: do not rewrite amounts after escrow pipeline / while active payment intent exists.
 */
...
import { attestationRegateMetadataPatch } from '@/lib/booking/inquiry-attestation-regate.js'

/** Statuses where rewriting price_thb / pricing_snapshot would mutate paid money SSOT. */
export const CHAT_INVOICE_SYNC_BLOCKED_BOOKING_STATUSES = Object.freeze([
  ...ESCROW_PIPELINE_STATUSES,
  'COMPLETED',
])

/** Live checkout intents (platform uses CREATED/INITIATED; Stripe-style aliases included for safety). */
const ACTIVE_CHECKOUT_INTENT_STATUSES = Object.freeze([
  'CREATED',
  'INITIATED',
  'requires_action',
  'processing',
 ...
 * @returns {boolean}
 */
export function isChatInvoiceSyncBlockedByBookingStatus(status) {
  const s = String(status || '').toUpperCase()
  return CHAT_INVOICE_SYNC_BLOCKED_BOOKING_STATUSES.map((x) => String(x).toUpperCase()).includes(s)
}
 ...
 * @returns {Promise<{ ok: boolean, transitioned?: boolean, error?: string, code?: string }>}
 */
export async function syncBookingForPayableChatInvoice({
  ...
  // AUDIT_03 C3.9 — status gate before mutating money columns
  if (isChatInvoiceSyncBlockedByBookingStatus(booking.status)) {
    logStructured({
  ...
  const { data: activeIntents, error: intentErr } = await supabaseAdmin
    .from('payment_intents')
    .select('id, status')
    .eq('booking_id', bookingId)
    .in('status', [...ACTIVE_CHECKOUT_INTENT_STATUSES])
    .limit(1)
  ...
  if (activeIntents?.length) {
    logStructured({
      ...
      intentStatus: activeIntents[0].status,
    })
    return {
      ok: false,
      error: 'BOOKING_ALREADY_PAID',
  ...
  const pricePatch = {
    ...
    updated_at: nowIso,
    metadata: attestationRegateMetadataPatch({
