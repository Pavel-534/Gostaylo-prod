/**
 * Stage 110.4 / 110.5 — SSOT server: POST/GET /api/v2/chat/messages.
 * Клиент: `lib/chat/post-chat-message.js` → fetch на route (тонкая обёртка).
 * Invoice с hold/commission: `post-chat-invoice.server.js` → `executePostChatMessageForUser`.
 */
...
import { buildContactSafetyWarningPayload } from '@/lib/chat/show-contact-safety-warning'
import { getContactSafetyMode, shouldMaskContactsOnWrite } from '@/lib/contact-safety-mode'
...
import { isChatE2eStressGarbageContent } from '@/lib/e2e/chat-stress-message-markers.js'
import { isProductionDeployment } from '@/lib/api/prod-perimeter-guard.js'
...
export async function executeGetChatMessages(request) {
  if (!SUPABASE_URL || !SERVICE_KEY) {
    return chatApiJson({ success: false, error: 'Server misconfigured' }, 500)
  }
  ...
  if (!conversationId) {
    return chatApiJson({ success: false, error: 'conversationId required' }, 400)
  }
  ...
  if (!res.ok) {
    return chatApiJson({ success: false, error: data }, 400)
  }
...
export async function executePostChatMessage(request) {
  if (!SUPABASE_URL || !SERVICE_KEY) {
    return chatApiJson({ success: false, error: 'Server misconfigured' }, 500)
  }
  ...
  const rl = await rateLimitCheck(request, 'chat', userId)
  if (rl) {
    return chatApiJson(rl.body, rl.status)
  }
  ...
  try {
    ...
    return chatApiJson({ success: false, error: 'Invalid JSON' }, 400)
  }

  return executePostChatMessageForUser(userId, body)
}

/**
 * Отправка сообщения от имени авторизованного userId (SSOT; вызывается route и /chat/invoice).
 * @param {string} userId
 * @param {object} body
 */
export async function executePostChatMessageForUser(userId, body) {
  if (!SUPABASE_URL || !SERVICE_KEY) {
    return chatApiJson({ success: false, error: 'Server misconfigured' }, 500)
  }
  if (!userId) {
  ...
  if (type === 'system' && !['ADMIN', 'MODERATOR'].includes(senderRole)) {
    ...
    const allowPartnerBooking =
      senderRole === 'PARTNER' &&
      ['booking_confirmed', 'booking_declined', 'invoice_payment_window_notice'].includes(
        finalMetadata?.system_key,
      ) &&
      userParticipatesInConversation(userId, conversation)
  ...
  if (type === 'invoice') {
    const prebuiltInvoice =
      finalMetadata?.invoice &&
      typeof finalMetadata.invoice === 'object' &&
      finalMetadata?.invoice_id
    if (prebuiltInvoice) {
      const inv = finalMetadata.invoice
      const curPre = String(inv.currency || currency || 'THB').toUpperCase()
      const sym = curPre === 'THB' ? '฿' : curPre === 'RUB' ? '₽' : '$'
      const amtPre = Number(inv.amount)
      content =
        content?.trim() ||
        `💳 Invoice: ${sym}${Number.isFinite(amtPre) ? amtPre.toLocaleString() : ''} ${curPre}`
      invoiceRow = {
        id: String(finalMetadata.invoice_id),
        conversation_id: conversationId,
        booking_id: inv.booking_id || bookingId || conversation.booking_id || null,
        amount: amtPre,
    ...
    if (!Number.isFinite(amt) || amt <= 0) {
      return chatApiJson({ success: false, error: 'invoice type requires positive amount' }, 400)
    }
    const invoiceId = `inv-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`
    const cur = String(currency || 'THB').toUpperCase()
    const settled = await settleInvoiceDisplayAmount(amt, cur)
    const amountThbForPayload = settled.amountThb
    const usdtAmount = settled.amountUsdt
    invoiceRow = {
      id: invoiceId,
      conversation_id: conversationId,
  ...
  if (type === 'text' && (!content || !String(content).trim())) {
    return chatApiJson({ success: false, error: 'content is required for text messages' }, 400)
  }

  const textBody = String(content ?? '').trim()
  if (type === 'text' && isProductionDeployment() && isChatE2eStressGarbageContent(textBody)) {
    return chatApiJson(
      { success: false, error: 'E2E chat stress messages are not allowed in production' },
      400,
    )
  }
  ...
  if (type === 'text' && hasSafetyTrigger && shouldMaskContactsOnWrite()) {
  ...
  if (!msgRes.ok) {
    ...
    return chatApiJson({ success: false, error: msgData }, 400)
  }
  ...
  const contactSafetyWarning =
    hasSafetyTrigger && safetyMode !== 'BLOCK'
      ? buildContactSafetyWarningPayload('ru')
      : null
