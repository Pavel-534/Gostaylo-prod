...
import { extractInvoiceSnapshotFromMessage } from '@/lib/chat/conversation-inbox-status.js'
...
export function mergeMessageInsertIntoConversation(conv, msg, uid) {
  ...
  const invoiceSnapshot = extractInvoiceSnapshotFromMessage(msg)

  return {
    updated: {
      ...conv,
      lastMessage: rawMsgToLastMessage(msg),
      latestInvoice: invoiceSnapshot || conv.latestInvoice || null,
