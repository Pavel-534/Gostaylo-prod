...
export function contractNeedsDiscoveryFetchHeadroom(contract) {
  if (contract.geo?.mode === 'bbox' || contract.geo?.mode === 'polygon') return true
...
export function discoveryPlanParitySnapshot(plan) {
  return JSON.stringify({
    ...
    service: {
      serviceLanguages: [...(plan.sql.serviceLanguages || [])].sort(),
      serviceExperienceMin: plan.sql.serviceExperienceMin ?? null,
      serviceSpecialization: plan.sql.serviceSpecialization ?? null,
      serviceHomeVisit: plan.sql.serviceHomeVisit === true,
    },
    ...
    spatial: {
      ...
      rpcArgs: plan.spatial.rpcArgs
        ? plan.spatial.rpc === 'listings_within_polygon_v1'
          ? {
              polygonFingerprint: plan.spatial.rpcArgs.geojson
                ? polygonGeoJsonCacheFingerprint(plan.spatial.rpcArgs.geojson)
                : null,
