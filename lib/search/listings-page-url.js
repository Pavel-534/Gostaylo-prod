...
/**
 * Stage 177.5.1 — raw `polygon=` (gzip+base64url) from URL; empty → null.
 * @param {URLSearchParams | { get: (k: string) => string | null }} sp
 * @returns {string | null}
 */
export function parsePolygonParamFromParams(sp) {
 ...
 * @property {string | null} [appliedPolygon] — gzip+base64url GeoJSON (Stage 177.5.1)
...
/** Smart search toggle: URL semantic → localStorage → default false (Stage 201.80 opt-in). */
...
export function buildPublicSearchParams(filters, options = {}) {
  ...
  const poly = appliedPolygon != null ? String(appliedPolygon).trim() : ''
  if (poly) {
    params.set('polygon', poly)
  } else {
