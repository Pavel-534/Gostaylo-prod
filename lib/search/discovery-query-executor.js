...
/**
 * Whether unified path already applied PostGIS spatial id prefilter (bbox or polygon).
 * Skip JS pointInBounds when true.
  ...
  return Boolean(
    plan &&
      ...
      (plan.registryFiltersApplied?.includes('geo.bbox') ||
        plan.registryFiltersApplied?.includes('geo.polygon')),
  )
}

/** @deprecated alias — prefer discoveryPlanUsedGistBbox (covers polygon too since 177.5.0) */
export function discoveryPlanUsedSpatialPrefilter(plan) {
  return discoveryPlanUsedGistBbox(plan)
}
