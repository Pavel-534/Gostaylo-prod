...
export function freezeDiscoveryContract(contract) {
  ...
  return Object.freeze({
    ...contract,
    geo: geo ? Object.freeze(geo) : null,
...
export function collectDiscoveryValidationIssues(contract) {
  ...
  if (contract.geo?.mode === 'polygon') {
    if (contract.geo._invalid) {
      issues.push({
        code: contract.geo._invalidCode || 'POLYGON_INVALID',
        path: 'geo.polygon',
        message: contract.geo._invalidMessage || 'Invalid polygon',
      })
    } else if (
      !contract.geo.geojson ||
      contract.geo.geojson.type !== 'Polygon' ||
      !Array.isArray(contract.geo.geojson.coordinates?.[0]) ||
      contract.geo.geojson.coordinates[0].length < 4
    ) {
      issues.push({
        code: 'POLYGON_INVALID',
        path: 'geo.polygon',
        message: 'Polygon GeoJSON is incomplete',
      })
    }
  }
