...
/** Не показывать результаты с similarity ниже порога (косинусная метрика из match_listings). Временно 0.1 для отладки строгости. */
export const SEMANTIC_MIN_SIMILARITY = 0.1
...
export async function fetchSemanticListingMatches(q, opts = {}) {
  ...
  if (!apiKey || !supabaseAdmin) {
    console.warn('[semantic-search] skip: no_openai_or_supabase', { q: queryText })
  ...
  if (!embed.ok || !embed.embeddingJson) {
    console.warn('[semantic-search] embed failed', { q: queryText, error: embed.error || 'embed_failed' })
    await logOnce(embed, {
      embed_ok: false,
  ...
  console.log('[semantic-search] match_listings RPC', {
    q: queryText,
    rpcRowCount: mapped.length,
    beforeThreshold: mapped.slice(0, 20).map((r) => ({ id: r.id, similarity: r.similarity })),
    threshold: SEMANTIC_MIN_SIMILARITY,
  })
  const filtered = mapped.filter((r) => r.similarity >= SEMANTIC_MIN_SIMILARITY)
  console.log('[semantic-search] after similarity threshold', {
    q: queryText,
