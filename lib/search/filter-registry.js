 ...
 * Stage 177.5.0 — geo.polygon (Wave E1 backend; gated by DISCOVERY_POLYGON_SEARCH).
 */
...
import { decodePolygonSearchParam } from '@/lib/search/discovery-geo-polygon'
import { isDiscoveryPolygonSearchEnabled } from '@/lib/search/discovery-pipeline-flag'
import {
...
import {
  isServiceFilterVerticalAllowed,
  isServiceRegistryFilterAllowedForContract,
} from '@/lib/search/discovery-services-vertical-guard'
import {
  parseUrlNannyLangsParam,
  parseUrlServiceExperienceMinParam,
  parseUrlServiceHomeVisitParam,
  parseUrlServiceSpecializationParam,
} from '@/lib/search/discovery-service-facet-parse'
import {
  buildDiscoveryJsonbTextEqCiPredicate,
  buildDiscoveryJsonbTextIlikeContainsPredicate,
...
/**
 * @typedef {Object} DiscoveryGeoPolygon
 * @property {'polygon'} mode
 * @property {{ type: 'Polygon', coordinates: number[][][] }} geojson
 * @property {string} [issueCode] set only when parse failed (draft); cleared on success path
 */

/**
 * @typedef {DiscoveryGeoBbox|DiscoveryGeoPolygon|null} DiscoveryGeoFilter
 */
 ...
 * @property {'@>'|'text_eq_ci'|'text_ilike_contains'|'jsonb_numeric_gte'} op
 * @property {string} path
...
export const FILTER_REGISTRY = {
  ...
  'geo.bbox': {
    ...
    surfaces: ['catalog', 'map'],
    parse(sp, draft) {
      // Precedence: polygon wins — skip bbox if polygon already applied or present + enabled.
      if (draft.geo?.mode === 'polygon') return
      if (isDiscoveryPolygonSearchEnabled() && sp.get('polygon')) return
  ...
  'geo.polygon': {
    ...
    surfaces: ['catalog', 'map'],
    parse(sp, draft) {
      if (!isDiscoveryPolygonSearchEnabled()) return
      const raw = sp.get('polygon')
      if (raw == null || !String(raw).trim()) return
      const decoded = decodePolygonSearchParam(String(raw))
      if (!decoded.ok) {
        draft.geo = {
          mode: 'polygon',
          geojson: { type: 'Polygon', coordinates: [] },
          _invalid: true,
          _invalidCode: decoded.code,
          _invalidMessage: decoded.message,
        }
        return
      }
      // Precedence over bbox: replace any prior geo.bbox from URL.
      draft.geo = { mode: 'polygon', geojson: decoded.geojson }
    },
    applyPlan(contract, plan) {
      if (contract.geo?.mode !== 'polygon') return
      if (contract.geo._invalid) return
      plan.spatial = {
        engine: 'postgis',
        rpc: 'listings_within_polygon_v1',
        rpcArgs: {
          geojson: contract.geo.geojson,
          categoryIds: plan.sql.categoryIds,
        },
        listingIds: null,
        bboxLegacy: null,
      }
      plan.registryFiltersApplied.push('geo.polygon')
    },
  },
  ...
  'service.languages': {
    ...
    surfaces: ['catalog', 'map'],
    parse(sp, draft) {
      const parsed = parseUrlNannyLangsParam(sp, 'nanny_langs')
      if (parsed.kind === 'active' && parsed.value?.length) {
        draft.vertical.nannyLangs = parsed.value
      } else if (parsed.kind === 'invalid') {
        draft.vertical._serviceLangInvalid = true
      }
    },
    applyPlan(contract, plan) {
      if (!isServiceFilterVerticalAllowed(contract)) return
      const langs = contract.vertical?.nannyLangs || []
      if (!langs.length) return
      if (!plan.sql.jsonbPredicates) {
        plan.sql.jsonbPredicates = []
      }
      plan.sql.serviceLanguages = [...langs].sort()
      plan.sql.jsonbPredicates.push({
        op: '@>',
        path: 'languages',
        value: [...langs].sort(),
      })
      plan.registryFiltersApplied.push('service.languages')
    },
  },

  'service.experience_min': {
    key: 'service.experience_min',
    urlKeys: ['nanny_experience_min', 'nannyExperienceMin'],
    verticals: ['service'],
    layer: 'sql',
    surfaces: ['catalog', 'map'],
    parse(sp, draft) {
      const parsed = parseUrlServiceExperienceMinParam(sp, 'nanny_experience_min', 'nannyExperienceMin')
      if (parsed.kind === 'active') {
        draft.vertical.nannyExperienceMin = parsed.value
      } else if (parsed.kind === 'invalid') {
    ...
    applyPlan(contract, plan) {
      if (!isServiceFilterVerticalAllowed(contract)) return
      const min = contract.vertical?.nannyExperienceMin
      if (!Number.isFinite(min) || min < 1) return
      if (!plan.sql.jsonbPredicates) {
        plan.sql.jsonbPredicates = []
      }
      plan.sql.serviceExperienceMin = min
      plan.sql.jsonbPredicates.push(buildDiscoveryJsonbNumericGtePredicate('experience_years', min))
      plan.registryFiltersApplied.push('service.experience_min')
    },
  },

  'service.specialization': {
    key: 'service.specialization',
    urlKeys: ['nanny_specialization', 'nannySpecialization'],
    verticals: ['service'],
    layer: 'sql',
    surfaces: ['catalog', 'map'],
    parse(sp, draft) {
      const parsed = parseUrlServiceSpecializationParam(
        sp,
        'nanny_specialization',
        'nannySpecialization',
      )
      if (parsed.kind === 'active' && parsed.value) {
        draft.vertical.nannySpecialization = parsed.value
      }
    },
    applyPlan(contract, plan) {
      if (!isServiceFilterVerticalAllowed(contract)) return
      const keyword = contract.vertical?.nannySpecialization
      if (!keyword) return
      if (!plan.sql.jsonbPredicates) {
        plan.sql.jsonbPredicates = []
      }
      plan.sql.serviceSpecialization = keyword
      plan.sql.jsonbPredicates.push(
        buildDiscoveryJsonbTextIlikeContainsPredicate('specialization', keyword),
      )
      plan.registryFiltersApplied.push('service.specialization')
    },
  },

  'service.home_visit': {
    key: 'service.home_visit',
    urlKeys: ['service_home_visit', 'home_visit_only', 'homeVisitOnly'],
    verticals: ['service'],
    layer: 'sql',
    surfaces: ['catalog', 'map'],
    parse(sp, draft) {
      const parsed = parseUrlServiceHomeVisitParam(
        sp,
        'service_home_visit',
        'home_visit_only',
        'homeVisitOnly',
      ...
      if (parsed.kind === 'active') {
        draft.vertical.serviceHomeVisitOnly = true
      } else if (parsed.kind === 'invalid') {
    ...
    applyPlan(contract, plan) {
      if (!isServiceFilterVerticalAllowed(contract)) return
      if (contract.vertical?.serviceHomeVisitOnly !== true) return
      if (!plan.sql.jsonbPredicates) {
        plan.sql.jsonbPredicates = []
      }
      plan.sql.serviceHomeVisit = true
      plan.sql.jsonbPredicates.push({
        op: '@>',
        path: 'home_visit',
        value: true,
      })
      plan.registryFiltersApplied.push('service.home_visit')
    },
  },
}
...
export function isRegistryFilterActive(contract, key) {
  ...
  if (!isServiceRegistryFilterAllowedForContract(contract, key)) {
    return false
  }
  ...
  switch (key) {
    ...
    case 'geo.polygon':
      return contract.geo?.mode === 'polygon' && !contract.geo._invalid
    case 'stay.dates':
    ...
    case 'service.languages':
      return (contract.vertical?.nannyLangs?.length || 0) > 0
    case 'service.experience_min':
      return (
        Number.isFinite(contract.vertical?.nannyExperienceMin) &&
        contract.vertical.nannyExperienceMin >= 1
      )
    case 'service.specialization':
      return Boolean(String(contract.vertical?.nannySpecialization || '').trim())
    case 'service.home_visit':
      return contract.vertical?.serviceHomeVisitOnly === true
    default:
      return false
  }
}
