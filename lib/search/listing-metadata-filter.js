 ...
 *
 * Stage 177.4 — unified discovery (`DISCOVERY_UNIFIED_PIPELINE=1`) парсит service/nanny
 * facets через `FILTER_REGISTRY` + `discovery-filter-contract.js`. Не дублируйте parse
 * service URL здесь для unified path; этот модуль остаётся oracle только для legacy flag `=0`.
 */
...
export function buildMetadataFiltersFromSearchParams(searchParams) {
  ...
  const captainParsed = parseUrlWithCaptainFilterParam(
    sp,
    'with_captain',
    'crew_included',
    'withCaptain',
  )
  const vesselRaw = sp.get('yacht_type') || sp.get('vessel_type')
  ...
  return {
    ...
    cabinsMin: firstInt('cabins_min', 'cabinsMin'),
    withCaptain: captainParsed.kind === 'active',
    vesselType: vesselRaw?.trim() || null,
    ...
    /** Stage 67.0 — маркетплейс услуг: только с выездом на дом */
    serviceHomeVisitOnly: boolParam('service_home_visit', 'home_visit_only', 'homeVisitOnly') === true,
  }
}
...
export function listingMatchesMetadataFilters(listing, f) {
  ...
  if (f.cabinsMin != null && f.cabinsMin > 0) {
    const cab = parseFloat(m.cabins ?? m.cabins_count ?? m.cabin_count ?? 0)
    if (!Number.isFinite(cab) || cab < f.cabinsMin) return false
  }

  if (f.withCaptain === true) {
    if (m.crew_included !== true && m.crew_included !== 'true') return false
  }
  if (f.vesselType) {
    const vessel = String(m.subcategory || m.vessel_type || m.boat_type || '').toLowerCase()
    if (!vessel.includes(String(f.vesselType).toLowerCase())) return false
  }
  ...
  if (f.serviceHomeVisitOnly === true) {
    if (m.home_visit !== true && m.home_visit !== 'true') return false
  }
