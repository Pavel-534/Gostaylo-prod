# Architectural Passport — ARCHIVE

> **ARCHIVED.** Живой SSOT разделён: [`CONSTITUTION.md`](./CONSTITUTION.md) · [`SYSTEM_MAP.md`](./SYSTEM_MAP.md) · [`HISTORY.md`](./HISTORY.md) · [`ROADMAP.md`](./ROADMAP.md). Индекс: [`ARCHITECTURAL_PASSPORT.md`](./ARCHITECTURAL_PASSPORT.md).
>
> **Version (last as monolith)**: 12.200.19.0 | **Last Updated**: 2026-07-31 | **Stage 200.19:** PDP map cooperative overlay no longer covers booking bar.
> Исторический текст ниже не является источником истины для новых изменений. **Порядок для агентов:** **`ARCHITECTURAL_DECISIONS.md`** → **`docs/TECHNICAL_MANIFESTO.md`** → **CONSTITUTION / SYSTEM_MAP**.
...
| **Недавно смотрели** | localStorage + **`listing_views`** после логина; merge + resolve ACTIVE; **`RecentlyViewedRail`** + compact **`RecommendationRailCard`** (title/category/price, **`RECOMMENDATION_RAIL_CARD_CLASS`**) on PDP + home | **`lib/recommendations/listing-views.service.js`**, **`components/recommendations/RecommendationRailCard.jsx`** |
| **Каталог map↔list** | **hover** → pin/card highlight only (no `flyTo`, no list scroll); **click / marker** → `selectedListingId` + `MapSelectionSync` pan if pin outside viewport; marker click → scroll card; cluster → `fitBounds(cellSizeM)`; **cluster mode:** выбранный listing pin поверх кластеров (171.14); **approximate fitBounds:** server fuzz coords only, no client 500m pad (171.15); **mobile ≤768px:** **`CatalogMobileMapSheet`** full-screen map (169.3, 173.3) + controlled bottom rail **`CatalogMapCardRail`** (177.6 Step 2, CSS snap) with shared `mapSelectedListingId`; map timings/debounce from **`lib/maps/catalog-map-ux-policy.js`**. | **`InteractiveSearchMap.jsx`**, **`CatalogSearchMapPanel.jsx`**, **`CatalogMobileMapSheet.jsx`**, **`CatalogMapCardRail.jsx`**, **`ListingSidebar.jsx`**, **`SearchMapWrapper.jsx`** |
| **Каталог mobile refetch UX (179.2)** | **`<md` only:** при `loading` / `isTransitioning` / `aiSearchPending` **`ListingSidebar`** рендерит **`ListingGridSkeleton`** × **`MOBILE_CATALOG_SKELETON_COUNT` (4)** с **`LISTING_CATALOG_GRID_CLASSES`**. **`md+`:** cross-fade (`opacity-50` на refetch). **Mobile sheet:** компактная сетка 2×2 (Select «Что?» + Where/Dates/Guests), без дублирующих badge-чипов; **`CategoryQuickChips`** — только hero главной. | Полная замена списка на mobile; горизонтальные chips + badges в шторке |
...
- **PostGIS spatial search (Stage 162.1–162.2):** миграции **`stage162_1_postgis.sql`**, **`stage162_2_postgis_knn_distance.sql`** — колонка **`listings.coordinates`**, триггер sync, RPC **`listings_ids_within_radius_v1`** (KNN + **`distance_meters`**); каталог **`GET /api/v2/search`** — **`lat`**, **`lng`|`lon`**, **`radius`|`radiusKm`**, **`sort=distance`** (default при координатах); карточка **`distance_from_center_km`**; meta **`spatialSortUsed`**, **`centerLat`**, **`centerLng`**; fallback Haversine (**`lib/api/postgis-probe.js`**).
- **Map pins API (Stage 163.0–163.3, 171.14–171.15):** **`GET /api/v2/search/map-pins`** — lean pins `{ id, lat, lng, price, status, isApproximate, locationPrivacyMode }`; privacy verticals → fuzzed coords via **`lib/geo/listing-public-coordinates.js`** (ADR-163); **`mode:clusters`** — centroid = **grid cell center** (`listings_map_clusters_grid_v1`, **`migrations/stage163_3_cluster_centroid_privacy.sql`**), meta **`clusterCentroidMode: grid_cell_center`**; GiST bbox **`listings_map_pin_ids_in_bbox_gist_v1`**. Клиент: **`CatalogSearchMapPanel`** (SSOT body; bbox quantize **`lib/geo/quantize-map-bbox.js`**, 3 decimals) in **`SearchMapWrapper`** (desktop) + **`CatalogMobileMapSheet`** (mobile, 169.3) + **`useMapPinsFetch`**; **`InteractiveSearchMap`** — selected pin overlay in cluster mode; **`InitialListingBoundsFit`** — point bounds on server coords only (no 500m client inflation); lazy popup **`ListingMapPopupLazy`**. Wave C privacy gap closed (pins fuzz + cluster cell center).
- **Карта / приватность координат:** **`lib/listing-location-privacy.js`** — UI mode (`privacy`|`exact`) из реестра; **серверный jitter** — **`lib/geo/listing-public-coordinates.js`** + **`lib/geo/public-coordinate-viewer-context.js`**.
...
- **PDP синхронизация:** **`lib/listing-card-spec-profile.js`** + **`ListingCardSpecsRow`** (`variant="pdp"`) — единственный ряд спек в шапке PDP (транспорт: cc, коробка, топливо, год, места; жильё: спальни, ванные, гости, м²); авто-dump **`extraMetadataSpecs`** снят. **`ListingGuestPolicies`** — stay: **`ListingStayPolicies`** (tiles заезд/выезд из metadata + правила + отмена в одной карточке); прочие вертикали — только **`ListingCancellationPolicy`**. Suppress keys: **`listing-public-metadata-specs.js`** (`fuel_type`, `city`, `seats`, …).
...
- **Карта / приватность координат:** **`lib/listing-location-privacy.js`** — UI mode (`privacy`|`exact`) из реестра; **серверный jitter (163.2)** — **`lib/geo/listing-public-coordinates.js`** (ADR **`docs/ADR/163-coordinate-privacy-ssot.md`**). Legacy **`categoryId`** — без изменений.
...
- **Viewport / клавиатура (Stage 170.8):** **`components/messages-viewport-shell.jsx`** — **`MessagesViewportShell`** на **`/messages*`**: **`window.visualViewport`** → inline **`height`** корня (fallback **`100dvh`**); scroll-контейнер ленты — **`[data-testid="chat-thread-scroll"]`** в **`ChatThreadChrome`**; композер **`flex-shrink-0`** in-flow; **`ChatMessageList`** — минимальный **`pb-4`** (legacy **`pb-28`** снят).
...
### 0.0k Modular public UI (Stage 8.1 → 171.0 ADR-101)

#### Public Search Chrome SSOT (ADR-101, normative — implementation staged)
...
- **Каталог:** expanded **`FilterBar`** fades `1-morphT`; compact cross-fade from progress **0.28**; CSS height **expanded → compact** lerp. **Mobile `<md`:** in-flow **`FilterBar`** заменён на **`CatalogSearchSummaryBar`** (`gsl-premium-glass`); поиск — **`CatalogMobileSearchSheet`** (overview: FAB / summary tap) с **`FilterBar`** / **`UnifiedSearchBar variant="filter"`** + **Stage 190.6–190.7** accordion **`pickerPresentation="wizardStep"`** (Where → Dates → Guests auto-advance / Done; «Фильтры» только если wired `extraFilters`); «Все фильтры» Apply — **`useDraftFilterResultCount`** + **`fetchSearchAvailableCount`** (draft extras, URL SSOT не трогается до Apply); «Что?» — **`CategoryQuickChips`** (179.1, **`selectHeroCategoryTabs`**), state **`usePublicSearchFilters`**. **Главная `<md`:** hero direct pickers (179.0); sheet FAB; hero/sheet share **`CategoryQuickChips`**. **Tab bar «Поиск»** (178.8): **`mobile-search-tab-action.js`**.
...
- **Stage 176.0.0 — Content Guard (instant publish):** миграция **`migrations/stage176_0_review_content_guard.sql`** — **`moderation_status`** (`approved` | `flagged`) на **`reviews`** и **`guest_reviews`**; RLS **`reviews_select_scope`** — публичный SELECT только **`approved`** на ACTIVE-листингах (автор + owner листинга видят любой статус); SSOT **`lib/reviews/content-guard.js`** + **`lib/reviews/moderation-status.js`**; **`POST /api/v2/reviews`** и **`POST /api/v2/partner/guest-reviews`** прогоняют **`comment`** через guard; **`refresh_listing_review_stats`** считает только approved; cron **`review-reminder`** — статусы **`THAWED`/`COMPLETED`** (без **`PAID_ESCROW`/`CHECKED_IN`**).
- **Stage 176.1.0 — Admin moderation + partner reply guard:** миграция **`stage176_1_review_moderation_admin.sql`** (`removed`); **`GET /api/admin/reviews`**, **`PATCH /api/admin/reviews/[id]/moderation`**; сервис **`lib/reviews/review-moderation-admin.service.js`**; UI **`/admin/reviews`**; **`PUT /api/v2/reviews/[id]/reply`** — **`guardReviewComment`** → **400** `REVIEW_CONTENT_BLOCKED`; toast **`reviewForm_toastModerationPending`**.
- **Stage 176.2.0 — Mobile UX (reviews & moderation):** **`components/review-modal.jsx`** — Radix **`Dialog`/`DialogContent`** ( **`visualViewport`** ), mobile bottom-sheet (`rounded-t-2xl`), sticky CTA footer, touch targets ≥44px; **`/admin/reviews`** — dual layout: card stack **`< md`**, table **`md+`**; **`invoice-bubble.jsx`** — truncate status badge; partner **`guest-review`** — CTA **`min-h-[48px]`**. Конституция: **`.cursorrules`** § Mobile Touch Areas & Ergonomics (Stage 176.2).
...
- **API:** **`GET /api/v2/admin/health`** — только **`profiles.role === 'ADMIN'`** или email из **`ADMIN_HEALTH_EMAILS`** (см. **`lib/admin-health-access.js`**); данные через **`supabaseAdmin`**. **Stage 142:** SSOT **`lib/admin/ops-job-health.js`**, **`lib/admin/notification-channel-health.js`**, **`lib/admin/critical-signals-health.js`**; query **`signalsKey`**, **`signalsQ`** (опционально). **Stage 164:** **`spatialStats`** (PostGIS probe, GiST, coords coverage, **`mapPinsMetrics`**), **`privacyStats`** (fuzz/reveal runtime + policy distribution), **`geoDrift`** (last run **`lib/services/geo-drift-detector.service.js`**), **`locationNormalize`** (Stage 161).
- **Geo drift (Stage 164–165):** RPC **`listings_geo_drift_scan_v1`**; cron **`POST /api/cron/geo-drift-detector`** (`runFullAudit`); CRITICAL signals **`GEO_COORDINATE_MISMATCH`**, **`GEO_PRIVACY_VIOLATION`**. **Stage 166 (Wave E):** partial indexes + GiST bbox count (`stage166_0_spatial_perf.sql`); spatial cache / circuit breaker / slow-query trace (`lib/ops/spatial-query-cache.js`, `spatial-circuit-breaker.js`, `slow-query-log.js`); rate limits **`spatial_map`** 45/min, **`spatial_catalog_user`** 90/min; health **`performanceStats`** (p95/p99, cache hit).
...
- **Stage 185.0 Phase 0 — Partner bookings compact list:** **`/partner/bookings`** рендерит **`UnifiedOrderCard`** с **`density="compact"`** (без timeline/детализации цены/PII; одна строка «Ваш доход»). Guest contacts **не отдаются** партнёрским API (**`sanitizePartnerBookingForClient`**, SELECT renter без email). Phase 1 — Master-Detail (**`PartnerBookingDetailDrawer`**).
- **Stage 185.0 Phase 1 — Master-Detail:** **`components/partner/bookings/`** — **`PartnerBookingList`**, **`PartnerBookingCard`**, **`PartnerBookingDetailDrawer`**, **`PartnerBookingStatusTabs`**; фильтр вкладок — **`lib/booking/partner-bookings-tabs.js`**; карточная модель — **`lib/partner/partner-booking-card-model.js`**; **`?booking=`** открывает drawer.
- **Stage 185.0 Phase 3 — mobile polish:** drawer **`layout="drawer"`** + sticky CTA (Stage **176.2**); **`PageSectionHeader`**; i18n **`partnerBookings_*`**. Partner workspace shell — **`WORKSPACE_SCROLL_CLASS`** (`app/partner/layout.js`); inner content width — **`PartnerPageShell`** (`components/product/PartnerPageShell.jsx`) on **`/partner/bookings`**.
- **Stage 185.1 — bookings UX rescue:** compact partner price block; INQUIRY i18n; dialog z-index above sheet; list quick decline; scrollable status tabs via **`PartnerFinancesPillSubNav`**.
- **Stage 185.0 Phase 3b — shared orders i18n + KPI:** **`OrdersSummary`** / **`OrderTypeFilter`** принимают **`language`**; ключи **`ordersSummary_*`**, **`orderTypeFilter_*`** (`order-flow.js`); partner KPI на **&lt; md** — компактная строка (ожидают · активные · доход); кнопки фильтра типа — **`min-h-[44px]`**.
- **Stage 186.0 Phase 0 — partner finances currency + ledger labels:** **`lib/partner/partner-money-display.js`**, **`lib/partner/ledger-display-labels.js`**; **`PartnerHostLedgerAmountCell`**; reconciliation calm UI (**`NEXT_PUBLIC_PARTNER_FINANCE_DEBUG=1`** для Δ).
- **Stage 186.0 Phase 1 — partner finances tabs:** **`/partner/finances`** — tabs **overview / ledger / reports** (`PartnerFinancesOverviewTab`, `PartnerFinancesLedgerTab`, `PartnerFinancesReportsTab`); **`PartnerPageShell`**; header без дубля escrow-card; **`PartnerFinancesBalanceStrip`** compact на mobile.
- **Stage 186.0 Phase 2 — ledger master-detail:** **`PartnerLedgerRow`**, **`PartnerLedgerDetailDrawer`**, **`hooks/use-partner-booking-detail.js`**; booking drawer in-place; **`?booking=`** deep link; mobile sticky withdraw on overview tab.
- **Stage 186.0 Phase 2b — final polish:** ledger pagination (Load More, **`ledgerPagination.hasMore`**); **`?ledgerEntry=`** deep link; **`PartnerFinancesLedgerSubNav`** (ledger vs booking history); withdraw eligibility tests.
- **Stage 186.0 Phase 2c — absolute perfection:** **`PartnerBookingTransactionRow`** mobile cards; **`PartnerFinancesReportsSubNav`** (statements / payouts / help); archive ledger entry inject + drawer banner.
...
- **Партнёр — календарь:** **`app/partner/calendar/page.js`** использует **`usePartnerReputationHealthQuery`** (**`hooks/use-partner-reputation-health.js`**, общий ключ кэша с дашбордом) и полоску **`getPartnerCalendarDominantHint`**. **Stage 188.0:** мобильные действия по ячейке — **`CalendarActionOverlay`** + **`ActionModals`** (bottom sheet); unblock manual blocks via **`useDeleteBlock`**; booked drill-down → **`/partner/bookings/[id]`**; bulk prices via **`applyBulkSeasonalPrices`** (concurrency 3); iCal blocks read-only sheet; batch API **`POST /api/v2/partner/calendar/batch`**; **two-tap range selection** (**`usePartnerCalendarRangeSelection`**) → prefilled block/booking forms; **`?onboarding=true`** welcome after first publish; **Post-188:** publish from **`/partner/listings`** card → same onboarding redirect (**`resolvePostPublishCalendarOnboardingUrl`**); E2E **`tests/e2e/partner-calendar-flow.spec.ts`** (`data-testid="partner-cal-cell"` / `partner-cal-action-overlay`).
- **Партнёр — wizard / listings:** **`ListingPhotoSortGrid`** — drop upload + reorder (cover = index 0); deep link **`/partner/listings/[id]?step=photos|pricing|…`** (**`listing-wizard-step-slugs.js`**); **bidirectional URL sync** — **`useListingWizardStepUrlSync`** (`history.replaceState` on step change, restore on F5); publish redirect → calendar onboarding (**`lib/partner/post-publish-redirect.js`**).
- **Ambassador hub FX (Stage 188.1 → 188.3):** ledger в THB на сервере; UI — **`useReferralLedgerDisplay`** / **`ReferralLedgerAmount`** + header **`useCurrency()`** (mid FX **`useAmbassadorDisplayFx`**, не retail); `/profile/referral|status|wallet`; **188.2:** только display-валюта (без `THB (≈ …)`); пороги и суммы — **`formatMinPayoutThreshold`** / **`formatAmbassadorAmountFromThb`**; **188.3:** вывод — ввод в display → **`convertDisplayToThbForWithdrawal`** (snap guard ≤2% / exact min display) → API; ошибки min — **`stage188_withdrawMinRequired`** + **`localizeWithdrawValidationError`**; E2E **`ambassador-currency-isolation.spec.ts`**.
...
TELEGRAM_ADMIN_GROUP_ID=***REDACTED***
RESEND_API_KEY=re_xxx  # Optional - falls back to mock
SENDER_EMAIL=Airento <noreply@example.com>
```
