...
Сервер: `PayoutBatchService.markBatchSettled` — **сначала** проверка открытых `payouts` (PENDING/PROCESSING) у партнёров пула → `409 open_partner_payout_requests`; иначе single-flight lock → проводки `PARTNER_PAYOUT_OBLIGATION_SETTLED`, `bookings` → `COMPLETED`, `syncPartnerBalanceColumns`. Concurrent settle → `409 settle_in_progress`.  
Список пулов: `GET /api/admin/finances/payout-batches` возвращает `canSettle` и `settleBlockers` для UI.

#### 3.1.1 Settle lock hygiene (AUDIT_02 / Stage 201)

Lock живёт **только** в `payout_batches.metadata` (`settle_in_progress_at`, `settle_lock_token`, `settle_lock_owner`) — **отдельной lock-таблицы нет**, строки не накапливаются.

| ✓ | Проверка | Ожидание |
|---|----------|----------|
| ☐ | После успешного settle | `metadata` без `settle_lock_token` / `settle_in_progress_at` (release в `finally`) |
| ☐ | После abort / 422 partial | токен снят; batch остаётся LOCKED/EXPORTED |
| ☐ | После crash worker mid-settle | через TTL (**1800s**) следующий claim reclaim’ит stale lock; heartbeat (~45s) не даёт украсть lock у живого worker |
| ☐ | SQL sanity (не должно расти «вечно») | нет orphaned keys старше TTL на **не**-SETTLING пулах |

```sql
-- Stale-looking locks (should be empty after TTL or after release)
SELECT id, status,
       metadata->>'settle_in_progress_at' AS lock_at,
       metadata->>'settle_lock_token' AS token
FROM payout_batches
WHERE metadata ? 'settle_lock_token'
  AND coalesce((metadata->>'settle_in_progress_at')::timestamptz, '-infinity'::timestamptz)
      < now() - interval '35 minutes';
```

Миграции: `stage201_01` → `stage201_02` → `stage201_03`. Smoke: `npm run smoke:audit02`.
