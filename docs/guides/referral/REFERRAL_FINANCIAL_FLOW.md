...
flowchart LR
  ...
  D --> F[referral_distribute_bonus_atomic]
...
3. При **earned**: **`referral_distribute_bonus_atomic`** с `reference_id = referral_ledger:{id}`; split internal/withdrawable по tier (`payout_to_internal_ratio`). Held → `earned_held` + `adjust_held_referral_balance_thb`, unlock credits via same atomic path.
