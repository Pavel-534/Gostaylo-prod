...
**Отдельная ветка (supply):** первая завершённая бронь у приглашённого **партнёра-хоста** → фиксированный бонус активации (**760 THB**, Stage 202.40) списывается из **Promo Tank**, **100% прямому L1** (L2 на host activation = 0).
...
- Фиксированная сумма из настроек **`partner_activation_bonus_thb`** (дефолт **760 THB**, Stage 202.40; RF mid display ~₽2500).
- Деньги берутся из **Marketing Budget (Promo Tank)**, **не** из маржи брони и **не** с последующих заказов хоста.
- Платится **только L1** (кто пригласил хоста) — **100%**; `mlm_level2_percent = 0` на host activation (guest L2/L3 не трогаем).
...
| Первая бронь приглашённого хоста (`host_activation`) | **100%** от 760 THB | **0** | **0** (без L3+) | — |

**Пример (цепочка вы → Борис → Катя едет):**  
С поездок Кати: **Борис** = L1, **вы** = L2. **L3** = кто пригласил вас — **0**, пока флаг L3 выключен; после cutover — 5% пула, если у этого человека ≥10 прямых партнёров и есть consent. Host activation (Stage 202.40) — **только L1 100%** от pot 760 THB.
...
1. **Guest L3 — код готов, live выключен** (ADR-131A): с поездок платит L1 + L2 (+ cashback); L3 ledger только после cutover (`ambassador_guest_l3_enabled=true`) + gate + consent. Host activation (Stage 202.40) — **L1 100% / L2 0%**, pot 760 THB. L4/L5 — Stage 131.A2 после gate §5.
