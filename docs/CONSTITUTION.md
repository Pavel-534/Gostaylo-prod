...
| **Storefront Search keep-alive** | `StorefrontSearchKeepAlivePane` in `StorefrontAppShell`. Parks catalog list across `/` ↔ `/listings` (not PDP). Frozen query: `useFrozenCatalogSearchParams`. Reveal: `revealStorefrontSearchKeepAlive`. |
