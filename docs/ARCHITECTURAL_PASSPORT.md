...
> **Version**: 7.12.0 | **Last Updated**: 2026-04-24 | **Status**: Production-Ready
> 
...
- **Stage 37.0 — Telegram delivery + reminder dedup:** `MarketingNotificationsService` подключён к реальному Telegram-боту через существующий adapter (`sendTelegramMessagePayload`) и отправляет партнёру reminder Flash Sale с inline-кнопкой быстрого действия. Кнопка ведёт в интерфейс партнёра (`/partner/promo?flashCode=...&extendHours=6`), где доступно one-click продление через **`POST /api/v2/partner/promo-codes/[code]/extend-flash-sale`**. В anti-spam слое реализована дедупликация: timestamp хранится в **`promo_codes.metadata.last_reminder_sent_at`**, повторная отправка блокируется при окне <45 минут. Крон **`/api/cron/flash-sale-reminder`** остаётся защищён `CRON_SECRET` (`x-cron-secret` или `Authorization: Bearer ...`) и возвращает telemetry (`notified`, `deduped`, `candidates`). Для performance hardening проверка применимости вынесена в кэшируемый слой `checkApplicabilityCached` (in-memory TTL ~45s) внутри `lib/promo/promo-engine.js`; этот слой используется в pricing/calendar/catalog для снижения нагрузки в hot-loop сценариях.
