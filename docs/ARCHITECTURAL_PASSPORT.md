...
- **SSOT медиа (Stage 84.3):** **`lib/services/media/media-upload.service.js`** — профили **`listing_photo`** / **`dispute_media`** / **`chat_image`** / **`kyc_document`** (лимиты + WebP); **`POST /api/v2/upload`** для растровых MIME прогоняет **`sharp`** и отдаёт **`image/webp`**; лог **`[MEDIA_PROFILE]: <профиль>`** после resolve (**`profile`/`mediaProfile`** в form-data или дефолт по bucket). Клиентские обёртки: **`lib/services/image-upload.service.js`**, **`lib/chat-upload.js`**, Telegram — **`compressImageBufferTelegramListing`**.
