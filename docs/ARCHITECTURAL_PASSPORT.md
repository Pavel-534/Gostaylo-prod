...
- **Главная — `components/PlatformHomeContent.jsx`:** тонкий оркестратор; **`components/home/`** — **`HomeHero`**, **`CategoryBar`**, **`TopListingsGrid`**, shared **`home-constants.js`**, **`useHomeFilters.js`** (What/Where/When/Who, дебаунс, сид из URL, semantic flag с сайта).
