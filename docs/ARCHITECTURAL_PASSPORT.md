...
> **Version**: 6.10.0 | **Last Updated**: 2026-04-23 | **Status**: Production-Ready
> 
...
- **Умная кнопка:** **`GET /api/v2/bookings/[id]/emergency-context`** (рентер) → **`bookingEligible`** + **`partnerInQuietHours`** (**`isPartnerInQuietHoursNow`**). В **`UnifiedOrderCard`** красная «Экстренная связь» только при **`partnerInQuietHours`**; иначе CTA **«Написать в чат»**. В модалке — **`orderHelp_emergencyNightDisclaimer`** (ночь / ЧП / лимит 24 ч).
