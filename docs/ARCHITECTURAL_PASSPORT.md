...
- **Чат (P1-3):** на странице треда **`deferThreadRealtime`** в **`useChatThreadMessages`** — INSERT/UPDATE активного треда через единый канал **`useConversationInbox`** (`activeConversationId` + callbacks).
...
- **Чат:** `lib/chat/post-chat-message.js` — единый POST messages; `useChatThreadMessages` + `UnifiedMessagesClient`.
...
### SSOT Цен (Stage 107.1–107.2)

| Слой | Правило | Файлы |
|------|---------|--------|
| **Формула витрины** | Гостевая цена = база (или календарный субтотал) + **guest service fee %** из комиссий платформы. Налоги и полный breakdown — только в блоке бронирования/checkout, не в hero каталога. | **`lib/pricing/guest-display-price.js`** (`computeGuestDisplayFromBaseThb`, `getGuestDisplayPerNight`, `getGuestDisplayForStay`, `getPdpHeroGuestPriceThb`, `formatCardPricePeriodSuffix`) |
| **API** | Поиск и карточка: `guestDisplayPriceThb` в payload; PDP detail: **`GET /api/v2/listings/[id]`** → `guestDisplayPriceThb`, `guestServiceFeePercent`. | **`lib/api/run-listings-search-get.js`**, **`app/api/v2/listings/[id]/route.js`** |
...
**Admin FinTech pulpit** — **`/admin/settings/finances`** (`components/admin/finances/AdminFinTechConsole.jsx`):
