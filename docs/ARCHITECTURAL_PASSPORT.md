...
- **Статусы брони (P0-2):** `lib/booking/status-transitions.js` — `PARTNER_BOOKING_STATUS_TRANSITIONS`, `OCCUPYING_BOOKING_STATUSES`, **CHECKED_IN ≠ THAWED**; импорт: partner `PUT`, `BookingService.updateStatus`, `booking-occupancy-statuses.js`, hint в `booking-status-display.js`.
