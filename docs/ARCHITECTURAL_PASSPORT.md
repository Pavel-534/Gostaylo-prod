...
### Stage 79 — Supabase OAuth (Google / Apple), 2026-05
...
- **Юридический гейт:** см. **`POST /api/v2/auth/accept-legal/`** и страница **`/auth/complete-legal/`** (если оферта не была принята перед OAuth через модалку).
- **UI:** модалка в **`contexts/auth-context.jsx`**, браузерный клиент **`lib/supabase/oauth-browser-client.js`**, i18n ключи **`auth_continueGoogle`** / **`auth_continueApple`**.
