...
> **Version**: 12.170.13.1 | **Last Updated**: 2026-06-20 | **Stage 170.13.1:** `lib/layout/workspace-shell.js` — partner/admin frame below AppHeader, scroll in body (no gray band / breadcrumb overlap). | **Stage 170.11:** Similar rail + catalog card height SSOT + yacht map geo gate. | **Stage 170.10:** PDP specs/policies SSOT all verticals. | **Stage 170.9:** `RecommendationRailCard` + PDP map pin. | **Stage 170.8:** VisualViewport chat shell. | **Stage 170.7:** App Shell CI guard (`npm run check:shell`). | **Stage 170.6:** Partner/admin desktop shell hotfix. | **Stage 170.5:** Long tail shell tokens. | **Stage 170.4:** Partner/admin workspace sidebar. | **Stage 170.0–170.3:** App Shell SSOT waves. |
...
| **Каталог map↔list** | `onListingCardSelect` → highlight + `MapSelectionSync`; marker click → scroll card; cluster → `fitBounds(cellSizeM)`; **mobile ≤768px:** **`CatalogMobileMapSheet`** full-screen map (169.3) | **`InteractiveSearchMap.jsx`**, **`CatalogSearchMapPanel.jsx`**, **`CatalogMobileMapSheet.jsx`**, **`ListingSidebar.jsx`**, **`SearchMapWrapper.jsx`** |
...
- **Map pins API (Stage 163.0–163.3):** **`GET /api/v2/search/map-pins`** — lean pins `{ id, lat, lng, price, status, isApproximate }`; privacy verticals → fuzzed coords via **`lib/geo/listing-public-coordinates.js`** (ADR-163); **`mode:clusters`** — centroid = **grid cell center** (`listings_map_clusters_grid_v1`, **`migrations/stage163_3_cluster_centroid_privacy.sql`**), meta **`clusterCentroidMode: grid_cell_center`**; GiST bbox **`listings_map_pin_ids_in_bbox_gist_v1`**. Клиент: **`CatalogSearchMapPanel`** (SSOT body) in **`SearchMapWrapper`** (desktop) + **`CatalogMobileMapSheet`** (mobile, 169.3) + **`useMapPinsFetch`**; lazy popup **`ListingMapPopupLazy`**. Wave C privacy gap closed (pins fuzz + cluster cell center).
