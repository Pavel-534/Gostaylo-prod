...
> **Version**: 12.178.9.0 | **Last Updated**: 2026-07-12 | **Stage 178.9:** dynamic popular destinations + recent search history (`PopularDestinationChips`, `fetchPopularDestinations`, `airento_recent_searches`). **Stage 178.8:** mobile tab «Поиск» context (`mobile-search-tab-action.js`). **Stage 178.7:** unified mobile search sheet home+catalog (`CatalogMobileSearchSheet`); legacy **`MobileSearchBottomSheet`** removed.
...
- **Каталог:** expanded **`FilterBar`** fades `1-morphT`; compact cross-fade from progress **0.28**; CSS height **expanded → compact** lerp. **Mobile `<md`:** in-flow **`FilterBar`** заменён на **`CatalogSearchSummaryBar`**; поиск — **`CatalogMobileSearchSheet`** с **`FilterBar`** / **`UnifiedSearchBar variant="filter"`** (сетка Что·Куда·Даты·Гости + Фильтры), state SSOT **`usePublicSearchFilters`**. **Главная `<md`:** тот же **`CatalogMobileSearchSheet`** (Stage 178.7); hero field tap → **`initialFocusSection`**; под «Куда» — **`PopularDestinationChips`**: недавние (`airento_recent_searches`, commit «Найти») + API **`fetchPopularDestinations`** (inventory-ranked suggest). **Tab bar «Поиск»** (Stage 178.8): **`lib/search/mobile-search-tab-action.js`** — home → `navigateToCatalog`, catalog → scroll + sheet.
