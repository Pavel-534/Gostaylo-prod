...
| **Каталог map↔list** | `onListingCardSelect` → highlight + `MapSelectionSync`; marker click → scroll card; cluster → `fitBounds(cellSizeM)`; **cluster mode:** выбранный listing pin поверх кластеров (171.14); **approximate fitBounds:** server fuzz coords only, no client 500m pad (171.15); **mobile ≤768px:** **`CatalogMobileMapSheet`** full-screen map (169.3, 173.3: below AppHeader + floating «Список») | **`InteractiveSearchMap.jsx`**, **`CatalogSearchMapPanel.jsx`**, **`CatalogMobileMapSheet.jsx`**, **`ListingSidebar.jsx`**, **`SearchMapWrapper.jsx`** |
