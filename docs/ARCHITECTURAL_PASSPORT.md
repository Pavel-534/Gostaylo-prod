...
- **P&L engine (Stage 109.1 → 131.0):** facade **`lib/services/marketing/referral-pnl.service.js`** → **`referral-policy.service.js`**, **`referral-calculation.js`**, **`referral-payout.service.js`**. **Stage 131.0:** параметры waterfall/caps — **`system_fintech_settings`** через **`SystemConfigService`**; acquiring от **`guest_payment_thb`**; USN/НДС/bank от **`owner_revenue`** (commission + `fx_markup_thb` владельцу); program cap **`referral-program-cap.service.js`**. Safety lock `safety_lock_max_share` (default 0.95) от commission gross. Promo tank остаётся в **`system_settings.general`**.
...
- **MLM depth=2:** распределение partner activation bonus по L1/L2 upline через `ancestor_path` + `referrer_id`; проценты — **`system_fintech_settings`** (Stage 131.0), legacy fallback из `general`.
...
**Public calculator (Stage 131.1):** **`GET /api/v2/referral/calculator`** — marketing-only (L1/L2/cashback); **`/about/referral`**. SSOT itemized targets: **`ADR131_REFERENCE_TARGETS`** in `lib/services/finance/fintech-waterfall.js`.

**L2 Shadow Mode (Stage 131.1):** при `ambassador_guest_l2_enabled=false` — `referral-guest-l2-shadow.service.js` в `distribute()` пишет **`metadata.fintech_snapshot.shadow_l2_thb`** (caps 500/бронь, 50k/мес); ledger L2 **не** создаётся.
