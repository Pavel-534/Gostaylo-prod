...
- **Branding:** ключи `partnerCtaTitle` и др. в переводах переведены на `{brand}`; поддержка email в маркетинге `/terms` и `/help` через `lib/config/legal-details.js` (`LEGAL_DETAILS.email`).
