...
- **Статусы брони (P0-2 / 119.2):** `lib/booking/status-transitions.js` — матрицы переходов; **SSOT применения** — **`lib/services/booking/booking-status.service.js`** (`transitionBookingStatus` → chat sync + **`referral-lifecycle-hook`**). Обёртка: `BookingService.updateStatus`. **CHECKED_IN ≠ THAWED**; hint в `booking-status-display.js`.
...
- **`POST /api/v2/payments/submit-txid` (124.18):** сессия + проверка **`bookings.renter_id`** (staff ADMIN/MODERATOR bypass).
- **Payment webhooks (124.18):** `POST /api/webhooks/payments/confirm`, `POST /api/webhooks/crypto/confirm` — **`assertWebhookGuestPaymentAllowed`** (`lib/payment/webhook-guest-payment-gate.js`: emergency pause, fiscal, treasury manual mode + structured audit log).
