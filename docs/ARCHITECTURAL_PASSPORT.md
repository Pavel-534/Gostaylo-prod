...
> **Version**: 12.178.8.0 | **Last Updated**: 2026-07-10 | **Stage 178.8:** mobile tab «Поиск» context (`mobile-search-tab-action.js`). **Stage 178.7:** unified mobile search sheet home+catalog (`CatalogMobileSearchSheet`); legacy **`MobileSearchBottomSheet`** removed. **Stage 178.6:** catalog mobile search restored — **`CatalogMobileSearchSheet`** + **`FilterBar`**. **Stage 178.3 Step 1:** **`CatalogSearchSummaryBar`**. **Stage 178.2:** PDP map cooperative touch (`<md`), vertical map title SSOT, always-active Book CTA → calendar anchor. **Stage 177.2c E3:** handler guard — no double `filterListingsByAvailability` when unified executor ran availability; `meta.pagination.refillAttempts`. **Stage 177.2c E2:** availability executor + cursor refill. **Stage 177.2c E1:** `stay.dates` registry + plan postSteps. **Stage 177.1.0:** unified discovery pipeline (`DISCOVERY_UNIFIED_PIPELINE`). |
...
- **Каталог:** expanded **`FilterBar`** fades `1-morphT`; compact cross-fade from progress **0.28**; CSS height **expanded → compact** lerp. **Mobile `<md`:** in-flow **`FilterBar`** заменён на **`CatalogSearchSummaryBar`**; поиск — **`CatalogMobileSearchSheet`** с **`FilterBar`** / **`UnifiedSearchBar variant="filter"`** (сетка Что·Куда·Даты·Гости + Фильтры), state SSOT **`usePublicSearchFilters`**. **Главная `<md`:** тот же **`CatalogMobileSearchSheet`** (Stage 178.7); hero field tap → **`initialFocusSection`**; популярные направления — **`PopularDestinationChips`** под «Куда». **Tab bar «Поиск»** (Stage 178.8): **`lib/search/mobile-search-tab-action.js`** — home → `navigateToCatalog`, catalog → scroll + sheet.
