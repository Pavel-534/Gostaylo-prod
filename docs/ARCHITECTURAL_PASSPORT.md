...
| **Избранное / недавние** | Клиент хранит/прокидывает `guestDisplayPriceThb` (favorites — `computeGuestDisplayFromBaseThb` при загрузке). | **`app/renter/favorites/page.js`**, **`lib/hooks/use-recently-viewed.js`**, **`hooks/useListingViewData.js`** |
...
### SSOT Chat (Stage 110.5–110.7)

| Слой | SSOT | Примечание |
|------|------|------------|
| **POST сообщение** | `lib/chat/post-chat-message.server.js` | Push, safety; route — делегат |
| **Клиент POST** | `lib/chat/post-chat-message.js` | Текст/voice/media/system — только через `useChatThreadMessages` на `/messages/[id]` |
| **POST invoice** | `lib/chat/post-chat-invoice.js` → API → `executePostChatMessageForUser` | `settleInvoiceDisplayAmount` (retail FX); префилл/гость — `fx-display-client.js` |
...
| **Захват оплаты** | `BOOKING_PAYMENT_CAPTURED` после PAID_ESCROW | `ledger/ledger-payment-capture.js` ← `escrow/ledger-capture.js` |
