...
> **Version**: 7.13.0 | **Last Updated**: 2026-04-24 | **Status**: Production-Ready
> 
...
- **Stage 38.0 — Idempotent reminder lock + social proof + `/promo` + request memo:** дедуп Flash reminder переведён на **атомарный** слой: ключ вида **`flash_1h_reminder_[YYYY-MM-DD-HH]`** (календарный час **Asia/Bangkok**) записывается в **`promo_codes.metadata.reminder_locks`** через RPC **`promo_try_acquire_reminder_lock`** (миграция **`008_stage38_promo_reminder_lock_rpc.sql`**); параллельные HTTP-вызовы кронов не создают дублей. **`last_reminder_sent_at`** по-прежнему обновляется после успешной отправки (аудит). **Social proof:** поиск и **`GET /api/v2/listings/[id]`** обогащают карточку полем **`catalog_flash_social_proof: { bookingsCreatedCount }`** — число броней с **`promo_code_used`** за **текущие сутки по Bangkok**, для того же flash-промо что и **`catalog_flash_urgency`**; UI: **`GostayloListingCard`** и **`/listings/[id]`** (i18n **`listingDetail_flashSocialProof`**). **Telegram:** команда **`/promo`** (партнёр) — список активных Flash с остатком времени и **`Броней: N`** (все созданные брони по коду, как KPI Stage 35). **Performance:** `isCodeApplicable` в **`lib/promo/promo-engine.js`** обёрнут в **`cache` из `react`** на сериализованный вход; **`checkApplicabilityCached`** — алиас без TTL-карты.
