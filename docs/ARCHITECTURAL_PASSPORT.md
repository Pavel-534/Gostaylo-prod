...
### Performance & Caching (Stage 113.0)

| Слой | Механизм | Где |
|------|----------|-----|
| **In-flight dedup** | `dedupeClientRequest(key, fn, { ttlMs })` — один параллельный запрос на ключ | `lib/api/client-request-dedup.js` |
...
| **FX / categories persist** | `localStorage` v3, 2h FX (`lib/client-data.js`) + categories in-memory TTL | главная, валюта |
| **Logout bust** | `invalidateAllClientRequests()` в `clearBrowserPersistedAuthState` | сессия / чат |
...
**Не используем** TanStack Query на Stage 113 (кроме уже существующих partner calendar / finances / reputation hooks).
...
| **Клиентский кеш** | `exchange_rates_retail` / `exchange_rates_mid` | **`lib/client-data.fetchExchangeRates({ retail })`** |
