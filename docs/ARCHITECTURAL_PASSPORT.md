...
- **Admin marketing IA (Stage 124.0):** **`/admin/marketing`** — обзорный дашборд (`MarketingDashboard`); **`/admin/marketing/promos`** — промокоды (бывший корень); горизонтальная навигация **`MarketingSubNav`** в `app/admin/marketing/layout.js`; sidebar **`lib/admin/admin-menu.ts`** — Обзор (exact), Кампании, A/B правила, Настройки, Fraud Queue, Аналитика (`attribution`), Бюджет и аудит (`/budget` hub → settings, wallet-audit, audit, payouts, analytics). Скрытые из sidebar маршруты сохранены.
