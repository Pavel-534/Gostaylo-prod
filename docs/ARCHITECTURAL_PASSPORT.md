...
- **Cron health (P0-4):** `lib/admin/financial-cron-health.js` — последние прогоны `escrow-thaw`, `promote-ready-for-payout` из **`ops_job_runs`**; UI **`FinTechCronHealthPanel`** на **`/admin/settings/finances`**; данные в **`GET /api/admin/finances/treasury-ops`** → `cronHealth`; stale **> 3 ч** → предупреждение + **`TREASURY_CRON_STALE`** в TG FINANCE.
