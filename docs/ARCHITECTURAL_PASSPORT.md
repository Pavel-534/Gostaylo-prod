...
- **P&L engine (Stage 109.1):** facade **`lib/services/marketing/referral-pnl.service.js`** → **`referral-calculation.js`**, **`referral-stats.service.js`**, **`referral-payout.service.js`**. Рассчитывает `AdjustedNetProfitOrder` (`PlatformGross - InsuranceReserve - AcquiringFee - OperationalReserve`) перед применением `referral_reinvestment_percent`; safety lock `95% PlatformGross` сохраняется.
