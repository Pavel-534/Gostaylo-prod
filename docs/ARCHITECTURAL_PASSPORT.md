...
- **Главная — `components/PlatformHomeContent.jsx`:** тонкий оркестратор; **`components/home/`** — **`HomeHeroLuxe`** (Airy Premium hero, единая 60-px геометрия `Where/Dates/Guests/CTA`, env-копии через **`lib/config/home-page-copy.js`** с поддержкой токена `'AUTO'`), **`StickySearchBar`**, **`TopListingsGrid`** (env-копия **`NEXT_PUBLIC_HOME_TOP_LISTINGS_TITLE`**), shared **`home-constants.js`**, **`useHomeFilters.js`** (What/Where/When/Who, дебаунс, сид из URL, semantic flag с сайта). **`WhereCombobox`** имеет `variant='flat'` для встраивания в 60-px поле. Гости унифицированы через SSOT-компонент **`components/search/GuestsPopover.jsx`** (Adults/Children/Infants, +/- controls, summary text в trigger) и переиспользуются в `HomeHeroLuxe`, `StickySearchBar` и `UnifiedSearchBar` (поиск). Overlay-контракт унифицирован: popover’ы рендерятся через portal в `body` (`components/ui/popover.jsx`), desktop/dropdown держатся выше search/header по z-index, mobile для where/guests/dates — через `Drawer` шторки. Дублирующий **`CategoryBar`** на главной не используется — категории остаются только как pill-фильтры в блоке поиска. Вертикальный ритм main-секций: `py-12 sm:py-16`. Скелетон **`ListingCardSkeleton`** соответствует реальной карточке (no CLS).
