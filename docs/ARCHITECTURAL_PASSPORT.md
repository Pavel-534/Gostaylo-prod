...
> **Version**: 12.178.3.3 | **Last Updated**: 2026-07-09 | **Stage 178.3 Step 3:** wizard step router `what→where→when→who`; **`SearchCalendar`** / **`GuestsPopover`** `presentation=wizardStep`; **`CatalogMobileSearchSheet`** deleted. **Step 2:** **`MobileSearchWizard`** + **`WhereCombobox.presentation=wizardStep`**. **Step 1:** catalog mobile **`CatalogSearchSummaryBar`**. **Stage 178.2:** PDP map cooperative touch (`<md`), vertical map title SSOT, always-active Book CTA → calendar anchor. **Stage 177.2c E3:** handler guard — no double `filterListingsByAvailability` when unified executor ran availability; `meta.pagination.refillAttempts`. **Stage 177.2c E2:** availability executor + cursor refill. **Stage 177.2c E1:** `stay.dates` registry + plan postSteps. **Stage 177.1.0:** unified discovery pipeline (`DISCOVERY_UNIFIED_PIPELINE`). |
...
- **Каталог:** expanded **`FilterBar`** fades `1-morphT`; compact cross-fade from progress **0.28**; CSS height **expanded → compact** lerp. **Mobile `<md` (Stage 178.3):** in-flow **`FilterBar`** заменён на **`CatalogSearchSummaryBar`**; поиск — один Vaul **`MobileSearchWizard`** (шаги **`what → where → when → who`**, draft **`useMobileSearchWizardDraft`**, Apply → **`commitToUrl({ snapshot })`**); inline панели **`WhereCombobox`**, **`SearchCalendar`**, **`GuestsPopover`** с **`presentation="wizardStep"`** (без вложенных Drawer/Popover). **`CatalogMobileSearchSheet`** удалён.
