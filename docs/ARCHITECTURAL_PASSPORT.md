...
|-------|-------------|------------|
| **RUB direct (банк РФ)** | `payout_batches.rail` **`TBANK_RU`**, метод **`pm-bank-ru`** / channel **BANK** | CSV реестр: **`POST /api/v2/admin/payouts/tbank-registry`**, UI **`/admin/financial-health`**. Партнёрский профиль: ФИО, ИНН, БИК, счёт. |
