...
> **Version**: 12.179.2.0 | **Last Updated**: 2026-07-13 | **Stage 179.2:** mobile catalog skeleton refetch (CLS). **Stage 179.1:** **`CategoryQuickChips`**. **Stage 179.0:** hero direct pickers; **`.gsl-premium-glass`**.
...
| **Каталог mobile refetch UX (179.2)** | **`<md` only:** при `loading` / `isTransitioning` / `aiSearchPending` **`ListingSidebar`** рендерит **`ListingGridSkeleton`** × **`MOBILE_CATALOG_SKELETON_COUNT` (4)** с **`LISTING_CATALOG_GRID_CLASSES`** (тот же `grid-cols-1 gap-5`, что у реальной сетки); **`ListingCardSkeleton`** повторяет **`aspect-[4/3]`** + min-heights из **`lib/listing/listing-card-layout.js`**. **`md+`:** прежний cross-fade (`opacity-50` на refetch). | Полная замена списка на mobile; дублирование grid-классов |
