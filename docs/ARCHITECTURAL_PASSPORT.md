...
- **Каталог:** expanded **`FilterBar`** fades `1-morphT`; compact cross-fade from progress **0.28**; CSS height **expanded → compact** lerp. **Mobile `<md` (Stage 178.3–178.5):** in-flow **`FilterBar`** заменён на **`CatalogSearchSummaryBar`**; поиск — один Vaul **`MobileSearchWizard`** (единый scrollable экран: **Куда** + **Даты** + **Гости** inline, `presentation="wizardStep"`, без step-router и под-шторок; draft **`useMobileSearchWizardDraft`**, Apply → **`commitToUrl({ snapshot })`**). **`CatalogMobileSearchSheet`** удалён.
