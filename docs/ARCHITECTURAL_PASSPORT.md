...
| **Недавно смотрели** | localStorage + **`listing_views`** после логина; merge в `useRecentlyViewed`; **`RecentlyViewedRail`** on PDP + home (`recent_pdp` / `recent_home`, min 2 on home); `POST/GET /api/v2/listing-views` | **`migrations/stage167_1_listing_views.sql`**, **`lib/recommendations/listing-views.service.js`**, **`components/PlatformHomeContent.jsx`** |
