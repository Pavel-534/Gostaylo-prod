...
> **Version**: 12.156.0 | **Last Updated**: 2026-06-17 | **Stage 156.0:** Wave 1E sealed — vertical guest payment notify copy (`guest-booking-labels.js`); Playwright `guest-inquiry-golden-path`; E2E `promote-booking-paid-escrow`. | **Stage 155.6:** PDP post-inquiry next steps; Order Help vertical ctx; PostHog guest journey events. | **Stage 155.5:** Resend transport guard — smoke/E2E/test recipients skip real Resend API (`lib/email/resend-transport-guard.js`; Cursor rule `gostaylo-resend-transport-guard.mdc`). | **Stage 155.3:** Core User Journey Wave 1C — guest vertical warm terminology (`guest-booking-labels.js`, `getUIText` placeholders); `GuestBookingNextStepsCard`; upcoming tab includes `PENDING`/`INQUIRY`; escrow badge post-thaw fix; checkout escape links; PWA manifest brand SSOT. | **Stage 155.2:** Core User Journey Wave 1B — chat **`AWAITING_PAYMENT`** pay-bar; invoice bubble self-heal + **`PAID_ESCROW`** sync; partner payout callouts via **`payout-release-config.js`**; dashboard **`?published=1`** URL cleanup; guest escrow notify dedup on reconcile; partner application **`rejectionReason`** in settings. | **Stage 155.1:** Core User Journey Wave 1A — guest notify on partner decline/cancel; guest escrow payment confirm (email/push/TG); checkout success escrow SSOT copy; white-label SEO metadata; honest listing wizard moderation CTA + `?published=1` dashboard banner. | **Stage 154.3:** Wave 1C — Telegram webhook **Lite-Protection** (structural `update` validation + **`TELEGRAM_BOT_TOKEN`** format gate; no `secret_token` / manual setWebhook). | **Stage 154.2:** Wave 1B — DB-backed **`requireSession`** (ban/role); private **`chat-attachments`** + signed URL guard route (`stage154_03`). | **Stage 154.1:** Security Wave 1A — **`GET /api/v2/profile`** session guard (self + staff); private **`verification_documents`** bucket + 15m signed URLs (`admin`/`partner` routes); referral **`referral_ledger`/`referral_codes`/`referral_relations`** RLS + `service_role` only (`stage154_01`/`02`); smoke **12j**. | **Stage 153.3:** Health Panel signals (audit/saga/bucket/fiscal/gateway/referral-shadow); **`runGatewayLedgerReconciliation`** 24h PAID intents vs ledger capture → **`GATEWAY_LEDGER_DRIFT`** (daily **`financial-health-monitor`**). | **Stage 153.2:** Dispute saga repair (`resolution_ledger_status`, `resolution-retry` ADMIN-only); booking **`financial-timeline`** API; soft **`POST_DISPUTE_BUCKET_DRIFT`** after arbitration. | **Stage 153.1:** Admin governance — **`admin_audit_logs`** append-only (`stage153_01`, service_role INSERT, partial unique **`idempotency_key`**); **`recordAdminAudit`** fail-safe (`ADMIN_AUDIT_WRITE_FAILED`); HTTP **`Idempotency-Key`** on dispute financial actions + payout batch **`settled`**; dispute **`force_refund`/`split`/`freeze_payment`** ADMIN-only; **`split`** terminal guard; smoke **12i**. | **Stage 152.3:** Batch search occupancy TZ parity — **`batch_check_listing_availability`** `booking_load` via **`resolve_listing_timezone_v1`** (`stage152_03`); JS catalog input dates — Wave 3b backlog (`calendar-update.service.js`). | **Stage 152.2:** Escrow thaw listing TZ (`listingYmdLocalWallTimeToUtcIso`, no `hour - 7`); atomic iCal replace RPC **`replace_calendar_blocks_for_source_v1`** (`stage152_02`). | **Stage 152.1:** Floating Local Date; **`THAWED`** occupying SQL (`stage152_01`); checkout-hold TTL cron (`checkout-hold-expiry.js`); iCal TZ hardening (no `hour - 7`); smoke **12h** capacity invariant. | **Stage 151.2:** Referral fail-fast (`assertReferralAtomicCreditResult`); host activation → atomic RPC; shadow **`PAYMENT_INSTRUMENT_COLLISION`** (`payment-instrument-fingerprint.js`, soft-hold); smoke **`assertWalletBucketIntegrity`** on **12f/12g**. | **Stage 151.1:** Referral **`referral_distribute_bonus_atomic`** RPC — ledger earn + wallet credit + bucket split in one TX (`referral-distribute-atomic.service.js`); smoke **`withSmokeRetry`** on **12f/12g**; agent/docs rebrand **Airento** via `getSiteDisplayName()`. | **Stage 150.4:** Success 4-step Next Steps + escrow SSOT hint; chat optimistic **`PAID_ESCROW`** + **`readGuestPaymentDisplay`** in **`DealDetailsCard`**; checkout initiate human errors. | **Stage 150.2:** Guest journey polish — PDP auth-resume (`sessionStorage` + `useAuthModalState`); checkout **`isBookingPayable`** FSM guard (UI + **`payment/initiate`**); success Next Steps + acquirer return poll fix; chat pay bar **`AWAITING_PAYMENT`** parity + **`fromPayment`** refetch; **In-flight PENDING policy** (pay in-flight bookings when listing → PENDING). | **Stage 149.4:** **`invoices.status`** CHECK + **`expired`** (`stage149_4`) — cron expiry больше не падает молча; **`toFiscalKassaPayload`** именованный импорт (был не-метод класса); `creation.js` return `feeSplit`→`activeFeeSplit`. | **Stage 149.3:** Smoke step **6e** — **`POST /api/v2/bookings`** on **`max_capacity=1`** (atomic RPC path) + **409 `DATES_CONFLICT`**; invoice expiry smoke → **`expireInvoiceHoldBlocks`**; smoke **`getSessionPayload`** override; prod requires **`stage149_2`** + **`stage149_3`** + **`stage149_4`**. | **Stage 149.2:** OG Guest-Gate; Payout Profile Guard; **`AWAITING_PAYMENT`** occupying; RPC phantom LEFT JOIN fix (`stage149_2` + `stage149_3` batch); invoice expiry → **`expireInvoiceHoldBlocks`**. | **Stage 149.1:** Post-publish share loop in `PartnerHostNextStepsCard`; moderation toast/banner; first-listing redirect; PATCH price Zod gate. | **Stage 149.0:** Payout release SSOT **`lib/booking/payout-release-config.js`** — vertical thaw copy + 24h hold; `wizard_profile` in thaw path; THAWED notifications fixed. | **Stage 148.1:** Host wizard Wave 1 — publish uses draft listing id after photo upload; StepPricing host net + payout blurb; WelcomePartnerModal → wizard; onboarding listing-first. | **Stage 148:** Read-only Host Flow audit (wizard/onboarding/money clarity). | **Stage 147.2:** Payout timing copy all locales → 24h after guest check-out; `formatBookingAmountForNotify` → `readGuestPaymentDisplay`. | **Stage 146.2:** Guest-facing copy — host/landlord/owner in renter UI (not «партнёр»); `get-guest-provider-label.js`; i18n sweep checkout/orders/chat/help/trust badges; `chatSystem_extensionConfirmed`; `messengerThread_label*`; invoice message lookup fallback. | **Stage 146:** Chat-Commerce trust — invoice message PAID sync + Realtime; post-escrow extension invoices (`payment/confirm` + `payments/confirm` webhook); checkout escrow trust copy + primary CTA `/my-bookings`; chat enrich `financial_snapshot`; `DealDetailsCard` role-aware money. | **Stage 145:** Email fallback on `success:false` (`notify-email-delivery.js`); Telegram legacy cleanup (`lib/telegram.js` → `telegram.service.js`); partner calendar page i18n (7 RU hardcodes removed). | **Stage 144:** Telegram notify i18n (`notify-telegram-copy.js`); `bookings.metadata.ui_locale` on create; `docs/SOFT_LAUNCH_GO_NO_GO.md`. | **Stage 143.1:** UX consistency polish — `WorkspaceEmptyState`, partner nav descriptions, `PartnerHostNextStepsCard`, referral ambassador guide (`ReferralAmbassadorWaveGuide`, `docs/AMBASSADOR_FIRST_WAVE_GUIDE.md`), `ReferralPartnerSupplyStrip`, welcome modal coherence, marketing email locale. | **Stage 143:** Referral funnel seamlessness (`ReferralCatalogFunnelStrip`, `useReferralModalFollowup`); notification locale SSOT (`resolve-notify-locale.js`). | **Stage 142:** Health dashboard hardening — sanitized **`ops_job_runs`**, notification channel metrics (Telegram/FCM 24h), critical signals filter/search. | **Stage 141.3:** Dispute UX widget on guest/partner booking cards; `booking_reviews` view; KYC light banner; admin health guard (`ADMIN` + `ADMIN_HEALTH_EMAILS` only). | **Stage 141.2:** Ledger timeline in admin dispute card — **`loadDisputeLedgerTimeline`**, **`AdminDisputeLedgerTimeline`**, **`financialSummary`**; **`settleDisputeHoldForRefund`** after **`REFUND_GUEST`**; **`close_dispute`** explicit **`resolutionStrategy`** (+ SPLIT). | **Stage 141.1:** Unified dispute arbitration — SSOT **`dispute-resolution-engine.js`** (`PAYOUT_PARTNER` / `REFUND_GUEST` / `SPLIT` / `DISMISS`); SLA + admin **`force_refund`** / **`close_dispute`** via **`executeDisputeResolution`**; **Split** — partial guest refund + **`settleDisputeHoldForSplit`** (`DISPUTE_SPLIT_HOLD_SETTLED`); admin **`action=split`** (`guestPercent`); UI slider + preview; **`GET …/disputes/[id]`** → **`splitPreview`**. | **Stage 141:** Dispute escrow hermetization — `getFrozenBookingIdSet` SSOT in **`partner-payout-eligibility.js`** (includes **`PENDING_MEDIATION`** soft block); **`markBatchSettled`** dispute guard + Telegram alert; SLA auto-resolve → **`resolveDisputeInFavorOfGuest`** (ledger refund, **`REFUNDED`**, no partner unfreeze); admin **`force_refund`** ledger + **`RESOLVED`**; **`freeze_payment`** → **`applyDisputePayoutFreeze`**; FSM **`READY_FOR_PAYOUT→REFUNDED`**; fix admin dispute GET `request` param. | **Stage 140.4:** Master Calendar iCal visual (`blockKind` manual/ical/inventory), legend + **`sync-partner-all`** button; **`ReferralBonusSavedBanner`** on catalog + PDP (`autoFromPendingRef`). | **Stage 140.3:** Host Realtime & Notifications — migration **`stage140_3_partner_realtime_publication.sql`** (`bookings`/`wallet_transactions` → `supabase_realtime`); **`hooks/usePartnerRealtime.js`** (invalidate partner stats/bookings/calendar/finances/wallet); SSOT **`lib/partner/partner-notification-events.js`** + **`PartnerNotificationProvider`**; bell **`PartnerNotificationFeed`** (desktop + mobile); foreground toasts **`PartnerForegroundNotifications`**; push fixes in **`payment-events.js`** (`PAYMENT_RECEIVED`/`PAYMENT_PENDING_PARTNER`); i18n **`partnerNotif_*`**. | **Stage 140.2:** Host P1 — SSOT денежного пути **`lib/booking/host-money-stage.js`** (`getHostMoneyStage`, без жаргона; интегрирован в finances-карточки + dashboard future-income); i18n-слайс **`lib/translations/slices/partner-ui.js`** (`partnerListings_*`/`payoutProfiles_*`/`hostMoney_*`) — `app/partner/listings` и `app/partner/payout-profiles` без захардкоженного RU; rejected-карточка показывает `rejectedAt`; design-system: `teal-*`/`indigo-600`/`#0d9488` → `variant="brand"`/`brand`-токены/`brandMintHex`; edit-mode `isDirty` через `wizardCompareKey` + серверный baseline. | **Stage 140.1:** Host P0 — wizard autosave SSOT **`lib/partner/wizard-draft-storage.js`** (localStorage `gostaylo_wizard_draft`, create-mode restore/`beforeunload`/`isDirty`), clear on publish; **`GET /api/v2/partner/listings`** returns **`rejectionReason`/`rejectedAt`**; `REJECTED` listing cards show reason + edit CTA. | **Stage 139.1:** auth modal SSOT **`useAuthModalState`** + **`closeLoginModal(outcome)`**; shared **`ReferralBonusSavedBanner`** + **`hasPendingReferralClient()`** for catalog/PDP embed. | **Stage 139:** host wallet skeleton (`UnifiedBalanceSummarySkeleton`, no CLS); partner dashboard fully i18n (`partnerDashboard_*`, RU/EN) + `isUpdatingBooking` fix; auth ctx exposes `loginModalOpen`; ambassador landing follow-up on modal close (`stage139_followup*`). | **Stage 138.3:** guest bookings canon **`/my-bookings`** (`UnifiedOrderCard`, status tabs, `?booking=`); **`/renter/bookings`** redirect; Super-App copy pass. | **Stage 138.2:** checkout **`CheckoutPaymentFailedView`** + return poll failure recovery (YooKassa cancel/decline/timeout). | **Stage 138.1:** guest Pay CTA SSOT **`lib/booking/booking-status-rules.js`** (`isBookingPayable`); instant-book → **`/checkout/[id]`**; inquiry → chat; **`/dashboard`** partner redirect → **`/partner/dashboard`**. | **Stage 133.0:** Team Analytics — RPC **`referral_team_analytics_for_referrer`**, **`GET /api/v2/referral/me` → `teamAnalytics`**, **`ReferralTeamAnalyticsCard`** + per-member **`earnedForReferrerThb`** on tab «Команда»; L1/L2 ADR-133 semantics; host retention KPI. | **Stage 132.3:** referral withdrawal status notifications (approve / registry / paid); wallet in-app banner + sticky mobile CTA; **`RELEASE_CHECKLIST_RU.md`**. | **Stage 132.2 (closed):** Sprint 3 — INN/BIK validation i18n keys; **`PartnerReferralWizardBanner`** on listing wizard; OG dual guest/host CTA (`stage1322_uMetaDescription`); zh/th **`stage1322_*`** EN fallbacks. Sprint 2 — Guest/Host share tabs, **`ReferralMentorStrip`**, RU dual-currency teasers, withdrawal i18n. Sprint 1 — **`GET /api/v2/partner/referral-context`**, **`PartnerReferralWelcomeStrip`**, OG on **`/go/[vanity]`**. | **Stage 132.1:** **`/admin/marketing/referral-payouts`** (Single Payout Ops); **`GET` tbank-registry preview**; wallet i18n; legacy strip removal. | **Stage 132.0:** **`ReferralBalanceBreakdown`** + **`payout.blockerDetails`**; fraud-queue resolve UX; client RU bank validation. | **Stage 131.8:** **`ReferralFraudResolveService`**; **`securityHeldReferralBalanceThb`**; INN checksum; CIS stubs; smoke **12g** approve. | **Stage 131.6:** Dual SSOT — marketing admin → `system_fintech_settings` only; `payouts.payout_rail` + `metadata`; **`REFERRAL_RUB_CARD`**; smoke **12f**. | **Stage 131.5:** FinTech Bridge (referral `user_wallets` queue → `payouts` PENDING RUB → T-Bank CSV); RU payout profile gate; Live L2 in `distribute()`; RUB-only withdrawal waterfall; team metrics split. | **Stage 131.4:** withdrawal waterfall UI; vanity welcome host; L2 live/shadow counter split; smoke webhook import fix. | **Stage 131.3:** калькулятор FX-тoggle (THB/RUB); vanity **`/go/{code}`**; withdrawal preview + 1.5% fee + debit on approve; **`createL2PendingLedgerRow`** stub. | **Stage 131.1:** itemized ADR recalc (pool 1300 @ 35k/15%); L2 **Shadow Mode** (`shadow_l2_thb` in booking snapshot, no ledger); public calculator **`/about/referral`** + **`GET /api/v2/referral/calculator`**. *(Stage 131.0: fintech_snapshot, admin API, audit.)* | **Stage 130.9:** Rebrand SSOT — `lib/site-url.js` multi-domain (`KNOWN_SITE_HOSTS`, `getSiteEmailDomain`, `buildDefaultFromAddress`); `lib/site-url.cjs`; email/auth/manifest Airento. | **Stage 130.8:** Supabase Split URL — `NEXT_PUBLIC_SUPABASE_URL` (browser proxy) + `SUPABASE_SERVER_URL` (direct); `lib/supabase.js` SSOT; OAuth callback `X-Forwarded-Host`; airento.ru/com whitelists; origin fallback `airento.ru`. | **Stage 130.4:** YooKassa Controlled Live — `YookassaOpsCard` + `GET /api/admin/finances/treasury-ops` → `yookassaOps`; smoke `test:true` assert; `controlled-live-mir-guard.js`; webhook IP `x-vercel-forwarded-for`. | **Stage 129.1:** Brand SSOT rollback — removed hardcoded «Airento» from Tailwind/docs; `lib/site-brand.cjs` CJS bridge; display name stays `getSiteDisplayName()`. | **Stage 129.0:** Design System Foundation — `lib/theme/tokens.ts` + `tokens.cjs`, Tailwind brand mint/navy, Inter via `var(--font-sans)`, `.gsl-*` tokenized, `.cursorrules` UI rules. | **Stage 128.4:** RQ foundation closed — **PAUSED** until first MIR; logout verify; Iterations 3–4 frozen. | **Stage 128.3:** TanStack Query Iteration 2 — FX SSOT (`useFxRatesQuery`), featured grid RQ, profiles (`useProfileMeQuery`). | **Stage 128.2:** unified `queryKeys.public.categories`, hover PDP prefetch. | **Stage 128.1:** catalog search RQ (no `searchCache`), home live count. | **Stage 128.0:** `clearClientQueryCache` on logout, `lib/query-keys.js`, `lib/api/query-fetch.js`. | **Stage 127.1:** Pre-flight — smoke 12c backoff, PAID_ESCROW sweep, USDT re-export, Prisma enum warning. | **Stage 127.0:** Payment core hardening — webhook idempotent escrow pipeline; `getExpectedUsdtForBooking` SSOT; marketplace-health ROI → `buildReferralRoiReport` (30d). | **Stage 126.3:** First MIR prep — ACTIVE banner, TG MIR enrichment, FI Real Payments Only preset. | **Stage 126.2:** Controlled Live minimalism (FI + TG, soft limit). | **Stage 126.0:** Controlled Live launch; `docs/CONTROLLED_LIVE_RUNBOOK.md`. | **Phase 1 CLOSED:** Pre-Live Hardening 125.0–125.8. | **Stage 125.7:** Pre-Live Readiness card on `/admin/settings/finances` (`loadPreLiveReadiness`, `PreLiveReadinessCard`); `PRE_REAL_PAYMENTS_CHECKLIST.md` Phase 1 owner sign-off. | **Stage 125.6:** fiscal idempotent reconcile on escrow retry (`PENDING_FISCAL`+attempts); smoke 6a. | **Stage 125.5:** treasury alerts → `critical_signal_events` only (no `system_settings` RMW ring); migration `stage125_5_clear_treasury_ops_alerts_ring.sql`. | **Stage 125.4:** `crypto/confirm` idempotent 2xx post-escrow (same status set as 125.1); smoke 7c. | **Stage 125.3:** FSM guard — `PAID_ESCROW` только через `moveToEscrow` RPC (`viaEscrowRpc`); smoke 6d. | **Stage 125.2:** `submit-txid` guard — no regression on `CONFIRMED`/`COMPLETED` payments or post-escrow bookings (409); smoke 6c. | **Stage 125.1:** `payments/confirm` idempotent 2xx for post-escrow booking statuses (`status-sets.js`); smoke 7b. | **Stage 125.0:** `moveToEscrow` killswitch + side-effect reconcile; smoke 6a. | **Stage 124.19:** HTTP checkout smoke (initiate + mock webhook); Cron Health 7 jobs + errors in owner UI; `referral-unlock` → `ops_job_runs`; `PRE_REAL_PAYMENTS_CHECKLIST` owner 10-step block. | **Stage 124.18:** P0 audit fixes — `AWAITING_PAYMENT` enum + RPC whitelist (`stage124_18_awaiting_payment_escrow_rpc.sql`); FSM `PENDING|INQUIRY` → `AWAITING_PAYMENT`; `submit-txid` session+renter; `verify-tron` ownership; webhooks `assertWebhookGuestPaymentAllowed`; FinTech Cron Health 6 jobs + `referral-reconciliation` → `ops_job_runs`; smoke step 6 checkout path. | **Stage 124.17:** Phase D official closure + `PHASE_D_CLOSURE_AND_ROADMAP.md` (debt review, 2–3 mo plan). | **Stage 124.16:** Phase D closed — owner guide card, FI↔ROI navigation polish. | **Stage 124.15:** LTV/retention, fraud-adjusted ROI, business summary. | **Stage 124.14:** Campaign ROI drill-down page + bookings→FI P&L; chart day/week rollup. | **Stage 124.13:** Owner weekly marketing digest (email/Telegram), realtime ROI/budget alerts, digest settings on ROI page, cron Mon 07:00 UTC. | **Stage 124.12:** Referral ROI CAC + budget alerts + CSV/Excel export; FI Referral Economics links. | **Stage 124.11:** Referral ROI dashboard, campaign/source metrics, FI Referral Economics widget. | **Stage 124.10:** FI smoke 19/19, bank reconciliation DB. | **Stage 124.9:** FI net profit after all deductions, ADR-097 jurisdiction panel, obligations + bank reconciliation, Excel xlsx export, P&L RU/KG/FX flow. | **Stage 124.7:** FI category/partner drill-down, treasury timeline, P&L flow UI, owner labels, filter presets. | **Stage 124.6:** FI P&L page, category rollup, partner liability drill-down, CSV export, FinTech CTA, cache on batch settle. | **Stage 124.5:** FI drill-down (bookings API, booking P&L, escrow aging, cash position, cache). | **Stage 124.4:** Financial Intelligence SSOT `lib/analytics/` + `/admin/finance/intelligence` + `GET /api/admin/finance/intelligence`. | **Stage 124.2:** marketing dashboard final polish (period compare abs+delta, chart, alerts). | **Stage 124.1:** marketing dashboard polish (period compare, alerts, chart); sub-nav 6 tabs; `/admin/marketing/rules` hub. | **Stage 124.0:** Admin marketing IA — dashboard `/admin/marketing`, promos `/admin/marketing/promos`, `MarketingSubNav`, budget hub, sidebar 6 items (no duplicate quickActions). | **Stage 121.2:** B2 closed — referral held UI strip + `vercel.json` unlock cron. | **Stage 121.1b:** Referral hold `earned_held` + `unlock_at`, cron `/api/cron/referral-unlock`, `held_referral_balance_thb`, admin KPI/filter. | **Stage 121.0b:** Referral funnel 2.0 tab + `computeReferralFunnelBundle` SSOT. | **Stage 121.1:** migration template GRANT+RLS (`migrations/_template_new_public_table.sql`, README, Cursor rule). | **Stage 121.0:** RLS security sweep (`stage121_0_rls_security_sweep.sql`). | **Stage 120.6:** Phase A closed — monetary dashboard UX polish. | **Stage 120.1:** E2E attribution chain smoke 12c, admin `/admin/marketing/attribution`, `GET /api/v2/admin/referral/attribution`. | **Stage 120.0:** referral attributions (`referral_attributions`), `GET /api/v2/referral/track`, cookie `gostaylo_ref`, ledger `metadata.attribution_id` (economics unchanged). | **Stage 119.3:** full booking status SSOT paths, manual referral reconcile, telemetry. | **Stage 119.2:** `booking-status.service.js` (SSOT status + referral hook), referral-reconciliation cron + admin health, E2E lifecycle1192. | **Stage 119.1:** referral financial lock — **`referral-lifecycle-hook.js`** (status + dispute), promo tank reversal, **`canceled_deficit`**, smoke step 12b. | **Stage 118.6:** search indexes + `request-access-cache`; **`docs/REFERRAL_PROGRAM_2_0_PLAN.md`**. | **Stage 118.5:** полный RBAC `/api/admin/*` + `/api/v2/admin/*`; меню Control → System Health; поиск с rank. | **Stage 118.4:** `admin-menu.ts` SSOT + `quickActions`; API RBAC `admin-api-access.ts` / `requireAdminStaff(request)`; `GET /api/admin/search`; `AdminGlobalSearch`, `AdminTableAmount`, `AdminStatusPill`. | **Stage 118.3:** `lib/admin/admin-menu.ts` (RBAC `allowedRoles`, breadcrumbs, quick actions); `AdminQuickActionsBar`; admin content `max-w-7xl`; фин. таблицы `format-signed-thb.js`. | **Stage 118.1:** chat-stress E2E только fixture-беседа; cleanup `st*-seq-*` messages; production block в `post-chat-message.server.js`. | **Stage 118.0:** `lib/admin/system-settings-store.js` — SSOT server read/write `system_settings`; миграция REST/fetch в API (commission, moderation, admin settings GET/PUT, iCal, chat-safety); THAWED→READY_FOR_PAYOUT date parse fallback; admin `formatAdminUserLabel` (wallet-audit, payouts); `cleanup:test-data` marketing/referral purge. | **Stage 117.4:** `cleanup:test-data:execute` — referral_ledger + marketing_promo_tank_ledger + wallet test purge (`test-marketing-referral-markers.js`). | **Stage 117.3:** `cleanup:test-data:execute` — aggressive test profile purge (`lib/e2e/test-user-markers.js`, `cleanup-test-users.service.js`: ledger_accounts, cascade, auth UUID only). | **Stage 117.2:** `OwnerLaunchReadinessCard` smoke/pause actions + ЮKassa hint; admin deep screens → `brand` tokens. | **Stage 117.1:** launch polish — `OwnerLaunchReadinessCard` quick links + plain copy; brand tokens on catalog/chat/partner listings; PDP OG fallback. | **Stage 117.0:** unified partner listings publish checklist; `OwnerPlatformStatusCard`; OG `resolve-og-image.js`. | **Stage 116.4:** owner launch checklist (`launchReadiness` in stats, `OwnerLaunchReadinessCard`); moderation list UX + revalidate on approve; security violators table polish. | **Stage 116.3:** admin owner ops (`ownerOps` in `GET /api/v2/admin/stats`); moderation filters (`GET /api/admin/moderation?partner&category&dateFrom&dateTo`); contact violators table + ban in `/admin/security`. | **Stage 116.2:** listing quality checklist UI + `GET /api/v2/partner/onboarding-status`; publish PENDING gate. | **Stage 116.1b:** contact safety default **ADVISORY** (rollback REDACT prod default); strikes → search demotion; admin strike PATCH + `/admin/security` listings. | **Stage 116.1:** quality gates, inquiry hold 48–72h, onboarding checklist. | **Stage 116.0:** launch blockers, PostHog MVP. | **Stage 115.4:** long tail Design System — auth modals, legal shell, renter profile, PDP booking, orders cards + ~80 files → `brand`/`variant="brand"`; components `teal-*` eliminated except logo SSOT. | **Stage 115.3:** final Design System migration — home marketing (HowItWorks, TrustBar, PartnerCTA, footer), FinTech hero gradient, empty states, chat thread/composer, partner calendar UI → `brand`/`gsl-*`; `GSL_FINTECH_HERO_GRADIENT` + brand shadows in `product-ui.js`. | **Stage 115.2:** deep product UI migration (home, messages, partner dashboard, PDP, AppHeader → `brand`/`gsl-*`, `MessagesAuthGate`, flow hints). | **Stage 115.0:** product UI cohesion — `docs/PRODUCT_UI_SYSTEM.md`, `.gsl-*` tokens, `components/product/*` (ProfileHubNav, ProductPageShell, GuestBookingFlowHint), `Button variant="brand"`, partner/renter nav + mobile bottom nav. | **Stage 114.7:** referral launch readiness — monthly spend approaching alert + FinTech bar, payout queue search/sort/filtered bulk, UX empty states + level/badge tooltips, runbook §16. | **Stage 114.6:** referral launch polish (UX skeletons, FinTech payout workflow + ledger bulk hold, GO_NO_GO referral checklist). | **Stage 114.5:** `docs/REFERRAL_ACCOUNTING.md`, FinTech accounting KPI + ledger hold/reject API, monthly spend alert, referral/me pagination + landing-meta cache. | **Stage 114.4:** referral transparency — FinTech liability filters/chart/export (`GET /api/v2/admin/referral/ledger-export`), admin alerts (`REFERRAL_ADMIN_ALERT`), `docs/REFERRAL_FINANCIAL_FLOW.md`, withdrawable strip on `/profile/referral` + `/profile/status`. | **Stage 114.3:** public `/u/[id]` ambassador landing, referral profile tabs, badges/levels, referral bulk payouts API. | **Stage 114.2:** referral push/email (`REFERRAL_BONUS_EARNED`, `REFERRAL_TEAMMATE_JOINED`), FinTech **Referral Liability** (`GET /api/v2/admin/referral/liability`), `withdrawable_referral` + `POST /api/v2/wallet/referral-withdrawal-request`. | **Stage 114.1:** referral clawback, completion trigger SSOT, `referral-me-client` + `useReferralMeQuery`, `/partner/referrals` redirect. | **Stage 113.1:** pre-deploy micro cleanup, `components/` 0 fetch. | **Stage 113.0:** client API dedup/caching (`client-request-dedup`, `client-fetch-policy`). | **Stage 112.3 final:** `admin-settings-api-client`; `components/` launch-path 0 fetch; Go/No-Go 9/10. | **Stage 111.1b/c:** prod perimeter + booking-create-guard. | **Stage 112.3:** API clients — seasonal, iCal, referral, push, geocode, renter cancel; auth realtime SSOT; booking-request accept fix. | **Stage 112.2:** partner-calendar/bookings/finances API clients; calendar + chat booking card без `fetch` в UI; platform-calendar SSOT. | **Stage 112.1:** Go/No-Go checklist; chat thread/read → API clients; FinTech prefs init без двойного load; home live-count dedup. | **Stage 112.0:** Pre-Launch Hardening — `chat-ui-api-client`, расширенный `catalog-public-client` (marketing ui-strings, search count); админ-чат без двойного enrich на mount; UI chat/admin/geo без прямого `fetch`. | **Stage 111.2:** FinTech panels → `admin-fintech-api-client` (0 fetch в `components/admin/finances`); `fmtThb` SSOT; главная — без двойного запроса listings на mount. | **Stage 111.1:** Final cleanup — `usePlatformHomePage`, `admin-fintech-api-client`, `catalog-public-client`, `auth-client`; главная и FinTech hook без `fetch` в UI. | **Stage 111.0:** Pre-Launch Cleanup — разгрузка P1-страниц (marketing, partner dashboard, renter profile, admin payments) → hooks + `*PageContent`; `admin-payments-api-client`; `DASH_DATE_LOCALE` export. | **Stage 110.8:** Chat/marketing/profile API clients; partner dashboard booking hook; без прямого `fetch` в ключевых UI. | **Stage 110.6b:** Chat invoice = storefront — `settleInvoiceDisplayAmount`, partner prefill `resolveInvoicePrefillFromStorefront`, guest dual-currency display (retail FX). | **Stage 110.7:** Chat final — `conversation-api-client.js`, `handleInboxMessageUpdate`, outbound `sendInvoice`/`appendMessage`; UI split: `partner-dashboard-widgets`, `PartnerApplicationModal`, `profile-completion`, `marketing-promo-helpers`. | **Stage 110.6:** Chat polish — `post-chat-invoice.js`, outbound только через thread SSOT; `handleInboxMessageInsert` (один merge); invoice → server POST path. | **Stage 110.5:** Chat SSOT — `post-chat-message.server.js` (POST/GET messages + invoice delegate); один Realtime на `/messages*` (`deferThreadRealtime` + `thread-inbox-bridge`). | **Stage 110.4:** SSOT FX — `lib/pricing/fx-display.js` (retail markup vs guest fee); `GET /api/v2/exchange-rates?retail=1|0`; витрина/wizard/PDP/карточки на одном `rateMap`; settlement без retail. | **Stage 110.3:** Financial Core — `ledger.service.js` facade + `lib/services/ledger/*` modules (capture, settlement, refund, balance, reconciliation, dispute). | **Stage 110.2:** единый реестр наборов статусов брони — `lib/booking/status-sets.js` (календарь, iCal, ROI, escrow, чат, cancel, FinTech). | **Stage 110.1:** SSOT гостевой цены — поиск/фильтр/гистограмма через `getGuestDisplayForSearchFilters`; API search + listing detail → `guestDisplayPriceThb` + `guestServiceFeePercent`; wizard preview разделяет витрину и chat-invoice reference. | **Stage 109.3:** UnifiedMessagesClient + ListingWizardContext hooks split (109.x complete). | **Stage 109.2:** dispute + payout-batch topic modules (facades unchanged). | **Stage 109.1:** i18n topic slices + referral-pnl/dispute module split. | **Stage 109.0:** Admin FinTech console split (composition + panels, `useAdminFinTechConsole`). | **Stage 108.5 (FIN):** pricing aliases, chat unread bridge (D-05), FX cache v3, owner FinTech polish. **Stage 108** блок закрыт. | **Stage 108.4:** schema verify, status SSOT, CHECKED_IN hints. | **Stage 108.3:** FinTech cron health (`ops_job_runs`), chat thread Realtime dedup, owner pool rail labels. | **Stage 108.2:** cleanup dead UI, chat POST SSOT, pricing map, owner FinTech labels. | **Stage 108.1:** legacy payout guard (D-01), `lib/booking/status-transitions.js` (P0-2), FinTech owner mode. | **Stage 107.2:** SSOT цен (полировка витрины + SEO/favorites), FinTech owner copy. | **Product flow map:** `docs/PRODUCT_FLOW_MAP.md` (SSOT paths, P0/P1 PR backlog). | **Stage 106.2:** launch status dashboard, payment ops guard (fiscal/treasury/mock), legal ZIP export, owner pause runbook §14. | **Stage 106.1:** P0 payment hardening (prod confirm gate, no mock adapters, webhook IP + idempotency, production readiness card on `/admin/settings/finances`). | **Stage 105:** treasury ops config, monitoring alerts + FinTech tab, emergency pause, PRE_REAL_PAYMENTS checklist. | **Stage 104:** dual payout rails (`payout-rails.js`), smoke `--rail`, FinTech system status + pool rail filter, ZIP registries per rail, `PRE_REAL_PAYMENTS_CHECKLIST.md`, runbook §12. | **Stage 103 (final):** smoke timing + test-full-package API, FinTechTreasuryHeroDashboard, PRE_LAUNCH §12, runbook §13. | **Stage 102.4:** legal drafts/publish, partner settlement docs tab, bank ZIP package. | **Stage 101.2:** conversions panel filters + margin % + CSV reconcile API/UI. | **Stage 101.1:** conversions panel CSV export (`GET /api/admin/finances/conversions/export`, `;`/Excel RU), margin widget polish with accepted guest RUB reference + THB SSOT math; stage 101 ledger tracking (`056_track_conversions.sql`) remains base. | **Stage 100.8:** Partner finances UI — all payout FX via `GET …/payouts/preview` + `POST …/preview-batch` (no client `calcPayoutMath` / live `exchange-rates` for payouts); admin shows `amount_in_payout_currency`. | **Stage 100.7:** Partner payout preview unified (`GET …/payouts/preview` + `PartnerPayoutPreviewFields`); `settlement_v3` uses snapshot `fx_raw_rate_to_thb`. | **Stage 100.6:** Partner payout FX (`lib/partner/partner-payout-fx.js`), `amount_in_payout_currency`, on-the-fly PENDING reserve, batch excludes open partner requests, T-Bank CSV fix, withdraw rail picker + preview API. | **Stage 100.5:** Treasury & Payouts v2 SSOT (ADR-097, Concierge, flags, migrations 053–054). | **Stage 100.4:** compliance CSV (`lib/admin/compliance-registry-csv.js`), FinTech/partner UI polish. | **Stage 100.3:** RUB acquirer + manual pool settle. | **Stage 98:** 24h partner withdraw hold (`partner-payout-eligibility.js`), `READY_FOR_PAYOUT`, crons `promote-ready-for-payout` + `payout-batch-pools`, admin `/admin/settings/finances`, checklist `docs/PRE_LAUNCH_CHECKLIST.md`. | **Stage 96.0:** wizard upload creates draft listing before `POST /api/v2/upload`; partner soft-delete via `metadata.is_deleted`; public stats excludes E2E rows. | **Stage 051:** RLS reset **`051_stage95_4_rls_recursion_fix.sql`** — drop all policies on core tables + canonical recreate (after **047–050**). | **Stage 95.2:** thumbnails **`thumb_*.webp`** (600×400) + bounded cleanup **`metrics`**. | **Stage 95.1:** **`lib/storage/image-processor.server.js`** (Sharp→WebP SSOT), **`lib/storage/storage-cleanup.service.js`**, cron **`GET/POST /api/cron/cleanup-storage`**, CLI **`npm run cleanup:storage`** (dry-run default). | **Stage 95.0:** Storage RLS **`049_stage95_storage_rls_policies.sql`**, **`lib/storage/*`**, hardened **`POST/DELETE /api/v2/upload`**, avatars bucket + legacy **`listing-images/avatars/`** read. | **Status**: Production-Ready — **Stage 94 prep:** `GET /api/admin/metrics/overview`, `GET/POST /api/admin/activity`, **`GET/PATCH /api/admin/system-settings`** (service_role, `requireAdminStaff`) вместо anon `/_db` для **`system_settings`**; роутер **`/dashboard`** — **`GET /api/v2/auth/me`** (без клиентского **`supabase.from('profiles')`**); **`GET /api/v2/admin/stats`** — **`requireAdminStaff`**; **`GET /api/v2/admin/telegram`** — staff, **`POST`** — только **ADMIN**; опционально **`database/migrations/048_stage94b_reassert_is_admin.sql`** после **047**; админ UI без anon `/_db` к `profiles`/`listings`/`bookings`; публичный `/test-db` — только публичные API. — **Stage 94.0:** DB **`047_stage94_*`** — `auth.users`→`profiles` trigger + `current_profile_id` fix + RLS (profiles/listings/bookings/wallets/ledger/commissions/payments/disputes/reviews/guest_reviews). — **Stage 93.0:** logout — сервер **`clearAuthSidecarCookies`** + клиент **`browser-auth-cleanup`** + порядок **`signOut`**; OAuth callback **`tryGetJwtSecret`**, **`reason`** `account_link` / `suspended`; **`oauth-profile-sync`** — конфликт **`auth_user_id`**, ban на race-merge; **`GET /api/v2/wallet/me`**, **`referral/me`**, **`referral/leaderboard`** — **`error_code`** на 401/404. — **Stage 92.0:** единая верификация **`gostaylo_session`** — **`lib/auth/verify-app-session-jwt.js`** (**HS256**); **`getSessionPayload`** / **`requirePartnerSession`** / admin health-security resolvers / партнёрские API (calendar, occupancy, ical-export-link, upload, iCal sync) без дублирующего **`jwt.verify`**; гейты **`requireAccess`**, **`api-guard`**, **`requirePartnerSession`** — ответы с **`error_code`** (**`AUTH_ACCESS_FORBIDDEN`**, **`API_BOOKING_*`**). **Stage 91.6:** **`/admin/marketplace-health`** — виджет **Marketing Efficiency** (ROI): **`loadMarketingReferralRoiStats`** (**`lib/admin/marketing-referral-roi.js`**) — маржа = Σ **`bookings.commission_thb`** для **`renter_id ∈ referral_relations.referee_id`** и статусов **PAID / PAID_ESCROW / CHECKED_IN / COMPLETED**; выплаты = Σ **`referral_ledger.amount_thb`** при **`type=bonus`**, **`status=earned`**; **`GET /api/v2/admin/marketplace-health`** → **`marketingRoi`**. **`/about/loyalty`**: **`generateMetadata`** — **`alternates.canonical`** + **`alternates.languages`** (hreflang **ru-RU / en-US / zh-CN / th-TH** + **x-default**, варианты с **`?lang=`** для не-RU), синхронизация UI через **`useSearchParams`** + **`AboutLoyaltyClient`**; ссылки **`/terms/`**, **`/legal/privacy/`**. — **Stage 91.5:** `/about/loyalty` — **`generateMetadata`** через **`getUIText('seo_loyalty_*', getLangFromRequest(...))`** (RU/EN/ZH/TH) + **`openGraph.locale`**; **`AboutLoyaltyClient`** — сумма welcome в **THB** + **`formatPrice`** в валюте **`useCurrency`** (**`fetchExchangeRates`**); **`ReferralMarketingKit`** — **`navigator.share`** при доступности (нативный шаринг), иначе fallback в Telegram. — **Stage 91.4:** `/about/loyalty` — динамический welcome (**`getCachedWelcomeBonusAmountThb`** = **`PricingService.getGeneralPricingSettings`**, **`lib/server/welcome-bonus-public.js`**), **`generateMetadata`** (SSOT суммы + бренд; i18n **`seo_loyalty_*`** — **Stage 91.5**), UI **`AboutLoyaltyClient`** + плейсхолдер **`{welcomeThb}`** в **`getUIText`**; **`ReferralMarketingKit`** — ссылка на **`/about/loyalty`** (цикл шаринг → регистрация → бонус для гостей без личной ссылки). — **Stage 91.3:** Visual Referral Tracking — блок **Activity** на **`/profile/referral`** (статусы приглашённых + **`timelineStage`** в **`buildReferralTeamMembers`**: registered / first_booking_pending / completed_bonus_paid), виджет **Calculate your potential** (**`ReferralEarningsEstimator`**, **`estimateReferrerIllustrationThb`**, поля **`referralEstimator`** в **`GET /api/v2/referral/me`**), публичная страница **`/about/loyalty`** + тизер на главной (**`PlatformHomeContent`**). — **Stage 91.2:** Human-Friendly Dashboard маркетинга — переименования **Marketing Budget (Pool)** / **Direct vs Sub-Referral Bonus**, tooltips к **`referral_reinvestment_%`** и **`payout_to_internal_ratio`** (**`/admin/marketing/settings`**, **`SystemSettingsMarketing`**), глоссарий на **`/admin/marketing/analytics`**, audit **`reference_label`** в **`GET /api/v2/admin/wallet/transactions`**; пользователь **`/profile/referral`** — блок **Why share?** и invitee-first копирайт с **`welcomeBonusAmount`**. — **Stage 90.2:** Audit Trail UI на **`/admin/marketplace-health`** (**`staffAuditFeed`** из **`critical_signal_events`**: **SYSTEM_AUTO_VERIFICATION**, **STAFF_LISTING_MODERATION**, **STAFF_USER_VERIFICATION**), запись модерации в **`app/api/admin/moderation`**, ручной KYC в **`PATCH /api/admin/users`**; i18n RU для споров в **`booking.js`**; звезда рейтинга только при **`rating > 0`** (**`ListingInfo`**, **`BookingWidget`**). **Stage 90.1:** все маршруты **`/api/admin/**`** (и **`POST /api/admin/revalidate`**, **`/api/admin/fix-enum`**) — **`requireAdminStaff()`** (**`ADMIN`/`MODERATOR`**, без анонима); одобрение **`partner_applications` → APPROVED** выставляет **`profiles.is_verified`** + **`verification_status: VERIFIED`**, аудит **`critical_signal_events.signal_key = SYSTEM_AUTO_VERIFICATION`**; **`GET /api/v2/admin/marketplace-health`** — **`ADMIN`/`MODERATOR`**, Pulse за 7d (**`snapshotRowsLast7Days`** из **`catalog_verified_snapshots`**, **`autoVerificationsLast7Days`** из журнала). **Stage 90.0:** полный цикл KYC профиля (партнёр/юзер → PATCH **`/api/admin/users`** → **`profiles.is_verified`** + **`verification_status: VERIFIED`**, не путать с **`partner_applications.status` `APPROVED`**), бейдж каталога/карты — **`listingQualifiesForTrustVerifiedMiniBadge`** + **`ownerVerified`** / **`owner.is_verified`**; охрана **GET/PATCH `/api/admin/users*`** через **`requireAdminStaff()`**; SEO: **`catalog-seo-geo`**, ключи **`catalogSeo_place_*_{ru|th|generic}`**, **`resolveWhereTarget`** в breadcrumbs + **`seoJsonLd_breadcrumb_countryRussia`**. Таблица **`046_catalog_verified_snapshots`** должна существовать на проекте для Marketplace Health. **Stage 89.0:** масштабирование каталог-карты (**`Leaflet.markercluster`** в **`InteractiveSearchMap`**, кластеры по масштабу **`disableClusteringAtZoom=13`**, цвет по доле Verified — **`createSearchMapClusterDivIcon`** / **`gslVerified`**), персист majority-телеметрии (**`catalog_verified_snapshots`**, **`insertCatalogVerifiedMajoritySnapshot`**), админ **`/admin/marketplace-health`** + **`GET /api/v2/admin/marketplace-health`**, партнёр **`?tab=verification`** на **`/partner/settings`** (KYC). **Stage 77.7:** `FINAL_POLISH_V3_LOGOTYPE_SYNC` — финальная визуальная подгонка: отдельный бренд-компонент `components/brand/airento-logo.jsx` интегрирован в header/shell, Turbo-badge получил мягкую pulse-анимацию, карточки и social-кнопки получили плавные hover transitions, mobile bottom nav приведён к стилистике `1code.html` (rounded top, blur, soft shadow, активный teal-pill). Сохранена единая Deep Teal эстетика и непрерывный UX при переходах между `/profile/referral`, `/profile/wallet`, `/profile/status`. **Stage 77.6:** `TOTAL_REDESIGN_REBOOT` — разделение реферального UX на 3 страницы: `/profile/referral` (Invite landing), `/profile/wallet` (Wallet), `/profile/status` (Status), с базовой сеткой и карточной эстетикой по HTML-макетам (`1code.html`, `2code.html`, `code.html`) в Deep Teal палитре (`#006666` + `#f7f9fb`). Функции сохранены: Turbo, payout blockers, `referral_display_currency`, `referral_monthly_goal_thb`, `iana_timezone`. Обновлены global shells для визуальной целостности: `components/universal-header.jsx`, `app/renter/layout.js`, `app/partner/layout.js` (`backdrop-blur`, мягкие тени). **Stage 77.5:** `REF_INTERFACE_REDESIGN_V3_NO_BLUE` — визуальный редизайн `/profile/referral` и `components/referral/*` в Deep Teal (`#006666`) + Surface (`#f7f9fb`) без blue/indigo-тонов; премиальные rounded-2xl карточки, мягкие тени, кошелёк в аналитичном формате (инсайты + таблица последних операций), invite/status блоки приведены к структуре Stitch при сохранении всей текущей логики. **Stage 77.4:** `/profile/referral` получил Turbo-индикатор в карточке статуса (tooltip «Ваш ускоритель бонусов» из `data.turbo`), в табе `Кошелек` добавлен dropdown `Валюта статистики` (`referral_display_currency` через `PATCH /api/v2/profile/me`) и понятная расшифровка причин блокировки payout по `wallet.payout.blockers`; в `Мой статус` добавлен accordion `Настройки профиля` (`referral_monthly_goal_thb`, `iana_timezone`) с сохранением через профильный API. **Stage 77.3:** `/profile/referral` перестроен в табы (`Кошелек`, `Пригласить`, `Мой статус`) с упрощённой лексикой для пользователя; invite-first UX (QR + ссылка, тексты в accordion), скрытие пустых секций. Админ UX: `/admin/marketing/settings` — блоки «Бонусы / Экономика сети / Лимиты безопасности», `/admin/marketing/audit` — сводка остатка бака и quick top-up. Glossary: payout flag в админке — «Доступен вывод». **Stage 77.2:** `/admin/marketing/settings` — promo tank explainer, Manual top-up → `POST /api/v2/admin/referral/pnl-monitor` (metadata `admin_user_id`/`admin_email`), лог manual из `GET /api/v2/admin/referral/tank-events`, retry host activation `POST /api/v2/admin/referral/retry-host-activation`; при пустом баке host activation → `bookings.metadata.host_activation_promo_tank.pending_tank_refill` + system alert; E2E `lib/e2e/deep-cleanup-e2e-actors.js`, `npm run clean:e2e-deep`, `PLAYWRIGHT_E2E_DEEP_PROFILE_CLEANUP`; Network Builder min partners **3** (SSOT со Stories). **Stage 77.1:** в `ReferralMarketingKit` добавлены multi-language ready-to-use тексты (short/medium/long) + copy buttons/toast; `ReferralActivityFeed` показывает статус партнёра Registered/Active; `/u/[id]/layout.js` отдает large social preview (`openGraph.images`, `twitter.images`); `exchange-rates-refresh` cron включает health-check `exchange_rates`. **Stage 76.2:** FX TTL **2h** в **`CurrencyService`**, без локального 1h кэша в referral converter; синхронизация **`profiles.referral_display_currency`** + **`profiles.preferred_currency`** через **`PATCH /api/v2/profile/me`**; **`GET /api/v2/exchange-rates`** возвращает **`ratesUpdatedAt`**; **`/profile/referral`** показывает freshness label и использует `retail=0`; cron endpoint **`/api/cron/exchange-rates-refresh`**: Vercel Free 1×/сутки, частый прогрев — внешний cron-job. **Stage 76.1:** **`profiles.referral_display_currency`**, **`lib/finance/currency-converter.js`** + **`PATCH /api/v2/profile/me`**, **`GET /api/v2/referral/me`** (**`stats.ledgerBaseCurrency`** / **`referralDisplayCurrency`**), клиент **`/profile/referral`** + **`GET /api/v2/exchange-rates?retail=0`**, SSR **`/u/[id]`** + geo teaser; **75.3:** unlock Stories по **`directPartnersInvited`**, **`buildReferralGamificationForUser`**, без **`profiles.metadata`** в referral/me; админ **`referralGamification`**; **75.2:** **`ReferralYourStatusCard`** (tier **`tierProgressPercent`** + медали), удалён **`ambassador.progressPercent`**, i18n **`stage73_brandSubtitle`**, PDF elite только **`pdfVariant === 'elite'`**; **75.1:** lock Stories (**`STORIES_TEAM_DIRECT_REFERRALS_REQUIRED`**), OG **`/u/[id]/opengraph-image`**; **74.4:** i18n **`{brand}`** + ADR §7a; **`landing-meta`** — **`friendsInvited`** / **`siteDisplayName`**; toast по **`referralLastTeammateJoinEventId`**; SEO **`/u/[id]`** (**`generateMetadata`**); **74.3:** публичная визитка **`/u/[id]`**, **`GET /api/v2/referral/landing-meta/[userId]`**, короткий URL в QR/PDF/Stories (**`lib/referral/public-landing-url.js`**), toast на рост **`friendsInvited`**; **74.2:** RPC лидерборда, **`GET /api/v2/admin/referral/leaderboard`** (UTC), бейджи + Stories copy; **74.1:** **`GET /api/v2/referral/leaderboard`**, L1/L2; **73.7:** TZ seed; **73.6:** **`resolveReferralStatsTimeZone`**; **73.5:** **`teammate_joined`**.
> 
...
| Слой | Механизм | Где |
|------|----------|-----|
| **TanStack Query (128.x)** | `AppQueryProvider` + domain hooks; SSOT ключей **`lib/query-keys.js`**; **`queryFetchJson`** для `queryFn`; logout → **`clearClientQueryCache()`** | публичная витрина (главная, каталог, PDP prefetch, FX, featured), профиль; partner/wallet/referral — ранее |
| **In-flight dedup** | `dedupeClientRequest(key, fn, { ttlMs })` — один параллельный запрос на ключ | `lib/api/client-request-dedup.js` — параллельно RQ на legacy paths (чат, FinTech) |
| **TTL policy** | Константы TTL в `lib/api/client-fetch-policy.js` | auth me 30s; chat favorites 30s; provider list 8s; categories/site-features 5–10m |
| **FX / categories persist** | `localStorage` v3, 2h FX (`lib/client-data.js`) + RQ **`useFxRatesQuery`**; categories — RQ **`usePublicCategoriesQuery`** (5m) | главная, каталог, checkout preview, PDP |
| **Logout bust** | `invalidateAllClientRequests()` + **`clearClientQueryCache()`** в `clearBrowserPersistedAuthState` | `lib/auth/browser-auth-cleanup.js` |
| **Chat mutations** | `invalidateChatListCaches()` после archive/favorite | `conversation-api-client.js` |
| **FinTech reload** | `invalidateFintechConsoleBundleCache()` перед каждым `load()` | `useAdminFinTechConsole` |
| **Server RSC** | `next: { revalidate }` — только в Server Components (не в browser clients) | будущие SSR-витрины |

**Не мигрировано на RQ (128.4 PAUSED):** чат, платежное ядро/checkout capture, smoke, админка; часть legacy **`fetchExchangeRates`**. **Новые RQ-PR — только post-MIR** (кроме hotfix). ADR — **`ARCHITECTURAL_DECISIONS.md`** § ADR-128.

### Stage 128.x — Client-Side Data Layer (TanStack Query) (2026-06-01)

#### Stage 128.0 — Foundation
- **`clearClientQueryCache()`** (`lib/query-client.js`) в **`clearBrowserPersistedAuthState`** (`lib/auth/browser-auth-cleanup.js`) вместе с **`invalidateAllClientRequests`**.
- **`lib/query-keys.js`** — фабрики ключей (`PUBLIC_SCOPE`, `queryScopeId`, домены catalog/home/fx/profile/chat/…).
- **`lib/api/query-fetch.js`** — стандартный browser `queryFn` для API routes с cookie-сессией.

#### Stage 128.1 — Public catalog + home (Iteration 1)
- **Главная:** `hooks/home/use-platform-home-page.js` — **`useHomeLiveCountQuery`** (`queryKeys.home.liveCount`, `lib/hooks/use-home-live-count-query.js`).
- **Каталог:** `lib/hooks/useListingsSearch.js` — module-level **`searchCache`** удалён; **`useQuery`** + **`queryKeys.catalog.search`** (`PUBLIC_SCOPE`, сегмент **`displayCurrency`**) + **`keepPreviousData`**; fetch — **`lib/catalog/fetch-catalog-search.js`**, **`build-catalog-search-params.js`**.
- **Категории (до 128.2):** отдельные home/catalog hooks; unified key — **128.2**.
...
- Потребители: главная, каталог (`listings-catalog-client.jsx`), **`useCheckoutPricing`** (preview only), **`useListingViewData`** (PDP).
- **Featured grid:** **`lib/home/fetch-home-featured.js`** + **`useQuery`** + **`queryKeys.home.featured`** + **`keepPreviousData`** в **`use-platform-home-page.js`**.
- **Профили:** **`useProfileMeQuery`**, **`usePartnerApplicationStatusQuery`** (`lib/hooks/use-profile-queries.js`); **`app/profile/page.js`**; wallet/referral — **`useWalletMeQuery`** / **`useReferralMeQuery`** (ранее).

#### Stage 128.4 — Closure & PAUSE
- **RQ Iteration 1–2 завершены**; фундамент закрыт; **TanStack Query на паузе** до первых MIR.
- **Logout:** `signOut` → `clearBrowserPersistedAuthState` → RQ `clear()` + dedupe clear (catalog/FX/profile/wallet); FX **localStorage** — публичный справочник, не сбрасывается.
- **Frozen:** Iteration 3 (чат), 4 (админка). Приоритет — Controlled Live / ops / `PRE_REAL_PAYMENTS_CHECKLIST`.

### Stage 129.0 — Design System Foundation (UI-0) (2026-06-01)

- **SSOT tokens:** `lib/theme/tokens.ts` (typed API) + `lib/theme/tokens.cjs` (Tailwind/Node bridge) — colors (brand + mint/navy), spacing, radii, shadows, typography.
- **Tailwind:** `toTailwindExtend()` из tokens; neutral comment + **`lib/site-brand.cjs`**; `brand.mint` `#0D9488`, `brand.navy` `#0F172A`; brand shadows.
- **Typography:** `font-stack.cjs` — `var(--font-sans)` первым (Inter из `app/layout.js`).
- **globals.css:** `--primary: 180 100% 20%` (= brand `#006666`); `.gsl-*` на `bg-brand`, `bg-brand-surface`, `shadow-brand*`.
- **Rules:** `.cursorrules` — UI / Design System Rules (Stage 129+).
- **Docs:** `docs/PRODUCT_UI_SYSTEM.md`, `docs/proposals/UI_SSOT_AUDIT.md` (baseline).
- **Out of scope:** component hex cleanup, partner `teal-*` bulk, FinTech inline styles → **UI-1+**.

### Stage 129.1 — Emergency Brand SSOT Fix (2026-06-01)

- **Regression (129.0):** hardcoded «Airento» in `tailwind.config.js` comment and docs.
- **Fix:** neutral Tailwind comment; **`lib/site-brand.cjs`** — CJS mirror of **`getSiteDisplayName()`** (`NEXT_PUBLIC_SITE_NAME` / `SITE_DISPLAY_NAME`).
- **Unchanged:** legal pages, i18n `{brand}`, emails, manifest — still **`lib/site-url.js`**.

### Stage 113.1 — Pre-deploy micro cleanup (2026-05-21)

- Удалён неиспользуемый `components/notification-bell.js` (legacy `/api/conversations`, не в shell).
- Dedup: `admin-settings-api-client`, `marketing-api-client` (read + bust на mutate), `partner-bookings-client`, `catalog-public` (geo, ui-strings).
- **`components/`:** 0 прямых `fetch` — только API clients / hooks.

### Stage 113.0 — API client performance (2026-05-21)

- **SSOT:** `lib/api/client-request-dedup.js`, `lib/api/client-fetch-policy.js`.
- **Клиенты:** `auth-client`, `conversation-api-client`, `catalog-public-client`, `platform-home-api-client`, `admin-fintech-api-client`, `partner-calendar-client`, `partner-listing-client`; `fetchCategories` в `client-data.js`.
- **Пример:** два одновременных `fetchAuthMe()` → один HTTP; `fetchChatProviderConversations({ bustCache: true })` на явный refresh.

### Stage 112.3 — Final Pre-Launch Optimization (2026-05-20)

- **API clients:** `partner-listing-client`, `partner-seasonal-prices-client`, `ical-sync-client`, `referral-public-client`, **`referral-me-client`** (`useReferralMeQuery`), `push-client`, `geocode-client`, `renter-bookings-client`, `realtime-diag-client`; `auth-client` + realtime JWT/claims.
- **UI без `fetch`:** calendar-sync, seasonal-price-manager, referral leaderboard/activity, push-init, MapPicker, cancel-booking, PartnerHealthWidget, RealtimeDiagOverlay, supabase-realtime-auth-sync; `booking-request-card` accept → `updatePartnerBookingStatus`.
- **Чат Realtime:** `realtime-session-jwt.js` → `fetchRealtimeToken`.
- **Проверки:** build + smoke 15/15.

### Stage 112.2 — Final Pre-Launch Optimization (2026-05-20)

- **API clients:** `lib/api/partner-calendar-client.js`, `lib/api/partner-bookings-client.js`; settlement docs в `partner-finances-client`; `fetchGeoCurrencyHint` в `catalog-public-client`.
- **Календарь:** `use-partner-calendar`, `availability-calendar`, `platform-calendar`, `PartnerChatCalendarPeek` — без прямого `fetch`.
- **Чат:** `booking-request-card` → `updatePartnerBookingStatus`; peek → `fetchPartnerBookingById`.
- **Партнёр finances:** `PartnerFinancesDocuments` → settlement API client.
- **Проверки:** build + smoke 15/15.

### Stage 112.1 — Final Pre-Launch Optimization (2026-05-20)

- **Чат (hooks):** `use-chat-thread-messages` → `fetchEnrichedConversation`, `fetchChatMessages`, `postChatMarkRead`; `use-mark-conversation-read` → `postChatMarkRead`.
- **Чат (UI):** `ReferralTeamSection` → `postChatConversationFromProfile` в `conversation-api-client`.
- **FinTech:** `useAdminFinTechConsole` — `realDataOnly` / `ownerMode` из localStorage при первом рендере (без лишнего `load()`).
- **Главная:** `handleDateChange` не дублирует `fetchLiveCount` (счётчик обновляет debounced effect).
- **Доки:** `docs/GO_NO_GO_FIRST_REAL_PAYMENT.md`; обновлены PRE_REAL_PAYMENTS + runbook.
- **Проверки:** build + smoke 15/15.

### Stage 112.0 — Pre-Launch Hardening (2026-05-20)

- **Chat UI SSOT:** `lib/chat/chat-ui-api-client.js` — перевод, сообщения, enrich conversations, support join, шаблоны, escalate; потребители: `message-bubble`, `QuickReplies`, `AdminDisputeChatPeek`, `AdminMessagesThreadClient`, `support-request-dialog`.
- **Публичная витрина:** `catalog-public-client` — `fetchPublicMarketingUiStrings`, `fetchSearchAvailableCount` (`ListingFlashHotStrip`, `GeoSuggestToast`).
- **Auth/legal:** `PartnerVerifiedBadgePromo` → `fetchAuthMe`; `AdminLegalFullPackageCard` → `postLegalTestFullPackage` (как launch dashboard).
- **Perf:** `AdminMessagesThreadClient` — один `fetchAdminEnrichedConversations` на mount (раньше два при наличии `conversationId`).
- **Проверки:** `npm run build`; `npm run smoke:full-financial` 15/15.

### Stage 111.2 — Final cleanup before Stage 112 (2026-05-21)

- **FinTech UI:** все панели в `components/admin/finances/*` переведены на `lib/admin/admin-fintech-api-client.js` (конвертации, журнал, treasury-ops, clean-test-data, launch status).
- **Дубли:** локальные `fmtThb` → `fintech-console-shared`; `hooks/use-debounce.js` — единая точка для debounce.
- **Оптимизация:** `usePlatformHomePage` — убран лишний повторный `fetchListings` при первом mount (debounced effect с skip ref).
- **Проверки:** build OK; smoke 15/15.

### Stage 111.1 — Final Pre-Launch Cleanup (2026-05-21)
...
### Stage 111.1b — Booking create security SSOT (2026-05-21)

- **`POST /api/v2/bookings`:** `lib/api/booking-create-guard.js` → `resolveBookingCreateSession` — cookie-сессия обязательна; `renterId` из тела для гостя игнорируется (IDOR); staff может передать `renterId`.
- **Цены:** `lib/services/booking/booking-price-gate.js` — общая аттестация subtotal/guest total для `creation.js` + `inquiry.service.js`.
- **Insert:** стандартная бронь — `booking-atomic-insert.js` → RPC `create_booking_atomic_v1`; inquiry — прямой INSERT `BOOKING_STATUS.INQUIRY` (без резерва слотов).
- **Legal:** акцепт оферты по-прежнему на оплате (`legal-consent.js`), не на create.

### Stage 111.0 — Pre-Launch Cleanup (2026-05-21)

- **P1-страницы (тонкие `page.js` ~9 строк):** `/admin/marketing/promos` → `useAdminMarketingPage` + `AdminMarketingPageContent`; `/admin/marketing` → `MarketingDashboard`; `/admin/finances` (верификация платежей) → `useAdminPaymentsPage` + `admin-payments-api-client.js` + `AdminPaymentsPageContent`; `/renter/profile` → `useRenterProfilePage` + `RenterProfilePageContent`; `/partner/dashboard` → `usePartnerDashboardPage` + `PartnerDashboardPageContent`.
- **FinTech console** (109.0) без изменений контракта — `/admin/settings/finances`.
- **Проверки:** `npm run build` OK; `npm run smoke:full-financial` 15/15 PASS.

#### Что почистили в 109–112.x (сводка)

| Блок | Суть |
|------|------|
| **109.0–109.3** | FinTech console, i18n/referral/dispute/payout mодули, UnifiedMessages + ListingWizard hooks. |
| **110.1–110.4** | SSOT цены, статусы, ledger, FX retail vs settlement. |
| **110.5–110.8** | Chat SSOT (server POST, inbox bridge, invoice=storefront, conversation-api-client, sound policy). |
| **111.0** | Финальная разгрузка 4 крупных страниц; платежи админки без `fetch` в UI; `DASH_DATE_LOCALE`. |
| **111.1** | Главная + FinTech hook → API clients; catalog-public + auth SSOT. |
| **111.1b** | POST bookings session guard + booking-price-gate + atomic insert SSOT. |
| **111.2** | FinTech panels без `fetch`; fmtThb SSOT; home mount без двойного search. |
| **112.0** | `chat-ui-api-client` + marketing/search public client; админ-чат один enrich на mount; мелкие UI без `fetch`. |
| **112.1** | Thread/read SSOT в hooks; FinTech один bundle-load; Go/No-Go doc; home live-count без дубля. |
| **112.2** | Calendar/bookings/finances API clients; platform + partner calendar UI без `fetch`. |
| **112.3** | iCal/seasonal/referral/push/geocode clients; realtime auth SSOT; остатки UI `fetch` в P2-зонах. |

### Stage 108.5 — Финальная уборка блока 108 (2026-05)

- **P1-8:** `PricingCatalogService` / `BookingSettlementPricing` — алиасы в `pricing.service.js` (каталог vs settlement).
- **D-05:** на `/messages*` `ChatContext` не дублирует Realtime списка; `chat-unread-bridge.js` + `inbox-realtime-merge.js`.
- **FX:** `lib/client-data.js` — кеш курсов v3, TTL 2 ч (согласовано с сервером); обновление при возврате на вкладку.
- **FinTech owner:** `resolveBatchStatusLabel`, подписи пулов без техкодов.

### Stage 108.4 — Статусы и schema 103.2 (2026-05)

- **P0-3:** `npm run verify:schema-103-2` → `scripts/verify-stage103-2-schema.mjs` (6 колонок lifecycle); чеклист `PRE_LAUNCH_CHECKLIST` §1.6f.
- **P1-1:** `validatePartnerBookingStatusTransition` / `buildPartnerBookingStatusPatch` в **`status-transitions.js`**; **`BookingService.updateStatus`** `@deprecated`; partner PUT использует тот же SSOT.
- **P1-6:** `BOOKING_STATUS_OWNER_HINTS_RU`, бейдж **CHECKED_IN** («Гость заехал»), подсказки в **`/partner/finances`**.
- **P1-7:** **`BOOKING_STATUS_CODES`**, расширен **`NO_PAY_TRAVEL_STATUSES`**, parity в **`escrow/constants.js`**.

### Stage 108.3 — Прозрачность cron и чат (2026-05)

- **Cron health (P0-4, 124.19):** `lib/admin/financial-cron-health.js` — **`ops_job_runs`**: `escrow-thaw`, `promote-ready-for-payout`, `payout-batch-pools`, `financial-health-monitor`, `exchange-rates-refresh`, `referral-reconciliation`, **`referral-unlock`**; UI **`FinTechCronHealthPanel`** (сводка ok/total, текст последней ошибки); **`GET /api/admin/finances/treasury-ops`** → `cronHealth`; stale → **`TREASURY_CRON_STALE`** в TG FINANCE.
- **Чат (P1-3, 110.5):** на `/messages/[id]` **`deferThreadRealtime`** отключает per-thread канал; INSERT/UPDATE треда — **`useConversationInbox`** + **`thread-inbox-bridge.js`**; глобальный бейдж — **`chat-unread-bridge.js`** (без Realtime списка в ChatContext).
- **FinTech owner:** селект пула — **`resolvePayoutRailLabel`** («Выплаты в рублях» / «Международные»); кнопка полной очистки тестовых данных — крупная в **`FinTechTestDataToolbar`**.

### Stage 108.2 — Консолидация и уборка (2026-05)

- **Legacy payout:** вызовов `processPayout` / `processAllPayoutsForToday` вне `payout.service.js` нет; cron — только `PayoutBatchService`.
- **Чат:** сервер POST — **`lib/chat/post-chat-message.server.js`** (`executePostChatMessage`, `executePostChatMessageForUser`); клиент — **`lib/chat/post-chat-message.js`**; route — тонкая обёртка; invoice POST делегирует в тот же SSOT.
- **Dead code:** удалены `booking-actions.jsx`, `chat-booking-announcement.jsx`, `chat-typing-bar.jsx`.
- **Pricing:** `lib/pricing/PRICING_SERVICES.md` — карта двух `pricing.service.js`.
- **FinTech owner:** `resolvePayoutRailLabel`, кнопка «Запустить полную очистку тестовых данных».

### Stage 108.1 — P0 риски перед реальными платежами (2026-05)

- **Legacy payout (D-01):** `lib/services/escrow/legacy-payout-guard.js` — блокирует `processPayout` / `processAllPayoutsForToday` при **production** или **`TREASURY_MANUAL_MODE`** (override: `ALLOW_LEGACY_PAYOUT=1`); алерт **TG FINANCE** + `critical_signal_events`. Prod path: **`PayoutBatchService`** только.
- **Статусы брони (P0-2 / 119.2, **125.3**):** `lib/booking/status-transitions.js` — матрицы переходов; **SSOT применения** — **`lib/services/booking/booking-status.service.js`** (`transitionBookingStatus` → chat sync + **`referral-lifecycle-hook`**). **125.3:** прямой FSM-переход в `PAID_ESCROW` без `viaEscrowRpc: true` → `direct_paid_escrow_transition_forbidden` (capture только `EscrowService.moveToEscrow` + RPC). Обёртка: `BookingService.updateStatus`. **CHECKED_IN ≠ THAWED**; hint в `booking-status-display.js`.
- **FinTech owner mode:** переключатель по умолчанию **вкл.** (`readFintechOwnerModePreference(true)`), понятные подписи журнала, упрощённый hero казначейства.

### SSOT Цен (Stage 107.1–107.2, уточнение **110.1**)
...
| Слой | Правило | Файлы |
|------|---------|--------|
| **Guest fee (THB)** | База + `guestServiceFeePercent` — без FX | **`lib/pricing/guest-display-price.js`** |
| **Retail FX** | `rateMap` с `applyRetailMarkup: true` (`chatInvoiceRateMultiplier` на курсах) | **`lib/pricing/fx-display.js`**, **`CurrencyService.getDisplayRateMap`** |
| **Mid FX** | Settlement, ledger RUB-поля, payout preview, referral stats, admin risk | **`getMidMarketDisplayRateMap`**, **`pricing-fx-helpers.getRawRateMap`**, **`GET /api/v2/exchange-rates?retail=0`** |
| **Единая витрина** | `getDisplayPriceInCurrency`, `formatDisplayPriceInCurrency`; карточка/PDP/SSR SEO | **`CardPriceDisplay`**, **`BookingWidget`**, **`app/listings/[id]/layout.js`**, **`lib/server-data.getExchangeRates`** |
| **API курсов** | Query **`retail`**: default `1` (storefront), `0` (mid); ответ: `rateMap`, `retailMode`, `retailMarkupMultiplier` | **`app/api/v2/exchange-rates/route.js`** |
...
| Слой | SSOT | Примечание |
|------|------|------------|
| **POST сообщение** | `lib/chat/post-chat-message.server.js` | Push, safety; route `messages/route.js` — делегат + re-export |
| **POST invoice (server)** | `lib/chat/post-chat-invoice.server.js` | hold, commission, calendar → `executePostChatMessageForUser`; route `invoice/route.js` — делегат |
| **Клиент POST** | `lib/chat/post-chat-message.js` | Текст/voice/media/system — только через `useChatThreadMessages` на `/messages/[id]` |
| **Клиент invoice** | `lib/chat/post-chat-invoice.js` → API | `settleInvoiceDisplayAmount` (retail FX); префилл/гость — `fx-display-client.js` |
| **Outbound UI** | `useUnifiedMessagesOutbound.js` | Делегат thread: `sendMessageText`, `sendVoiceFromUrl`, `sendInvoice`, `appendMessage`; без `postChatMessage` / лишних props |
| **Inbox API (client)** | `lib/chat/conversation-api-client.js` | Список/избранное/unarchive/enriched merge — ChatContext + `useConversationInbox` |
| **Inbox Realtime merge** | `lib/chat/inbox-realtime-merge.js` | `handleInboxMessageInsert` + `handleInboxMessageUpdate` — ChatContext (вне `/messages*`) + inbox (на `/messages*`) |
| **Thread bridge** | `lib/chat/thread-inbox-bridge.js` | Один канал `inbox-messages` на треде |
| **Глобальный unread** | `lib/chat/chat-unread-bridge.js` | `publishChatInboxUnreadTotal`; ChatContext без messages RT на `/messages*` |

### SSOT наборов статусов брони (Stage 110.2)
...
| **`ESCROW_PIPELINE_STATUSES`** | Эскроу / выплаты (alias `BOOKING_ESCROW_PIPELINE_STATUSES`) | PAID_ESCROW → READY_FOR_PAYOUT |
| **`NO_PAY_TRAVEL_STATUSES`** | Чат: скрыть Pay now | Set |
| **`TRANSPORT_CONFIRM_STATUSES`** | Transport calendar confirm CSV | Без PENDING/INQUIRY |
| **Прочие** | cancel ledger/simple, review cron, FinTech dashboard, dispute/emergency, draft cleanup | См. **`lib/booking/status-sets.js`** |
| **110.2b** | mask contacts, compliance export, order card, admin stats, calendar query | Импорт наборов из **`status-sets.js`** (без смены membership) |

- **Коды enum:** `lib/config/app-constants.js` → `BOOKING_STATUS`
- **FSM / PATCH:** `lib/booking/status-transitions.js` (реэкспорт занятости из `status-sets`)
- **Фасад календаря:** `lib/booking-occupancy-statuses.js` → `status-sets`

### Financial Core SSOT (Stage 110.3)

| Слой | Роль | Файлы |
|------|------|--------|
| **Ноги захвата** | Расчёт DEBIT/CREDIT из `pricing_snapshot` / booking row | `lib/services/ledger/ledger-capture-legs.js` |
| **Захват оплаты** | `BOOKING_PAYMENT_CAPTURED` atomically с PAID_ESCROW (RPC); JS replay — `ledger-payment-capture.js` | `move_to_escrow_and_post_ledger_v1` + `ledger/ledger-payment-capture.js` |
| **Выплаты** | `PARTNER_PAYOUT_OBLIGATION_SETTLED` (single + batch) | `ledger/ledger-settlement.js` ← **`PayoutBatchService.markBatchSettled`** |
| **Возврат** | `BOOKING_REFUND_PARTIAL` | `ledger/ledger-refund.js` |
| **Споры** | Hold / release partner earnings | `ledger/ledger-dispute.js` → `dispute-hold.js` |
| **Балансы / сверка** | Net по account_id, MVP margin leakage | `ledger/ledger-balance.js`, `ledger/ledger-reconciliation.js` |
| **Facade** | Публичный API без смены импортов | `lib/services/ledger.service.js` (~70 строк) |
| **Escrow** | PAID_ESCROW, thaw, **не** prod auto-payout | `escrow.service.js` + `legacy-payout-guard.js` |
| **Payout batch** | Mon/Thu pools | `payout-batch.service.js` + `payout-batch/*.js` |

---

### Stage 95.0 — Storage RLS + unified upload, 2026-05

- **SQL:** `database/migrations/049_stage95_storage_rls_policies.sql` — bucket limits 10MB; policies on **`storage.objects`** using **`public.current_profile_id()`** (requires **047/048**).
- **SSOT code:** `lib/storage/storage-buckets.js`, `storage-validation.js`, `storage-authorization.js`, `storage-upload.server.js`, `storage-upload.client.js`.
- **API:** `POST /api/v2/upload`, `DELETE /api/v2/upload` — session + ownership checks before service_role upload; signed URLs via `createStorageSignedUrl` (server).
- **Avatars:** new uploads → bucket **`avatars`** (`{profileId}/*.webp`); existing **`listing-images/avatars/{id}/…`** unchanged.
- **Listing photos:** path `{listingId}/…`; owner = `listings.owner_id`.

### Stage 95.2 — Thumbnails + bounded storage cleanup, 2026-05

- **Thumbnails:** **`processImageMainAndThumb`** — один decode; путь **`{dir}/thumb_{base}.webp`**; профили **`listing_photo`**, **`avatar`**, **`chat_image`** only; API adds **`thumbUrl`**, **`thumbPath`** (backward compatible).
- **Cleanup caps:** per bucket list ≤ **600** objects, delete ≤ **120**/run, messages sample ≤ **400**, **`budgetMs`** env/query (Hobby **~9s** default).
- **Metrics:** cron/CLI JSON **`metrics`**: `durationMs`, `objectsListed`, `candidatesFound`, `deleted`, `skipped`, `listTruncated`, `abortedDueToTimeout`.

### Stage 95.1 — Image processor SSOT + storage orphan cleanup, 2026-05

- **Processing:** **`lib/storage/image-processor.server.js`** — Sharp rotate/resize/WebP по **`lib/services/media/media-profiles.js`**; **`POST /api/v2/upload`** импортирует отсюда; **`media-upload.service.js`** — re-export. Опционально **`processImageWithVariants`** + **`MEDIA_VARIANTS.thumb`** (не пишется в upload по умолчанию; превью каталога — Next/Image).
- **Cleanup:** **`lib/storage/storage-cleanup.service.js`** — индекс ссылок из БД (**`profiles`**, **`listings`**, **`reviews`**, **`partner_applications`**, **`disputes`**, **`messages`**), рекурсивный list Storage, удаление только **unreferenced** объектов старше **`minAgeDays`** (default **7**) с **`graceHours`** (default **48**); защита префикса существующего **`listingId`** до истечения min age.
- **Cron:** **`GET/POST /api/cron/cleanup-storage`** — **`CRON_SECRET`**; query **`dryRun=false`** для удаления (default **`dryRun=true`**). Внешний планировщик — **README.md**.
- **CLI:** **`npm run cleanup:storage`** (dry-run), **`npm run cleanup:storage:execute`** — **`scripts/cleanup-storage.mjs`**.
- **Related:** черновики листингов — **`/api/cron/cleanup-drafts`**; DELETE listing — trigger **`004_listing_storage_cleanup_and_audit_logs.sql`** + **`cleanupListingStorage`** в **`app/api/v2/listings/[id]`**.

### Stage 96.0 — Partner wizard upload + delete + public stats, 2026-05

- **`lib/partner/ensure-wizard-draft-listing.js`** — `POST /api/v2/partner/listings` draft before photos.
- **`ListingWizardContext`**: `resolveListingIdForUpload()`; no `wizard-*` storage prefix.
- **`DELETE /api/v2/partner/listings/[id]`** — `INACTIVE` + `metadata.is_deleted` (no `listing_status.DELETED`).
- **`GET /api/v2/public/stats`** — `listingsCount` excludes test rows (`isTestListingRow`).

### Stage 051 — RLS cleanup & standardization, 2026-05

- **SQL:** `database/migrations/051_stage95_4_rls_recursion_fix.sql` — после **047–050**; не удаляет строки данных.
- **Шаги:** `ENABLE ROW LEVEL SECURITY` на **`profiles`**, **`listings`**, **`bookings`**, **`system_settings`**; цикл **`DROP POLICY`** по **`pg_policies`** (с **`RAISE NOTICE`**); создание **`profiles_*`**, **`listings_*`**, **`bookings_select_party`**, **`bookings_insert_as_renter`**, **`system_settings_staff_only`**.
- **Prerequisites:** **`current_profile_id()`**, **`is_admin()`** с anti-recursion (**`050_stage95_4_rls_recursion_fix.sql`**).
- **Bookings column:** политики ссылаются на **`renter_id`** — на БД без этой колонки применять только после выравнивания схемы или правки SQL (см. динамический выбор гостя в **047**).
- **Anon PostgREST:** **`profiles`** / **`system_settings`** — deny; **`listings`** — read **ACTIVE** only (каталог).

### Stage 94.0 — `auth.users` → `profiles` trigger + core RLS, 2026-05

- **SQL:** `database/migrations/047_stage94_auth_profile_trigger_and_core_rls.sql` — триггер **`on_auth_user_created`**, функции **`handle_new_user`**, правка **`current_profile_id()`**, политики на перечисленных таблицах. Колонка гостя в **`bookings`**: при применении выбирается **`renter_id`**, иначе **`user_id`**, иначе **`guest_id`** (через `information_schema` + динамический SQL). Аналогично **`reviews`**: **`user_id`**, иначе **`renter_id`**.
- **Поведение:** PostgREST/anon больше не читает произвольные строки **`profiles`** (только self + admin + service); каталог/PDP по-прежнему через **service_role** API.
- **`public.is_admin()`:** каноническое определение в **`database/migrations/022_realtime_profile_claim_rls.sql`** (профиль из **`current_profile_id()`**, **`role = 'ADMIN'`**). Если проект пропустил **022**, примените **`048_stage94b_reassert_is_admin.sql`** после **047** (idempotent **`CREATE OR REPLACE`**).

### Stage 94b — Admin `system_settings` API + `is_admin` safety net, 2026-05

- **`GET/PATCH /api/admin/system-settings`** — **`supabaseAdmin`**, **`requireAdminStaff`**; query **`keys`** (список через запятую); body PATCH **`{ key, value }`** (**JSONB**).
- **UI:** **`hooks/useAdminSystemPage.js`** — maintenance / iCal prefs / статус через этот API + **`credentials: 'include'`** на **`/api/v2/admin/telegram`** и **`/api/ical/sync`** где нужна сессия.

### Stage 93.0 — Logout + OAuth sync edge cases, 2026-05

- **Logout:** **`POST /api/v2/auth/logout`** — **`clearGostayloSessionCookie`** + **`clearAuthSidecarCookies`** (`gostaylo_pending_ref`, `gostaylo_oauth_legal`). Клиент **`lib/auth.js` `signOut`**: server POST → **`clearBrowserPersistedAuthState`** → Supabase **`signOut({ scope: 'local' })`**. **`contexts/auth-context.jsx`** — событие **`gostaylo-refresh-session`** после выхода.
- **OAuth callback:** **`tryGetJwtSecret`**; при ошибке профиля редирект с **`reason`**: **`account_link`** (конфликт привязки email/auth), **`suspended`**, иначе **`sync`**.
- **`oauth-profile-sync.service.js`:** отказ если у найденного профиля **`auth_user_id`** задан и не равен текущему OAuth uid; при 23505 merge — проверка **`is_banned`** и того же конфликта **`auth_user_id`**.

### Stage 92.0 — SSOT верификации `gostaylo_session` (Node), 2026-05

- **Модуль:** **`lib/auth/verify-app-session-jwt.js`** — **`verifyAppSessionJwt(token, secret)`**; опции **`JWT_HS256_VERIFY_OPTIONS`** (`algorithms: ['HS256']`), согласовано с **`lib/auth/app-session-issue.js`**.
- **Сервис сессии:** **`lib/services/session-service.js`** — **`getSessionPayload`**, **`requirePartnerSession`** (JWT + **`verifyPartnerAccess`** для партнёрской зоны).
- **Маршруты/хелперы:** убраны прямые **`jwt.verify`** для куки **`gostaylo_session`** в пользу SSOT (в т.ч. **`app/api/v2/partner/listings/[id]/calendar`**, **`occupancy`**, **`ical-export-link`**, **`application-status`**, **`app/api/v2/upload`**, **`app/api/ical/sync`**, **`lib/admin-health-access.js`**, **`lib/admin-security-access.js`**).
- **Ответы гейтов:** **`lib/security/access-guard.js`**, **`lib/api/api-guard.js`** — **`authErrorJson`** + **`AuthErrorCode`** (без legacy-строки **`error`** в теле для новых отказов).

### Stage 79 — Supabase OAuth (Google), 2026-05

- **DB:** `migrations/stage79_0_profiles_oauth_bridge.sql` — колонка `profiles.auth_user_id` (TEXT, UNIQUE внешне); `password_hash` nullable для OAuth-only; индекс по `auth_user_id`.
- **Callback:** `app/auth/callback/route.js` (PKCE через anon key + SSR cookies), синк **`lib/services/auth/oauth-profile-sync.service.js`** → приложенческая сессия **`gostaylo_session`** плюс очистка `gostaylo_pending_ref` и `gostaylo_oauth_legal`.
- **Юридический гейт:** см. **`POST /api/v2/auth/accept-legal/`** и страница **`/auth/complete-legal/`** (если оферта не была принята перед OAuth через модалку); после успеха — событие **`gostaylo-close-auth-modal`** + обновление сессии.
- **UI:** модалка в **`contexts/auth-context.jsx`** — сначала поля (имя/промо/email/пароль), оферта при регистрации, primary **Войти / Создать**, затем разделитель и **Google**; язык только из **`useI18n().language`**; ошибки API — **`getAuthErrorMessage(error_code)`** + слайсы **`auth-errors`** / **`promo-errors`** (мердж в **`translation-state`**). Пароль регистрации: **`lib/auth/password-policy.js`**. Клиент OAuth: **`lib/supabase/oauth-browser-client.js`**.
- **Admin:** строка OAuth id в **`/admin/users/[id]`**.

### Stage 86.0 — JSON-LD для ИИ/генеративного поиска (WebSite SearchAction + PDP `@graph`), 2026-05

- **Глобально:** **`app/layout.js`** → **`GlobalSiteJsonLd`**; SSOT сборки **`lib/seo/site-website-schema.js`** — **`SearchAction`** с **`urlTemplate`**: канонический origin + **`/listings?q={search_term_string}&semantic=1`** (умный каталог **`runListingsSearchGet`**). **Stage 87.0:** **`lang`** запроса → **`inLanguage`** / описание **`WebSite`** (см. блок Stage **87.0**).
- **PDP (`app/listings/[id]/layout.js` + ListingSchema):** **`buildListingStructuredDataPayload`** в **`lib/seo/listing-schema-org.js`** — один JSON-LD script с **`@graph`**: объект листинга + **`lib/seo/listing-breadcrumbs.js`** (**`BreadcrumbList`**: Home → Thailand → город/канон острова (**`resolveCanonicalCityLabelForGeo`**) → район (**`district`**) → карточка). Основная сущность: **`Accommodation`** (жильё), **`Vehicle`**, **`Motorcycle`**, **`Boat`** (нативно **`vehicleEngine`** / **`vehicleTransmission`**, яхты: **`numberOfRooms`** как каюты); сохранённые туры / вертолёт (**`Product`/`Service`**) без ломания контрактов.
- **`buildListingJsonLd`** — упрощённый один объект + **`@context`** (обратная совместимость, не используется в **`ListingSchema`** после 86.0).

### Stage 87.0 — Локализация SEO-графа и trust-слой на карточках, 2026-05

- **JSON-LD i18n:** подписи **`BreadcrumbList`** — **`lib/seo/listing-breadcrumbs.js`** через **`getUIText`** (**`seoJsonLd_breadcrumb_*`** в **`lib/translations/booking.js`**). Глобальный **`WebSite`** — **`lib/seo/site-website-schema.js`**: **`buildWebSiteSearchActionJsonLd(..., lang)`** → **`inLanguage`** + **`description`** (**`seoJsonLd_webSiteDescription`**). Локаль запроса — **`getLangFromRequest`** в **`components/seo/GlobalSiteJsonLd.jsx`** и **`components/seo/ListingSchema.jsx`**; PDP граф — **`buildListingStructuredDataPayload(..., lang)`**.
- **Метаданные `/listings`:** шаблоны **`catalogSeo_*`** подставляют **`{brand}`** через **`getSiteDisplayName()`** в **`lib/seo/listings-catalog-seo-templates.js`**; название категории — **`getCategoryDisplayName`** (см. **`buildListingsCatalogMetadata`**).
- **Trust UI (каталог):** компактный бейдж «Verified» — **`listingQualifiesForTrustVerifiedMiniBadge`** в **`lib/listing-card-spec-profile.js`** (**`partnerTrust.reliabilityPercent` > 80% или > 0.8 в шкале 0–1**; флаги metadata **`verified_partner`**, **`partner_trust_verified`**, **`trust_verified`**); рендер — **`components/listing/ListingCardSpecsRow.jsx`** (**`partnerTrust_verified`**).

### Stage 87.1 — Trust-слой синхронизирован с картой и телеметрией выдачи, 2026-05

- **Карта:** попап маркера **`InteractiveSearchMap`** → **`ListingPopupCard`**: бейдж через **`components/listing/ListingTrustVerifiedMiniBadge.jsx`** (та же логика, что **`listingQualifiesForTrustVerifiedMiniBadge`**); в **`ListingCardSpecsRow`** для попапа включён **`suppressTrustVerifiedMiniBadge`**, чтобы не дублировать бейдж со строкой спек.
- **Lite payload:** **`listing-search-payload.js`** пробрасывает в **`metadata`** ключи **`verified_partner`**, **`partner_trust_verified`**, **`trust_verified`** для parity с полной карточкой.
- **Подсказка бейджа:** Radix **`Tooltip`**, текст **`listingCard_verifiedPartner`** (RU/EN/ZH/TH) — полная формулировка «проверенный партнёр» для иконки верификации.
- **Телеметрия:** **`lib/api/run-listings-search-get.js`** — **`console.info('[SEARCH TELEMETRY] catalog_verified_majority_anon', …)`**, если **> 50%** строк выдачи помечены как Verified (**без id листингов**); в логе только агрегаты и грубые подсказки контекста (**`category`**, укороченный **`whereHint`**, флаги области/семантики).

### Stage 88.0 — Визуальная отделка карты и карточек каталога + промо верификации партнёра, 2026-05
...
- **Карта:** **`components/listing/InteractiveSearchMap.jsx`** — **`MarkerClusterGroup`** (**`@changey/react-leaflet-markercluster`**, **`leaflet.markercluster`**, chunked loading); при zoom **≥ 13** кластеры отключены (**`disableClusteringAtZoom={13}`**, semantics plugin). Иконка кластера — **`lib/maps/search-map-cluster-icon.js`** (низкая / средняя / высокая плотность verified по **`marker.options.gslVerified`**, задаётся в **`ListingPriceMarker`** через **`listingQualifiesForTrustVerifiedMiniBadge`**). Стили: **`app/globals.css`** (**.gostaylo-map-cluster\***).
- **Персист телеметрии 87.1:** таблица **`public.catalog_verified_snapshots`** (**`database/migrations/046_catalog_verified_snapshots.sql`**); запись из **`lib/api/run-listings-search-get.js`** при доле verified **> 50%** через **`lib/services/telemetry/catalog-verified-snapshot.js`** (**`insertCatalogVerifiedMajoritySnapshot`**). Лог **`catalog_verified_majority_anon`** в stdout сохраняется.
- **Админ:** **`/admin/marketplace-health`** — таблица средних **`verified_share_approx`** по **`where_hint`** за **30** дней + Pulse за **7** дней (**`snapshotRowsLast7Days`**, опционально **`autoVerificationsLast7Days`**); **`GET /api/v2/admin/marketplace-health`** (**ADMIN** и **MODERATOR**, Stage **90.1**).
- **Партнёрский deep-link:** **`PartnerVerifiedBadgePromo`** — CTA **`/partner/settings?tab=verification`**; **`app/partner/settings/page.js`** — якорь **`id="partner-verification-panel"`**, **`KycUploader`** + **`PATCH /api/v2/partner/applications`** для заявки **`PENDING`**; без заявки — ссылка на **`/renter/profile`**. i18n: **`partnerSettingsVerification_*`** в **`partner-shell.js`**.

### Stage 90.0 — Админ-верификация профиля (KYC) и SEO гео-паритет RU/TH, 2026-05

- **Не дублировать статусы:** **`profiles.verification_status`** — enum **`PENDING` / `VERIFIED` / `REJECTED`**; партнёрская заявка — **`partner_applications.status`** — **`PENDING` / `APPROVED` / `REJECTED`**. Экран **`/admin/users/[id]`** — одна секция **KYC / Verification**: файлы (**`verification_doc_url`** из заявки и профиля) + действия **`PATCH /api/admin/users`** (**`requireAdminStaff`**, Stage **90.1**): **`is_verified: true`** + **`verification_status: 'VERIFIED'`** («Подтвердить верификацию») или **`REJECTED`**. Ранее UI ошибочно писал **`APPROVED`**, что не совпадает с enum профиля.
- **Бейдж доверия в каталоге (Stage 87 + 90):** **`listingQualifiesForTrustVerifiedMiniBadge`** теперь учитывает **`ownerVerified`** / **`listing.owner?.is_verified`**, чтобы подтверждение профиля сразу отражалось на карте/в lite-поиске (без необходимости отдельно выставлять metadata-флаги).
- **SEO:** **`lib/seo/catalog-seo-geo.js`** + ключи **`catalogSeo_place_*_{ru|th|generic}`** (**`catalog-seo.js`**); общий barrel **`lib/seo/catalog-seo-templates.js`**. PDP breadcrumbs: **`lib/seo/listing-breadcrumbs.js`** — узел страны **`RU`** vs **`Таиланд`**, ключи **`seoJsonLd_breadcrumb_countryRussia`** (**`booking.js`**). Карта алиасов канона: **`lib/locations/city-district-map.js`** (**Moscow**, Bangkok, …).

### Stage 90.1 — Защита Admin API и автоматическая выдача VERIFIED при одобрении заявки, 2026-05
...
- **Audit Trail:** **`lib/services/audit/staff-audit.js`** — **`recordStaffListingModeration`** / **`recordStaffUserVerificationUpdate`** → **`critical_signal_events`**; **`loadStaffAuditFeed`** собирает человекочитаемые строки (**`summaryRu`**) для UI. **`GET /api/v2/admin/marketplace-health`** возвращает **`staffAuditFeed`**, **`auditLogAvailable`**. Одобрение партнёра по-прежнему пишет **`SYSTEM_AUTO_VERIFICATION`** (**`lib/services/audit/system-auto-verification.js`**).
- **Админ-UI:** секция «Последние действия персонала» на **`/admin/marketplace-health`** (роли с доступом к странице видят ленту).
- **Тексты RU (споры):** **`lib/translations/booking.js`** — дружелюбные формулировки с опорой на **`{brand}`**, без сухого «система закрыла».
- **Рейтинг на карточке:** блок со звездой не показывается при отсутствии числового рейтинга (**`ListingInfo`**, десктопный **`BookingWidget`**).

### Stage 85.5 — Универсальный профиль спецификаций карточек (Stay, Transport, Yacht, Tour), 2026-05

- **Цель:** единые «главные качества» в поиске, на главной, во всплывающем окне карты (**`InteractiveSearchMap` → `ListingPopupCard`**) и без дублей на PDP в авто-блоке метаданных.
- **SSOT вертикали:** **`lib/listing-card-spec-profile.js`** — **`resolveListingCardSpecVertical`** (slug + **`categories.wizard_profile`**), хелперы полей (**`getListingCardBedrooms`**, **cc**, **каюты**, **transmission** и т.д.).
- **UI:** **`components/listing/ListingCardSpecsRow.jsx`** подключён в **`ListingCard`**, **`TopListingsGrid`**, **`InteractiveSearchMap`** (попап маркера). Цена в попапе — **`CardPriceDisplay`** + **`initialDates`** (проброс из **`listings-catalog-client`** через **`SearchMapWrapper.initialDates`**), подпись ночь/сутки — **`getListingRentalPeriodMode`**.
...
### Stage 103 — Soft launch E2E smoke + owner UX (2026-05)

- **Smoke:** `lib/smoke/financial-smoke-run.js` (guest+partner → booking → escrow → thaw → READY_FOR_PAYOUT → pool lock/settle → ZIP → partner docs); `npm run smoke:full-financial`; `POST /api/admin/smoke/financial-run` (ADMIN).
- **FinTech:** `GET /api/admin/finances/dashboard` → `treasury` (`openBatchesCount`, `awaitingActsLines`, `lastSettledBatch`); Concierge Launch banner; карточки «Готово к выплате» / «В открытых пулах» / «Последний пакет».
- **Legal:** `POST /api/admin/settings/legal/test-act` — демо PDF-акт; UI на `/admin/settings/legal`.
- **Уведомления:** Telegram FINANCE при `PayoutBatchService.markBatchSettled` (ссылка на финпульт + ZIP в строке пула).
- **Партнёр:** вкладка **Документы** на `/partner/finances` — бейдж количества, подсказка при 0 актов.
- **Доки:** `docs/STAGE_103_PRE_LAUNCH_CHECKLIST.md`, runbook `docs/CONCIERGE_LAUNCH_TREASURY_RUNBOOK.md` §12.

### Stage 102.4 — Legal polish: drafts, partner docs, bank ZIP (2026-05)

- **Партнёр:** `/partner/finances` → вкладка **Документы**; `GET /api/v2/partner/settlement-documents`, signed URL 30 дней — `GET …/settlement-documents/download`.
- **Админ legal:** черновик → превью → **Опубликовать** (`POST …/legal/draft`, `POST …/legal/publish`); история версий; дата обновления текста (`legal_pages_meta`); предупреждение о новых оплатах.
- **Банк:** `GET /api/admin/finances/payout-batches/[id]/bank-package` — ZIP (CSV + legal PDF + акты); кнопка в пуле FinTech.

### Stage 102.3 — Admin legal console + payout settlement PDF (2026-05)

- **`/admin/settings/legal`** — версии оферты/партнёра, кнопка новой версии, журнал акцептов, ссылки на `/legal/*`, PDF-справка.
- **Версии SSOT:** `LegalVersionsService` → `system_settings.general.legal_versions` (fallback `lib/config/legal-terms-version.js`); `lib/legal-consent.js` читает активную версию из сервиса.
- **API (ADMIN):** `GET /api/admin/settings/legal`, `POST …/legal/bump` (`doc: guest|partner`), `GET …/legal/consents`, `GET …/legal/pdf`.
- **Выплаты:** при `PayoutBatchService.markBatchSettled` и `PATCH /api/v2/admin/payouts/[id]` → `PAID` — `lib/services/payout-document.service.js` (pdfkit, нейтральный акт), Storage bucket **`payout-documents`**, email партнёру; `payouts.documents` JSONB (migration **`migrations/stage102_3_legal_admin_payout_documents.sql`**).
- **FinTech:** блок «Юридические документы» на `/admin/settings/finances` → ссылка на legal.
...
- **Register:** `POST /api/v2/auth/register` — обязательное тело `acceptedLegalTerms: true`, при создании строки выставляется `legal_terms_accepted_at = now()`.
- **Session:** `POST /api/v2/auth/login` и `GET /api/v2/auth/me` включают `legalTermsAcceptedAt` (и snake_case дубль там, где уже принят в DTO).
- **Payments:** до списания/подтверждения оплаты гостя с `bookings.renter_id` сервер вызывает `ensureProfileLegalConsentForPayment(sessionUserId, body.acceptedLegalTerms)` (`lib/legal-consent.js`). Точки входа: `POST /api/v2/bookings/[id]/payment/initiate`, `POST /api/v2/bookings/[id]/payment/confirm`, `POST /api/v2/payments/verify-tron` (ключ `acceptedLegalTerms` в JSON).
- **UI:** `components/legal/LegalConsentCheckboxRow.jsx` — ссылки на `/legal/public-offer/` и `/legal/privacy/`; используется в `contexts/auth-context.jsx` (регистрация) и `app/checkout/[bookingId]/components/PaymentMethods.jsx` (если у пользователя нет отметки в БД).
- **Admin:** `app/admin/users/[id]/page.js` — блок «Юр. согласие» по `profile.legal_terms_accepted_at`.
- **Branding:** ключи `partnerCtaTitle` и др. в переводах переведены на `{brand}`. Публичный support email: **`NEXT_PUBLIC_SUPPORT_EMAIL`** и **`getPublicSupportEmail()`** (`/terms`, `/help`, блок оператора и футер **`legal-doc-shell`**, колонка «Поддержка» на главной).
- **Компоненты (нейтральные имена):** главная — **`components/PlatformHomeContent.jsx`**, календарь PDP — **`components/platform-calendar.jsx`** (`PlatformCalendar`), карточка листинга — **`components/listing-card.jsx`** (`ListingCard`).

### Stage 77.8 — Booking atomic lock hotfix (2026-04)

- **DB hotfix:** миграция `migrations/stage77_8_booking_atomic_lock_fix.sql` переопределяет `public.create_booking_atomic_v1` и меняет lock scope на `FOR UPDATE OF l` при `listings LEFT JOIN categories`, чтобы Postgres не пытался блокировать nullable-сторону outer join.
- **Симптом и причина:** ошибка `FOR UPDATE cannot be applied to the nullable side of an outer join` больше не должна возникать на этапе атомарного `POST /api/v2/bookings`.
- **Антиспам алертов:** `lib/services/booking/creation.js` переводит этот конкретный DB-сбой в `recordCriticalSignal('BOOKING_ATOMIC_FOR_UPDATE_OUTER_JOIN')` с cooldown вместо отправки отдельного Telegram system alert на каждый повтор.

### Stage 77.9 — Daily guard for system alerts (2026-04)

- **Глобальный лимит:** `lib/services/system-alert-notify.js` ограничивает системные Telegram-алерты по умолчанию до **1 сообщения в UTC-сутки** (`SYSTEM_ALERT_DAILY_LIMIT`, default `1`).
- **Хранилище guard:** таблица `public.system_alert_daily_guard` (миграция `migrations/stage77_9_system_alert_daily_guard.sql`) хранит `sent_count`, `suppressed_count`, `last_seen_at`, `last_message_preview`.
- **Fallback:** если таблица/БД временно недоступна, включается in-memory guard в том же суточном лимите, чтобы продолжать подавлять штормы.
...
- **P&L engine (Stage 109.1 → 131.0):** facade **`lib/services/marketing/referral-pnl.service.js`** → **`referral-policy.service.js`**, **`referral-calculation.js`**, **`referral-payout.service.js`**. **Stage 131.0 / 131.6:** параметры waterfall/caps — **`system_fintech_settings`** через **`SystemConfigService`** (admin marketing save → **`referral-fintech-admin-sync.js`**); promo tank остаётся в **`system_settings.general`**.
...
- **Attribution 2.0 (Stage 120.0–120.5):** `referral_attributions` + **`lib/referral/attribution.service.js`**; финансовые метрики SSOT — **`lib/finance/reporting.service.js`** (будущий Financial Intelligence Dashboard); админ **`/admin/marketing/attribution`** (`referrers-csv`, **`cohort-csv`**, gross/net margin, clawback) — без изменения margin-based payout.
- **Campaigns B3.1 (Stage 122.1):** отдельный admin экран **`/admin/marketing/campaigns`** (создание/редактирование/пауза), API **`/api/v2/admin/referral/campaigns*`** и self-service bind **`/api/v2/referral/campaign-binding`**. Реестр кампаний хранится в `system_settings` (`referral_campaigns`), но runtime guard остаётся в `referral_codes.metadata` (`campaign_is_active`, `campaign_expires_at`, `max_budget_thb`, `current_spent_thb`, `override_hold_days`) через синхронизацию.
- **Campaigns B3.2 (Stage 122.2):** drill-down **`/admin/marketing/campaigns/[slug]`** — KPI, воронка, рефереры, когорты, коды; CSV export (list + detail); budget top-up через **`PATCH` `action: top_up_budget`**; SSOT `buildReferralCampaignDrilldown` в `referral-campaigns.service.js`.
- **Campaigns B3.3 (Stage 122.3):** бюджет ≥90% — баннер в списке и карточке; 100% — `setReferralCampaignState(false)` + Telegram FINANCE (`REFERRAL_CAMPAIGN_BUDGET_EXHAUSTED`); аудит пополнений `metadata.budget_topups[]`; колонка «До окончания»; attribution — отдельная карточка фильтра кампании.
- **Reward rules B4 (Stage 123.0–123.1):** `referral_reward_rules` + SSOT facade `resolveReferralAccrualPolicy()`; приоритет hold: campaign override → production rule → global. Rollout-флаг `rules.apply_split_in_production` включает реальное влияние `split_ratio` и `min_booking_value_thb`; `metadata.reward_rule_applied=true`. `referral_ledger` колонки `rule_version` / `reward_rule_id` + metadata `reward_rule_*`. Admin **`/admin/marketing/reward-rules`** (campaign picker), API **`GET/POST /api/v2/admin/referral/reward-rules`**, **`PATCH …/[id]`**.
- **Anti-fraud B5 (Stage 123.2):** `ReferralGuardService` v2: device graph (fingerprint + IP token), velocity окна по track/convert, self-referral chain loop, pattern checks (throwaway domain, domain/name clusters). Suspicious/block сигналы пишутся в `critical_signal_events` и очередь `referral_fraud_queue` (admin **`/admin/marketing/fraud-queue`**, API **`/api/v2/admin/referral/fraud-queue`**, **`PATCH …/[id]`**). Fraud markers (`fraud_suspicious`, `fraud_rule_codes`, `fraud_severity`) сохраняются в `referral_attributions.metadata` и прокидываются в `referral_ledger.metadata`.
- **Phase B closure (Stage 123.3):** observability-polish без смены экономики — KPI suspicious count/% на верхнем уровне (`/admin/marketing/attribution`, `/admin/marketing/campaigns/[slug]`), deep-link расследование в fraud queue (queue → attribution / ledger / profile), документы Referral 2.0 переведены в статус завершённой Phase B.
- **Phase D closure (124.17):** SSOT **`docs/PHASE_D_CLOSURE_AND_ROADMAP.md`** — owner summary, tech debt, roadmap; аналитика в коде не расширяется без нового ADR.
- **Admin marketing IA (Stage 124.0–124.17, Phase D ✅):** **`/admin/marketing/roi`** — ROI-пульт, `#owner-guide` (`RoiOwnerGuideCard`, `REFERRAL_ROI_OWNER_GUIDE`), business summary, fraud-adjusted, Owner Digest; **`/admin/marketing/roi/[campaignSlug]`** — drill-down + LTV/retention; FI **`ReferralEconomicsWidget`** + header «Referral ROI»; брони → **`/admin/finance/intelligence/bookings/[id]`**; SSOT `lib/analytics/`.
- **Как владелец использует Phase D (playbook):** (1) FI за период → (2) ROI-пульт и карточка «Как пользоваться» → (3) клик кампании → (4) клик брони → P&L; еженедельно — Owner Digest; при фроде — fraud-adjusted + `/admin/marketing/fraud-queue`. Подробности — `docs/TECHNICAL_MANIFESTO.md` § Phase D.
- **Attribution campaigns analytics (Stage 122.1):** `/admin/marketing/attribution` поддерживает фильтр `campaignSlug` и отдельную таблицу campaign metrics (clicks, registrations, first/repeat bookings, earned/spend/ROI) из `FinancialReportingService.buildCampaignMetricsRows`.
- **Ledger health:** **`GET /api/v2/admin/ledger-balances`** — chunked balance reads для `/admin/financial-health`.
...
- **MLM depth=2:** распределение partner activation bonus по L1/L2 upline через `ancestor_path` + `referrer_id`; проценты и **`partner_activation_bonus_thb`** — только **`system_fintech_settings`** (Stage 131.0 / 131.6; marketing admin PUT синхронизирует fintech, не `general`).
...
- **Admin payout control:** `GET/PATCH /api/v2/admin/wallet/payouts` + UI **`/admin/marketing/referral-payouts`** (tabs: queue / T-Bank registry / processing statuses; legacy `/admin/marketing/payouts` redirects).
...
- **Auth gateway doc:** `docs/AUTH_GATEWAY_OAUTH.md` — текущая JWT-сессия, включение Google в Supabase, redirect checklist, связка с referral после OAuth.
...
- **`lib/services/notifications/email.service.js`:** Resend для простых писем/фолбэков (`sendResendEmail`, text→HTML). **Stage 155.5:** перед HTTP — **`shouldMockResendDelivery`** (`lib/email/resend-transport-guard.js`) для smoke/E2E/тестовых адресов. Брендированные React-шаблоны остаются в **`lib/services/email.service.js`** (**`EmailService`**, тот же guard в **`sendEmail`**).
...
- **Канон ссылок «к брони» в списках (гость):** `lib/email/booking-routes.js` — **`renterBookingsListPath(id)`** → **`/my-bookings?booking=…`** (Stage 138.3; **`/renter/bookings`** — redirect). Активные premium-шаблоны в **`lib/services/email.service.js`**: `bookingRequested`, `newLeadAlert` (для партнёра — **`partnerBookingsListPath`**), `paymentSuccessGuest` (как и ранее, **`bookingsUrl`** строится в хабе уведомлений). Мёртвый шаблон/метод **`sendBookingStatusChange` / `bookingStatusChange`** удалён (не использовался).
...
- **Tron verify API:** `POST /api/v2/payments/verify-tron` при **`bookingId` + успешной верификации** вызывает **`PaymentsV3Service.confirmPayment`** (а не «тихий» UPDATE `payments`), чтобы сработали **эскроу + фоновый ledger**, как у админского confirm. **124.18:** при `bookingId` обязательны сессия и **`renter_id`** (или staff); checkout fetch с **`credentials: 'include'`**.
- **`POST /api/v2/payments/submit-txid` (124.18, **125.2**):** сессия + проверка **`bookings.renter_id`** (staff ADMIN/MODERATOR bypass). **125.2:** до UPDATE — если последний `payments.status` ∈ `{CONFIRMED, COMPLETED}` → **409** `payment_already_finalized`; если `bookings.status` ∈ post-escrow (`PAYMENT_ACQUIRING_WEBHOOK_IDEMPOTENT_BOOKING_STATUSES`) → **409** `booking_payment_past_escrow`. Smoke **6c**.
- **Payment webhooks (124.18, 125.1, **125.4**, **127.0**):** `POST /api/webhooks/payments/confirm`, `POST /api/webhooks/crypto/confirm` — **`assertWebhookGuestPaymentAllowed`** (`lib/payment/webhook-guest-payment-gate.js`: emergency pause, fiscal, treasury manual mode + structured audit log). **125.1 / 125.4 / 127.0:** ранний **2xx** `{ success, idempotent, alreadyProcessed }` для escrow pipeline / `COMPLETED` (`PAYMENT_ACQUIRING_WEBHOOK_IDEMPOTENT_BOOKING_STATUSES`); acquiring после `markPaid` — повторная проверка статуса брони (без `recordTreasuryWebhookError` на idempotent). Crypto USDT: **`getExpectedUsdtForBooking`** (`lib/services/currency.service.js` + `guestPayableRoundedThbFromBooking`). Acquiring smoke **7b**, crypto **7c**.
...
- **Канон в БД:** таблица **`categories`**, поля **`slug`**, **`wizard_profile`** (Stage 67.0 — вертикаль для визарда/реестра/поиска), **`parent_id`** (Stage 68.0 — иерархия для Super-App), **`name_i18n`** (Stage 69.0 — JSONB **`{ru,en,zh,th}`**, публичный API **`nameI18n`**, резолв имён **`lib/category-display-name.js`**), **`description`** (Stage 69.1 — копирайт под заголовком каталога для родительских категорий + SEO-снимок), **`is_coming_soon`** + **`is_preview_only`** (Stage 83.0 — smart visibility), связь **`listings.category_id`** (объявления по-прежнему привязаны к конкретной строке категории; фильтр по родителю в поиске расширяется на детей). Нормативно: **`ARCHITECTURAL_DECISIONS.md`** §**10**.
- **Публичная выдача категорий (Stage 85.0):** **`GET /api/v2/categories`** для обычных пользователей возвращает только **`is_active=true`** (и `!is_preview_only`); категории с `is_active=false` больше не попадают в публичные home/search/wizard поверхности. Для **ADMIN** сохраняется режим полного списка + вычисляемый `isPreview` для скрытых от публики строк.
- **Waitlist по “Soon”:** **`POST /api/v2/leads/waiting-list`** сохраняет email-лиды в **`leads_waiting_list`** (категория, язык, источник) только для категорий с **`is_coming_soon=true`**; уникальность по паре `(category_slug,email)` (lowercase).
- **Операционный контроль спроса:** **`GET /api/v2/admin/categories`** возвращает `waitlistLeads` на категорию; экран **`/admin/waitlist`** использует **`GET /api/v2/admin/waitlist`** (фильтр `?category=<slug>`, экспорт `?format=csv`) и показывает Email/категорию/язык/дату. При вставке waitlist-лида backend отправляет Telegram-уведомление админу.
- **Security hardening (Stage 83.1):** общий server guard **`lib/security/access-guard.js`** валидирует JWT и **обязательно** перепроверяет роль/ban в `profiles`; при `is_banned=true` очищает `gostaylo_session` и возвращает 403. Для open-redirect используется **`lib/security/safe-internal-path.js`** (`/api/v2/auth/login`, client auth redirect, OAuth callback `next`). TTL cookie-сессии приложения: **7 дней** (SSOT: `lib/auth/app-session-issue.js`).
- **Security hardening (Stage 83.2 / Phase 1):** financial/admin routes закреплены на едином guard **`requireAccess({ roles: ['ADMIN'] })`**. Ключевые точки: **`/api/v2/payments`**, **`/api/v2/admin/payment-adapters/health`**, **`/api/v2/admin/payouts*`**, **`/api/v2/admin/payout-methods*`**, **`/api/v2/admin/ledger-*`**, **`/api/v2/admin/wallet/transactions`**, **`/api/v2/admin/exchange-rates-health`**, плюс custom admin-ai/impersonation/ban routes. Cron endpoints (`/api/cron/*`) не используют cookie-session и остаются на проверке `CRON_SECRET`.
- **iCal enum safety:** **`/api/cron/ical-sync`** использует whitelist допустимых `listing_status` (`PENDING|ACTIVE|BOOKED|INACTIVE|REJECTED`) вместо `neq('DELETED')`, чтобы исключить Postgres enum error 42704.
- **Push sweeper runtime safety:** в `PushService.runStaleChatPushSweeper` и `cron/push-token-hygiene` добавлена проверка `supabaseAdmin.from` перед DB-операциями; при некорректной инициализации возвращается контролируемая ошибка без падения рантайма.
- **P3 archival cleanup:** legacy mock endpoint **`app/api/v2/bookings/[id]/payment-status/route.js`** и материалы **`legacy/unused-cron-logic/*`** перенесены в **`legacy/archive/`** (не runtime).
- **Stage 84.0 (Priority 1 hardening):**
- **Admin DB re-check everywhere:** все маршруты **`app/api/v2/admin/**`** перешли на **`requireAccess({ roles: ['ADMIN'] })`**; удалены route-level JWT-only проверки.
- **Search anti-bypass for hidden categories:** в **`app/api/v2/listings/search/route.js`** добавлен visibility guard: если `category` скрыта (`is_active=false` или `is_preview_only=true`), публичный ответ принудительно пустой; полный результат видит только ADMIN.
- **Stage 85.0 — Dynamic Category Visibility:** публичные UI-точки выбора категорий (`HomeHeroLuxe`, `UnifiedSearchBar`, partner wizard через `PartnerCategoryPickerTwoStep` + `ListingWizardContext`) используют strict active-only фильтрацию (`isActive===true`) из API/БД, без удаления транспорта/яхтного кода; выключение категории в админке автоматически убирает ее из навигации и доступных селекторов.
- **Atomic escrow + ledger posting contract:** миграция **`migrations/stage84_0_atomic_escrow_ledger_contract.sql`** добавляет функцию **`move_to_escrow_and_post_ledger_v1`**; `EscrowService.moveToEscrow` теперь вызывает RPC, чтобы переход `PAID_ESCROW` и posting в `ledger_journals/ledger_entries` выполнялись в одной DB-транзакции.
- **Mock transparency in payment intent:** payment initiate API отдает `isTestMode` при mock/fallback-провайдере; сервер пишет warning `⚠️ ВНИМАНИЕ: Проведен тестовый платеж (MOCK_MODE) для брони [ID]` в initiate/confirm flow.
...
- **Время разморозки эскроу (финансовое «ведро»):** **`lib/escrow-thaw-rules.js`** — **`getEscrowThawBucketFromCategorySlug`** (читает реестр) → **`housing` \| `transport` \| `service`**, затем **`computeEscrowThawAt`** с **`listing.metadata.timezone`** / **`resolveListingTimeZoneFromMetadata`** и **`listingYmdLocalWallTimeToUtcIso`** (Stage 152.2 — без `hour - 7`). Туры (**`tours`**, подстрока **`tour`**) попадают в ведро **`service`** (+2 ч от старта), **не** в отдельный thaw-тип.
...
- **`hooks/useCheckoutPayment.js`:** загрузка брони/инвойса, `GET` payment-intent, `allowedMethods`, проверка доступа, `language`, **`chatConversationId`**: сначала **`b.conversation_id`** из ответа **`GET /api/v2/bookings/[id]`** (см. **`getBookingById` + `attachConversationIdsToBookings`**), иначе fallback на **`GET /api/v2/chat/conversations`**. Initiate, **`POST .../payment/confirm`**, крипта: `POST /api/v2/payments/verify-tron` с **`{ txid, bookingId }`**, обработка **`paymentSettled`** (toast, **`loadPaymentStatus`**, success UI). Ошибки оплаты/верификации — через **toast** (не молчать). **124.19 smoke:** `lib/smoke/checkout-http-escrow-step.js` — in-process **`POST …/payment/initiate`** + **`POST /api/webhooks/payments/confirm`** (YooKassa-style mock, HMAC); шаг 6 в **`npm run smoke:full-financial`**.
...
- **Хуки треда (109.3):** `useUnifiedMessagesThread` (inbox + realtime bridge), `useUnifiedMessagesPeer` (presence/typing/payNowHref), `useUnifiedMessagesBookingActions`, `useUnifiedMessagesOutbound` (text/voice/invoice/passport), `useUnifiedMessagesNavigation`.
- **SSoT треда (не путать с глобальным инбоксом):** `app/messages/[id]/context/ChatContext.js` — **`MessengerThreadProvider`**, **`useMessengerThread`**. Глобальные бейджи/список — **`@/lib/context/ChatContext.jsx`** + **`lib/chat/chat-unread-bridge.js`**.
- **Данные и Realtime:** **`hooks/use-chat-thread-messages.js`**; **`useChatRealtime`** / **`use-realtime-chat.js`**. Точка входа: **`app/messages/[id]/hooks/useChatRealtime.js`** (реэкспорт).
...
- **TTFB главной:** `app/page.js` — **статическая** оболочка (убрано **`export const dynamic = 'force-dynamic'`**); список/курсы тянет **`PlatformHomeContent`** на клиенте. **Suspense fallback** — **`HomePageSkeleton`** (`components/home-page-skeleton.jsx`), а не полноэкранный спиннер.
...
- **Главная — `components/PlatformHomeContent.jsx`:** тонкий оркестратор; **`components/home/`** — **`HomeHeroLuxe`** (Airy Premium hero, единая 60-px геометрия `Where/Dates/Guests/CTA`, env-копии через **`lib/config/home-page-copy.js`** с поддержкой токена `'AUTO'`), **`StickySearchBar`**, **`TopListingsGrid`** (env-копия **`NEXT_PUBLIC_HOME_TOP_LISTINGS_TITLE`**), shared **`home-constants.js`**, **`useHomeFilters.js`** (What/Where/When/Who, дебаунс, сид из URL, semantic flag с сайта). **`WhereCombobox`** имеет `variant='flat'` для встраивания в 60-px поле. Гости унифицированы через SSOT-компонент **`components/search/GuestsPopover.jsx`** (Adults/Children/Infants, +/- controls, summary text в trigger) и переиспользуются в `HomeHeroLuxe`, `StickySearchBar` и `UnifiedSearchBar` (поиск). Overlay-контракт унифицирован: popover’ы рендерятся через portal в `body` (`components/ui/popover.jsx`), desktop/dropdown держатся выше search/header по z-index, mobile для where/guests/dates — через `Drawer` шторки. Финальный visual SSOT: desktop-панели календаря/гостей/городов имеют `rounded-3xl` + `shadow-2xl`; календарь строго непрозрачный (`bg-white opacity-100`) и контраст `text-slate-900`; интерактивные элементы в поповерах используют teal hover-feedback (`hover:bg-teal-50`). Дублирующий **`CategoryBar`** на главной не используется — категории остаются только как pill-фильтры в блоке поиска. Вертикальный ритм main-секций: `py-12 sm:py-16`. Скелетон **`ListingCardSkeleton`** соответствует реальной карточке (no CLS).
...
- **Skeleton главной — `components/home-page-skeleton.jsx`:** зоны **hero / категории / сетка+футер** согласованы по высотам/отступам с реальной вёрсткой, **`min-h`** у блока сетки, чтобы снизить скачок при гидрации `PlatformHomeContent`.
- **Stage 85.5 — см. секцию выше:** карточки и маркеры карты используют **`ListingCardSpecsRow`**; PDP не дублирует **`transmission`/`gearbox`** (см. **`listing-public-metadata-specs`**).

### Treasury & Payouts v2 (ADR-097, Concierge Launch)

**Норматив:** **`docs/ADR/097-financial-model-v2.md`** · операции: **`docs/CONCIERGE_LAUNCH_TREASURY_RUNBOOK.md`** · чеклист: **`docs/PRE_LAUNCH_CHECKLIST.md`** · soft launch: **`docs/SOFT_LAUNCH_PLAN.md`**.

| Слой | Поведение |
|------|-----------|
| **Pricing v2 (production)** | `PRICING_ENGINE_V2=true` → **`lib/pricing-engine/PricingEngine`**, `bookings.pricing_snapshot.v=2`, округление гостя до **1 THB**, внутренний split RU (ИП) + KR (ОсОО IT) + FX markup из **`pricing_profiles`**. Attestation: **`lib/booking-price-integrity.js`**. |
| **Concierge Launch** | Учёт и статусы в Postgres; **исходящий перевод партнёру — вручную** (банк / реестр). Авто `payout-batch-pools` **выкл**; пул только из пульта **«Сформировать пул на сегодня»** → Lock → CSV → банк → **«Отметить как оплаченный»** (`PATCH …/payout-batches/[id]` `action: settled`). |
| **RUB acquiring + spread** | Гость платит в **RUB** (YooKassa/MIR); ledger и партнёрские суммы — **THB**. SSOT суммы в PSP: **`lib/services/payment-adapters/acquirer-charge-amount.js`** (snapshot parse → **`lib/booking/guest-payment-display.js`**). UI/dispute guest amount — **`readGuestPaymentDisplay`**. Курсовой спред — поле **`fx_markup_thb`** в snapshot (не путать с FX TTL **`CurrencyService`**). |
| **24h hold** | После thaw: **`PARTNER_WITHDRAWAL_HOLD_MS`** в **`lib/partner/partner-payout-eligibility.js`** → статус **`READY_FOR_PAYOUT`** (cron или кнопка пула). |

**Четыре слоя FX (не смешивать):**

| # | Слой | SSOT | Когда | Спред |
|---|------|------|-------|-------|
| **1** | Гость (оплата) | `pricing_snapshot.final_breakdown` (`fx_customer_rate_to_thb`, `fx_raw_rate_to_thb`), `bookings.exchange_rate` / `price_paid`, **`acquirer-charge-amount.js`** | Создание брони → оплата | `fx_markup_thb` (доход платформы у гостя) |
| **2** | Settlement (история брони) | `settlement_v3.partner_net.preferred_amount` via **`mergeSnapshotLockedRawRateMap`** (`booking/pricing.service.js`) | Confirm / attach snapshot | **Нет** выплатного спреда |
| **3** | ~~UI справочно~~ (deprecated для выплат) | Раньше: live `exchange-rates` на портфеле/истории | — | **Stage 100.8:** убрано для payout UI |
| **4** | Исходящая выплата | **`lib/partner/partner-payout-fx.js`** + **`GET /api/v2/partner/payouts/preview`** + **`POST …/preview-batch`** | Заявка / пул / модалка / строки истории | RUB: `PARTNER_PAYOUT_FX_RUB_SPREAD_PCT`; USDT: mid |

Партнёрский кабинет (**Stage 100.8**): баланс в **THB**; ₽/USDT — только из preview (карточка расчёта, модалка, портфель net, строки истории). Дисклеймер: ориентир на момент просмотра. Цифры в карточке брони (`settlement_v3`) — ориентир слоя 2. Админ: **`amount_in_payout_currency`** в `/admin/financial-health` и в БД заявок.

**Stage 101 (первый шаг):** учёт реальных конвертаций в treasury-пульте. API **`GET/POST /api/admin/finances/conversions`**, миграция **`056_track_conversions.sql`** (`ledger_entries.conversion_*`, `external_tx_reference`, `conversion_loss_thb`). Финтех-блок **«Конвертации и потери»** на `/admin/settings/finances` фиксирует фактические операции (курс/комиссия/tx) и считает виджет **«Реальная маржа»** за период:
`accepted guest THB - paid payouts THB - conversion losses THB`.
...
| Рельс | Код / метод | Назначение |
|-------|-------------|------------|
| **RUB direct (банк РФ)** | `payout_batches.rail` **`TBANK_RU`**, метод **`pm-bank-ru`** / channel **BANK** | CSV реестр: **`POST /api/v2/admin/payouts/tbank-registry`**, preview **`GET …/tbank-registry`**, UI **`/admin/marketing/referral-payouts?tab=registry`**. Партнёрский профиль: ФИО, ИНН, БИК, счёт. |
| **KR / USDT (крипто)** | **`KG_CRYPTO`** (ADR), channel **CRYPTO** | Профиль: сеть (TRC20) + адрес USDT; batch export plugin по ADR-097 (scaffolding в **`payout_batches`**). |
| **Партнёрская заявка** | **`POST /api/v2/partner/payouts`**, preview **`GET …/payouts/preview`** | PENDING + резерв available (THB в ledger); FX в **`amount_in_payout_currency`** (RUB spread / USDT mid). Concierge — staff вручную. Alias **`/payouts/request`**. |
| **Legacy THB auto** | **`processAllPayoutsForToday`** | **Запрещён** на prod Concierge (см. runbook §5). |

**Feature flags (env, production):**

| Flag | Prod | Роль |
|------|------|------|
| `PRICING_ENGINE_V2` | `true` | v2 breakdown + integer rounding |
| `FISCAL_SANDBOX` | `false` | Боевые чеки 54-ФЗ |
| `PAYMENT_ACQUIRER_RUB_ENABLED` | `1` (default) | Charge/capture в RUB для MIR_RU |
| `PAYMENT_ACQUIRER_RUB_SHADOW` | `0` | `1` = лог RUB, в PSP уходит THB (staging only) |
| `PARTNER_PAYOUT_FX_RUB_SPREAD_PCT` | `1.75` | Выплатной спред RUB (партнёр получает меньше ₽ за ฿); SSOT **`partner-payout-fx.js`** |
| `CRON_SECRET` | set | Внешний cron-job.org для hourly thaw/promote (Hobby) |

Полный список — **`docs/PRODUCTION_ENV.md`**, **`docs/CRON_EXTERNAL_FINANCIAL.md`**.

**Миграции (financial v2):**

| File | Содержание |
|------|------------|
| **`database/migrations/053_financial_model_v2.sql`** | `pricing_profiles`, `payout_batches`, `payout_batch_items`, RUB cols on ledger, assignments |
| **`database/migrations/054_add_ready_for_payout_status.sql`** | enum **`booking_status`**: `READY_FOR_PAYOUT` |
| **`database/migrations/055_stage100_6_partner_payout_multicurrency.sql`** | `payouts` / `payout_batch_items`: `amount_in_payout_currency`, `payout_currency` |

**Admin FinTech pulpit** — **`/admin/settings/finances`** (`components/admin/finances/AdminFinTechConsole.jsx`, Stage 109.0 — композиция):

**Financial Intelligence (Stage 124.4–124.10)** — **`/admin/finance/intelligence`**; SSOT **`lib/analytics/`**; bank reconciliation **`finance_bank_reconciliation_entries`** + **`GET/POST …/intelligence/bank-reconciliation`**; export CSV/XLSX/PDF; cache **`invalidateFinancialIntelligenceCache`** on batch settle, FX POST, payout batch settled.

- **Состояние/API:** `hooks/useAdminFinTechConsole.js`, утилиты **`lib/admin/fintech-console-shared.js`**.
- **Панели (вкладки/блоки):** `FinTechConsoleHeader`, `FinTechOverviewDashboard`, `PayoutBatchesPanel`, `ConversionsPanel` (→ `FinTechTreasuryConversionsPanel`), `MovementsJournal` (→ `FinTechMovementJournal`), `FinTechExportsPanel`, `LaunchReadinessCard` (→ `FinTechLaunchStatusDashboard`), `TestDataToolbar` (→ `FinTechTestDataToolbar`, **режим владельца** `localStorage` `fintech-owner-mode`).
- **Уже вынесено ранее:** `FinTechTreasuryHeroDashboard`, `FinTechEmergencyPauseCard`, `FinTechCronHealthPanel`, `PayoutBatchRow`, `FiscalSandboxReceiptDialog`.
- APIs: `GET /api/admin/finances/dashboard`, `pricing-v2`, `payout-batches`, `compliance-export`, `fiscal-test`, `pricing-profiles`.
- Compliance CSV: **`lib/admin/compliance-registry-csv.js`** — период по **дате оплаты**; не использовать `listings.category_slug` (только `categories.slug`).
- Stage 101 placeholder: **`FinTechTreasuryConversionsStub`** (ручной журнал FX-потерь).

**Не путать:** **`/admin/finances`** — верификация входящих платежей (MOCK_MODE); treasury — только **`/admin/settings/finances`**.
...
- **PR-#2 баланс по эскроу:** **`GET /api/v2/partner/balance-breakdown`** — **`frozenBalanceThb`** / **`thawHoldBalanceThb`** (THAWED &lt;24h) / **`availableBalanceThb`** (**`THAWED`≥24h** + **`READY_FOR_PAYOUT`**), **`byCategory`**, **`recentLedgerTransactions`**. **`GET /api/v2/partner/finances-summary`** — корзины **Pending / Escrow / Thaw hold / Available / Total paid** + reconciliation. **Stage 98–100:** cron **`/api/cron/promote-ready-for-payout`**; admin FinTech **`/admin/settings/finances`** (см. § **Treasury & Payouts v2**); compliance **`GET /api/admin/finances/compliance-export`** + **`…/compliance/[bookingId]?format=csv`** (SSOT CSV — **`lib/admin/compliance-registry-csv.js`**). Колонки **`profiles.frozen_balance_thb`**, **`profiles.available_balance_thb`** синхронизируются в **`EscrowService`**. Разморозка: **`POST /api/cron/escrow-thaw`**. Из‑за лимитов **Vercel Hobby** (cron не чаще 1/день) критичные кроны запускаются гибридно: Vercel расписание остаётся daily для валидного деплоя, а **hourly** для **`escrow-thaw`** дублируется внешним планировщиком (cron-job.org) с заголовком `Authorization: Bearer CRON_SECRET` (или `x-cron-secret`). Роут **`/api/cron/payouts`** удалён из приложения (исторический автопул); см. архив **`legacy/unused-cron-logic/`**. Партнёрский UI: **`/partner/finances`** (карточка баланса + таблица ledger + сводка); вывод — **`PartnerFinancesWithdrawDialog`** + реквизиты **`/partner/payout-profiles`**.
...
- **PR-#5 отмена брони и UI политики:** … Рентер: **`/my-bookings`** (канон, Stage 138.3), **`/checkout/[bookingId]`** (**`components/renter/cancel-booking-dialog.jsx`**).
- **PR-#7 Smart Extension Cockpit (этап 1):** chat invoice расширен без breaking changes: **`POST /api/v2/chat/invoice`** принимает **`intent=extension`** + **`new_check_out`** (optional). Оплата инвойса через checkout прокидывает **`invoiceId`** в **`POST /api/v2/bookings/[id]/payment/confirm`**. После успешного **`EscrowService.moveToEscrow`** (или post-escrow path при **`PAID_ESCROW` + invoiceId**, **Stage 146**) сервер применяет post-payment effects: invoice → **paid**, **`messages.metadata.invoice.status = PAID`** (Realtime UPDATE), extension идемпотентно меняет **`bookings.check_out`** через **`bookings.metadata.appliedExtensionInvoiceIds`**, пишет system message в чат (**`system_key=booking_extension_confirmed`**) и досрочно закрывает soft-hold блока счёта в **`calendar_blocks`** (**`source=invoice_hold`**, `expires_at=now`) для корректной доступности календаря.
...
- **UI:** `app/admin/health/page.jsx` — маршрут **`/admin/health`**, карточки **`rounded-2xl`**: агрегаты **`ops_job_runs`** (7 дн.) для **`ical-sync`**, **`push-sweeper`**, **`push-token-hygiene`**, **`partner-sla-telegram-nudge`** (метрики + «покрытие» TG vs **`partner_sla_nudge_events`**). **Stage 142:** лента **`jobFailures`** (ошибки в **`<details>`** monospace); блок **каналов уведомлений** (Telegram success % / FCM tokens cleaned, 24 ч, WARNING **>15%** fail); **`CriticalSignalsPanel`** — фильтр/поиск по **`critical_signal_events`** (`PRICE_TAMPERING`, `CONTACT_LEAK_ATTEMPT`, `FCM_TOKEN_CLEANED`, …) + ссылки на детали (read-only).
...
- **API:** **`GET /api/v2/admin/health`** — только **`profiles.role === 'ADMIN'`** или email из **`ADMIN_HEALTH_EMAILS`** (см. **`lib/admin-health-access.js`**); данные через **`supabaseAdmin`**. **Stage 142:** SSOT **`lib/admin/ops-job-health.js`**, **`lib/admin/notification-channel-health.js`**, **`lib/admin/critical-signals-health.js`**; query **`signalsKey`**, **`signalsQ`** (опционально).
...
- **Route:** `POST /api/webhooks/crypto/confirm` — **не** публичный: требуется **`CRYPTO_WEBHOOK_SHARED_SECRET`** (заголовок **`x-crypto-webhook-secret`** или поле **`webhookSecret`** в JSON), иначе **401/503**. **125.4:** после загрузки брони — idempotent **2xx** на post-escrow статусах (тот же набор, что acquiring 125.1); без `verifyTron` / `moveToEscrow`.
...
- **Route/UI:** `app/my-bookings/page.js` — **канонический** список гостевых бронирований (Stage 138.3): `UnifiedOrderCard`, вкладки статуса (**`lib/booking/my-bookings-tabs.js`**), фильтр типа, deep link **`?booking=`**; **`app/renter/bookings/page.js`** — redirect на **`/my-bookings`**.
...
- **Dispute modules (Stage 109.2):** facade **`lib/services/dispute.service.js`** (импорты без изменений); **`dispute-shared.js`** (константы, FSM, evidence paths); **`dispute-notifications.js`** (`runDisputeOpenedSideEffects`); **`dispute-resolution.js`** (unfreeze, notify parties); **`dispute-update.js`** (events, FSM transitions, evidence URLs, SLA crons); **`dispute-create.service.js`** (`createOfficialDispute`); **`dispute-payout-freeze.js`** (release freeze).
- **Payout batch modules (Stage 109.2):** facade **`lib/services/payout-batch.service.js`**; **`payout-batch/payout-batch-shared.js`**; **`payout-batch-creation.js`** (promote, draft pool, list, lock); **`payout-batch-export.js`** (registry + period CSV); **`payout-batch-settlement.js`** (`getBatchSettleBlockers`, `markBatchSettled`). Re-export **`isScheduledPayoutPoolDay`** из facade.
- **Admin levers groundwork:** `POST /api/v2/admin/disputes/[id]/action` (ADMIN/MODERATOR) поддерживает команды `freeze_payment`, `force_refund`, `add_penalty` (запись в `dispute_penalties` + flags в `disputes.admin_action_flags`). Stage 84.1: при первом staff-действии по `OPEN` кейсу статус автоматически переводится в `IN_REVIEW` (FSM guard в `DisputeService`).
...
- **Панель решения:** `Sheet` справа: **Stage 141.2** — summary (эскроу, статусы, холд, даты **`resolvedAt`/`rejectedAt`**, доказательства); **`AdminDisputeLedgerTimeline`** (ledger по брони); **`UnifiedOrderCard`** с `role="admin"` (карточка + блок гость/партнёр), компактная лента чата **`components/admin/AdminDisputeChatPeek.jsx`** (чтение через **`GET /api/v2/chat/messages?conversationId=`**), поле **вердикта**, выбор **финансовой стратегии** (`DISMISS` / `PAYOUT_PARTNER` / `REFUND_GUEST` / `SPLIT`) и **виновной стороны** (штраф) и кнопки рычагов. **Stage 141.1:** блок **Split** — слайдер **`guestPercent`** (1–99), preview сумм (гость / партнёр), кнопка «Применить Split» → toast + refresh.
- **API:** **`GET /api/v2/admin/disputes`** (список с embed `bookings` + `listing` + opener), **`GET /api/v2/admin/disputes/[id]`** (детальный кейс + `unified_order` + **`splitPreview`** + **`ledgerTimeline`** / **`holdStatus`** / **`financialSummary`**); расширение **`POST /api/v2/admin/disputes/[id]/action`**: добавлено **`close_dispute`** (статус **CLOSED**, `freeze_payment=false`, `resolved_at`, `closed_by`, метаданные вердикта); при выбранной виновной стороне — запись в **`dispute_penalties`** (тот же контур, что **`add_penalty`**). **Stage 141.2:** **`close_dispute`** принимает **`resolutionStrategy`** (`PAYOUT_PARTNER` / `REFUND_GUEST` / `SPLIT` / `DISMISS`) + **`guestPercent`** для SPLIT; **`settleDisputeHoldForRefund`** (`DISPUTE_REFUND_HOLD_SETTLED`). **Stage 141.1:** SSOT **`lib/services/dispute/dispute-resolution-engine.js`** — **`executeDisputeResolution`** (`PAYOUT_PARTNER` / `REFUND_GUEST` / `SPLIT` / `DISMISS`); **`force_refund`** / **`close_dispute`** / SLA auto-resolve делегируют в engine; **`action=split`** (`guestPercent` 1–99) → partial guest refund + **`settleDisputeHoldForSplit`** (`DISPUTE_SPLIT_HOLD_SETTLED`). Ответы действий возвращают снимок **`dispute`** для мгновенного обновления таблицы на клиенте.
...
- **Доказательства к спору:** до **3** URL в **`POST /api/v2/disputes/create`** (`evidenceUrls`), сохраняются в **`disputes.metadata.evidence_urls`**; загрузка файлов — **`POST /api/v2/upload`** с **`bucket=dispute-evidence`** (allowlist в **`app/api/v2/upload/route.js`**), поддержаны фото + `video/mp4|video/quicktime|video/webm`. Миграция бакета: **`database/migrations/039_dispute_evidence_storage.sql`**. Админка **`/admin/disputes`**: таймлайн **`dispute_events`** + вложения только **signed URLs** из **`GET /api/v2/admin/disputes/[id]`** (`evidenceSignedUrls`); закрытие — **`disputes.resolution_reason`**, пуш **`DISPUTE_RESOLVED`**. Таблица событий и колонка вердикта: **`044_dispute_events_timeline.sql`**. Чтение `/_storage/dispute-evidence/*` проходит через серверный guard route `app/_storage/dispute-evidence/[...path]/route.js` (доступ: участники брони или ADMIN/MODERATOR).
...
- **Retention:** **`GET/POST /api/cron/cleanup-drafts`** (тот же janitor: черновики листингов + SLA броней + **checkout-hold TTL** **`AWAITING_PAYMENT`** → **`CANCELLED`** via **`processExpiredAwaitingPaymentCheckouts`**, default **30m**, env **`CHECKOUT_HOLD_TTL_MINUTES`**) + просроченные invoices) — дополнительно **Dispute Evidence Retention**: **`RESOLVED`/`REJECTED`/`CLOSED`**, закрытие **`resolved_at`** (fallback **`updated_at`**) старше **180** суток → удаление объектов **`dispute-evidence`** из **`metadata.evidence_urls`**, маркер **`metadata.dispute_evidence_storage_purged_at`**. Ранее без черновиков janitor делал ранний `return` без SLA/invoice — исправлено: вспомогательные задачи выполняются всегда.
- **Humane SLA 72h (Stage 84.5):** конфиг **`lib/config/dispute-sla.js`** (`DISPUTE_SLA_MS = 72h`, env override), БД **`disputes.current_deadline_at`** (**`045_disputes_humane_sla_72h.sql`**), при открытии официального спора сохраняется дедлайн `now + 72h`; сервис **`DisputeService.processSlaBreaches`**: напоминания **T-24h** и **T-2h** (push+Telegram по `profiles.telegram_id`) и auto-resolution **`OPEN -> RESOLVED`** при breach с `Actor=SYSTEM`, `resolution_reason`, событием в **`dispute_events`**. Подключено в janitor **`/api/cron/cleanup-drafts`**.
- **SLA i18n + user timer (Stage 84.6):** тексты SLA уведомлений централизованы в **`lib/translations/booking.js`** (`booking.dispute_opened_sla`, `booking.dispute_sla_reminder`, `booking.dispute_auto_resolved`) на 4 языках, бренд берётся из env через `{brand}` (без хардкода). В push/TG deep-link для кейса: `.../bookings?id={bookingId}&action=dispute`. В пользовательской карточке заказа **`UnifiedOrderCard`** (данные из **`BookingService.getBookings/getBookingById`** + `active_dispute_current_deadline_at`) отображается живой SLA-индикатор «Осталось времени на ответ: HH:MM».
- **SLA countdown UX (Stage 84.7):** таймер обновляется каждую секунду (`HH:MM:SS`), цветовая шкала зона/внимание/критично, по истечении дедлайна — **`booking.sla_awaiting_admin`**.
...
Status: PUBLIC (Lite-Protection — no session cookie)
Runtime: nodejs
Pattern: Immediate Response + Fire-and-Forget
Protection (Stage 154.3): lib/webhooks/telegram-webhook-auth.js
  — update_id + (message | callback_query) shape check → 400 on bad payload
  — TELEGRAM_BOT_TOKEN format gate → 503 if unset/invalid (no external API)
  — no TELEGRAM_BOT_SECRET_TOKEN / no setWebhook secret_token required
```

**Inline approve/decline:** … Гостевые TG/push (check-in, review reminder) и письма об оплате — deep link **`/my-bookings?booking={id}`** (через **`renterBookingsListPath`**).
...
- **SSOT медиа (Stage 84.3):** **`lib/services/media/media-profiles.js`** — профили **`listing_photo`** / **`dispute_media`** / **`chat_image`** / **`kyc_document`**; клиент **`lib/services/media/compress-image-browser.js`** (`compressImageForBrowser`); сервер **`lib/services/media/media-upload.service.js`** — **`sharp`** в **`POST /api/v2/upload`** для растровых MIME → **`image/webp`**; лог **`[MEDIA_PROFILE]: <профиль>`** после resolve (**`profile`/`mediaProfile`** в form-data или дефолт по bucket). Обёртки: **`lib/services/image-upload.service.js`**, **`lib/chat-upload.js`**, Telegram — **`compressImageBufferTelegramListing`** (импорт с сервера из **`media-upload.service.js`**).
...
```
🔒 Ваши средства защищены системой эскроу платформы ({brand}) и выплачиваются 
...
#### `system_fintech_settings` (singleton `id = 'global'`, Stage 131.0)
| Column | Launch | Purpose |
|--------|--------|---------|
| `acquiring_fee_percent` | 4.3 | % от **`guest_payment_thb`** (snapshot) |
| `usn_provision_percent` / `vat_provision_percent` | 6 / 5 | % от **`owner_revenue`** |
| `reserve_bank_percent` | 0.5 | % от commission gross |
| `referral_reinvestment_percent` | 45 | Доля adjusted net → referral pool |
| `referral_monthly_program_cap_thb` | 250000 | Жёсткий cap accruals / UTC month |
| `ambassador_guest_l2_enabled` | false | P1: L2 на guest_booking |
| `ambassador_guest_pool_*_percent` | 45/12/43 | Split пула при L2 on |
| `referral_withdrawal_fee_percent` | 1.5 | P1: fee на выводе амбассадора |
| `mlm_level*_percent`, `partner_activation_bonus_thb` | 70/30, 500 | Supply Builder (host activation) |

SSOT reader: **`lib/services/finance/system-config.service.js`** (cache 60s). Bootstrap: **`lib/config/fintech-config-defaults.js`**. Migration: **`migrations/stage131_0_system_fintech_settings.sql`**. Policy: **`docs/ADR/131-ambassador-3-0.md`**.

**Immutable booking lock:** `booking.metadata.fintech_snapshot` (v1) пишется в **`POST /api/v2/bookings/[id]/payment/initiate`** при переходе в `AWAITING_PAYMENT` — **`lib/services/finance/fintech-snapshot.service.js`**. Расчёты referral waterfall/distribute читают snapshot через **`getReferralSettings({ booking })`**.

**Admin API:** **`GET|PUT /api/admin/finances/fintech-settings`** — Zod validation, preview waterfall; audit **`audit_logs`** + **`system_fintech_settings_audit`** (`migrations/stage131_1_fintech_settings_audit.sql`); Telegram topic **FINANCE**. UI: **`FinTechAmbassadorSettingsPanel`** на `/admin/settings/finances`.

**Public calculator (Stage 131.1 / 131.3):** **`GET /api/v2/referral/calculator`** — `guestPaymentMode=THB|RUB_CROSS`; бонусы от base THB @ mid; guest RUB @ retail FX markup; **`/about/referral`**. SSOT itemized targets: **`ADR131_REFERENCE_TARGETS`** in `lib/services/finance/fintech-waterfall.js`.

**Vanity links (Stage 131.3):** **`referral_codes.custom_vanity_code`**; **`/go/[vanity]`** → track + redirect `/u/[id]`; **`PATCH /api/v2/referral/vanity`**.

**Referral withdrawal (Stage 131.3 / 131.4):** **`GET /api/v2/wallet/referral-withdrawal-preview?grossThb=&payoutCurrency=`** — gross / 1.5% fee / net; UI **`ReferralWithdrawalWaterfall`** on `/profile/wallet`; approve debits gross. RUB @ mid, 0% spread (`referral-payout-fx.service.js`).

**Vanity welcome (Stage 131.4):** **`GET /api/v2/referral/vanity-welcome`**, session `vanity-welcome-session.js`, banner on `/u/[id]` + homepage.

**L2 counters (Stage 131.4):** shadow → `referral-guest-l2-shadow.service.js` (booking metadata only); live → `referral-guest-l2-live.service.js` (ledger `split_role=l2_mentor`).

**L2 Shadow Mode (Stage 131.1):** при `ambassador_guest_l2_enabled=false` — `referral-guest-l2-shadow.service.js` в `distribute()` пишет **`metadata.fintech_snapshot.shadow_l2_thb`** (caps 500/бронь, 50k/мес); ledger L2 **не** создаётся. **Live L2 stub (131.3):** `createL2PendingLedgerRow()` — только при flag=true.
...
# Split mode (Stage 130.8): browser proxy + direct server URL
NEXT_PUBLIC_SUPABASE_URL=https://airento.ru/supabase
SUPABASE_SERVER_URL=https://xxx.supabase.co
NEXT_PUBLIC_SUPABASE_ANON_KEY=eyJxxx
SUPABASE_SERVICE_ROLE_KEY=eyJxxx

# SSOT helpers: lib/supabase.js → getSupabasePublicUrl() / getSupabaseServerUrl()
# Never use fallback values
const url = process.env.SUPABASE_SERVER_URL || process.env.NEXT_PUBLIC_SUPABASE_URL  // ✅ server
const url = process.env.NEXT_PUBLIC_SUPABASE_URL  // ✅ browser only
...
| POST | `/api/v2/bookings/[id]/payment/confirm` | Подтверждение оплаты гостем → **`PAID_ESCROW`** через **`EscrowService.moveToEscrow`** (ledger); **Stage 146:** при **`PAID_ESCROW` + invoiceId** — post-escrow extension path (**`applyInvoicePostPaymentEffects`**, без повторного capture); **Stage 106.1:** на production CARD/MIR — **403** без intent **PAID** от webhook (`payment-production-guard.js`); CRYPTO — webhook или on-chain verify |
...
| GET | `/api/v2/admin/payouts/tbank-registry` | ADMIN: preview referral RU bank registry (no status change) |
| POST | `/api/v2/admin/payouts/tbank-registry` | ADMIN: CSV реестр Т-Банка + PROCESSING (referralOnly default) |
...
| Method | Path | Description |
|--------|------|-------------|
| GET | `/api/admin/metrics/overview` | **ADMIN/MODERATOR** (`requireAdminStaff`): счётчики таблиц + seed **`profiles.id=admin-777`** + агрегаты **`buildAdminStatsPayload`** (service_role) — без anon `/_db` к core-таблицам |
| GET | `/api/admin/activity/recent?limit=` | **Staff:** последние строки **`activity_log`** |
| POST | `/api/admin/activity` | **Staff:** вставка в **`activity_log`** (**`activity_type`**, **`description`**, **`metadata`**) |
| GET | `/api/admin/moderation` | MODERATOR/ADMIN: PENDING (без черновиков), embed owner + **`categories(slug,name)`**; query **`partner`**, **`category`** (slug), **`dateFrom`**, **`dateTo`**; ответ **`facets`**, **`totalPending`**; SSOT фильтров **`lib/admin/moderation-queue.js`** |
