...
| **RUB acquiring + spread** | Гость платит в **RUB** (YooKassa/MIR); ledger и партнёрские суммы — **THB**. SSOT суммы в PSP: **`lib/services/payment-adapters/acquirer-charge-amount.js`**. Курсовой спред — поле **`fx_markup_thb`** в snapshot (не путать с FX TTL **`CurrencyService`**). |
...
| POST | `/api/v2/bookings/[id]/payment/confirm` | Подтверждение оплаты гостем → **`PAID_ESCROW`** через **`EscrowService.moveToEscrow`** (ledger); **Stage 106.1:** на production CARD/MIR — **403** без intent **PAID** от webhook (`payment-production-guard.js`); CRYPTO — webhook или on-chain verify |
