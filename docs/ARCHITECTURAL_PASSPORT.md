...
> **Version**: 8.0.2 | **Last Updated**: 2026-04-25 | **Status**: Production-Ready — Stage 50.2: centralized **`lib/http-client-identity.js`** (slug + Nominatim / image proxy / importer / iCal sync / admin probe User-Agents from **`getSiteDisplayName()`** + **`getPublicSiteUrl()`**); admin dashboard Telegram status fallbacks removed, test alerts via **`POST /api/v2/telegram/test`** with credentials; system activity log uses probe UA. Stage 50.1: public brand SSOT (manifesto §50.1).
> 
...
- **Время разморозки эскроу (финансовое «ведро»):** **`lib/escrow-thaw-rules.js`** — **`getEscrowThawBucketFromCategorySlug`** → **`housing` \| `transport` \| `service`**, затем **`computeEscrowThawAt`**. Туры (**`tours`**, подстрока **`tour`**) попадают в ведро **`service`** (+2 ч от старта), **не** в отдельный thaw-тип.
- **Тип услуги для визарда / отзывов (четыре логических типа):** **`lib/partner/listing-service-type.js`** — **`inferListingServiceTypeFromCategorySlug`**: **`stay` \| `transport` \| `service` \| `tour`**. Для туров здесь **`tour`** отделён от **`service`** (дефолты metadata, UX). **Не смешивать** с ведром thaw без явного ADR: для денег по-прежнему **`escrow-thaw-rules`**.
- **Копирайт уведомлений по ведру thaw:** **`lib/notification-category-terminology.js`** — те же **buckets**, что **`getEscrowThawBucketFromCategorySlug`**.
- **Карта / приватность координат:** **`lib/listing-location-privacy.js`** — отдельные regex/legacy **`categoryId`**; при новой **`categories.slug`** проверить, нужен ли режим **`exact`** vs **`privacy`**.
...
- **Чеклист при новой строке в `categories`:** (1) нужен ли апдейт **`getEscrowThawBucketFromCategorySlug`** / **`computeEscrowThawAt`**; (2) **`inferListingServiceTypeFromCategorySlug`** и **`defaultMetadataForListingServiceType`**; (3) **`listing-location-privacy.js`**; (4) **`partner-calendar-filters.js`**; (5) копирайт в **`notification-category-terminology`** / **`review-criteria-labels`** при пользовательских текстах; (6) поиск/календарь, если категория как **`vehicles`** требует интервального режима.
...
| **Escrow thaw (время разморозки)** | **3 ведра** | **`getEscrowThawBucketFromCategorySlug`** → **`housing` \| `transport` \| `service`**, затем **`computeEscrowThawAt`**. Подстрока **`tour`** в slug → ведро **`service`** (не отдельный thaw-тип). | **`lib/escrow-thaw-rules.js`** |
...
| **UI / визард / отзывы** | **4 типа** | **`inferListingServiceTypeFromCategorySlug`** → **`stay` \| `transport` \| `service` \| `tour`** | **`lib/partner/listing-service-type.js`**, **`defaultMetadataForListingServiceType`** |
| **Партнёр: список доходов** | **4 бейджа** (в т.ч. **Тур**) | Те же 4 типа, что визард; **деньги/thaw для тура остаются в ведре `service`** — партнёр видит разницу только в UI. | **`app/partner/finances/page.js`** (`PartnerBookingIncomeKindBadge`), строки **`lib/translations/ui.js`** |
...
- **Stage 30.0 — партнёрский UI:** визард шаг 1 (**`StepGeneralInfo.jsx`**) — **`check_in_instructions`** (textarea, плейсхолдеры по **`listingServiceType`**); транспорт — **`PartnerListingSearchMetadataFields`**: поле **`fuel_policy`** (`full_to_full` \| `other`). Snapshot брони по-прежнему только при insert.
