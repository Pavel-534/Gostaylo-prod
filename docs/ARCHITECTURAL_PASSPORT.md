...
- **Публичная выдача категорий (Stage 83.0):** **`GET /api/v2/categories`** для обычных пользователей возвращает только `(is_active OR is_coming_soon) AND !is_preview_only`; для **ADMIN** — все категории + вычисляемый флаг **`isPreview`** для скрытых от публики строк (UI-превью без открытия для остальных).
