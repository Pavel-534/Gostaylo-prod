...
- **Глобально:** **`app/layout.js`** → **`GlobalSiteJsonLd`**; SSOT сборки **`lib/seo/site-website-schema.js`** — **`SearchAction`** с **`urlTemplate`**: канонический origin + **`/listings?q={search_term_string}&semantic=1`** (умный каталог **`runListingsSearchGet`**).
