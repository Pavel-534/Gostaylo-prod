...
> **Version**: 7.27.0 | **Last Updated**: 2026-04-25 | **Status**: Production-Ready — Stage 47.0 notifications + chat→finance + PDF Unicode
> 
...
- **Stage 47.0 — Ecosystem notifications + chat→finance + PDF Unicode:** **`NEW_BOOKING_REQUEST`**: FCM партнёру (**`BOOKING_REQUEST`** / **`BOOKING_INSTANT_PARTNER`** при **`CONFIRMED`**, ссылка **`/partner/bookings?booking=`**), письма гостю/партнёру с ветвлением **instant vs request** (**`EmailService.sendBookingRequested`** / **`sendNewLeadAlert`**, опции **`instantConfirmed`**). **`PARTNER_FUNDS_THAWED_AVAILABLE`** после **`PAID_ESCROW` → `THAWED`** (**`thaw.service.js`**): **`readBookingFinancialSnapshot`** → push **`FUNDS_THAWED_PARTNER`** + **`EmailService.sendPartnerFundsThawedEmail`** (net THB, CTA **`/partner/finances`**); затем по-прежнему **`PARTNER_GUEST_REVIEW_INVITE`**. **`PartnerChatCalendarPeek`**: i18n **`getUIText`** (`chatCalendarPeek_*`), кнопка «Финансы брони» → **`PartnerFinancialSnapshotDialog`** (данные из **`GET /api/v2/partner/bookings/[id]`** или **`financial_snapshot`** из треда). PDF: **`lib/services/partner-pdf-fonts.js`** + шрифты в **`lib/assets/fonts/partner-pdf/`** (Noto LGC / Thai / SC) в **`renderPartnerFinancialStatementPdf`**. PDP: бейдж **`listingCard_verifiedPartner`** под заголовком (**`GuestListingTitleBlock`**). Календарь: демо-фолбэк **`NEXT_PUBLIC_PARTNER_CALENDAR_DEMO_FALLBACK`** только при **`NODE_ENV !== 'production'`** (**`use-partner-calendar.js`**).
