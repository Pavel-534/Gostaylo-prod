...
- **Retention:** **`GET/POST /api/cron/cleanup-drafts`** (тот же janitor: черновики листингов + SLA броней + просроченные invoices) — дополнительно **Dispute Evidence Retention**: **`RESOLVED`/`REJECTED`/`CLOSED`**, закрытие **`resolved_at`** (fallback **`updated_at`**) старше **180** суток → удаление объектов **`dispute-evidence`** из **`metadata.evidence_urls`**, маркер **`metadata.dispute_evidence_storage_purged_at`**. Ранее без черновиков janitor делал ранний `return` без SLA/invoice — исправлено: вспомогательные задачи выполняются всегда.
