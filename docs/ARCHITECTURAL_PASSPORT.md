...
- **SSOT guard:** **`lib/security/admin-staff-access.js`** — **`requireAdminStaff()`** → **`requireAccess({ roles: ['ADMIN', 'MODERATOR'] })`**. Применено ко всем обработчикам под **`app/api/admin/**`**, включая **`settings`**, **`promo-codes`**, **`marketing/*`**, **`notification-outbox/*`**, **`moderation`**, **`users`**, **`revalidate`**, **`fix-enum`** (для **`fix-enum`** сохранён дополнительный секрет в теле/`x-admin-secret`).
