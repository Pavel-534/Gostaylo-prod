...
- **Admin marketing IA (Stage 124.0–124.1):** **`/admin/marketing`** — обзорный дашборд (`MarketingDashboard`, KPI vs пред. 30 дн., алерты, multi-line chart); **`/admin/marketing/promos`** — промокоды; **`/admin/marketing/rules`** — hub A/B + глобальные настройки; **`MarketingSubNav`** (6 вкладок, sticky); sidebar — Обзор, Кампании, Аналитика, Правила и настройки, Fraud Queue, Бюджет и аудит (`/budget` hub → wallet-audit, audit, payouts, analytics, promos).
