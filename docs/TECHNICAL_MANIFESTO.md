...
**Версия:** 2.1.9 (апрель 2026) — как 2.1.8, плюс: устранение гонки Realtime/списка бесед в **`ChatContext`** (ref + `fetchOneConversation` для «чужих» INSERT); единый канал **`typing:global:v1`** для инбокса и треда (**`typing-global-channel.js`**); dev-предупреждения по обрыву Realtime (**`realtime-dev-warn.js`**); **критическое исправление — `useRealtimeMessages` в `hooks/use-realtime-chat.js` убрана серверная фильтрация `filter: conversation_id=eq.…` (ненадёжна для не-UUID id вида `conv-…`), фильтрация по беседе перенесена в JS-обработчик**; диагностика — `GET /api/v2/realtime-diag` (JWT, publication, RLS), компонент `RealtimeDiagOverlay` (admin-only); **миграция `database/migrations/020_realtime_publication_fix.sql`** — добавляет `messages` и `conversations` в `supabase_realtime` publication + `REPLICA IDENTITY FULL` (ОБЯЗАТЕЛЬНО запустить в Supabase SQL Editor).
