...
| **Cron** | **`escrow-thaw`** (ежечасно :15 — **`EscrowService.processDueEscrowThaws`**, PAID_ESCROW→THAWED по **`escrow_thaw_at`**), `checkin-reminder` (только **`PAID_ESCROW`** на день заезда), `review-reminder` (напоминание об отзыве на день после **`check_out`**, listing TZ), `draft-digest`, `cleanup-drafts`, `ical-sync` (ошибка или исключение). **`/api/cron/payouts`** в расписании Vercel **нет**; **`POST`** отключён (**503**). |
