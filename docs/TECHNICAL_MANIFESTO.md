...
**Версия:** 2.1.9 (апрель 2026) — как 2.1.8, плюс: устранение гонки Realtime/списка бесед в **`ChatContext`** (ref + `fetchOneConversation` для «чужих» INSERT); единый канал **`typing:global:v1`** для инбокса и треда (**`typing-global-channel.js`**); dev-предупреждения по обрыву Realtime (**`realtime-dev-warn.js`**); **`useRealtimeMessages`** — без server-side `filter` по `conversation_id`, фильтр в JS; **мост треда** — **`lib/chat/realtime-thread-bridge.js`**: после INSERT в **`ChatContext`** диспатчится событие `gostaylo:realtime-message-insert`, **`useChatThreadMessages`** подписан и вызывает тот же `handleRealtimeInsert`, если вторая подписка postgres_changes не отдала событие в ленту; **`GET /api/v2/realtime-diag`** — JWT + текстовые подсказки (pg_catalog не через PostgREST); **`RealtimeDiagOverlay`** (admin); **`020_realtime_publication_fix.sql`** — publication + REPLICA IDENTITY (идемпотентно).
