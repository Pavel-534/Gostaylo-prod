...
**Financial model version:** **3.5.1** (Split Fee + payout rails + rounding pot + **ledger** + **T-Bank CSV** + **manual payout PAID/FAILED** + **acquiring webhook** + **admin payout verification** + **partner payout history**).
...
| **Cron** | `payouts`, `checkin-reminder` (только **`PAID_ESCROW`** на день заезда), `review-reminder` (напоминание об отзыве на день после **`check_out`**, listing TZ), `draft-digest`, `cleanup-drafts`, `ical-sync` (ошибка или исключение) |
