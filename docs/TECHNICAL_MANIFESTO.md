...
**Версия:** 2.1.9 (апрель 2026) — как 2.1.8, плюс: устранение гонки Realtime/списка бесед в **`ChatContext`** (ref + `fetchOneConversation` для «чужих» INSERT); единый канал **`typing:global:v1`** для инбокса и треда (**`typing-global-channel.js`**); dev-предупреждения по обрыву Realtime (**`realtime-dev-warn.js`**); **тред:** **`lib/chat/realtime-messages-filter.js`** + **`useRealtimeMessages`** — server-side `filter` через **`realtime-messages-filter.js`** (`conversation_id=eq."…"` — кавычки для id с дефисами, напр. `conv-…`); клиентская проверка `rowMatchesConversation` остаётся запасной; в development в консоль: **`NEW_MESSAGE_RECEIVED_IN_THREAD`** и лог hook при INSERT; **`GET /api/v2/realtime-diag`**; **`RealtimeDiagOverlay`** (admin); **`020_realtime_publication_fix.sql`** (идемпотентно).
