...
| **Crypto webhook** | `POST /api/webhooks/crypto/confirm` — секрет **`CRYPTO_WEBHOOK_SHARED_SECRET`**; **125.4:** idempotent **2xx** на post-escrow (`bookings.status` SSOT); иначе **`verifyTronTransaction`** → **`confirmPayment`** / intent + **`moveToEscrow`** |
