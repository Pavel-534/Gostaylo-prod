...
**Версия:** 2.3.0 (апрель 2026) — включает 2.2.0; плюс **единые курсы** для дашборда утечек (**`getDisplayRateMap` без розничной надбавки**), **`general.chatSafety`** в админ-настройках, **авто-shadowban** по порогу страйков (**`hidden_from_recipient`** + фильтр GET/Realtime), **`triggerTextSample`** в **`critical_signal_events`**.
