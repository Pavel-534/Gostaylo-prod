...
- **Typing (Broadcast v2):** канал **`typing:global:v1`**, события **`typing_start`** / **`typing_stop`** с payload `{ conversationId, userId, name }`. Источники: **`hooks/use-chat-typing.js`** (отправка + индикация в треде) и **`lib/context/ChatContext.jsx`** (агрегация для списка диалогов).
...
**Версия:** 2.1.8 (апрель 2026) — messenger-grade: persisted `profiles.last_seen_at` + `POST /api/v2/presence/last-seen`, Broadcast typing_start/typing_stop (`typing:global:v1`) в треде и списке, read-receipts с `is_read=true` в `/api/v2/chat/read`, multi-session E2E `tests/e2e/chat-sync.spec.ts`.
