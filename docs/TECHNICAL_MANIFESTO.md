...
- **Partner confirm + транспорт:** **`verifyInventoryBeforePartnerConfirm`** подгружает **`listings.category_id`** / fallback **`metadata.category_slug`**, при **`vehicles`** передаёт в **`CalendarService.checkAvailability`** **`guestsCount: 1`** и **`listingCategorySlugOverride: 'vehicles'`**, чтобы финальная проверка не зависела от потери slug в **`getCalendar`** и не сравнивала party size с «остатком мест». В **`getCalendar`** slug можно переопределить опцией; при отсутствии **`category_id`** используется **`listings.metadata.category_slug`**. Текст inquiry в чате для транспорта без строки «минимум свободных мест» — см. **`ensureInquiryConversation`**.
