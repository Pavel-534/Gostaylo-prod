...
**Financial model version:** **3.5.5** (на базе **3.5.4** + **PR-#5**: UI политики отмены в редакторе листинга и на странице объявления; **`computeRefundEstimateForBooking`** в **`lib/services/booking-refund-calculator.service.js`**; **`POST /api/v2/bookings/[id]/cancel`** — расчёт возврата + **`LedgerService.postPartialRefundForBooking`** для **`PAID_ESCROW` / `CHECKED_IN` / `THAWED`**, иначе отмена без ledger (**`PENDING`**, **`INQUIRY`**, **`CONFIRMED`**, **`AWAITING_PAYMENT`**, **`PAID`**).)
