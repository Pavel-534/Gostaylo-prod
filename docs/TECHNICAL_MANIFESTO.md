...
- **Тред `/messages/[id]` (хост, мобилка):** при **PENDING** / **INQUIRY** кнопки **Подтвердить / Отклонить** на узком экране дублируются под **`ChatMilestoneCard`** (системные ключи inquiry и **`booking_created`**); **`ChatHeaderActions`** по-прежнему показывает те же действия только на **`lg+`** (**`hidden lg:flex`**). Чтобы не занимать высоту дважды, **`ChatActionBar`** получает **`suppressMobileHostBar`**, когда в ленту передан **`partnerInquiryActions`**. Композер: **`PartnerChatComposer`** + общая оболочка **`CHAT_COMPOSER_SHELL_CLASS`** (**`lib/chat-ui.js`**), поле — **`ChatGrowingTextarea`** (компактная стартовая высота, рост по мере ввода).
