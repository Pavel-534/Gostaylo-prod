-- Stage 152.1 — THAWED in occupying status SSOT (align SQL with OCCUPYING_BOOKING_STATUSES).
...
COMMENT ON FUNCTION public.create_booking_atomic_v1 IS
  'Stage 152.1 — atomic booking insert; occupying statuses incl. THAWED (SSOT lib/booking/status-sets.js).';
...
COMMENT ON FUNCTION public.batch_check_listing_availability(text[], date, date, integer, text[]) IS
  'Stage 152.1 — batch availability; default occupying incl. THAWED (SSOT status-sets.js).';
