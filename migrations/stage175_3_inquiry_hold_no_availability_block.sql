-- Stage 175.3.0 — Inquiry soft-hold must not reduce availability (first paid wins).
-- JS SSOT: lib/calendar/block-source-display.js → countsTowardCalendarAvailability().
-- Payment window policy: lib/booking/payment-window-policy.js (separate app change).

-- Stage 175.3 (inquiry_hold excluded from block_load) вЂ” batch_check_listing_availability: per-listing TZ for booking night overlap.
-- Aligns search occupancy with create_booking_atomic_v1 (timezone(listing_tz, check_in/out)::date).
-- JS SSOT: lib/geo/listing-timezone-ssot.js + resolve_listing_timezone_v1(metadata).
...
COMMENT ON FUNCTION public.batch_check_listing_availability(text[], date, date, integer, text[]) IS
  'Stage 175.3 (inquiry_hold excluded from block_load) вЂ” batch availability; booking nights via resolve_listing_timezone_v1 (parity with create_booking_atomic_v1).';

GRANT EXECUTE ON FUNCTION public.batch_check_listing_availability(text[], date, date, integer, text[]) TO service_role;


-- Stage 152.1 вЂ” THAWED in occupying status SSOT (align SQL with OCCUPYING_BOOKING_STATUSES).
-- JS SSOT: lib/booking/status-sets.js (incl. THAWED since Stage 110.2).
...
COMMENT ON FUNCTION public.create_booking_atomic_v1 IS
  'Stage 152.1 вЂ” atomic booking insert; occupying statuses incl. THAWED (SSOT lib/booking/status-sets.js).';
...
COMMENT ON FUNCTION public.batch_check_listing_availability(text[], date, date, integer, text[]) IS
  'Stage 152.1 вЂ” batch availability; default occupying incl. THAWED (SSOT status-sets.js).';
