-- Stage 124.18 (P0-1): checkout flow — AWAITING_PAYMENT → PAID_ESCROW via move_to_escrow_and_post_ledger_v1
-- Aligns RPC with payment/initiate + payment/confirm + acquiring webhook (SYSTEM_BOOKING_STATUS_TRANSITIONS).
...
BEGIN
  ...
  IF v_booking.status = 'PAID_ESCROW' THEN
    ...
    SELECT lj.id INTO v_journal_id
    FROM public.ledger_journals lj
    WHERE lj.idempotency_key = v_idempotency_key
    LIMIT 1;
    RETURN QUERY SELECT true, true, p_booking_id, v_journal_id, NULL::text;
    RETURN;
  END IF;

  IF v_booking.status::text NOT IN ('PENDING', 'CONFIRMED', 'PAID', 'AWAITING_PAYMENT') THEN
  ...
  SELECT COALESCE(sum(le.amount_thb), 0)::numeric(14,2)
  INTO v_debit_sum
