-- Stage 149.3 — batch_check_listing_availability: do not treat NULL LEFT JOIN rows as 1 booked/blocked unit.
-- Symptom: max_capacity=1 listings looked fully booked in catalog search with zero bookings/blocks.
-- JS passes p_occupying_statuses from OCCUPYING_BOOKING_STATUSES (incl. AWAITING_PAYMENT since 149.2).
...
CREATE OR REPLACE FUNCTION public.batch_check_listing_availability(
  ...
  p_occupying_statuses text[] DEFAULT ARRAY[
    'PENDING','CONFIRMED','AWAITING_PAYMENT','PAID','PAID_ESCROW','CHECKED_IN'
  ]
)
RETURNS TABLE (
  listing_id text,
...
COMMENT ON FUNCTION public.batch_check_listing_availability(text[], date, date, integer, text[]) IS
  'Stage 149.3: batch availability — phantom LEFT JOIN fix; default occupying incl. AWAITING_PAYMENT.';
