-- Stage 201.02 — Single-flight guard for Concierge payout batch settle.
-- CAS on payout_batches.metadata (settle_in_progress_*), TTL reclaim for crashed workers.
-- Prefer over Redis (optional) and session pg_advisory_lock (pooler / multi-statement settle).
--
-- Apply order (no shared objects / no race with 201.01):
--   1) stage201_01_adjust_held_referral_balance_atomic.sql  (wallet held RPC)
--   2) this file                                            (settle lock RPCs)
-- Both are CREATE OR REPLACE + GRANT service_role only; safe to re-run.
-- Heartbeat refresh: stage201_03_payout_batch_settle_lock_refresh.sql
...
CREATE OR REPLACE FUNCTION public.release_payout_batch_settle_lock(
  p_batch_id text,
...
BEGIN
  ...
  IF v_id = '' THEN
    RETURN QUERY SELECT false, 'BATCH_ID_REQUIRED';
    RETURN;
  END IF;
  ...
  -- Token mismatch = another worker reclaimed after TTL; do not wipe their lock.
  IF v_token IS NOT NULL AND v_existing IS NOT NULL AND v_existing <> v_token THEN
    RETURN QUERY SELECT false, 'token_mismatch';
    RETURN;
  END IF;

  UPDATE public.payout_batches pb
     SET metadata = (coalesce(pb.metadata, '{}'::jsonb)
                     - 'settle_in_progress_at'
                     - 'settle_lock_owner'
                     - 'settle_lock_token'),
         updated_at = now()
   WHERE pb.id = v_id;
